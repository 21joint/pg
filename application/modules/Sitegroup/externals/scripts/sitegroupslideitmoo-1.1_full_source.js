...
/**
	...
	- navigators ( forward/back ) no longer added by script. Instead, add them into overallContainer making their display from CSS and after add the CSS selector class to navs parameter
		IE: navs:{ 
				fwd:'.Sitegroup_SlideItMoo_forward',
				bk:'.Sitegroup_SlideItMoo_back'
			}
...
var SocialengineSlideItMoo = new Class({
  ...
  options: {
    ...
    itemsSelector: null, /* css class for inner elements ( ie: .Sitegroup_SlideItMoo_element ) */
      ...
      if (this.options.call_count == 1) {
        ...
        var request = new Request.HTML({
          ...
          {	
            ...
            if(module_name == 'Sitegroupvideo' && en4.sitevideoview)
              en4.sitevideoview.attachClickEvent(Array('item_photo_sitegroupvideo_video','sitegroupvideo_title'));        
          }
        });
        request.send();
      }
