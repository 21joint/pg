...
var hideLeftContainer = function (ads_display, group_communityad_integration, adwithoutpackage) {
    if (group_communityads == 1 && ads_display != 0 && group_communityad_integration != 0 && adwithoutpackage == 1) {
        ...
        if (typeof group_hide_left_container != 'undefined' && group_hide_left_container == 0)
            return;
        ...
        if ($('global_content').getElement('.layout_sitegroup_group_cover_information_sitegroup')) {
            $('global_content').getElement('.layout_sitegroup_group_cover_information_sitegroup').style.display = 'none';
        }
        ...
        if ($('global_content').getElement('.layout_sitegroupmember_groupcover_photo_sitegroupmembers')) {
            $('global_content').getElement('.layout_sitegroupmember_groupcover_photo_sitegroupmembers').style.display = 'none';
        }
    }
}
...
$(window).on('load', function () {
    if ($('global_wrapper') && $('global_wrapper').getElementById('global_content')) {
...
en4.sitegroup.ajaxTab = {
    ...
    attachEvent: function (widget_id, params) {
        ...
        $('.tab_' + widget_id).each(function (j, el) {
            if (el.get('tag') == 'li') {
                element = el;
                return;
            }
...
function scrollToTopForGroup(id) {
    ...
    if (typeof sitegroup_slding_effect !== 'undefined' && sitegroup_slding_effect == 0)
        return false;
