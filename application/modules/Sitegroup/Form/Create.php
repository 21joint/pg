...
class Sitegroup_Form_Create extends Engine_Form {
    ...
    public function init() {
        ...
        if (Engine_Api::_()->getDbtable('modules', 'core')->isModuleEnabled('sitecrowdfunding') && (Engine_Api::_()->hasModuleBootstrap('sitecrowdfundingintegration') && Engine_Api::_()->getDbtable('modules', 'sitecrowdfunding')->getIntegratedModules(array('enabled' => 1, 'item_type' => 'sitegroup_group', 'item_module' => 'sitegroup')))) {
...
//       $description = Zend_Registry::get('Zend_Translate')->_('This will be the end of your group URL. It should be indicative of the title and can contain alphabets, numbers, underscores and  dashes only. Its length should be in the range of 3-255 characters. The complete URL of your group will be:');
        ...
        if (!empty($createFormFields) && in_array('location', $createFormFields) && $coreSettings->getSetting('sitegroup.locationfield', 1)) {
            ...
            $this->addElement('Hidden', 'locationParams', array('order' => $i++,));


            include_once APPLICATION_PATH . '/application/modules/Seaocore/Form/specificLocationElement.php';
        }

        // FOR RESTRICTED GROUP
        $restrictedGroupText = "Do you want to create a restricted group? Only group admin or group member can invite / add members in this group.";
        if (!empty($createFormFields) && in_array('restrictedGroup', $createFormFields)) {
            $this->addElement('Radio', 'restricted',  array(
                    'label' => 'Restricted Group',
                    'description' => $restrictedGroupText,
                    'multiOptions' => array(
                        '1' => 'Yes, create this group as a restricted group.',
                        '0' => 'No, create this group as a normal group.',
                    ),
                    'value' => '0',
            ));
        }
...
//            'attribs' => array('style' => 'max-height:150px; '),
        ...
        if (!empty($createFormFields) && in_array('projectPrivacy', $createFormFields)) {
            if (Engine_Api::_()->getDbtable('modules', 'core')->isModuleEnabled('sitecrowdfunding') && (Engine_Api::_()->hasModuleBootstrap('sitecrowdfundingintegration') && Engine_Api::_()->getDbtable('modules', 'sitecrowdfunding')->getIntegratedModules(array('enabled' => 1, 'item_type' => 'sitegroup_group', 'item_module' => 'sitegroup')))) {
                ...
                $options = (array) Engine_Api::_()->authorization()->getAdapter('levels')->getAllowed('sitegroup_group', $user, 'auth_sprcreate');
                $options_create = array_intersect_key($availableLabels, array_flip($options));

                if (!empty($options_create)) {
                    $can_show_list = true;
                    if (Engine_Api::_()->sitegroup()->hasPackageEnable()) {
                        if (!Engine_Api::_()->sitegroup()->allowPackageContent($this->getPackageId(), "modules", "sitecrowdfunding")) {
                            $can_show_list = false;
                        ...
                        $can_create = Engine_Api::_()->authorization()->getPermission($userlevel_id, 'sitegroup_group', 'sprcreate');
                        if (!$can_create) {
                            $can_show_list = false;
                            $this->addElement('Hidden', 'sprcreate', array(
                                'order' => $i++,
                    ...
                    if ($can_show_list) {
                        ...
                        if (count($options_create) > 1) {
                            $this->addElement('Select', 'sprcreate', array(
                                ...
                                'description' => 'Who may create projects for this group?',
                                'multiOptions' => $options_create,
                                'value' => @array_search(@end($options_create), $options_create),
