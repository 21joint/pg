 ...
 */
class Sitegroup_Form_Admin_Import_Import extends Engine_Form {

  public function init() {

    $this
            ->setTitle('Import a File')
            ->setDescription("Add a CSV file to import Groups corresponding to the entries in it, then click 'Submit'. Below, you can also set the privacy of those Groups")
    ...
    if (count($view_options) >= 1) {
      $this->addElement('Select', 'auth_view', array(
          'label' => 'View Privacy',
          'description' => 'Who may see these groups?',
    ...
    if (count($comment_options) >= 1) {
      $this->addElement('Select', 'auth_comment', array(
          ...
          'description' => 'Who may post comments on these groups?',
