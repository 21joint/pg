 ...
 */
class Sitegroup_Form_Managesearch extends Fields_Form_Search {
  ...
  public function getAdditionalOptionsElement() {
    ...
    $row = $this->_searchForm->getFieldsOptions('sitegroup', 'price');
    ...
    $rowLocation = $this->_searchForm->getFieldsOptions('sitegroup', 'location');
    if (!empty($rowLocation) && !empty($rowLocation->display)) {
      ...
      if (!empty($enableLocation)) {
        $row = $this->_searchForm->getFieldsOptions('sitegroup', 'locationmiles');
        if (!empty($row) && !empty($row->display)) {
    ...
    $row = $this->_searchForm->getFieldsOptions('sitegroup', 'search');
    if (!empty($row) && !empty($row->display)) {
    ...
    $row = $this->_searchForm->getFieldsOptions('sitegroup', 'orderby');
		$sitegroupreviewEnabled = Engine_Api::_()->getDbtable('modules', 'core')->isModuleEnabled('sitegroupreview');
    if (!empty($row) && !empty($row->display) && !empty($sitegroupreviewEnabled)) {
    ...
    $row = $this->_searchForm->getFieldsOptions('sitegroup', 'show');
    if (!empty($row) && !empty($row->display)) {
    ...
    $row = $this->_searchForm->getFieldsOptions('sitegroup', 'closed');
    if (Engine_Api::_()->getApi('settings', 'core')->getSetting('sitegroup.status.show', 1) && !empty($row) && !empty($row->display)) {
//      $enableStatus = Engine_Api::_()->getApi('settings', 'core')->getSetting('sitegroup.status.show', 1);
//      if ($enableStatus) {
...
//      }
    ...
    $row = $this->_searchForm->getFieldsOptions('sitegroup', 'category_id');
    if (!empty($row) && !empty($row->display)) {
    ...
    $row = $this->_searchForm->getFieldsOptions('sitegroup', 'has_photo');
    if (!empty($row) && !empty($row->display)) {
