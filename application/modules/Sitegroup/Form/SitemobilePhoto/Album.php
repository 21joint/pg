 ...
 */
class Sitegroup_Form_SitemobilePhoto_Album extends Engine_Form {
  ...
  public function init() {
    ...
    if (!empty($tagOptions) && count($tagOptions) >= 1) {
      if (count($tagOptions) == 1) {
        $this->addElement('hidden', 'auth_tag', array('order' => 335,'value' => key($tagOptions)));
      } else {
...
//    $this->addElement('Hidden', 'fancyuploadfileids');
  ...
  public function saveValues($values) {
    ...
    $params = Array();
    $getPackageAlbum = Engine_Api::_()->sitegroup()->getPackageAuthInfo('sitegroupalbum');
...
//     $linked_album_title = '<a target = "_parent" href = http://' . $_SERVER['HTTP_HOST'] . Zend_Controller_Front::getInstance()->getRouter()->assemble(array('action' => 'view', 'group_id' => $album->group_id, 'album_id' => $album->album_id, 'slug' => $album->getSlug(), 'tab' => $content_id), 'sitegroup_albumphoto_general') . "><b>$album->title</b></a>";
    ...
    if (count($values['file'] > 0)) {
      ...
      if ($activityFeedType) {
        $action = $api->addActivity(Engine_Api::_()->user()->getViewer(), $sitegroup, $activityFeedType, null, array('count' => count($values['file']), 'linked_album_title' => $linked_album_title));
      ...
      //GROUP ALBUMS CREATE NOTIFICATION AND EMAIL WORK
			...
			if ($sitegroupVersion >= '4.2.9p3') {
				Engine_Api::_()->sitegroup()->sendNotificationEmail($album, $action, 'sitegroupalbum_create', 'SITEGROUPALBUM_CREATENOTIFICATION_EMAIL', 'Groupevent Invite');
				...
				if (!empty($isGroupAdmins)) {
					...
					Engine_Api::_()->sitegroup()->sendNotificationToFollowers($album, $action, 'sitegroupalbum_create');
				}
      }
