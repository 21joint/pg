 ...
 */
class Sitegroup_Form_Search extends Fields_Form_Search {
  ...
  public function init() {
    ...
    $this->_searchFormSettings = Engine_Api::_()->getDbTable('searchformsetting', 'seaocore')->getModuleOptions('sitegroup');     
    
    //if (Engine_Api::_()->getApi('settings', 'core')->getSetting('sitegroup.profile.search', 1)) {
    ...
    $this->setAction($view->url(array('action' => 'index'), 'sitegroup_general', true))->getDecorator('HtmlTag')->setOption('class', '');
  }

  public function getMemberTypeElement() {
//    $row = $this->_searchForm->getFieldsOptions('sitegroup', 'profile_type');
  ...
  public function getAdditionalOptionsElement() {
    ...
    if (!empty($this->_searchFormSettings['price']) && !empty($this->_searchFormSettings['price']['display'])) {
      ...
      if (!empty($enablePrice)) {
        ...
        $this->addSubForm($subformPrice, 'sitegroup_price');
      }
    }
    ...
    if (!empty($this->_searchFormSettings['location']) && !empty($this->_searchFormSettings['location']['display'])) {
      ...
      if (!empty($enableLocation)) {
        ...
        $this->addElement('Text', 'sitegroup_location', array(
            'label' => 'Location',
        ...
        $myLocationDetails = Engine_Api::_()->seaocore()->getMyLocationDetailsCookie();
        if (isset($_GET['sitegroup_location'])) {
          $this->sitegroup_location->setValue($_GET['sitegroup_location']);
        } elseif (isset($_GET['locationSearch'])) {
          $this->sitegroup_location->setValue($_GET['locationSearch']);
        } elseif (isset($myLocationDetails['location'])) {
          $this->sitegroup_location->setValue($myLocationDetails['location']);
          if(!Engine_API::_()->seaocore()->checkSitemobileMode('fullsite-mode') && empty($myLocationDetails['location']) && isset($_GET['is_ajax_load'])){ 
            $this->sitegroup_location->setValue(true);
          }
        }

        if (isset($_GET['sitegroup_location']) || isset($_GET['locationSearch'])) {
          Engine_Api::_()->seaocore()->setMyLocationDetailsCookie($myLocationDetails);
        }

        if (!isset($_GET['sitegroup_location']) && !isset($_GET['locationSearch']) && isset($this->_widgetSettings['locationDetection']) && empty($this->_widgetSettings['locationDetection'])) {
          $this->sitegroup_location->setValue('');
        }



        if (!empty($this->_searchFormSettings['street']) && !empty($this->_searchFormSettings['street']['display'])) {
          $this->addElement('Text', 'sitegroup_street', array(
              'label' => 'Street',
        ...
        if (!empty($this->_searchFormSettings['city']) && !empty($this->_searchFormSettings['city']['display'])) {
          $this->addElement('Text', 'sitegroup_city', array(
              'label' => 'City',
        ...
        if (!empty($this->_searchFormSettings['state']) && !empty($this->_searchFormSettings['state']['display'])) {
          $this->addElement('Text', 'sitegroup_state', array(
              'label' => 'State',
        ...
        if (!empty($this->_searchFormSettings['country']) && !empty($this->_searchFormSettings['country']['display'])) {
          $this->addElement('Text', 'sitegroup_country', array(
              'label' => 'Country',
    ...
    if (Engine_Api::_()->getDbtable('modules', 'core')->isModuleEnabled('sitegroupbadge') && Engine_Api::_()->getApi('settings', 'core')->getSetting('sitegroupbadge.seaching.bybadge', 1) && !empty($this->_searchFormSettings['badge_id']) && !empty($this->_searchFormSettings['badge_id']['display'])) {
        ...
        if (!empty($badgeData)) {
          ...
          if (!empty($badgeCount)) {
            ...
            $this->addElement('Select', 'badge_id', array(
                ...
                'onchange' => $this->gethasMobileMode() ? '' : 'searchSitegroups();',
    ...
    if (!empty($sitegroupreviewEnabled) && !empty($this->_searchFormSettings['orderby']) && !empty($this->_searchFormSettings['orderby']['display'])) {
    ...
    if (!empty($sitegroupofferEnabled) && !empty($this->_searchFormSettings['offer_type']) && !empty($this->_searchFormSettings['offer_type']['display'])) {
    ...
    if (!empty($this->_searchFormSettings['show']) && !empty($this->_searchFormSettings['show']['display'])) {
      ...
      $show_multiOptions["1"] = 'Everyone\'s Groups';
      $show_multiOptions["2"] = 'Only My Friends\' Groups';
      $show_multiOptions["4"] = 'Groups I Like';
      $show_multiOptions["5"] = 'Featured Groups'; 
      $value_deault = 1;
      ...
      if (empty($enableNetwork)) {
        ...
        if (!empty($viewerNetwork) || Engine_Api::_()->getApi('subCore', 'sitegroup')->groupBaseNetworkEnable()) {
          $show_multiOptions["3"] = 'Only My Networks';
          $browseDefaulNetwork = Engine_Api::_()->getApi('settings', 'core')->getSetting('sitegroup.default.show', 0);
    ...
    if (Engine_Api::_()->getApi('settings', 'core')->getSetting('sitegroup.status.show', 1) && !empty($this->_searchFormSettings['closed']) && !empty($this->_searchFormSettings['closed']['display'])) {
...
// echo preg_replace($pattern, '&', $category->category_name);
      if (count($categories) != 0) {
        ...
        if (Engine_API::_()->seaocore()->checkSitemobileMode('fullsite-mode')) {
          ...
          $categoryFiles = 'application/modules/Sitegroup/views/scripts/_Subcategory.tpl';
        } else {
          $onChangeEvent = "sm4.core.category.set(this.value, 'subcategory');";
          $categoryFiles = 'application/modules/Sitegroup/views/sitemobile/scripts/_Subcategory.tpl';
        }
...
//      ));
    ...
    if (Engine_Api::_()->getDbtable('modules', 'core')->isModuleEnabled('sitegroupreview') && !empty($this->_searchFormSettings['has_review']) && !empty($this->_searchFormSettings['has_review']['display'])) {
      $this->addElement('Checkbox', 'has_review', array(
          'label' => 'Only Groups With Reviews',
      ...
      if (Engine_Api::_()->getDbtable('modules', 'core')->isModuleEnabled('sitegroupgeolocation') && !empty($this->_searchFormSettings['has_currentlocation']) && !empty($this->_searchFormSettings['has_currentlocation']['display'])) {
    ...
    $subform->addElement('Button', 'done', array(
        'label' => 'Search',
        'type' => 'submit',
        'onclick' => $this->gethasMobileMode() ? '' : 'searchSitegroups();',
        'ignore' => true,
        'decorators' => array(
            'ViewHelper',
            array('HtmlTag', array('tag' => 'li'))
