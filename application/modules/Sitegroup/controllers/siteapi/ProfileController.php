...
class Sitegroup_ProfileController extends Siteapi_Controller_Action_Standard {
    ...
    public function viewAction() {
        ...
        $bodyParams['response']['body'] = @str_replace('src="/', 'src="' . Engine_Api::_()->getApi('Core', 'siteapi')->getHost() . '/', $bodyParams['response']['body']);
    ...
    public function editAction() {
        ...
        foreach ($roles as $roleString) {
            ...
            if (1 == $auth->isAllowed($sitegroup, $role, 'view')) {
                $authValues['auth_view'] = $roleString;
            }

            if (1 == $auth->isAllowed($sitegroup, $role, 'comment')) {
            ...
            if (1 == $sitegroupAllow->isAllowed($sitegroup, $role, 'sspcreate')) {
                $authValues['auth_sspcreate'] = $roleString;
            }
            // PHOTO PRIVACY WORK
            if (1 == $sitegroupAllow->isAllowed($sitegroup, $role, 'spcreate')) {
                $authValues['spcreate'] = $roleString;
            }
            // DISCUSSION PRIVACY WORK
            if (1 == $sitegroupAllow->isAllowed($sitegroup, $role, 'sdicreate')) {
                $authValues['sdicreate'] = $roleString;
            }
            //SITEGROUPDOCUMENT PRIVACY WORK
            if (1 == $sitegroupAllow->isAllowed($sitegroup, $role, 'sdcreate')) {
                $authValues['sdcreate'] = $roleString;
            }
            // SITEGROUPVIDEO PRIVACY WORK
            if (1 == $sitegroupAllow->isAllowed($sitegroup, $role, 'svcreate')) {
                $authValues['svcreate'] = $roleString;
            }
            //START SITEGROUPPOLL PRIVACY WORK
            if (1 == $sitegroupAllow->isAllowed($sitegroup, $role, 'splcreate')) {
                $authValues['splcreate'] = $roleString;
            }
            //START SITEGROUPNOTE PRIVACY WORK
            if (1 == $sitegroupAllow->isAllowed($sitegroup, $role, 'sncreate')) {
                $authValues['sncreate'] = $roleString;
            }
            //START SITEGROUPMUSIC PRIVACY WORK
            if (1 == $sitegroupAllow->isAllowed($sitegroup, $role, 'smcreate')) {
                $authValues['smcreate'] = $roleString;
            }
            //START SITEGROUPEVENT PRIVACY WORK
            if (1 == $sitegroupAllow->isAllowed($sitegroup, $role, 'secreate')) {
                $authValues['secreate'] = $roleString;
            }
        }

        if (Engine_Api::_()->getApi('subCore', 'sitegroup')->groupBaseNetworkEnable()) {
            if (!empty($sitegroup->networks_privacy)) {
                $authValues['secreate'] = explode(',', $sitegroup->networks_privacy);
            } else {
                $authValues['secreate'] = array(0);
            }
        }
        ...
        if ($this->getRequest()->isGet()) {
            $form_fields = Engine_Api::_()->getApi('Siteapi_Core', 'sitegroup')->getForm($sitegroup, 0, 1);
            ...
            if (isset($authValues) && !empty($authValues))
                $form_fields['formValues'] = array_merge($form_fields['formValues'], $authValues);
            ...
            $profile_type = Engine_Api::_()->getDbTable('categories', 'sitegroup')->getProfileType($sitegroup->category_id, 0, 'profile_type');
            ...
            try {
                ...
                $values['profile_type'] = $data['profile_type'] = Engine_Api::_()->getDbTable('categories', 'sitegroup')->getProfileType($values['category_id'], 0, 'profile_type');
                ...
                $values['profile_type'] = Engine_Api::_()->getDbTable('categories', 'sitegroup')->getProfileType($values['category_id'], 0, 'profile_type');
