...
class Sitegroup_PaymentController extends Core_Controller_Action_Standard {
  ...
  public function gatewayAction() {
    ...
    foreach ($gateways as $gateway) {
      if ($gateway->plugin == 'Sitegateway_Plugin_Gateway_MangoPay' || $gateway->plugin == 'Sitegateway_Plugin_Gateway_PayPalAdaptive')
                continue;
    ...
    $creditAllow=false;
    $isSitecreditEnabled = Engine_Api::_()->getDbtable( "modules" , "core" )->isModuleEnabled("sitecredit");
        ...
        if($isSitecreditEnabled) {
            ...
            if(!empty($viewer_id)) {
                ...
                $select=$CreditModuleTable->select()->where('name = ?','sitegroup')->where('flag=?','package');
      ...
      if (Engine_Api::_()->hasModuleBootstrap('sitecredit')) {
            ...
            if (!empty($creditSession->packagePaymentCreditDetail)) {
                ...
                if(empty($package->price)){
                    ...
                    $group_user = $order->getUser();
                    $group_group = $order->getSource();
                    $group_package = $group_group->getPackage();
                    $order->state = 'complete';
                    $order->save();
                    $group_group->onPaymentSuccess();
                    if ($group_group->didStatusChange()) {
                        // SEND ACTIVE MAIL HERE
                        Engine_Api::_()->sitepage()->sendMail("ACTIVE", $group_group->group_id);
                    }
                    $param['type_id']=$order->order_id;
                    $param['credit_point']=-$creditDetail['credit_points'];
                    $param['type']=strtolower($group_package->getType());
                    ...
                    $this->_success->succes_id = $group_group->group_id;
