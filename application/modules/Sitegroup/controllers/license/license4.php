...
if ($PRODUCT_TYPE != 'sitegroup') {
    include APPLICATION_PATH . "/application/modules/Sitegroup/controllers/license/sitegroup_plugin_check.php";
    if (empty($sitegroup_version_correct))
        return;
}
        ...
        if ($shouldUpgrade) {
            ...
            if (empty($module_auth)) {
                if (empty($currentmod_info)) {
                    ...
                    return $this->_error('<div class="global_form"><div><div><a href="' . $final_url_1 . '" class="smoothbox">Click here</a> to proceed with the installation of: "<strong>' . $PRODUCT_TITLE . '</strong>" package.. (If you get a "Group Not Found" in the next step, then <a href="' . $final_url_2 . '" class="smoothbox">click here</a>.)</div></div></div>');
                } else {
