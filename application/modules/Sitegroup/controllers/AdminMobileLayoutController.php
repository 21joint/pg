 ...
 */
class Sitegroup_AdminMobileLayoutController extends Core_Controller_Action_Admin {
  ...
  public function saveAction() {
    ...
    $groupcoreTable = Engine_Api::_()->getDbtable('pages', 'sitemobile');
...
//    }
  ...
  public function layoutAction() {
    ...
    $userlayoutcontentgroupsTable = Engine_Api::_()->getDbtable('mobileContentgroups', 'sitegroup');
    ...
    $groupTable = Engine_Api::_()->getDbtable('pages', 'sitemobile');
    $contentTable = Engine_Api::_()->getDbtable('mobileadmincontent', 'sitegroup');
    $userlayoutcontentTable = Engine_Api::_()->getDbtable('mobileContent', 'sitegroup');
    ...
    $select = $userlayoutcontentTable->select()->from($userlayoutcontentTablename, array('mobilecontentgroup_id'))->limit($limit, $offset)->group('mobilecontentgroup_id');
    ...
    try {
      ...
      if (isset($session->structure)) {
        ...
        $trimStructure = trim($structure, '()');
        $session->structure = !empty($trimStructure) ? Zend_Json::decode($trimStructure) : array();


        if (is_string($structure)) {
          $structure = !empty($trimStructure) ? Zend_Json::decode($trimStructure) : array();
        }
      ...
      foreach ($structure as $element) {
        ...
        if (!empty($contentRow) && is_object($contentRow)) {
          ...
          foreach ($content_groups_id as $value) {
            ...
            $content_group_id = $value->mobilecontentgroup_id;

            if ($contentRow->type == 'container' && $contentRow->name == 'middle') {
              $select_parent = $userlayoutcontentTable->select()->where('type =?', 'container')->where('name =?', $parentContentRow->name)->where('mobilecontentgroup_id =?', $content_group_id);
              $content_parent = $userlayoutcontentTable->fetchRow($select_parent);
              $select = $userlayoutcontentTable->select()->where('type =?', $contentRow->type)->where('name =?', $contentRow->name)->where('mobilecontentgroup_id =?', $content_group_id)->where('parent_content_id = ?', $content_parent->mobilecontent_id);
            } else {
              $select = $userlayoutcontentTable->select()->where('type =?', $contentRow->type)->where('name =?', $contentRow->name)->where('mobilecontentgroup_id =?', $content_group_id);
            }
            ...
            if ($usercontentRow->type == 'container') {
              ...
              $userlayoutcontentTable->update(array('order' => $newOrder), array('type = ?' => $contentRow->type, 'mobilecontentgroup_id = ?' => $content_group_id, 'name = ?' => $contentRow->name, 'widget_admin = ?' => 1));
            }

            if (isset($session->edittitles) && in_array($element['name'], $session->edittitles)) {
							$userlayoutcontentTable->update(array('params' => $element['params']), array('type = ?' => $contentRow->type, 'mobilecontentgroup_id = ?' => $content_group_id, 'name = ?' => $contentRow->name, 'widget_admin = ?' => 1, 'parent_content_id = ?' => $usercontentRow->parent_content_id, 'params =?' => $oldContentRowParams));
            }

            $parent_id = 0;
            if (!empty($parentContentRow)) {
              $select = $userlayoutcontentTable->select()->where('type =?', $parentContentRow->type)->where('name =?', $parentContentRow->name)->where('mobilecontentgroup_id =?', $content_group_id);
              ...
              if (!empty($parentContentRow->parent_content_id)) {
                ...
                if (!empty($parentparentContentRow)) {
                  $select = $userlayoutcontentTable->select()->where('type =?', $parentparentContentRow->type)->where('name =?', $parentparentContentRow->name)->where('mobilecontentgroup_id =?', $content_group_id);
                  $contentParent_groups = $userlayoutcontentTable->fetchRow($select);
                  if ($contentParent_groups->name != $parentparentContentRow->name) {
                    $select = $userlayoutcontentTable->select()->where('type =?', $parentContentRow->type)->where('name =?', $parentContentRow->name)->where('mobilecontentgroup_id =?', $content_group_id)->where('parent_content_id <> ?', $content_groups->parent_content_id);
              ...
              $parent_id = $content_groups->mobilecontent_id;
            } else {
              $userlayoutcontentTable->update(array('parent_content_id' => new Zend_Db_Expr('NULL'), 'order' => $newOrder), array('type = ?' => $contentRow->type, 'mobilecontentgroup_id = ?' => $content_group_id, 'name = ?' => $contentRow->name, 'widget_admin = ?' => 1));
            }
          ...
          foreach ($content_groups_id as $value) {
            ...
            if ($element['type'] == 'container') {
              ...
              if ($element['name'] != 'core.html-block' && $element['name'] != 'core.ad-campaign' && $element['name'] != 'sitegroupintegration.profile-items' && $element['name'] != 'sitegroupmember.profile-sitegroupmembers') {
                $select = $userlayoutcontentTable->select()->where('name =?', $element['name'])->where('mobilecontentgroup_id =?', $content_group_id);
                $userlayoutgroups = $userlayoutcontentTable->fetchRow($select);
              } else {
                $select = $userlayoutcontentTable->select()->where('name =?', $element['name'])->where('mobilecontentgroup_id =?', $content_group_id)->where('params =?', json_encode($element['params']));
                $userlayoutgroups = $userlayoutcontentTable->fetchRow($select);
              }
            }
            ...
            if (empty($userlayoutgroups) && isset($element['type']) && isset($element['name'])) {
              ...
              $usercontentRow->mobilecontentgroup_id = $content_group_id;
              $usercontentRow->order = $newOrder;
              $usercontentRow->type = $element['type'];
              $usercontentRow->name = $element['name'];
              if (!empty($parentContentRow)) {
                $select = $userlayoutcontentTable->select()->where('type =?', $parentContentRow->type)->where('name =?', $parentContentRow->name)->where('mobilecontentgroup_id =?', $content_group_id)->order('mobilecontent_id DESC')->order('mobilecontent_id DESC');
                $content_groups = $userlayoutcontentTable->fetchRow($select);
                if (!empty($content_groups)) {
                  $usercontentRow->parent_content_id = $content_groups->mobilecontent_id;
                }
                if (empty($usercontentRow->parent_content_id)) {
          ...
          foreach ($content_groups_id as $value) {
            ...
            if (!empty($parentContentRow)) {
              ...
              if (!empty($parentparentContentRow)) {
                ...
                $contentParent_groups = $userlayoutcontentTable->fetchRow($select);
                if ($contentParent_groups->name != $parentparentContentRow->name) {
                  $select = $userlayoutcontentTable->select()->where('type =?', $parentContentRow->type)->where('name =?', $parentContentRow->name)->where('mobilecontentgroup_id =?', $content_group_id)->where('parent_content_id <> ?', $contentParent_groups->mobilecontent_id);
                  $content_groups = $userlayoutcontentTable->fetchRow($select);
                }
              }
              $parent_id = $content_groups->mobilecontent_id;
            ...
            if (!empty($parent_id)) {
              if ( $contentRow->name != 'core.ad-campaign' && $contentRow->name != 'sitegroupintegration.profile-items') {
                $select = $userlayoutcontentTable->select()->where('type =?', $contentRow->type)->where('name =?', $contentRow->name)->where('mobilecontentgroup_id =?', $content_group_id)->where('parent_content_id =?', $parent_id);
              } else {
                $select = $userlayoutcontentTable->select()->where('type =?', $contentRow->type)->where('name =?', $contentRow->name)->where('mobilecontentgroup_id =?', $content_group_id)->where('parent_content_id =?', $parent_id)->where('params =?', $oldContentRow);
              }
              $content_groups = $userlayoutcontentTable->fetchRow($select);
              if (!empty($content_groups)) {
                $userlayoutcontentTable->delete(array(
                    'name =?' => $contentRow->name, 'mobilecontentgroup_id =?' => $content_group_id, 'parent_content_id =?' => $parent_id, 'mobilecontent_id =?' => $content_groups->mobilecontent_id, 'widget_admin = ?' => 1
                ));
              }
            } else {
              if (($contentRow->name == 'top' || $contentRow->name == 'bottom') && $contentRow->type == 'container') {
                ...
                $userlayoutcontentTable->delete(array('type =?' => 'container',
                    'name =?' => 'middle', 'mobilecontentgroup_id =?' => $content_group_id, 'parent_content_id = ?' => $content_groups->mobilecontent_id, 'widget_admin = ?' => 1
                ));
                $userlayoutcontentTable->delete(array(
                    'name =?' => $contentRow->name, 'mobilecontentgroup_id =?' => $content_group_id, 'mobilecontent_id =?' => $content_groups->mobilecontent_id, 'widget_admin = ?' => 1
                ));
              }
            }
          }
      ...
      if (!empty($top)) {
        $select = $contentTable->select()->where('group_id =?', $group)->where('name =?', 'middle')->where('parent_content_id =?', $top->mobileadmincontent_id);
        $middle = $contentTable->fetchRow($select);
        $select = $contentTable->select()->where('group_id =?', $group)->where('parent_content_id =?', $middle->mobileadmincontent_id);
        ...
        foreach ($content_groups_id as $value) {
          $content_group_id = $value->mobilecontentgroup_id;
          $select = $userlayoutcontentTable->select()->where('name =?', 'top')->where('mobilecontentgroup_id =?', $content_group_id);
          $user_top = $userlayoutcontentTable->fetchRow($select);
          if (!empty($user_top)) {
            $select = $userlayoutcontentTable->select()->where('name =?', 'middle')->where('mobilecontentgroup_id =?', $content_group_id)->where('parent_content_id =?', $user_top->mobilecontent_id);
            $user_middile = $userlayoutcontentTable->fetchRow($select);
            if ($user_middile) {
              foreach ($name as $name_value) {
                $userlayoutcontentTable->update(array('parent_content_id' => $user_middile->mobilecontent_id), array('mobilecontentgroup_id = ?' => $content_group_id, 'name = ?' => $name_value, 'widget_admin = ?' => 1));
              }
            }
          }
        }
        ...
        if (!empty($bottom)) {
          $select = $contentTable->select()->where('group_id =?', $group)->where('name =?', 'middle')->where('parent_content_id =?', $bottom->mobileadmincontent_id);
          $middle = $contentTable->fetchRow($select);
          $select = $contentTable->select()->where('group_id =?', $group)->where('parent_content_id =?', $middle->mobileadmincontent_id);
          $middle_block = $contentTable->fetchAll($select);
          $name = array();
          ...
          foreach ($content_groups_id as $value) {
            $content_group_id = $value->mobilecontentgroup_id;
            $select = $userlayoutcontentTable->select()->where('name =?', 'bottom')->where('mobilecontentgroup_id =?', $content_group_id);
            $user_bottom = $userlayoutcontentTable->fetchRow($select);
            if (!empty($user_bottom)) {
              $select = $userlayoutcontentTable->select()->where('name =?', 'middle')->where('mobilecontentgroup_id =?', $content_group_id)->where('parent_content_id =?', $user_bottom->mobilecontent_id);
              $user_middile = $userlayoutcontentTable->fetchRow($select);
              if ($user_middile) {
                foreach ($name as $name_value) {
                  $userlayoutcontentTable->update(array('parent_content_id' => $user_middile->mobilecontent_id), array('mobilecontentgroup_id = ?' => $content_group_id, 'name = ?' => $name_value, 'widget_admin = ?' => 1));
      ...
      if (empty($edit_layout_setting)) {
        ...
        if (empty($user_widgetlink)) {
          foreach ($content_groups_id as $value) {
            $content_group_id = $value->mobilecontentgroup_id;
            $userlayoutcontentTable->delete(array(
                'name =?' => 'sitegroup.widgetlinks-sitegroup', 'mobilecontentgroup_id =?' => $content_group_id));
          }
        }
      }
    ...
    if ($group_reload <= $reload_count) {
      ...
      $this->_forward('success', 'utility', 'core', array(
          ...
          'redirect' => $this->view->url(array('module' => 'sitegroup', 'controller' => 'mobile-layout', 'action' => 'update', 'group' => $group, 'group_reload' => $group_reload, 'admin_sitegroup_layout' => $newLayout), 'admin_default', true),
      ...
      $this->_forward('success', 'utility', 'core', array(
          'smoothboxClose' => false,
          'redirect' => $this->view->url(array('module' => 'sitegroup', 'controller' => 'mobile-layout', 'action' => 'layout', 'group' => $group), 'admin_default', true),
          'parentRedirectTime' => '10',
          //'format' => 'smoothbox',
  ...
  public function showHideWidgetAction() {
    ...
    if ($option == 1) {
      $contentgroupstable = Engine_Api::_()->getDbTable('mobileContentgroups', 'sitegroup');
      ...
      if (empty($admincontentrow)) {
        foreach ($contentgroupsrow as $value) {
          Engine_Api::_()->getDbTable('mobileContent', 'sitegroup')->delete(array('name =?' => $widgetname));
        }
      }
      if (empty($row)) {
