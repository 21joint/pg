...
class Sitegroup_DashboardController extends Core_Controller_Action_Standard {
    ...
    public function appAction() {
        ...
        //START THE GROUP CROWDFUNDING WORK
        $sitecrowdfundingEnabled = Engine_Api::_()->getDbtable('modules', 'core')->isModuleEnabled('sitecrowdfunding');
        if ($sitecrowdfundingEnabled && (Engine_Api::_()->hasModuleBootstrap('sitecrowdfundingintegration') && Engine_Api::_()->getDbtable('modules', 'sitecrowdfunding')->getIntegratedModules(array('enabled' => 1, 'item_type' => 'sitegroup_group', 'item_module' => 'sitegroup')))) {

            //PACKAGE BASE PRIYACY START
            if (Engine_Api::_()->sitegroup()->hasPackageEnable()) {
                if (Engine_Api::_()->sitegroup()->allowPackageContent($subject->package_id, "modules", "sitecrowdfunding")) {
                    $this->view->can_create_project = 1;
                }
            } else {
                $isGroupOwnerAllow = Engine_Api::_()->sitegroup()->isGroupOwnerAllow($subject, 'sprcreate');
                if (!empty($isGroupOwnerAllow)) {
                    $this->view->can_create_project = 1;
                }
            }
            //PACKAGE BASE PRIYACY END
            $this->view->projecttab_id = Engine_Api::_()->sitegroup()->GetTabIdinfo('sitecrowdfunding.contenttype-projects', $group_id, $layout);
            
        }
        //END THE GROUP CROWDFUNDING WORK
    ...
    public function getStartedAction() {
        ...
        if ($sitecrowdfundingEnabled && (Engine_Api::_()->hasModuleBootstrap('sitecrowdfundingintegration') && Engine_Api::_()->getDbtable('modules', 'sitecrowdfunding')->getIntegratedModules(array('enabled' => 1, 'item_type' => 'sitegroup_group', 'item_module' => 'sitegroup')))) {
            ...
            //PACKAGE BASE PRIYACY END
            $this->view->projecttab_id = Engine_Api::_()->sitegroup()->GetTabIdinfo('sitecrowdfunding.contenttype-projects', $page_id, $layout);
    ...
    public function chooseProjectAction() { 
        ...
        if (!$this->_helper->requireUser()->isValid())
            return;
        //DO NOT SHOW THE PROJECTS TAB IF ADMIN HAVE PLACED ON THE GROUP PROFILE PAGE
        ...
        $this->view->group_id = $group_id = $this->_getParam('group_id');
        $this->view->sitegroup = $sitegroup = Engine_Api::_()->getItem('sitegroup_group', $group_id); 
        $this->view->parent_type = $sitegroup->getType();
        $this->view->owner_id = $sitegroup->getOwner()->getIdentity();
        //SET PAGE SUBJECT 
        if (!$sitegroup->authorization()->isAllowed($viewer, 'edit')) {
            return $this->_forwardCustom('requireauth', 'error', 'core');
        } 
        $this->view->form = $form = new Sitecrowdfunding_Form_ChooseProjectContentModule(array('item' => $sitegroup)); 
        //CHECK METHOD
        if (!$this->getRequest()->isPost()) {
            return;
        } 
