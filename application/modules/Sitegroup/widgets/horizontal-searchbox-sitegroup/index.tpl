...
<?php if ($this->loaded_by_ajax): ?>
    <script type="text/javascript">
        var params = {
            ...
            responseContainer: $$('.layout_sitegroup_horizontal_searchbox_sitegroup')
        }
        en4.sitegroup.ajaxTab.attachEvent('<?php echo $this->identity ?>', params);
    </script>
<?php endif; ?>

<?php $this->headLink()->prependStylesheet($this->layout()->staticBaseUrl . 'application/modules/Sitegroup/externals/styles/style_sitegroup.css'); ?>

<?php if ($this->showContent): ?>
<div class="sitegroup_form_quick_search">
    <?php echo $this->form->setAttrib('class', 'sitegroup-search-box')->render($this) ?>
</div>	
<?php endif; ?>
...
<script type="text/javascript">
    ...
    var doSearching = function(searchboxcategory_id) {
        ...
        if (searchboxcategory_id != 0) {
            ...
            var categoriesArray = <?php echo Zend_Json_Encoder::encode(Engine_Api::_()->getDbTable('categories', 'sitegroup')->getCategoriesDetails($this->categoriesLevel)); ?>;
    ...
    en4.core.runonce.add(function()
    {
        var item_count = 0;
        if($('titleAjax')) {
        var contentAutocomplete = new Autocompleter.Request.JSON('titleAjax', '<?php echo $this->url(array('action' => 'get-search-events'), "sitegroup_general", true) ?>', {
            'postVar': 'text',
            'minLength': 1,
            'selectMode': 'pick',
            ...
            'injectChoice': function(token) {
                if (typeof token.label != 'undefined') {
                    if (token.sitegroup_url != 'seeMoreLink') {
                        var choice = new Element('li', {'class': 'autocompleter-choices', 'html': token.photo, 'id': token.label, 'sitegroup_url': token.sitegroup_url, onclick: 'javascript:getPageResults("' + token.sitegroup_url + '")'});
                    ...
                    if (token.sitegroup_url == 'seeMoreLink') {
                        ...
                        var choice = new Element('li', {'class': 'autocompleter-choices1', 'html': '', 'id': 'stopevent', 'sitegroup_url': ''});
            ...
            window.addEvent('keyup', function(e) {
                if (e.key == 'enter') {
                    if (selected.retrieve('autocompleteChoice') != 'null') {
                        var url = selected.retrieve('autocompleteChoice').sitegroup_url;
                        if (url == 'seeMoreLink') {
                            Seemore();
                        }
                        else {
...
//        });
    ...
    function Seemore() {
        ...
        var url = '<?php echo $this->url(array('action' => 'index'), "sitegroup_general", true); ?>';
