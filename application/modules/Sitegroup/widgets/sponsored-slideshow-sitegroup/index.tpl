...
<?php $this->headScript()->appendFile($this->layout()->staticBaseUrl.'application/modules/Sitegroup/externals/scripts/_class.noobSlide.packed.js'); ?>
<?php
...
$image_count = 1;
$priceEnable = Engine_Api::_()->getApi('settings', 'core')->getSetting('sitegroup.price.field', 1);
$locationEnable = Engine_Api::_()->sitegroup()->enableLocation();
foreach ($this->show_slideshow_object as $type => $item) {
  ...
  if ($priceEnable && $item->price) {
    $content_price = $this->translate("Price: ") . " " . Engine_Api::_()->sitegroup()->getPriceWithCurrency($item->price); 
  }

  $description = $item->body;

  $content_link = $this->htmlLink(Engine_Api::_()->sitegroup()->getHref($item->group_id, $item->owner_id, $item->getSlug()), $this->translate('View Group &raquo;'), array('class' => 'seaocore_slideshow_view_link'));
  ...
  $image_text_var .= "<div class='seaocore_slidebox'>";
  $image_text_var .= "<div class='seaocore_slidshow_img'>" . $itemPhoto . "</div>";


  if (!empty($content_info)) {
    $image_text_var .= "<div class='seaocore_slidshow_content'>";
  }
  if (!empty($item->title)) {
    $tmpBody = strip_tags($item->title);
    $item->title = ( Engine_String::strlen($tmpBody) > 45 ? Engine_String::substr($tmpBody, 0, 45) . '..' : $tmpBody );

    $image_text_var .='<h5>' . $this->htmlLink(Engine_Api::_()->sitegroup()->getHref($item->group_id, $item->owner_id, $item->getSlug()), $item->title) . '</h5>';
  }
  ...
  if (!empty($content_info)) {
    $image_text_var .= "<span class='seaocore_slidshow_info'>" . $content_info . "</span>";
  }
  $descriptionSize = 220;
  if (!empty($content_price)) {

    $descriptionSize-=30;
    $image_text_var .= "<span class='seaocore_slidshow_info'>" . $content_price . "</span>";
  }
  if (!empty($content_price)&& !empty($content_location)) {
     $image_text_var .="<span class='seaocore_slidshow_info'>, </span>";
  }
  if (!empty($content_location)) {
    $descriptionSize-=30;

    $image_text_var .= "<span class='seaocore_slidshow_info'>" . $content_location . "</span>";
  }
  ...
  if (!empty($description)) {
    ...
    $image_text_var .= "<p>" . $truncate_description . " " . $this->htmlLink(Engine_Api::_()->sitegroup()->getHref($item->group_id, $item->owner_id, $item->getSlug()), $this->translate('More &raquo;')) . "</p>";
  }

  $image_text_var .= "</div></div>";
}
if (!empty($this->sitegroup_sponsored)) {
  ?>
  <script type="text/javascript">
    window.addEvent('domready',function(){
      ...
      var width=$('global_content').getElement(".seaocore_slideshow_wrapper").clientWidth;
      $('global_content').getElement(".seaocore_slideshow_mask").style.width= (width-10)+"px";
      var divElements=document.getElementsByClassName('seaocore_slidebox');   
     for(var i=0;i < divElements.length;i++)
      ...
      var nS8 = new noobSlide({
        box: $('sitegroup_seaocore_te_advanced_box'),
        items: $$('#sitegroup_seaocore_te_advanced_box h3'),
        size: (width-10),
        handles: $$('#handles8 span'),
        addButtons: {previous: $('sitegroup_seaocore_prev8'), stop: $('sitegroup_seaocore_stop8'), play: $('sitegroup_seaocore_play8'), next: $('sitegroup_seaocore_next8') },
        interval: 5000,
        fxOptions: {
          duration: 500,
          transition: '',
        ...
        onWalk: function(currentItem,currentHandle){
          ...
          {
            $('sitegroup_seaocore_caption').innerHTML =   current_caption_link;
          }
          else {
            $('sitegroup_seaocore_caption').innerHTML =  '';
          }


          $('sitegroup_seaocore_current_numbering').innerHTML =  current_index + '/' + num_of_slidehsow ;
        }
      });
...
<div class="seaocore_slideshow_wrapper">
  <div class="seaocore_slideshow_mask">
    <div id="sitegroup_seaocore_te_advanced_box" class="seaocore_slideshow_advanced_box">
<?php echo $image_text_var ?>
    </div>
  </div>

  <div class="seaocore_slideshow_option_bar">
    <div>
      <p class="buttons">
        <span id="sitegroup_seaocore_prev8" class="seaocore_slideshow_controllers-prev seaocore_slideshow_controllers prev" title="<?php echo $this->translate('Previous'); ?> "></span>
        <span id="sitegroup_seaocore_stop8" class="seaocore_slideshow_controllers-stop seaocore_slideshow_controllers" title="<?php echo $this->translate('Stop'); ?> "></span>
        <span id="sitegroup_seaocore_play8" class="seaocore_slideshow_controllers-play seaocore_slideshow_controllers" title="<?php echo $this->translate('Play'); ?>"></span>
        <span id="sitegroup_seaocore_next8" class="seaocore_slideshow_controllers-next seaocore_slideshow_controllers" title="<?php echo $this->translate('Next'); ?> "></span>
      </p>
    </div>
    <span id="sitegroup_seaocore_caption"></span>
    <span id="sitegroup_seaocore_current_numbering" class="seaocore_slideshow_pagination"></span>
  </div>
</div>