...
foreach ($this->show_slideshow_object as $type => $item) {
  ...
  $content_link = $this->htmlLink(Engine_Api::_()->sitegroup()->getHref($item->group_id, $item->owner_id, $item->getSlug()), $this->translate('View Group &raquo;'), array('class' => 'featured_slideshow_view_link'));
  ...
  $image_text_var .= "<div class='featured_slidebox featured_slidebox_" . $this->identity . "'>";
...
if (!empty($this->sitegroup_featured)) {
  ?>
    ...
    window.addEvent('domready',function(){
        ...
        document.getElementsByClassName = function(className)
        {
          ...
          return results;
			...
			var width=$('global_content').getElement(".featured_slideshow_wrapper_<?php echo $this->identity ?>").clientWidth;
      $('global_content').getElement(".featured_slideshow_mask_<?php echo $this->identity ?>").style.width= (width-10)+"px";
			var divElements=document.getElementsByClassName('featured_slidebox_<?php echo $this->identity ?>');
      ...
      var nS8 = new noobSlide({
        box: $('sitegroup_featured_te_advanced_box'),
        items: $$('#sitegroup_featured_te_advanced_box h3'),
        size: (width-10),
        handles: $$('#handles8 span'),
        addButtons: {previous: $('sitegroup_featured_prev8'), stop: $('sitegroup_featured_stop8'), play: $('sitegroup_featured_play8'), next: $('sitegroup_featured_next8') },
        interval: 5000,
        fxOptions: {
          duration: 500,
          transition: '',
        ...
        onWalk: function(currentItem,currentHandle){
          ...
          {
            $('sitegroup_featured_caption').innerHTML =   current_caption_link;
          }
          else {
            $('sitegroup_featured_caption').innerHTML =  '';
          }


          $('sitegroup_featured_current_numbering').innerHTML =  current_index + '/' + num_of_slidehsow ;
        }
      });
...
<div class="featured_slideshow_wrapper featured_slideshow_wrapper_<?php echo $this->identity ?>">
  <div class="featured_slideshow_mask featured_slideshow_mask_<?php echo $this->identity ?>">
    <div id="sitegroup_featured_te_advanced_box" class="featured_slideshow_advanced_box">
<?php echo $image_text_var ?>
  ...
  <div class="featured_slideshow_option_bar">
    <div>
      <p class="buttons">
        <span id="sitegroup_featured_prev8" class="featured_slideshow_controllers-prev featured_slideshow_controllers prev" title="<?php echo $this->translate('Previous'); ?> "></span>
        <span id="sitegroup_featured_stop8" class="featured_slideshow_controllers-stop featured_slideshow_controllers" title="<?php echo $this->translate('Stop'); ?> "></span>
        <span id="sitegroup_featured_play8" class="featured_slideshow_controllers-play featured_slideshow_controllers" title="<?php echo $this->translate('Play'); ?>"></span>
        <span id="sitegroup_featured_next8" class="featured_slideshow_controllers-next featured_slideshow_controllers" title="<?php echo $this->translate('Next'); ?> "></span>
      </p>
    </div>
    <span id="sitegroup_featured_caption"></span>
    <span id="sitegroup_featured_current_numbering" class="featured_slideshow_pagination"></span>
  </div>
</div>