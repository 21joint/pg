...
en4.core.runonce.add(function()
    {
        var item_count = 0;
        var contentAutocomplete = new Autocompleter.Request.JSON('search', '<?php echo $this->url(array('action' => 'get-search-groups'), "sitegroup_general", true) ?>', {
            'postVar': 'text',
            'minLength': 1,
            'selectMode': 'pick',
            ...
            'injectChoice': function(token) {
                if (typeof token.label != 'undefined') {
                    if (token.group_url != 'seeMoreLink') {
                        var choice = new Element('li', {'class': 'autocompleter-choices', 'html': token.photo, 'id': token.label, 'group_url': token.group_url, onclick: 'javascript:getGroupResultsSearch("' + token.group_url + '")'});
                    ...
                    if (token.group_url == 'seeMoreLink') {
                        ...
                        var choice = new Element('li', {'class': 'autocompleter-choices1', 'html': '', 'id': 'stopevent', 'group_url': ''});
            ...
            window.addEvent('keyup', function(e) {
                if (e.key == 'enter') {
                    if (selected.retrieve('autocompleteChoice') != 'null') {
                        var url = selected.retrieve('autocompleteChoice').group_url;
                        if (url == 'seeMoreLink') {
                            SeemoreSearch();
                        }
                        else {
   ...
   function SeemoreSearch() {
    ...
    function getGroupResultsSearch(url) {
        if (url != 'null') {
            if (url == 'seeMoreLink') {
                SeemoreSearch();
            }
...
<div class="seaocore_searchform_criteria <?php if($this->viewType == 'horizontal'): ?>seaocore_searchform_criteria_horizontal<?php endif; ?>">
	<?php if ($this->sitegroup_post == 'enabled') {
...
<script>
  en4.core.runonce.add(function() {
    if ($('sitegroup_location')) {
      var params = {
        'detactLocation': <?php echo $this->locationDetection; ?>,
        'fieldName': 'sitegroup_location',
        'noSendReq': 1,
  ...
  locationAutoSuggest('<?php echo Engine_Api::_()->getApi('settings', 'core')->getSetting('seaocore.countrycities'); ?>', 'sitegroup_location', 'sitegroup_city');
