...
//				$versionInfo = 0;
                ...
                if (!empty($running_version) && !empty($product_version)) {
                    ...
                    <tr>
                        ...
                        <td>
                            <?php
                            $url = $this->url(array('module' => 'seaocore', 'controller' => 'settings', 'action' => 'upgrade-plugin', 'name' => @base64_encode($item['name']), 'version' => $item['product_version'], 'ptype' => $item['ptype'], 'key' => $item['key'], 'title' => str_replace("/", "_", @base64_encode($item['title'])), 'calling' => 'sitegroup'), 'admin_default', true);
...
<?php if (empty($this->OnSiteModules)) : ?>
    <div class="tip">
        <span>
            <?php echo $this->translate('There are no extensions of "Group Plugin" available on your site. To see the available extensions for this plugin, please %1$sclick here%2$s', '<a href="http://www.socialengineaddons.com/catalog/directory-groups-extensions" target="_blank">', '</a>.'); ?>
        </span>
    </div>
<?php endif; ?>