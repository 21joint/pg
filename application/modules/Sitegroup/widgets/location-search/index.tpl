...
?>
<?php
//if(empty($this->sitegroup_post)){return;}
...
?>
<?php if( $this->form ): ?>
  <div class="global_form_box sitegroup_advanced_search_form">
    <?php echo $this->form->render($this) ?>
  </div>
  <div class="" id="group_location_pops_loding_image" style="display: none;">
  ...
  var myLatlng;
	window.addEvent('domready', function() {
	  ...
	  if(document.getElementById('sitegroup_location')) {
			if(document.getElementById('sitegroup_location').value == '') {
				submiForm();
			} 
		} else {
			submiForm();
	  ...
	  $('group_location_pops_loding_image').injectAfter($('done-element'));
    ...
    if (navigator.geolocation) {
      navigator.geolocation.getCurrentPosition(function(position){
			...
			mapGetDirection = new google.maps.Map(document.getElementById("sitegroup_location_map_none"), myOptions);
        ...
        if(!position.address) {
          ...
          service.search(request, function(results, status) { 
            if (status  ==  'OK') {
              ...
              document.getElementById('sitegroup_location').value = (results[index].vicinity) ? results[index].vicinity :'';
          ...
          document.getElementById('sitegroup_location').value = location;
					document.getElementById('Latitude').value = lat;
					document.getElementById('Longitude').value = lng;
					document.getElementById('locationmiles').value = 1000;
          //form submit by ajax
	...
	function submiForm() {
		...
		document.getElementById('group_location_pops_loding_image').style.display ='';
		en4.core.request.send(new Request.HTML({
			method : 'post',
			'url' : url,
			'data' : param,
			onSuccess : function(responseTree, responseElements, responseHTML, responseJavaScript) {
				document.getElementById('group_location_pops_loding_image').style.display ='none';
        if($('sitegroup_map_container_topbar')) {
            $('sitegroup_map_container_topbar').style.display ='block';
        }
        if(document.getElementById('sitegroup_location_map_anchor')) {
            document.getElementById('sitegroup_location_map_anchor').getParent().innerHTML = responseHTML;
        }
				setMarker();
			  en4.core.runonce.trigger();
        if($('sitegroup_map_container'))
				$('sitegroup_map_container').style.visibility = 'visible'; 
	...
	function locationGroup() {
		var  sitegroup_location = document.getElementById('sitegroup_location');
		
// 		if (document.getElementById('sitegroup_location').value) {
// 			document.getElementById('sitegroup_location') = 0; 
// 	  }
	...
	function advancedSearchSitegroups() {
		...
		if (flag == 0) {
		  if ($('fieldset-grp2'))
			...
			if ($('sitegroup_street'))
			$('sitegroup_street').value = '';
// 			if ($('sitegroup_postalcode'))
// 			$('sitegroup_postalcode').value = '';
			if ($('sitegroup_country'))
			$('sitegroup_country').value = '';
			if ($('sitegroup_state'))
			$('sitegroup_state').value = '';
			if ($('sitegroup_city'))
			$('sitegroup_city').value = '';
			if ($('profile_type'))
			$('profile_type').value = '';
...
//     var contentAutocomplete = new Autocompleter.Request.JSON('sitegroup_location', '<?php //echo $this->url(array('module' => 'sitegroupmember', 'controller' => 'widgets', 'action' => 'getitem'), 'admin_default', true) ?>', {
//       'postVar' : 'text',
//       'minLength': 1,
//       'selectMode': 'pick',
