...
?>
<?php if ($this->form): ?>
  <?php echo $this->form->setAttrib('class', 'global_form_box sitegroup_advanced_search_form horizontal_search_' . $this->identity)->render($this) ?>
...
<script type="text/javascript">
  ...
  window.addEvent('domready', function() {
    ...
    $$('.<?php echo 'horizontal_search_' . $this->identity ?>').addEvent('submit',function(event){
		 if($('sitegroup_location') && !$('sitegroup_location').get('placeholder')  && $('sitegroup_location').value == "<?php echo $this->string()->escapeJavascript($this->translate('Enter a location'))?>"){
          $('sitegroup_location').value ='';
      }
