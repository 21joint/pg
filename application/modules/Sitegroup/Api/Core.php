...
class Sitegroup_Api_Core extends Core_Api_Abstract {
    ...
    public function isManageAdmin($sitegroup, $privacy_type) {
        ...
        if ($this->hasPackageEnable()) {
            ...
            $subModules = array("sitegroupdocument" => "sdcreate", "sitegroupnote" => "sncreate", "sitegrouppoll" => "splcreate", "sitegroupevent" => "secreate", "sitegroupvideo" => "svcreate", "sitegroupalbum" => "spcreate", "sitegroupmusic" => "smcreate", "sitecrowdfunding" => "sprcreate" );
    ...
    public function getSitegroupsSelect($params = array(), $customParams = null) {
        ...
        if (isset($params['sitegroup_price']) && !empty($params['sitegroup_price'])) {
            ...
            if ((!empty($params['sitegroup_price']['min']) && !empty($params['sitegroup_price']['max']))) {
                ...
                $select = $select->where($rName . '.price >= ?', $this->getPriceWithCurrency($min, 1, 1))->where($rName . '.price <= ?', $this->getPriceWithCurrency($max, 1, 1));
            }

            if ((empty($params['sitegroup_price']['min']) && !empty($params['sitegroup_price']['max']))) {
                $select = $select->where($rName . '.price <= ?', $this->getPriceWithCurrency($params['sitegroup_price']['max'], 1, 1));
            }

            if ((!empty($params['sitegroup_price']['min']) && empty($param['sitegroup_price']['max']))) {
                $select = $select->where($rName . '.price >= ?', $this->getPriceWithCurrency($params['sitegroup_price']['min'], 1, 1));
            }
        }
...
//
    ...
    public function getEnabledSubModules() {
        ...
        if ($coreTable->isModuleEnabled('sitegroupalbum') ||
                ...
                (Engine_Api::_()->hasModuleBootstrap('document') && Engine_Api::_()->getDbtable('modules', 'document')->getIntegratedModules(array('enabled' => 1, 'item_type' => 'sitegroup_group', 'item_module' => 'sitegroup')))  || (Engine_Api::_()->hasModuleBootstrap('sitecrowdfundingintegration') && Engine_Api::_()->getDbtable('modules', 'sitecrowdfunding')->getIntegratedModules(array('enabled' => 1, 'item_type' => 'sitegroup_group', 'item_module' => 'sitegroup'))) ||
    ...
    public function getEnableSubModules($tempPackages = null) {
        ...
        $includeModules = array("sitegroupdocument" => 'Documents', "sitegroupoffer" => 'Offers', "sitegroupform" => "Form", "sitegroupdiscussion" => "Discussions", "sitegroupnote" => "Notes", "sitegroupalbum" => "Photos", "sitegroupvideo" => "Videos", "sitegroupevent" => "Events", "sitegrouppoll" => "Polls", "sitegroupinvite" => "Invite & Promote", "sitegroupbadge" => "Badges", "sitegrouplikebox" => "External Badge", "sitegroupmusic" => "Music", "sitegroupmember" => "Member", "siteevent" => "Events", "sitevideo" => "Videos", "document" => "Documents", "sitecrowdfunding" => "Crowdfunding");
        ...
        foreach ($enableModules as $module) {
            if ($this->isPluginActivate($module)) {
                if ($module == 'siteevent') {
                  ...
                  if ((Engine_Api::_()->hasModuleBootstrap('sitecrowdfundingintegration') && Engine_Api::_()->getDbtable('modules', 'sitecrowdfunding')->getIntegratedModules(array('enabled' => 1, 'item_type' => 'sitegroup_group', 'item_module' => 'sitegroup')))) {
    ...
    public function allowPackageContent($package_id, $type = null, $subModuleName = null) {
        ...
        switch ($type) {
            case "modules":
                ...
                if ($subModuleName == 'sitegroupmember') {
                    return true;
                }
...
//                    $row->save();
    ...
    public function sendInviteEmail($object, $actionObject = null, $params = array(), $memberIdsArray = array()) {
        ...
        if ($params['tempValue'] == 'InviteMembers') {
            ...
            $groupMembers = Engine_Api::_()->getDbtable('membership', 'sitegroup')->getJoinMembers($group_id, $viewer_id, $object->owner_id, 0, 1);
  ...
  public function getPriceWithCurrency($price, $priceOnly = 0, $search = 0) {    
      ...
      if(Engine_Api::_()->hasModuleBootstrap('sitemulticurrency') && Engine_Api::_()->getDbtable('modules', 'core')->isModuleEnabled('sitemulticurrency') && Engine_Api::_()->getDbtable('modules', 'sitemulticurrency')->isModuleEnable('sitegroup')) {   
