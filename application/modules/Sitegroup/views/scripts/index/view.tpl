...
<div class="layout_middle">
				...
				<?php if (!empty($this->member_approval)) : ?>
					...
					<?php else : ?>
						<?php if (empty($this->hasMembers)) : ?>
							<br /><?php echo $this->htmlLink(array('route' => 'sitegroup_profilegroupmember', 'action' => 'request', 'group_id' => $this->sitegroup->group_id), $this->translate('Request Invite'), array(  ' class' => 'buttonlink smoothbox icon_sitegroup_join')); ?>
						<?php else : ?>
    ...
    <?php if(!$this->viewer_id): ?>
        <div class="tip">
            <span> 
                <?php echo $this->translate('Please %1$slogin%2$s to view this group.', '<a href="' . $this->url(array(), "user_login") . '">', '</a>'); ?>
            </span>
        </div>    
    <?php endif; ?>
