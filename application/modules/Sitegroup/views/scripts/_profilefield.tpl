  ...
  <div class="form-element">
    <ul class="form-options-wrapper">
      <li style="clear:both; margin-bottom:10px;">
        <?php
        $options = Engine_Api::_()->getDBTable('options', 'sitegroup')->getAllProfileTypes();
        foreach ($options->toarray() as $opt) :
          $selectOption = Engine_Api::_()->getDBTable('metas', 'sitegroup')->getFields($opt['option_id']);
        
         ?>
          <div class="form-label"  id="profile_<?php echo $opt['option_id'] ?>" >
            <label style="font-weight: bold;">
              <a href="javascript:void(0);" onclick="profile_show('<?php echo $opt['option_id'] ?>')" >  <span id="image_<?php echo $opt['option_id'] ?>" > <?php echo  $this->htmlImage('application/modules/Sitegroup/externals/images/icons/plus16.gif', '', array('title'=> $this->translate('Show Profile Fields.'))) ?></span> </a>
...
<script type="text/javascript">
  function profile_show(id) {
    ...
    if($(profile_fields_id).style.display == 'block') {
      ...
      $(image_id).innerHTML='<?php echo  $this->htmlImage('application/modules/Sitegroup/externals/images/icons/plus16.gif', '', array('title'=> $this->translate('Show Profile Fields.'))) ?>';
    } else {
      $(profile_fields_id).style.display = 'block';
      $(image_id).innerHTML='<?php echo  $this->htmlImage('application/modules/Sitegroup/externals/images/icons/minus16.gif', '', array('title'=> $this->translate('Hide Profile Fields.'))) ?>';
