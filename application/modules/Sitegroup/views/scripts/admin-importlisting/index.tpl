...
<script type="text/javascript">
    ...
    var import_confirmation =  confirm('<?php echo $this->string()->escapeJavascript($this->translate("Are you sure you want to start importing Groups ?")) ?>');
		...
		if(import_confirmation) {
			...
			Smoothbox.open("<div><center><b>" + '<?php echo $this->string()->escapeJavascript($this->translate("Importing Groups...")) ?>' + "</b><br /><img src='"+en4.core.staticBaseUrl+"application/modules/Sitegroup/externals/images/loader.gif' alt='' /></center></div>");

			en4.core.request.send(new Request.JSON({
				url : en4.core.baseUrl+'admin/sitegroup/importlisting',
				method: 'get',
				data : {
					'start_import' : 1,
...
<?php echo $this->htmlLink(array('route' => 'admin_default', 'module' => 'sitegroup', 'controller' => 'log', 'action' => 'index'), $this->translate('Import History'), array('class'=> 'buttonlink icon_sitegroups_log')) ?>

<?php echo $this->htmlLink(array('route' => 'admin_default', 'module' => 'sitegroup', 'controller' => 'importlisting', 'action' => 'manage'), $this->translate('Manage CSV Import Files'), array('class'=> 'buttonlink icon_sitegroup_admin_import_manage')) ?>
  
<?php echo $this->htmlLink(array('route' => 'admin_default', 'module' => 'sitegroupmember', 'controller' => 'importgroup', 'action' => 'index'), $this->translate('Import official SocialEngine Groups into Groups'), array('class'=> 'buttonlink icon_sitegroup_admin_import_manage')) ?><br/><br/>  

<?php if($this->first_listing_id && 0): ?>
	<div class="importlisting_form">
		<div>
			<h3><?php echo $this->translate('Import Listings into Groups');?></h3>
			<p>
				<?php echo $this->translate("This Importing tool is designed to migrate content directly from a Listing to a Group. Using this, you can convert all the listings on your site into Groups. Please note that we try to import all the data corresponding to a Listing but there is a possibility of some data losses too.<br />Below are the conditions which are required to be true for this import. Please check the points carefully and if some condition is yet to be fulfilled then do that first and then start importing your Listings. Once the import gets started, it is recommended not to close the lightbox, otherwise it will not be completed successfully and some data losses may occur.");?>
			</p>
			...
			<?php if(!Engine_Api::_()->getDbtable('modules', 'core')->isModuleEnabled('sitegroupvideo') && Engine_Api::_()->getApi('settings', 'core')->getSetting('sitegroupvideo.isActivate') || !Engine_Api::_()->getDbtable('modules', 'core')->isModuleEnabled('video')): ?>
				<div class="tip">
					<span>
						<?php  echo $this->translate("Groups/Communities - Videos Extension or Videos Plugin is either not installed or disabled at your site. So, this import tool will not be able to import the data corresponding to Listing Videos.") ?>
					</span>
				</div>
			<?php endif; ?>

			<?php if(!Engine_Api::_()->getDbtable('modules', 'core')->isModuleEnabled('sitegroupreview') && Engine_Api::_()->getApi('settings', 'core')->getSetting('sitegroupreview.isActivate')): ?>
				<div class="tip">
					<span>
						<?php  echo $this->translate("Groups/Communities - Reviews and Rating Extension Plugin is not installed or disabled at your site, so listing import functionality will not import the listing review datas.") ?>
					</span>
				</div>
			<?php endif; ?>
			...
			<div class="importlisting_elements" id="importlisting_elements" >
				<?php 
					...
					if(Engine_Api::_()->getDbtable('modules', 'core')->isModuleEnabled('sitegroupreview') && Engine_Api::_()->getApi('settings', 'core')->getSetting('sitegroupreview.isActivate')) {
						$error_msg1 = $this->translate("Groups/Communities - Reviews and Ratings Extension Plugin is installed and enabled!");
						echo "<span class='green'><img src='".  $this->layout()->staticBaseUrl . "application/modules/Sitegroup/externals/images/tick.png'/><b>$error_msg1</b></span>";
					}
			
					if(Engine_Api::_()->getDbtable('modules', 'core')->isModuleEnabled('sitegroupvideo') && Engine_Api::_()->getApi('settings', 'core')->getSetting('sitegroupvideo.isActivate') && Engine_Api::_()->getDbtable('modules', 'core')->isModuleEnabled('video')) {
						$error_msg1 = $this->translate("Groups/Communities - Videos Extension and Videos Plugin is installed and enabled!");
						echo "<span class='green'><img src='". $this->layout()->staticBaseUrl . "application/modules/Sitegroup/externals/images/tick.png'/><b>$error_msg1</b></span>";
					}
...
<div class="importlisting_form">
	<div>
		<h3><?php echo $this->translate('Import Groups from a CSV file');?></h3>

		<p>
		 <?php echo "This tool allows you to import Groups corresponding to the entries from a .csv file. Here, you can also generate your own .csv template file by selecting the Profile Fields to be included in it by using the “Generate new CSV template file” link below. Before starting to use this tool, please read the following points carefully. ";?>
		</p>
		...
		<ul class="importlisting_form_list">
			...
			<li>
				<?php echo $this->translate("Group title and category name are the required fields for all the entries in the file.");?>
			</li>

			<li>
				<?php echo $this->translate("For making groups claimable, admin should be added as 'Claimable Group Creators' from");?>
				<a href="<?php echo $this->url(array('module' => 'sitegroup', 'controller' => 'claim', 'action' => 'index'), 'admin_default', true) ?>"><?php echo $this->translate(" Manage Claims ") ?></a>
				<?php echo $this->translate("tab. Claim functionality also depends on Member level settings and Global settings. 'Claim a Group' value should be 1 or 0 in your csv file. Value 1 indicates the group is claimable and 0 indicates the group is not claimable.");?>
			</li>

			<li>
				<?php echo $this->translate("Category and sub-category name should exactly match with the existing categories and sub-categories, otherwise category or sub-category will be considered as null for that Group.");?>
			</li>
    <li>
      <?php echo $this->translate('Now, you can also import profile pictures for groups via the CSV file by following method:<br />
  a) Make a zipped folder containing all the photos to be uploaded and mention the name of the pictures in the csv file with associated group to be imported. You can later upload the folder from the “Manage CSV Import Files” section. [Note: To allow photos folder to be uploaded on your site successfully, please make sure that the php_zip extension is installed on your server.]');?>
    </li>
			<li>
			...
			<li>
				<?php echo $this->translate("Before starting the importing process, it is recommended that you should create category, Profile Fileds and do Category-Profile mappings from the 'Category-Group Profile Mapping' section.");?>
			</li>

			<li>
				<?php echo $this->translate("You can import the maximum of 2000 groups at a time and if you want to import more, you would have to then repeat the whole process. For example, you have to import 3500 groups. Then, you would have to create 2 CSV files - one having 2000 entries and another having 1500 entries corresponding to the Groups. After that, just upload both the files and import groups from them one by one from 'Manage CSV Import Files' section.");?>
			</li>

			<li>
				<?php echo $this->translate("You can also 'Stop' and 'Rollback' the import process. 'Stop' will just stop the import process going on at that time from that file and 'Rollback' will undo or delete all the Groups created from that CSV import file till that time.");?>
			</li>
		...
		<a href=<?php echo $this->url(array('action' => 'download-sample')) ?><?php echo '?path=' . urlencode('example_group_import.csv');?> target='downloadframe' class="buttonlink icon_sitegroups_download_csv"><?php echo $this->translate('Download the CSV template')?></a>
  
  <a href="<?php echo $this->url(array('module' => 'sitegroup', 'controller' => 'fields', 'action' => 'show-customfields')) ?>" class="buttonlink icon_sitegroup_generate_csv"><?php echo $this->translate('Generate new CSV template file')?></a> 
		
		<?php echo $this->htmlLink(array('route' => 'default', 'module' => 'sitegroup', 'controller' => 'admin-importlisting', 'action' => 'import'), $this->translate('Import a file'), array('class' => 'smoothbox buttonlink icon_sitegroups_import')) ?>
