...
<?php if ($this->status == 'pending'): // Check for pending status ?>
  <?php echo $this->translate('Your group is pending payment. You will receive an email when the
		payment completes.'); ?>
<?php else: ?>

  <form method="get" action="<?php echo $this->escape($this->url(array(), "sitegroup_process_payment", true)) ?>"
    ...
    <div>
      <div>
            ...
            <br/><br/>      
        <h3>
          <?php echo $this->translate('Order your Group') ?>
        </h3>
        <p class="form-description">
          <?php echo $this->translate('You have created a group that requires payment. You will be taken to a secure checkout area where you can pay for your group.') ?>
        </p>
        <p style="font-weight: bold; padding-top: 15px; padding-bottom: 15px;max-width:none;">
          <?php if ($this->package->recurrence): ?>
            <?php echo $this->translate('Your Group requires payment:') ?>
          <?php else: ?>
        ...
        function applyCreditcode() {
            ...
            en4.core.request.send(new Request.JSON({
              ...
              data: {
                ...
                package_type: 'sitegroup_package',
                package_id:<?php echo $this->group->package_id ?>,
              },
              onRequest: function(){
