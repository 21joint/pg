  ...
  <div class="seaocore_popup_top">
    <div class="seaocore_popup_des">
      <?php if ($this->album_id || $this->recentAdded): ?>
        <b><?php echo $this->translate("Choose Group Cover Photo") ?></b>
      <?php else: ?>
         <b><?php echo $this->translate("Select Group Album to choose Group Cover Photo") ?></b>
      <?php endif; ?>
    </div>
  </div>
  <?php if ($this->album_id || $this->recentAdded): ?>
    <div class="seaocore_popup_options">
      <div class="seaocore_popup_options_left">
        <b><?php echo $this->album_id ? $this->translate("%s's Photos", $this->album->getTitle()) : $this->translate("Recent Group Photos"); ?></b>
      </div>
      <div class="fright"><a href="<?php echo $this->url(array('action' => 'get-albums-photos', 'group_id' => $this->sitegroup->group_id, 'format' => 'smoothbox'), 'sitegroup_dashboard', true); ?>"><b><?php echo $this->translate("View Albums") ?></b></a></div>
    </div>
  <?php endif; ?>
    ...
    <div class="seaocore_popup_content_inner">
      <?php if ($this->album_id || $this->recentAdded): ?>
        <?php if (count($this->paginator) > 0) : ?>
					<div class="clr sitegroup_choose_cover_content">
          	<ul class="thumbs">
            <?php foreach ($this->paginator as $photo): ?>
							<li> 
								<a href="<?php echo $this->url(array('action' => 'upload-cover-photo', 'group_id' => $this->sitegroup->group_id, 'photo_id' => $photo->photo_id, 'format' => 'smoothbox'), 'sitegroup_dashboard', true); ?>" title="<?php echo $photo->title; ?>" class="thumbs_photo">
            ...
            <?php foreach ($this->paginator as $albums): ?>
              ...
              <li> 
                <?php if ($albums->photo_id != 0): ?>
                  <a href="<?php echo $this->url(array('action' => 'get-albums-photos', 'group_id' => $this->sitegroup->group_id, 'album_id' => $albums->album_id, 'format' => 'smoothbox'), 'sitegroup_dashboard', true); ?>" title="<?php echo $albums->title; ?>" class="thumbs_photo">
                    <span style="background-image: url(<?php echo $albums->getPhotoUrl('thumb.normal'); ?>);"></span></a>
                <?php else: ?>
                  <a href="<?php echo $this->url(array('action' => 'get-albums-photos', 'group_id' => $this->sitegroup->group_id, 'album_id' => $albums->album_id, 'format' => 'smoothbox'), 'sitegroup_dashboard', true); ?>" class="thumbs_photo" title="<?php echo $albums->title; ?>" >
