  ...
  <div class="top">
    <?php if ($this->album_id || $this->recentAdded): ?>
      <div class="view-album">
        ...
        <a class="ui-link-inhirit" href="<?php echo $this->url(array('action' => 'get-albums-photos', 'group_id' => $this->sitegroup->group_id, 'special' => 'cover'), 'sitegroup_profilegroupmobile', true); ?>"><b><?php echo $this->translate("View Albums") ?></b></a>
      </div> 
    <?php endif; ?>
      ...
      <?php if ($this->paginator && $this->paginator->getTotalItemCount() > 0) : ?>
        <ul class="thumbs thumbs_nocaptions">
          <?php foreach ($this->paginator as $photo): ?>
            <li> 
              <a data-ajax="false" href="<?php echo $this->url(array('action' => 'upload-cover-photo', 'group_id' => $photo->group_id, 'photo_id' => $photo->photo_id, 'special' => 'cover'), 'sitegroup_profilegroupmobile', true); ?>" title="<?php echo $photo->title; ?>">
          ...
          <?php foreach ($this->paginator as $albums): ?>
            ...
            <li class="sm-ui-browse-items"> 
              <?php if ($albums->photo_id != 0): ?>
                <a data-ajax="false" href="<?php echo $this->url(array('group_id' => $this->sitegroup->group_id, 'album_id' => $albums->album_id, 'slug' => $albums->getSlug(), 'tab' => $this->identity_temp, 'special' => 'cover'), 'sitegroup_profilegroupmobile') ?>">
              ...
              <?php else: ?>
                <a data-ajax="false" href="<?php echo $this->url(array('action' => 'get-albums-photos', 'group_id' => $this->sitegroup->group_id, 'album_id' => $albums->album_id, 'special' => 'cover'), 'sitegroup_profilegroupmobile', true); ?>">
                  <p class="ui-li-aside-show ui-li-aside" style="display:none;"><?php echo $this->locale()->toNumber($albums->count())?></p>
                  <img src="<?php echo $albums->getPhotoUrl('thumb.icon'); ?>" />
                  <div class="ui-list-content">
                    <h3><?php echo $this->string()->chunk($this->string()->truncate($albums->getTitle(), 45), 10); ?></h3>
