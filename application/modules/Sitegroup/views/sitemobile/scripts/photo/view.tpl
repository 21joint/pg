...
$this->headScriptSM()
        ->appendFile($this->layout()->staticBaseUrl . 'application/modules/Sitegroupalbum/externals/scripts/sitemobile/core.js');
?>
<script type="text/javascript"> 
  ...
  sm4.core.runonce.add(function(){
    ...
    sm4.core.Module.autoCompleter.attach("tags_<?php echo $this->subject()->getGuid() ?>", '<?php echo $this->url(array('module' => 'sitegroup', 'controller' => 'photo', 'action' => 'suggest','sendNow' => true, 'includeSelf' => true, 'group_id' => $this->sitegroup->group_id), 'default', true) ?>', {'singletextbox': false, 'limit':10, 'minLength': 1, 'showPhoto' : true, 'search' : 'search'}, "toValues_<?php echo $this->subject()->getGuid() ?>");
  ...
  <?php endif; ?>
	<?php if ($this->canEdit): ?> 
		<div class="albums_viewmedia_info_actions">
			<a class="buttonlink icon_photos_rotate_ccw" href="javascript:void(0)" onclick="$(this).attr('class', 'buttonlink icon_loading');sm4.sitegroupalbum.rotate(<?php echo $this->image->getIdentity() ?>, 90, $(this), 'buttonlink icon_photos_rotate_ccw', sm4.core.baseUrl + 'sitegroup/photo/rotate');">&nbsp;</a>
			<a class="buttonlink icon_photos_rotate_cw" href="javascript:void(0)" onclick="$(this).attr('class', 'buttonlink icon_loading');sm4.sitegroupalbum.rotate(<?php echo $this->image->getIdentity() ?>, 270, $(this), 'buttonlink icon_photos_rotate_cw', sm4.core.baseUrl + 'sitegroup/photo/rotate');">&nbsp;</a>
			<a class="buttonlink icon_photos_flip_horizontal" href="javascript:void(0)" onclick="$(this).attr('class', 'buttonlink icon_loading');sm4.sitegroupalbum.flip(<?php echo $this->image->getIdentity() ?>, 'horizontal', $(this), 'buttonlink icon_photos_flip_horizontal', sm4.core.baseUrl + 'sitegroup/photo/flip');">&nbsp;</a>
			<a class="buttonlink icon_photos_flip_vertical" href="javascript:void(0)" onclick="$(this).attr('class', 'buttonlink icon_loading');sm4.sitegroupalbum.flip(<?php echo $this->image->getIdentity() ?>, 'vertical', $(this), 'buttonlink icon_photos_flip_vertical', sm4.core.baseUrl + 'sitegroup/photo/flip');">&nbsp;</a>
		</div>
	<?php endif; ?>
	...
	<?php if ($this->canTag): ?>
		<div id="tagit_<?php echo $this->subject()->getGuid() ?>" style="display:none;">
			<div class="global_form_popup sm-ui-photo-add-tag-group">
				<div id="tags_<?php echo $this->subject()->getGuid() ?>-wrapper">
					...
					<div class="ui-group-content sm-photo-form-buttons">
...
<script type="text/javascript">
	$(document).bind( "groupinit", function( event, data ) {  
    var $group = $( event.target );   
		$group.find("#media_photo_div").on( "swipeleft swiperight",  function( event ) {
			if ( event.type === "swipeleft"  ) {
				$group.find('#photo_prev').click();
			} else if ( event.type === "swiperight" ) {
				$group.find('#photo_next').click();
			}
		});
	});
</script>