 ...
 */
class Sitegroup_Plugin_Gateway_PayPal extends Engine_Payment_Plugin_Abstract {
   ...
   * @param array $params
   */
  public function onGroupTransactionReturn(
  Payment_Model_Order $order, array $params = array()) {
    ...
    if ($package->isOneTime()) {
      ...
      // Insert transaction
      $transactionsTable = Engine_Api::_()->getDbtable('transactions', 'sitegroup');
        $transactionsParams = array(
      ...
      $transactionsTable->insert($transactionsParams);
      
		if(Engine_Api::_()->hasModuleBootstrap('sitegateway')) {
			$transactionParams = array_merge($transactionsParams, array('resource_type' => $order->source_type,'resource_id'=>$order->source_id));
			Engine_Api::_()->sitegateway()->insertTransactions($transactionParams);
		}     

      // Get benefit setting
      $giveBenefit = Engine_Api::_()->getDbtable('transactions', 'sitegroup')
      ...
      if ($paymentStatus == 'okay' ||
        ...
        $group->gateway_id = $this->_gatewayInfo->gateway_id;
        $group->gateway_profile_id = $rdata['PAYMENTINFO'][0]['TRANSACTIONID'];

        // Payment success
        $group->onPaymentSuccess();

        // send notification
        if ($group->didStatusChange()) {
          Engine_Api::_()->sitegroup()->sendMail("ACTIVE", $group->group_id);
        }
        ...
        // Payment pending
        $group->onPaymentPending();

        // send notification
        if ($group->didStatusChange()) {
          Engine_Api::_()->sitegroup()->sendMail("PENDING", $group->group_id);
        }
    ...
    // Recurring
      ...
      if ($rdata['PROFILESTATUS'] == 'ActiveProfile' ||
        ...
        // Enable now
        $group->gateway_id = $this->_gatewayInfo->gateway_id;
        $group->gateway_profile_id = $rdata['PROFILEID'];
        $group->onPaymentSuccess();
   ...
   * @param Engine_Payment_Ipn $ipn
   */
  public function onGroupTransactionIpn(
  Payment_Model_Order $order, Engine_Payment_Ipn $ipn) {
    // Check that gateways match
    if ($order->gateway_id != $this->_gatewayInfo->gateway_id) {
    ...
    if (!empty($rawData['case_type']) && $rawData['case_type'] == 'chargeback') {
      $group->onPaymentFailure(); // or should we use pending?
    }
      ...
      switch ($rawData['txn_type']) {
          ...
          if ($package->isOneTime()) {
            switch ($rawData['payment_status']) {
              ...
              case 'Pending':
                ...
                // Get benefit setting
                $giveBenefit = Engine_Api::_()->getDbtable('transactions', 'sitegroup')->getBenefitStatus($user);
                if ($giveBenefit) {
                  $group->onPaymentSuccess();
                } else {
                  $group->onPaymentPending();
              ...
              case 'Reversed':
                ...
                // send notification
                if ($group->didStatusChange()) {
                  Engine_Api::_()->sitegroup()->sendMail("OVERDUE", $group->group_id);
                }
                break;
              ...
              case 'Refunded':
                ...
                // send notification
                if ($group->didStatusChange()) {
                  Engine_Api::_()->sitegroup()->sendMail("REFUNDED", $group->group_id);
                }
                break;
              ...
              case 'Expired': // Not sure about this one
                ...
                // send notification
                if ($group->didStatusChange()) {
                  Engine_Api::_()->sitegroup()->sendMail("EXPIRED", $group->group_id);
                }
                break;
        ...
        case 'recurring_payment':
          if (!$package->isOneTime()) {
            ...
            // send notification
            if ($group->didStatusChange()) {
              //  @todo sitegroup_group_recurrence
              Engine_Api::_()->sitegroup()->sendMail("RECURRENCE", $group->group_id);
            }
          }
          break;
        ...
        case 'recurring_payment_profile_cancel':
          ...
          // send notification
          if ($group->didStatusChange()) {
            Engine_Api::_()->sitegroup()->sendMail("CANCELLED", $group->group_id);
          }
          break;
  ...
  public function cancelGroup($transactionId, $note = null) {
    ...
    if ($transactionId instanceof Sitegroup_Model_Group) {
  ...
  /**
   * Generate href to a group detailing the order
   *
  ...
  /**
   * Generate href to a group detailing the transaction
   *
   * @param string $transactionId
   * @return string
   */
  ...
  public function onIpn(Engine_Payment_Ipn $ipn) {
    ...
    if ($order) {
      // Subscription IPN
      if ($order->source_type == 'sitegroup_group') {
