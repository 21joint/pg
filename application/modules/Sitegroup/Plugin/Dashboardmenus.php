...
class Sitegroup_Plugin_Dashboardmenus {
  ...
  public function onMenuInitialize_SitegroupDashboardProjects($row) { 
      ...
      $viewer = Engine_Api::_()->user()->getViewer();
      $editPrivacy = $sitegroup->authorization()->isAllowed($viewer, "edit");
      if (empty($editPrivacy)) {
          return false;
      } 
      ...
      //DO NOT SHOW THE PROJECTS TAB IF ADMIN HAVE SELECTED PROJECT FOR THE GROUP PROFILE PAGE
      ...
      if (Engine_Api::_()->sitegroup()->hasPackageEnable()) {
          $allowProject = Engine_Api::_()->sitegroup()->allowPackageContent($sitegroup->package_id, "modules", 'sitecrowdfunding') ? 1 : 0; 
      } else {
          //permission to create project in above modules(Not Available Yet )
          $allowProject = Engine_Api::_()->sitegroup()->isGroupOwnerAllow($sitegroup, 'sprcreate') ? 1 : 0;
      } 
