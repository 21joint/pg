 ...
 */
class Sitegroup_Model_Group extends Core_Model_Item_Abstract {
    ...
    public function getMediaType() {
        ...
        return $translate->translate('MEDIA_TYPE_SITEGROUP_GROUP');
    }
    ...
    public function getCategory() {
        ...
        return Engine_Api::_()->getItem('sitegroup_category', $this->category_id);
    }
    ...
    public function getHref($params = array()) {
        ...
        if (!empty($sitegroupUrlEnabled)) {
            ...
            if ((!empty($change_url)) && $this->like_count >= $group_likes && !in_array($group_url, $banneUrlArray)) {
                $urlO = $replaceStr;
            }
        }
        return $urlO;
    ...
    public function membership() {
        return new Engine_ProxyObject($this, Engine_Api::_()->getDbtable('membership', 'sitegroup'));
    }
                ...
                if (Engine_Api::_()->getApi('subCore', 'sitegroup')->groupBaseNetworkEnable()) {
                    ...
                    if ($this->networks_privacy) {
                        if (!empty($viewerNetworkIds)) {
                            $groupNetworkId = explode(",", $this->networks_privacy);
                            $commanIds = array_intersect($groupNetworkId, $viewerNetworkIds);
                            if (empty($commanIds))
                                $flage = false;
                        } else {
                            $flage = false;
    ...
    public function isOwner($owner) {
        ...
        return Engine_Api::_()->sitegroup()->isGroupOwner($this, $owner);
    }
    ...
    public function setPhoto($photo) {
        if ($photo instanceof Zend_Form_Element_File) {
            ...
            throw new Sitegroup_Model_Exception($erro_msgg);
        }
        ...
        $album_id = '';
        $sitegroupAlbum = $this->getSingletonAlbum($album_id);
        $photoItem = $photoTable->createRow();
        $photoItem->setFromArray(array(
            'group_id' => $this->getIdentity(),
            'album_id' => $sitegroupAlbum->getIdentity(),
            'user_id' => $viewer->getIdentity(),
            'file_id' => $iMain->getIdentity(),
            'collection_id' => $sitegroupAlbum->getIdentity(),
        ));
        $photoItem->save();
    ...
    public function setLocation($params = null, $location_name = null) {
        ...
        if ($locationFieldEnable) {
            ...
            if (!empty($location)) {
                ...
                if (empty($params)) {
                    //$locationRow = Engine_Api::_()->getDbtable('locations', 'sitegroup')->getLocation(array('id' => $id));
                }
    ...
    public function getOffer() {
        ...
        $str = null;
        if (empty($sitegroupoffer_getInfo)) {
            return;
        }

        $getPackageOffer = Engine_Api::_()->sitegroup()->getPackageAuthInfo('sitegroupoffer');
        if (empty($getPackageOffer)) {
            return;
        }

        if (!Engine_Api::_()->sitegroup()->isGroupOwnerAllow($this, 'offer')) {
            return;
        }

        if (!Engine_Api::_()->sitegroup()->hasPackageEnable() || !Engine_Api::_()->sitegroup()->allowPackageContent($this->package_id, "modules", "sitegroupoffer")) {
            return;
        }

        if ($this->offer) {
            $sitegroupoffersTable = Engine_Api::_()->getDbtable('offers', 'sitegroupoffer');
            $result = $sitegroupoffersTable->getSitegroupoffer($this->group_id);
            if (empty($result)) {
                return;
            }
            ...
            if ($var == 1 || $var == 2) {
                ...
                if (!Engine_API::_()->seaocore()->checkSitemobileMode('fullsite-mode')) {
                    $tab_id = Engine_Api::_()->sitegroup()->GetTabIdinfo('sitegroupoffer.sitemobile-profile-sitegroupoffers', $this->group_id, $layout);
                } else {
                    $tab_id = Engine_Api::_()->sitegroup()->GetTabIdinfo('sitegroupoffer.profile-sitegroupoffers', $this->group_id, $layout);
                }
                $offer_tabinformation = $view->url(array('user_id' => $result->owner_id, 'offer_id' => $result->offer_id, 'tab' => $tab_id, 'slug' => $result->getSlug()), 'sitegroupoffer_view');
                ...
                $claim_value = Engine_Api::_()->getDbTable('claims', 'sitegroupoffer')->getClaimValue($viewer_id, $result->offer_id, $result->group_id);

                $content_claim = '';
                $view = Zend_Registry::isRegistered('Zend_View') ? Zend_Registry::get('Zend_View') : null;
                if (Engine_Api::_()->getApi('settings', 'core')->getSetting('sitegroupoffer.getofferlink', 1)) {
                    if (!empty($show_offer_claim) && empty($claim_value)) {

                        if (!empty($viewer_id)) {
                            $content_claim .= '<span><img src="' . $view->layout()->staticBaseUrl . 'application/modules/Sitegroupoffer/externals/images/invite.png" alt="" class="get_offer_icon" />';
                            $content_claim .= $view->htmlLink(array('route' => 'sitegroupoffer_general', 'action' => 'getoffer', 'id' => $result->offer_id), Zend_Registry::get('Zend_Translate')->_('Get Offer'), array(
                                        'onclick' => 'owner(this);return false',
                                    )) . '</span>';
                        } else {
                            $offer_tabinformation = $view->url(array('action' => 'getoffer', 'id' => $result->offer_id, 'param' => $param, 'request_url' => $request_url['1']), 'sitegroupoffer_general') . "?" . "return_url=" . $return_url . $_SERVER['HTTP_HOST'] . $currentUrl;
                            $title = $view->translate('Get Offer');
                            $content_claim .= '<span><img src="' . $view->layout()->staticBaseUrl . 'application/modules/Sitegroupoffer/externals/images/invite.png" alt="" class="get_offer_icon" />' . "<a href=$offer_tabinformation>$title</a>" . '</span>';
                        }
                    } elseif (!empty($claim_value) && !empty($show_offer_claim)) {
                        $content_claim .= '<span><img src="' . $view->layout()->staticBaseUrl . 'application/modules/Sitegroupoffer/externals/images/invite.png" alt="" class="get_offer_icon" />';
                        $content_claim .= $view->htmlLink(array('route' => 'sitegroupoffer_general', 'action' => 'resendoffer', 'id' => $result->offer_id), Zend_Registry::get('Zend_Translate')->_('Resend Offer'), array(
                                    'onclick' => 'owner(this);return false',
                                )) . '</span>';
                    } else {
                ...
                if (empty($result->photo_id)) {
                    if ($result)
                        $offer_image = "<a href=$offer_tabinformation><img src='" . $view->layout()->staticBaseUrl . "application/modules/Sitegroupoffer/externals/images/nophoto_offer_thumb_icon.png' alt='' /></a>";
                    $str = "<div class='sitegroup_offer_block'><div class='sitegroup_offer_photo'>$offer_image</div><div class='sitegroup_offer_details'><div class='sitegroup_offer_title'>" . $offer_title . "</div>" . '<div class="sitegroup_offer_stats">' . $content . '</div><div class="sitegroup_offer_date seaocore_txt_light">' . $content_claim . "</div></div></div>";
                }
                else {
                    if ($result) {
    ...
    public function getPackage() {
        ...
        if (null === $this->_package) {
            $this->_package = Engine_Api::_()->getItem('sitegroup_package', $this->package_id);
        }
    ...
    public function cancel($is_upgrade_package = false) {
        ...
        if (!empty($this->gateway_id) && !empty($this->gateway_profile_id)) {
            try {
                $gateway = Engine_Api::_()->getItem('sitegroup_gateway', $this->gateway_id);
                $gatewayPlugin = $gateway->getPlugin();
                if (method_exists($gatewayPlugin, 'cancelGroup')) {
                    $gatewayPlugin->cancelGroup($this->gateway_profile_id);
                }
            } catch (Exception $e) {
                // Silence?
            }
    ...
    public function setActive($flag = true, $deactivateOthers = null) {
        ...
        if (empty($this->aprrove_date)) {
            ...
            if (!empty($this->draft) && empty($this->pending)) {
                Engine_Api::_()->sitegroup()->attachGroupActivity($this);
            }
        }
    ...
    public function onPaymentIpn(Payment_Model_Order $order, Engine_Payment_Ipn $ipn) {
        ...
        $gateway->getPlugin()->onGroupTransactionIpn($order, $ipn);
        return true;
    }
    ...
    /**
     * Gets a group admin accroding to user id.
     *
     * @return group admin
     * */
    public function isGroupAdmin($user_id) {
        return Engine_Api::_()->getDbTable('manageadmins', 'sitegroup')->isGroupAdmins($user_id, $this->getIdentity());
    }

    public function getGroupOwnerList() {

        $owner_id = $this->owner_id;
        $table = Engine_Api::_()->getItemTable('sitegroup_list');
        $select = $table->select()
                ->where('owner_id = ?', $owner_id)
                ->where('group_id = ?', $this->getIdentity())
                ->where('title = ?', 'SITEGROUP_LIKE')
                ->limit(1);
    ...
    public function setImportPhoto($photo, $flag) {
        ...
        $siteGroupAlbum = $this->getSingletonAlbum();
        $photoItem = $photoTable->createRow();
