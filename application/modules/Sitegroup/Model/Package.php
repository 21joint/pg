 ...
 * @version    $Id: package.php 2011-05-05 9:40:21Z SocialEngineAddOns $
 * @author     SocialEngineAddOns
 */
class Sitegroup_Model_Package extends Core_Model_Item_Abstract {
  ...
  public function getProduct() {
    if (null === $this->_product) {
      ...
      $this->_product = $productsTable->fetchRow($productsTable->select()
                              ->where('extension_type = ?', 'sitegroup_group')
  ...
  public function getProductParams() {
    return array(
        ...
        'price' => $this->price,
        'extension_type' => 'sitegroup_group',
  ...
  public function getPackageDescription() {
    ...
    $priceStr = Engine_Api::_()->sitegroup()->getPriceWithCurrency($this->price); 
...
//      $typeStr = $translate->translate($typeStr);
  ...
  public function getPackageQuantity() {
    ...
    // Add duration, if not forever
    if ($this->duration > 0 && $this->duration_type != 'forever') {
       $str = $view->translate(array("%s $this->duration_type", "%s $this->duration_type".'s', $this->duration), $view->locale()->toNumber($this->duration));
  ...
  public function getSubModulesString() {
    $translate = Zend_Registry::get('Zend_Translate');
    $includeArray = Engine_Api::_()->sitegroup()->getEnableSubModules();
    $modulesArray = array();
    ...
    if (Engine_Api::_()->getDbtable( 'modules' , 'core' )->isModuleEnabled('sitegroupintegration') && Engine_Api::_()->getDbtable( 'modules' , 'core' )->isModuleEnabled('sitereview')) {
			$mixsettingsTable = Engine_Api::_()->getDbtable('mixsettings', 'sitegroupintegration');
