 ...
 */
class Sitegroup_Model_DbTable_Mobileadmincontent extends Engine_Db_Table {
    ...
    public function getContentId($mobilecontentgroup_id, $sitegroup) {
        ...
        $select_content = $select
                ->from($this->info('name'))
                ->where('group_id = ?', $mobilecontentgroup_id)
        ...
        if (!empty($mobilecontent_id)) {
            ...
            $select_photo = $select
                    ...
                    ->where('type = ?', 'widget')
                    ->where('name = ?', 'sitegroup.photos-sitegroup')->where('group_id = ?', $mobilecontentgroup_id)
                    ...
                    case 'siteevent.contenttype-events':
                        ...
                        case 'sitecrowdfunding.contenttype-projects':
                            if (Engine_Api::_()->getDbtable('modules', 'core')->isModuleEnabled('sitecrowdfunding')) {
                                if (Engine_Api::_()->sitegroup()->hasPackageEnable()) {
                                    if (Engine_Api::_()->sitegroup()->allowPackageContent($sitegroup->package_id, "modules", "sitecrowdfunding") == 1) {
                                        $flag = true;
                                    }
                                } else {
                                    $isGroupOwnerAllow = Engine_Api::_()->sitegroup()->isGroupOwnerAllow($sitegroup, 'sprcreate');
                                    if (!empty($isGroupOwnerAllow)) {
                                        $flag = true;
                                    }
                                }
                                //TOTAL PROJECTS
                                $projectCount = Engine_Api::_()->sitegroup()->getTotalCount($sitegroup->group_id, 'sitecrowdfunding', 'projects');
                                $isManageAdmin = Engine_Api::_()->sitegroup()->isManageAdmin($sitegroup, 'sprcreate');
    ...
    public function getContentInformation($group_id) {
        $select = $this->select()->from($this->info('name'), array('mobileadmincontent_id', 'name'))
                        ...
                        ->where('group_id = ?', $group_id)->order('mobileadmincontent_id ASC');
                        ...
                        if ($name != 'sitegroupintegration.profile-items') {
                            ...
                            if (!empty($check_list)) {
                                ...
                                foreach ($results as $value) {
                                    ...
                                    $select = new Zend_Db_Select($db);
                                    $select
                                            ->from('engine4_sitegroup_mobileadmincontent')
                                    ...
                                    if (empty($info)) {
                                        ...
                                        // tab on profile
                                        $db->insert('engine4_sitegroup_mobileadmincontent', array(
                                            'group_id' => $group_id,
                                            'type' => 'widget',
