...
class Sitegroup_Installer extends Engine_Package_Installer_Module {
    ...
    function onInstall() {
        ...
        //START: PUT THE BACK WIDGET ON GROUP PROFILE PAGE IF GROUP IS INTEGRATED WITH CROWDFUNDING
        $select = new Zend_Db_Select($db);
        $select
        ...
        if (!empty($page_id)) {
            ...
            if(!empty($versionCheckCrowdfunding)) {
                ...
                $select
                        ...
                        ->where('enabled = ?', 1)
                        ->where('item_type = ?', 'sitegroup_group')
                        ->where('item_module = ?', 'sitegroup');
                $sitegroupIntegratedWithCrowdfunding = $select->query()->fetchObject();
                
                if (!empty($sitecrowdfundingintegrationEnabled) && !empty($sitegroupIntegratedWithCrowdfunding)) {  
        ...
        //END: PUT THE BACK WIDGET ON GROUP PROFILE PAGE IF GROUP IS INTEGRATED WITH CROWDFUNDING
        ...
        $db->query("INSERT IGNORE INTO `engine4_core_mailtemplates` (`type`, `module`, `vars`) VALUES ('SITEGROUPDISCUSSION_CREATENOTIFICATION_EMAIL', 'sitegroup', '[host],[group_title],[item_title],[body_content]');");
        ...
        // ADDING RESTRICTED COLUMN IN ENGINE4_SITEGROUP_GROUPS TABLE FOR RESTRICTED GROUPS
        $sitegroupTable = $db->query('SHOW TABLES LIKE \'engine4_sitegroup_groups\'')->fetch();
        if (!empty($sitegroupTable)) {

            $restrictedColumn = $db->query("SHOW COLUMNS FROM engine4_sitegroup_groups LIKE 'restricted'")->fetch();
            if (empty($restrictedColumn)) {
                $db->query("ALTER TABLE `engine4_sitegroup_groups` ADD `restricted` INT(1) NOT NULL DEFAULT '0';");
            }
        }
...
("sitetagcheckin_sgal_photo_new", "sitetagcheckin", "{item:$object} added {var:$count} photo(s) to the album {var:$linked_album_title} - {var:$prefixadd} {var:$location}.", 1, 5, 1, 3, 1, 1)');
	...
	("sitetagcheckin_sbal_photo_new", "sitetagcheckin", "{item:$object} added {var:$count} photo(s) to the album {var:$linked_album_title} - {var:$prefixadd} {var:$location}.", 1, 5, 1, 3, 1, 1)');
        ...
        if ($is_documentintegration_object) {
            ...
            //entry of the member level settings for the document creation options
            ...
            $db->query("INSERT IGNORE INTO `engine4_authorization_permissions`
              SELECT
                ...
                5 as `value`,
                '[\"registered\",\"owner_network\",\"owner_member_member\", \"owner_member\", \"like_member\", \"owner\"]' as `params`
              FROM `engine4_authorization_levels` WHERE `type` NOT IN('public');
            ");
        }
        ...
        if ($is_sitevideointegration_object) {
            ...
            //entry of the member level settings for the video creation options
            $db->query("INSERT IGNORE INTO `engine4_authorization_permissions`
              SELECT
                level_id as `level_id`,
                'sitegroup_group' as `type`,
    ...
    if(!empty($crowdfundingEnabled)) {
        ...
        if ($is_sitecrowdfundingintegration_object) {
            ...
            if ($sitegroup_isActivate_object) {
                ...
                $db->query('INSERT IGNORE INTO `engine4_core_settings` ( `name`, `value`) VALUES( "sitecrowdfunding.project.leader.owner.sitegroup.group", "1");');
            }

            //entry of the member level settings for the project creation options
            $db->query("INSERT IGNORE INTO `engine4_authorization_permissions`
              SELECT
                level_id as `level_id`,
                'sitegroup_group' as `type`,
