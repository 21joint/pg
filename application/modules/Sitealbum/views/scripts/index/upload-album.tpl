...
<div class="o_hidden seao_add_photo_lightbox_header">
  ...
  <div class="fright txt_right">
    ...
    <div class="seaocore_button fleft">
      <a class="buttonlink" href="javascript:void(0);" id="demo1-browse"><i
          class="seaocore_icon_add"></i><?php echo $this->translate(
          'Add More Photos'
        ) ?></a>
    </div>
    <?php if (Engine_Api::_()->authorization()->isAllowed(
      'album', $this->viewer(), 'create'
    )
    ) : ?>
      <div class="seaocore_button fleft">
        <a class="buttonlink" href="<?php echo $this->url(
          array('action' => 'manage'), 'sitealbum_general', true
        ); ?>"><?php echo $this->translate('My Albums') ?></a>
      </div>
    <?php endif; ?>
    <div class="seaocore_button fleft">
      <a class="buttonlink" href="javascript:void(0);" id="demo1-clear"
         style='display: none;'><?php echo $this->translate('Clear List') ?></a>
    </div>
    <?php if (Engine_Api::_()->sitealbum()->openAddNewPhotosInLightbox()) : ?>
      <a class="bold" name="cancel" id="cancel" type="button"
         href="javascript:void(0)" onclick="openCancelSmoothbox();"></a>
    <?php endif; ?>
  </div>
</div>
...
<script type="text/javascript">
  ...
  function openCancelSmoothbox() {
    ...
    if (fileids.value == '') {
      ...
      Smoothbox.open('<div class="settings global_form_popup" style="width:96%;box-sizing:border-box;padding:2%;"><h3><?php echo $this->string(
      )->escapeJavascript(
        $this->translate("Cancel and Delete Photos?")
      ) ?></h3><br /><div><?php echo $this->string()->escapeJavascript(
        $this->translate(
          "You have already added some photos to this album. Do you want to cancel and delete these photos?"
        )
      ) ?></div><br/><button onclick="deletePhotos();"><?php echo $this->string(
      )->escapeJavascript(
        $this->translate("Yes, Delete Photos")
      ) ?></button> <?php echo $this->string()->escapeJavascript(
        $this->translate("or")
      ) ?> <a href="javascript:void(0);" onclick="albumSubmitForm();return false;"><?php echo $this->string(
      )->escapeJavascript($this->translate("No, Finish Album")) ?></a></div>');
    }
  }
  ...
  function albumSubmitForm() {
    if (isFlash == false) {
      file1 = $('fallback').clone(true);
      file1.hide();
      obj = $('form-upload').getElementById('search-wrapper');
      file1.inject(obj, 'after');
    }
  ...
  if (!Autocompleter) {
    SmoothboxSEAO.addScriptFiles.push("<?php echo $this->layout(
    )->staticBaseUrl ?>externals/autocompleter/Observer.js");
    SmoothboxSEAO.addScriptFiles.push("<?php echo $this->layout(
    )->staticBaseUrl ?>externals/autocompleter/Autocompleter.js");
    SmoothboxSEAO.addScriptFiles.push("<?php echo $this->layout(
    )->staticBaseUrl ?>externals/autocompleter/Autocompleter.Local.js");
    SmoothboxSEAO.addScriptFiles.push("<?php echo $this->layout(
    )->staticBaseUrl ?>externals/autocompleter/Autocompleter.Request.js");
  }
  <?php endif; ?>
  function openSmoothbox(photo_id) {
    Smoothbox.open('<div class="settings global_form_popup" style="width:96%;box-sizing:border-box;padding:2%;"><h3><?php echo $this->string(
    )->escapeJavascript(
      $this->translate("Make Album Main Photo")
    ) ?></h3><br /><div><?php echo $this->string()->escapeJavascript(
      $this->translate("Do you want to make this photo main photo of album?")
    ) ?></div><br/><button onclick=setMainPhoto(' + photo_id + ');Smoothbox.close();><?php echo $this->string(
    )->escapeJavascript(
      $this->translate("Save")
    ) ?></button> <?php echo $this->string()->escapeJavascript(
      $this->translate("or")
    ) ?> <a href="javascript:void(0);" onclick=closeSmoothbox(' + photo_id + ');><?php echo $this->string(
    )->escapeJavascript($this->translate("Cancel")) ?></a></div>');
  }
  ...
  setTimeout(function () {
    ...
    window.addEvent('domready', function () {
      if ($('sitealbum_location') && (('<?php echo !Engine_Api::_()->getApi(
        'settings', 'core'
      )->getSetting(
        'seaocore.locationspecific', 0
      ); ?>') || ('<?php echo Engine_Api::_()->getApi('settings', 'core')
        ->getSetting(
          'seaocore.locationspecific', 0
        ); ?>' && '<?php echo !Engine_Api::_()->getApi('settings', 'core')
        ->getSetting('seaocore.locationspecificcontent', 0); ?>'))) {
  ...
  <?php if (Engine_Api::_()->getApi('settings', 'core')->getSetting(
    'sitealbum.tags.enabled', 1
  )): ?>
  en4.core.runonce.add(function () {
    new Autocompleter.Request.JSON('tags', '<?php echo $this->url(
      array('module' => 'seaocore', 'controller' => 'index',
            'action' => 'tag-suggest', 'resourceType' => 'album'), 'default',
      true
    ) ?>', {
      'postVar': 'text',
      'minLength': 1,
  ...
  var getProfileType = function (category_id) {
    var mapping = <?php echo Zend_Json_Encoder::encode(
      Engine_Api::_()->getDbTable('categories', 'sitealbum')->getMapping(
        array('category_id', 'profile_type')
      )
    ); ?>;
...
<?php if (!Engine_Api::_()->seaocore()->isMobile()): ?>
  ...
  <div class="layout_middle">
    ...
    <ul>
      ...
      <li class="sitealbum_addphotos_btn" id="sitealbum_addphotos_btn">
        <a class="buttonlink" href="javascript:void(0);"
           id="demo-browse"><?php echo $this->translate('Add Photos') ?></a>
        <a class="buttonlink icon_clearlist" href="javascript:void(0);"
           id="demo-clear" style='display: none;'><?php echo $this->translate(
            'Clear List'
          ) ?></a>
      </li>
    </ul>
    ...
    <div id="demo-status" class="hide">
      ...
      <div class="demo-status-overall" id="demo-status-overall"
           style="display:none">
        <div class="overall-title" style="display:none;"></div>
        <img src="<?php echo $this->layout()->staticBaseUrl
          . 'application/modules/Seaocore/externals/images/loading.gif'; ?>"
             class="progress overall-progress"/>
      </div>

      <div class="demo-status-current" id="demo-status-current"
           style="display:none">
        <div class="current-title" style="display:none;"></div>
        <img src="<?php echo $this->layout()->staticBaseUrl
          . 'externals/fancyupload/assets/progress-bar/bar.gif'; ?>"
             class="progress current-progress" style="display:none;"/>
      </div>
...
//  $this->headScript()->appendFile(
//    $this->layout()->staticBaseUrl
//    . 'application/modules/Sitealbum/externals/scripts/core.js'
//  );
//  $this->headScript()
//    ->appendFile(
//      $this->layout()->staticBaseUrl . 'externals/fancyupload/Swiff.Uploader.js'
//    )
//    ->appendFile(
//      $this->layout()->staticBaseUrl . 'externals/fancyupload/Fx.ProgressBar.js'
//    )
//    ->appendFile(
//      $this->layout()->staticBaseUrl . 'externals/fancyupload/FancyUpload2.js'
//    );
//
//  $this->headLink()
//    ->appendStylesheet(
...
//    );
  ...
  <script type="text/javascript">
    ...
    setTimeout(function () {
      var uploaderContent = function (target) { // wait for the content
        ...
        var up = new FancyUpload2($('demo-status'), $('demo-list'), {// options object
          ...
          path: '<?php echo $this->layout()->staticBaseUrl
            . 'externals/fancyupload/Swiff.Uploader.swf'; ?>',
          ...
          onFileRemove: function (file) {
            ...
            request = new Request.JSON({
              ...
              'url': '<?php echo $this->url(
                Array('module' => 'sitealbum', 'controller' => 'photo',
          ...
          onFileSuccess: function (file, response) {
            ...
            if (json.get('status') == '1') {
              ...
              var mediaPhotoDetails = "<a class='buttonlink icon_photos_rotate_ccw' href='javascript:void(0)' onclick='rotatePhoto(this, " + json.get('photo_id') + ")' title='<?php echo $this->translate(
                "Rotate This Photo"
              ); ?>'>&nbsp;</a><img id='media_photo_" + json.get('photo_id') + "' style=''src=" + json.get('src') + " />";
              var photoLocation = '';

              <?php if (SEA_PHOTOLIGHTBOX_EDITLOCATION
            && Engine_Api::_()->getApi('settings', 'core')->getSetting(
              'sitealbum.location', 1
            )): ?>
              ...
              photoLocation = "<div><a href='javascript:void(0);' onclick='" + editLocationLink + "'><?php echo $this->translate(
                "Edit Location"
              ) ?></a><div class='media_option_location' id='media_option_location_" + json.get('photo_id') + "' style='display:none;'><ul><input type='text' id='media_photo_location_" + json.get('photo_id') + "'  /></ul><button  onclick='" + saveLocationLink + "' id='media_photo_save_location_" + json.get('photo_id') + "'><?php echo $this->translate(
                "Save"
              ) ?></button><button id='media_photo_cancel_location_" + json.get('photo_id') + "' onclick='" + cancelLocationLink + "' ><?php echo $this->translate(
                "Cancel"
              ) ?></button></div></div>";
              ...
              var albumMainLink = "<div><a href='javascript:void(0);' onclick='openSmoothbox(" + photo_id + ");'><?php echo $this->translate(
                "Make Album Main Photo"
              ) ?></a></div>";

              var changeDateLink = "<div><a href='javascript:void(0);' onclick='changeDateOptions(" + photo_id + ");'><?php echo $this->translate(
                "Change Date"
              ) ?></a></div>";
              ...
              file.info.set('html', '<span><?php echo $this->string()
                ->escapeJavascript(
                  $this->translate('An error occurred:')
                ) ?></span> ' + (json.get('error') ? (json.get('error')) : response));
            }
          ...
          onFail: function (error) {
            switch (error) {
              case 'hidden': // works after enabling the movie and clicking refresh
                alert(<?php echo Zend_Json::encode(
                  $this->translate(
              ...
              case 'empty': // Oh oh, wrong path
                alert(<?php echo Zend_Json::encode(
                  $this->translate(
                    'A required file was not found, please be patient and we will fix this.'
                  )
                ) ?>);
                break;
              case 'flash':
                ...
                isFlash = false;
                if ($('sitealbum_addphotos_btn'))
                  $('sitealbum_addphotos_btn').destroy();
    ...
    function makeLocationAsInput(photo_id) {
      ...
      setTimeout(function () {
        window.addEvent('domready', function () {
          if ($('media_photo_location_' + photo_id) && (('<?php echo !Engine_Api::_(
            ...
            google.maps.event.addListener(autocomplete, 'place_changed', function () {
              ...
              var address = '', country = '', state = '', zip_code = '',
                city = '';
              var data = {};
              if (place.address_components) {
                var len_add = place.address_components.length;
  ...
  <div style="display:none;" id="photo-add-date">
    <select id="year-album" name="year">
      <option label="Year" value="Year"
              disabled="disabled"><?php echo $this->translate(
          'Year'
        ); ?></option>
      <?php $curYear = date('Y'); ?>
      <?php for ($i = 0; $i <= 110; $i++) : ?>
        <option label="<?php echo $curYear; ?>"
                value="<?php echo $curYear; ?>" <?php if ($i
          == 0
        ): ?> selected="selected" <?php endif; ?>><?php echo $curYear; ?></option>
        <?php $curYear--; ?>
      <?php endfor; ?>
    </select>
    <a onclick="showMonth(0, 'album');" href="javascript:void(0);"
       id="addmonth-album" style="display:none;"><?php echo $this->translate(
        '+ Add Month'
      ); ?></a>
    <select id="month-album" name="month" onblur="showAddmonth(2, 'album')"
            onclick="showMonth(1, 'album')" onchange="showAddday(2, 'album')"
            style="display:block;">
      <option label="Month" value="0"><?php echo $this->translate(
          'Month'
        ); ?></option>
      <?php $curMonth = (int)date('m'); ?>
      <?php for ($k = 1; $k <= 12; $k++): ?>
        <?php $month = date('F', mktime(0, 0, 0, $k, 1)); ?>
        <option label="<?php echo $month; ?>"
                value="<?php echo $k; ?>" <?php if ($k
          == $curMonth
        ): ?> selected="selected" <?php endif; ?>><?php echo $this->translate(
            $month
          ); ?></option>
      <?php endfor; ?>
    </select>
    <a style="display:none;" id="addday-album" onclick="showDay(0, 'album');"
       href="javascript:void(0);"><?php echo $this->translate(
        '+ Add Day'
      ); ?></a>
    <select id="day-album" name="day" style="display:block;"></select>
    <button id="photo-add-date-save"
            onclick="savePhotoDate('album');"><?php echo $this->translate(
        'Save'
      ); ?></button>
    <button id="photo-add-date-cancel"
            onclick="cancelPhotoDate('album');"><?php echo $this->translate(
        'Cancel'
      ); ?></button>
  </div>
  ...
  <script type="text/javascript">
    ...
    function showMonth(month, ele) {
      ...
      if (selectedTextMonth != 'Month') {
        if (parseInt(selectedValueMonth) > '<?php echo (int)date(
          "m"
        ); ?>' && (currentYear == parseInt(selectedTextYear))) {
    ...
    function showDay(day, ele) {
      ...
      if (document.getElementById('day-' + ele))
        addOptionDate($('day-' + ele), '<?php echo $this->translate(
          "Day"
        ); ?>', 0);
    ...
    function addOptionDate(selectbox, text, value) {
    ...
    function showAddday(day, ele) {
      ...
      if (addDay == 0 || day == 2) {
        ...
        if (selectedText == 'Day') {
          ...
          if (parseInt(selectedValue) > '<?php echo (int)date(
            "d"
          ); ?>' && (currentYear == parseInt(selectedTextYear)) && parseInt(selectedMonthValue) == '<?php echo (int)date(
            "m"
          ); ?>') {
