...
//    if ( !empty($feed_filter) && empty($feedCountOnly) && empty($feedOnly) && empty($subject) && !$isForCategoryPage ) {
        if (!empty($feed_filter) && empty($feedCountOnly) && empty($feedOnly) && empty($subject)) {
            ...
            foreach ($contentTabs as $value) {
                ...
                if ((_CLIENT_TYPE && (_CLIENT_TYPE == 'ios') && _IOS_VERSION && _IOS_VERSION < '2.0.8') || (_CLIENT_TYPE == 'both')) {
                    if (strstr($value->filter_type, 'memories') || strstr($value->filter_type, 'advertise') || strstr($value->filter_type, 'schedule_post'))
                        continue;
                }

                if ((_CLIENT_TYPE && (_CLIENT_TYPE == 'android') && _ANDROID_VERSION && _ANDROID_VERSION < '2.2') || (_CLIENT_TYPE == 'both')) {
                    if (strstr($value->filter_type, 'memories') || strstr($value->filter_type, 'advertise') || strstr($value->filter_type, 'schedule_post'))
                        continue;
                }
...
//            $subject = Engine_Api::_()->getItem('user', $viewer_id);
        ...
        try {
            do {
                // Get current batch
                if (isset($_REQUEST['action_id']) && !empty($_REQUEST['action_id']) && empty($subject)) {
                    try {
                        $actObj = Engine_Api::_()->getItem('activity_action', $action_id);
                        $OnwerObj = $actObj->getOwner();
                        if (isset($OnwerObj) && !empty($OnwerObj)) {
                            if ($OnwerObj->getType() == 'user')
                                $subject = $OnwerObj;
                        }
                    } catch (Exception $ex) {
...
//            $this->respondWithSuccess();
        ...
        if (!isset($_REQUEST['hashtag']) && $this->getRequestParam('post_menus', true) && $this->getRequestParam('post_elements', true) && !empty($getBodyResponse['enable_composer'])) {
