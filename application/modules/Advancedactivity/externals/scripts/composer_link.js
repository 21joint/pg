...
Composer.Plugin.Link = new Class({
  ...
  doShowPreview : function() {
    ...
    if (this.params.richHtml) {
      this.elements.previewHtml = new Element('div', {
        'id': 'compose-link-preview-html',
        'class': 'compose-preview-link-html',
        'html': this.params.richHtml
      }).inject(this.elements.body);
      var arr = this.elements.previewHtml.getElementsByTagName('script');
      for (var n = 0; n < arr.length; n++) {
        if (arr[n].get('src')) {
          var scriptTag = document.createElement("script");
          scriptTag.src = arr[n].get('src');
          scriptTag.inject(this.elements.body);
        }
      }
      } else {
        var uriParse = document.createElement("a");
        uriParse.href = this.params.uri;
        var pathSplit = uriParse.pathname.split('/');
        if (uriParse.hostname === 'soundcloud.com' && pathSplit.length > 1) {
          this.elements.new = new Element('div', {
            'class': 'soundcloud-preview',
            'styles': {
              width: '100%'
            },
            'html': '<iframe frameborder="no"width="100%" height="400" src="https://w.soundcloud.com/player/?visual=true&amp;url=' + this.params.uri + '&amp;show_artwork=true" scrolling="no"></iframe>'
          }).inject(this.elements.body);
          return;
        }
      }
