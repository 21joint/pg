...
class Advancedactivity_Bootstrap extends Engine_Application_Bootstrap_Abstract
{
  ...
  public function __construct($application)
  {
    ...
    if (isset($session->siteViewModeSM)
      ...
      if ($settings->advancedmembers_enabled) {
        Zend_Registry::set(
          'advancedmembers_enabled', $settings->advancedmembers_enabled
        );
      ...
      if (substr(str_replace($subDirectory, '', $_SERVER['REQUEST_URI']), 0, 9)
        == '/profile/'
      ) {
        $_SERVER['REQUEST_URI'] = str_replace(
          '/profile/', '/profileSM/', $_SERVER['REQUEST_URI']
        );
      }
      //  RESOLVED CONFLICTED WITH PROFILE URL ]
    }
  ...
  protected function _initOverwriteManifest()
  {
    $manifest = Zend_Registry::get('Engine_Manifest');
    $manifest['core']['composer']['link']['plugin']
      = 'Advancedactivity_Plugin_Composer';
  ...
  protected function _initFrontController()
  {
    ...
    $notificationIsEnable = Engine_Api::_()->getApi('settings', 'core')
      ->getSetting('aaf.isenable.notification', 1);
    if ($notificationIsEnable) {
      if (Zend_Registry::isRegistered('StaticBaseUrl')) {
//@TODO add bundles
        //        $headScript->appendFile(Zend_Registry::get('StaticBaseUrl').'application/modules/Advancedactivity/externals/scripts/notification.js');
      } else {
//@TODO add bundles
    ...
    $front->registerPlugin(new Advancedactivity_Plugin_Core);
  }
