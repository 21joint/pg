...
(function() { // START NAMESPACE
  ...
  ComposerCheckin.Plugin.Photo = new Class({
    ...
    activate : function() {
      ...
      if(!DetectMobileQuick() && this.options.fancyUploadEnabled && this.options.fancyUploadOptions ) {
      ...
      if(DetectMobileQuick() ) {
        this.elements.formSubmit = new Element('button', {
          ...
          'events' : {
            'click' : function(e) {
              e.stop();
              this.elements.formSubmit.style.display='none';
              this.doAttach();           
            }.bind(this)
          }
        }).inject(this.elements.body);
