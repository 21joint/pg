...
ComposerCheckin = new Class({
      ...
      if( !$type(this.elements.menu) ) {
        this.elements.menu = new Element('div',{
          ...
          'class' : 'compose-menu'
				...
				this.elements.menu.inject($('seocheckinphotoactivator'), "before"); 
      }
    }
        ...
        this.elements.tray =  new Element('div',{
          ...
          'styles' : {
            'display' : 'none'
				...
				this.elements.tray.inject($('seocheckinphotoactivator'), "before");
      }
    }
  ...
  attach : function() {
    ...
    this.elements.container = new Element('div', {
      'id' : 'sitetagcheckin-compose-container',
        ...
        if( self.options.hideSubmitOnBlur ) {
          (function() {
            if( !self.hasActivePlugin() ) {
              self.getMenu().setStyle('display', 'block');
            }
          }).delay(250);
        }
      }
    ...
    if( self.options.hideSubmitOnBlur ) {
      this.getMenu().setStyle('display', 'block');
    ...
    if( supportsContentEditable ) {
      ...
      this.selection = new ComposerCheckin.Selection(this.elements.body);
    } else {
      this.elements.textarea.setStyle('display', '');
    }

    if( this.options.overText ) {
      new ComposerCheckin.OverText(this.elements.body, $merge({
...
ComposerCheckin.Selection = new Class({
...
ComposerCheckin.OverText = new Class({
...
ComposerCheckin.Plugin = {};

ComposerCheckin.Plugin.Interface = new Class({
  ...
  makeActivator : function() {
    if( !this.elements.activator ) {
      this.elements.activator = new Element('span', {
        ...
        'class' : 'compose-activator sitetag_checkin_post_composer_button',
        'href' : 'javascript:void(0);',
        'html' : '<p class="seaocheckin_composer_tip">'+en4.core.language.translate('Add Photo')+'<img alt="" src="./application/modules/Advancedactivity/externals/images/tooltip-arrow-down.png" /></p>',
        'events' : {
          'click' : this.activate.bind(this)
        }
      }).inject($('stcheckin-checkin-compose-menu'));
