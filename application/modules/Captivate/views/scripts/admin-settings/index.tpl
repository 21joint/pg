...
if (!empty($this->isModsSupport)):
    foreach ($this->isModsSupport as $modName) {
        echo "<div class='tip'><span>" . $this->translate("Note: You do not have the latest version of the '%s'. Please upgrade it to the latest version to enable its integration with Responsive Captivate Theme.", ucfirst($modName)) . "</span></div>";
    }
endif;
?>
...
if (($coreSettings->getSetting('captivate.isActivate', 0)) && empty($captivateThemeActivated)):
    ?>
...
<?php if ((!Engine_Api::_()->hasModuleBootstrap('sitehomepagevideo') || !$coreSettings->getSetting('sitehomepagevideo.isActivate', 0)) && $coreSettings->getSetting('captivate.isActivate')) : ?>
...
<?php
if ($coreSettings->getSetting('captivate.isActivate', 0)):
    include APPLICATION_PATH . '/application/modules/Captivate/views/scripts/_theme_message.tpl';
endif;
?>
