...
var MooRainbow = new Class({
    options: {
        id: 'mooRainbow',
        prefix: 'moor-',
        imgPath: en4.core.staticBaseUrl + 'application/modules/Seaocore/externals/images/',
        startColor: [255, 0, 0],
        wheel: false,
        onComplete: $empty,
        onChange: $empty
    ...
    eventKeys: function (e, el, id) {
        ...
        if (e.type == 'keydown') {
            if (e.key == 'up')
                ...
                return;
        } else if (e.type == Element.Events.mousewheel.base)
            wheel = (e.wheel > 0) ? 1 : -1;

        if (this.arrRGB.contains(el))
            type = 'rgb';
        else if (this.arrHSB.contains(el))
            type = 'hsb';
        else
            type = 'hsb';
