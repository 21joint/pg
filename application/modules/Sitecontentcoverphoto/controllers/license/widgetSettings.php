...
if (!empty($channelView)) {

    $page_id = $channelView->page_id;

    if ($page_id) {
        $select = new Zend_Db_Select($db);
        $topContainer_content_id = $select
                ->from('engine4_core_content', 'content_id')
                ->where('type = ?', 'container')
                ->where('name = ?', 'top')
                ->where('page_id = ?', $page_id)
        ...
        if ($topContainer_content_id) {
            $select = new Zend_Db_Select($db);
            $topContainer_middle_content_id = $select
                    ->from('engine4_core_content', 'content_id')
                    ->where('type = ?', 'container')
                    ->where('name = ?', 'middle')
                    ->where('page_id = ?', $page_id)
                    ->where('parent_content_id = ?', $topContainer_content_id)
                    ->query()
                    ->fetchColumn();
            if ($topContainer_middle_content_id) {

                $select = new Zend_Db_Select($db);
                $content_cover_photo_content_id = $select
                        ->from('engine4_core_content', 'content_id')
                        ->where('type = ?', 'widget')
                        ->where('name = ?', 'sitecontentcoverphoto.content-cover-photo')
                        ->where('page_id = ?', $page_id)
                        ->where('parent_content_id = ?', $topContainer_middle_content_id)
                        ->query()
                        ->fetchColumn();
                if (!$content_cover_photo_content_id) {
                    $db->insert('engine4_core_content', array(
                        'page_id' => $page_id,
                        'type' => 'widget',
                        'name' => 'sitecontentcoverphoto.content-cover-photo',
                        'parent_content_id' => $topContainer_middle_content_id,
                        'order' => 2,
                        'params' => '{"modulename":"sitevideo_channel","showContent_0":"","showContent_sitebusiness_business":"","showContent_siteevent_event":"","showContent_sitepage_page":"","showContent_sitevideo_channel":["title","subscribe","rating","optionsButton","shareOptions","uploadVideos"],"profile_like_button":"1","columnHeight":"400","showMember":"1","memberCount":"8","onlyMemberWithPhoto":"1","contentFullWidth":"1","sitecontentcoverphotoChangeTabPosition":"1","contacts":["1","2","3"],"showMemberLevelBasedPhoto":"0","emailme":"1","editFontColor":"1","title":"","nomobile":"0","name":"sitecontentcoverphoto.content-cover-photo"}',
                    ));
                }
            }
            $db->query("DELETE FROM `engine4_core_content` WHERE `engine4_core_content`.`name` = 'sitevideo.profile-breadcrumb'");
            $db->query("DELETE FROM `engine4_core_content` WHERE `engine4_core_content`.`name` = 'sitevideo.top-content-of-channel'");
            $db->query("DELETE FROM `engine4_core_content` WHERE `engine4_core_content`.`name` = 'sitevideo.user-ratings'");
        }
    }
...
//Code to set Content Cover Photo widget in project profile page
$select = new Zend_Db_Select($db);
$projectView = $select
        ->from('engine4_core_pages')
            ...
            if ($topContainer_middle_content_id) {
                ...
                if (!$content_cover_photo_content_id) {
                    $db->insert('engine4_core_content', array(
                        ...
                        'parent_content_id' => $topContainer_middle_content_id,
                        'order' => 2,
                        'params' => '{"modulename":"sitecrowdfunding_project","showContent_0":"","showContent_sitebusiness_business":"","showContent_siteevent_event":"","showContent_sitepage_page":"","showContent_sitecrowdfunding_project":["title","optionsButton","shareOptions"],"profile_like_button":"1","columnHeight":"400","showMember":"1","memberCount":"8","onlyMemberWithPhoto":"1","contentFullWidth":"1","sitecontentcoverphotoChangeTabPosition":"1","contacts":["1","2","3"],"showMemberLevelBasedPhoto":"0","emailme":"1","editFontColor":"1","title":"","nomobile":"0","name":"sitecontentcoverphoto.content-cover-photo"}',
                    ));
                }
            }
            $db->query("DELETE FROM `engine4_core_content` WHERE `engine4_core_content`.`name` = 'sitecrowdfunding.browse-breadcrumb'");

        }
    }
    $db->query('INSERT IGNORE INTO `engine4_sitecontentcoverphoto_modules` (`module`, `resource_type`, `resource_id`, `enabled`) VALUES
("sitecrowdfunding", "sitecrowdfunding_project", "project_id", 1)');
    $db->query("INSERT IGNORE INTO `engine4_authorization_permissions`
            SELECT
                    level_id as `level_id`,
                    'sitecontentcoverphoto_sitecrowdfunding_project' as `type`,
                    'upload' as `name`,
                    1 as `value`,
                    NULL as `params`
            FROM `engine4_authorization_levels` WHERE `type` IN('moderator', 'admin', 'user');");
