...
<div class="seaocore_profile_cover_head_section_inner" id="seaocore_profile_cover_head_section_inner">
  ...
  <?php if(($this->profile_like_button == 1) || (in_array('followButton', $this->showContent)) || (in_array('joinButton', $this->showContent)) || (in_array('addButton', $this->showContent)) || (in_array('optionsButton', $this->showContent)) || in_array('backButton', $this->showContent)):?>
    <div class="seaocore_profile_coverinfo_buttons">
      ...
      <?php if (is_array($this->showContent) && Engine_Api::_()->getDbtable('modules', 'core')->isModuleEnabled('sitegroupmember')) : ?>
				...
				if (empty($joinMembers) && in_array('joinButton', $this->showContent) && $this->viewer->getIdentity() != $this->subject()->owner_id ): ?>
					<div class="seaocore_button">
					<?php if ($this->viewer->getIdentity() && empty($this->subject()->restricted)) : ?>
						<?php if (!empty($this->subject()->member_approval)): ?>
							<a href="javascript:void(0);" onclick="Smoothbox.open('<?php echo $this->escape($this->url(array( 'action' => 'join', 'group_id' => $this->subject()->group_id), 'sitegroup_profilegroupmember', true)); ?>'); return false;" ><i class="fa plus_icon"></i><span><?php echo $this->translate("Join Group"); ?></span></a>
						<?php else: ?>
							<a href="javascript:void(0);" onclick="Smoothbox.open('<?php echo $this->escape($this->url(array( 'action' => 'request', 'group_id' => $this->subject()->group_id), 'sitegroup_profilegroupmember', true)); ?>'); return false;" ><i class="fa plus_icon"></i><span><?php echo $this->translate("Join Group"); ?></span></a>
						<?php endif; ?>
					<?php endif; ?>
					</div>
				<?php endif; ?> 
        ...
        if (!empty($hasMembers) && in_array('leaveButton', $this->showContent) && $this->viewer->getIdentity() != $this->subject()->owner_id ): ?>
					<div class="seaocore_button">
            <?php if ($this->viewer->getIdentity()) : ?>
              <a href="javascript:void(0);" onclick="Smoothbox.open('<?php echo $this->escape($this->url(array( 'action' => 'leave', 'group_id' => $this->subject()->group_id), 'sitegroup_profilegroupmember', true)); ?>'); return false;" ><i class="fa minus_icon"></i><span><?php echo $this->translate("Leave Group"); ?></span></a>
            <?php endif; ?>
					</div>
				<?php endif; ?>
				...
				<?php if (is_array($this->showContent) && in_array('addButton', $this->showContent)): ?>
					...
					<div>
						<a class="sitegroup_button" href="javascript:void(0);" onclick="Smoothbox.open('<?php echo $this->escape($this->url(array( 'action' => 'invite-members', 'group_id' => $this->subject()->group_id), 'sitegroup_profilegroupmember', true)); ?>'); return false;" ><i class="fa add_people"></i><span><?php echo $this->translate("Add People"); ?></span></a>	
					</div>
      ...
      <?php if (is_array($this->showContent) && in_array('backButton', $this->showContent)) : ?>
        <?php 
          $adminSelectedProject = Engine_Api::_()->sitecrowdfunding()->adminSelectedProject('sitegroup_index_view');
          $ownerProjectId = Engine_Api::_()->getApi('settings', 'core')->getSetting("sitegroup".$this->subject()->getIdentity().".choosed.project", 0); 
          $project_id = null; 
          if($adminSelectedProject) {
            $project_id = $adminSelectedProject;
            $backTitle = Engine_Api::_()->sitecrowdfunding()->adminSelectedProject('sitegroup_index_view', true);
          } elseif($ownerProjectId) {
            $project_id = $ownerProjectId;
            $backTitle = Engine_Api::_()->getApi('settings', 'core')->getSetting("sitegroup".$this->subject()->getIdentity().".back.project.label", 'Donate Now');
          }  
        ?>       
