...
<div class="seaocore_profile_cover_head_section_inner" id="seaocore_profile_cover_head_section_inner">
  ...
  <?php if(($this->profile_like_button == 1) || (in_array('followButton', $this->showContent)) || (in_array('optionsButton', $this->showContent)) || in_array('backButton', $this->showContent)):?>
    <div class="seaocore_profile_coverinfo_buttons">
      ...
      <?php if (is_array($this->showContent) && in_array('backButton', $this->showContent)) : ?>
        <?php 
          $adminSelectedProject = Engine_Api::_()->sitecrowdfunding()->adminSelectedProject("sitereview_index_view_listtype_".$this->subject()->listingtype_id); 
          $ownerProjectId = Engine_Api::_()->getApi('settings', 'core')->getSetting("sitereview_".$this->subject()->listingtype_id.$this->subject()->getIdentity().".choosed.project", 0);
          $project_id = null; 
          if($adminSelectedProject) {
            $project_id = $adminSelectedProject;
            $backTitle = Engine_Api::_()->sitecrowdfunding()->adminSelectedProject("sitereview_index_view_listtype_".$this->subject()->listingtype_id, true);
          } elseif($ownerProjectId) {
            $project_id = $ownerProjectId;
            $backTitle = Engine_Api::_()->getApi('settings', 'core')->getSetting("sitereview_".$this->subject()->listingtype_id.$this->subject()->getIdentity().".back.project.label", 'Donate Now');
          }  
        ?>       
