  ...
  <div class="seaocore_profile_coverinfo_status">
    ...
    <?php if (is_array($this->showContent) && in_array('hostName', $this->showContent) && $this->subject()->getHost()): ?>
        <div class="siteevent_listings_stats">
          <div class="o_hidden f_small" title="<?php echo $this->translate("Host") ?>">
...
<i class="siteevent_icon_strip siteevent_icon siteevent_icon_venue" title="<?php echo $this->translate("Venue") ?>"></i>           
 <?php echo $this->subject()->venue_name; ?>
  ...
  <?php if(is_array($this->showContent) && (($this->profile_like_button == 1) || in_array('inviteGuest', $this->showContent) || in_array('joinButton', $this->showContent) || in_array('updateInfoButton', $this->showContent) || in_array('inviteRsvpButton', $this->showContent)  || (in_array('optionsButton', $this->showContent) ) || in_array('backButton', $this->showContent))):?>
    <div class="seaocore_profile_coverinfo_buttons">
      ...
      <?php endif;?>
      <?php // event-rsvp-label switching
                $setting = Engine_Api::_()->getApi('core', 'siteevent');
                $rsvpMemberSetting = $setting->getRsvpMembershipSetting();
      ?>
    ...
    <?php endif;?>
    <?php if ($rsvpMemberSetting && is_array($this->showContent) && in_array('inviteRsvpButton', $this->showContent)) : ?>
                <div>
    ...
    <?php endif; ?>
      ...
      <?php
        ...
        ?>
       <div id="sitecontentcoverphoto_calender_buttons" class="siteevent_grid_footer seaocore_button">
   <?php if(!$this->contentFullWidth):?><div class="seaocore_like_button"><?php endif;?><a <?php if(!$this->contentFullWidth):?> style="background-color: #fff;line-height:20px;" <?php endif;?> class="siteevent_share_links_toggle siteevent_buttonlink" >
           
            <span class="" id="sitecontentcoverphoto_calendar_toggle" onclick="showCoverPhotoCalendarLinks()">Add To Calendar</span></a>
            <?php if(!$this->contentFullWidth):?></div><?php endif;?>
             <ul id="sitecontentcoverphoto_calendar_links" class="siteevent_calendar_links dropdown_menu" style="display:none;<?php if(!$this->contentFullWidth):?>top:30px;<?php endif;?>">
      ...
      <?php if (is_array($this->showContent) && in_array('backButton', $this->showContent)) : ?>
        <?php 
          $adminSelectedProject = Engine_Api::_()->sitecrowdfunding()->adminSelectedProject('siteevent_index_view');
          $ownerProjectId = Engine_Api::_()->getApi('settings', 'core')->getSetting("siteevent".$this->subject()->getIdentity().".choosed.project", 0);
          $project_id = null; 
          if($adminSelectedProject) {
            $project_id = $adminSelectedProject;
            $backTitle = Engine_Api::_()->sitecrowdfunding()->adminSelectedProject('siteevent_index_view', true);
          } elseif($ownerProjectId) {
            $project_id = $ownerProjectId;
            $backTitle = Engine_Api::_()->getApi('settings', 'core')->getSetting("siteevent".$this->subject()->getIdentity().".back.project.label", 'Donate Now');
          }  
        ?>       
      ...
      <?php // event-rsvp-label switching
                $setting = Engine_Api::_()->getApi('core', 'siteevent');
                $rsvpLabel = $setting->getRsvpMembershipSetting();
      ?>
      <?php if (!$rsvpMemberSetting && is_array($this->showContent) && in_array('inviteRsvpButton', $this->showContent)) : ?>
				<div>
					<?php echo $this->content()->renderWidget("siteevent.invite-rsvp-siteevent"); ?>
