  ...
  attachEvent: function(widget_id, params) {
    ...
    if (element) {
      ...
      element.addEvent('click', function() {
        if (en4.sitefaq.ajaxTab.click_elment_id == widget_id)
          return;
        en4.sitefaq.ajaxTab.click_elment_id = widget_id;
        en4.sitefaq.ajaxTab.sendReq(params);
      });
          ...
          element.getElementById('main_tabs').getElements('li:first-child').each(function(el) { 
            ...
            el.get('class').split(' ').each(function(className) {
              ...
              if (className.match(/^tab_[0-9]+$/) && className == "tab_" + widget_id) {
                attachOnLoadEvent = true;
                if (addActiveTab || tab_content_id_sitefaq == widget_id) {
