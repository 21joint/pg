...
<script type="text/javascript">
  ...
  function setLikeBox(){
    ...
    var lodingImage='<center><img src="<?php echo $this->layout()->staticBaseUrl; ?>application/modules/Sitepage/externals/images/loader.gif" alt="Loading..."  /></center>' ;
    ...
    var tempsrcUrl = '';  
    <?php foreach ($this->multiOptions as $key => $option): ?>
            if ($("<?php echo $option?>")) {
            var <?php echo $option?> = ($("<?php echo $option?>").checked);
            tempsrcUrl = tempsrcUrl + "&<?php echo $option?>="+<?php echo $option?>;
        }
        <?php endforeach; ?>
    var srcUrl = '';
    var page_id = '<?php echo $this->page_id; ?>';
		...
		if( likebox_type == 1 ) {
			srcUrl="<?php echo $this->url( array ( 'action' => 'index' ) , 'sitepagelikebox_general' , true ) ?>?page_id="+page_id+"&width="+width+"&height="+height+"&titleturncation="+titleturncation+"&border_color="+border_color+"&colorscheme="+colorscheme+"&faces="+faces+"&stream="+stream+"&streamupdatefeed="+streamupdatefeed+"&streaminfo="+streaminfo+"&streammap="+streammap+"&streamreview="+streamreview+"&streamalbum="+streamalbum+"&streamdiscussion="+streamdiscussion+"&streamevent="+streamevent+"&streampoll="+streampoll+"&streamnote="+streamnote+"&streamoffer="+streamoffer+"&streamvideo="+streamvideo+"&streamdocument="+streamdocument+"&streammusic="+streammusic+ tempsrcUrl +"&header="+header+"&edit="+1;
		}
  ...
  function getCode()	{
		...
		if( likebox_type == 1 ) {
			...
			srcUrl="<?php echo $this->url( array ( 'action' => 'get-like-code' ) , 'sitepagelikebox_general' , true ) ?>?page_id="+page_id;
