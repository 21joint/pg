...
<?php
$this->headLink()->appendStylesheet($this->layout()->staticBaseUrl . 'application/modules/Sitepagelikebox/externals/styles/sitepagelikebox.css');
...
<?php if (!Engine_Api::_()->getApi( 'settings' , 'core' )->getSetting( 'likebox.colorschme' , 1 )): ?>
	...
	<?php
$this->headLink()->appendStylesheet($this->layout()->staticBaseUrl . "application/modules/Sitepagelikebox/externals/styles/$colorScheme.css");
...
<?php else: ?>
	...
	<?php if($this->value['colorscheme']=="dark"):  ?>
		<?php
$this->headLink()->appendStylesheet($this->layout()->staticBaseUrl . 'application/modules/Sitepagelikebox/externals/styles/dark.css');
//$this->headLink()->appendStylesheet( $this->baseUrl( '/application/modules/Sitepagelikebox/externals/styles/dark.css' )) ;	?>
	<?php else:?>
		<?php
$this->headLink()->appendStylesheet($this->layout()->staticBaseUrl . 'application/modules/Sitepagelikebox/externals/styles/light.css');
			...
			<div class='splb_tabs_alt'>
				<ul class="splb_tabs">
					...
					?>
                                    ...
                                    <?php if($this->EnabledListingTypes):?>
                                    <?php foreach($this->EnabledListingTypes as $listingTypeId): ?>
                                            <?php $listingtTypeName = Engine_Api::_()->getDbTable('listingtypes', 'sitereview')->getListingTypeColumn($listingTypeId); 
                                                $resultName = 'sitereview_'.$listingtTypeName.'_likebox_result';
                                                $activeName = 'sitereview_'.$listingtTypeName.'_likebox_active';
                                                $listings = Engine_Api::_()->getDbTable('listingtypes', 'sitereview')->getPageListings($listingTypeId, $this->sitepage->page_id);
                                            ?>
                                  <?php if(count($listings)):
                                      $listingtTypeName = str_replace(' ','_',$listingtTypeName);?>
                                    <?php   if($this->value["$listingtTypeName"]=="true"): ?>
                                    <li class="<?php echo $activeClass; ?>" onclick= "showmoduleResult('<?php echo $resultName; ?>', '<?php echo $activeName; ?>')" id= "<?php echo $activeName; ?>"  >
											<a><?php echo $this->translate($listingtTypeName); ?></a>
										</li>
                                    <?php endif;?>
                                    <?php endif;?>
                                     <?php    endforeach;
?>
    ...
    $str =  $this->PageProfileFieldValueLoop($this->sitepage, $this->fieldStructure, $params)?>
		<?php if($str): ?>
			<h4 >
				<span><?php  echo $this->translate('Profile Information');  ?></span>
			</h4>
			<?php echo $this->PageProfileFieldValueLoop($this->sitepage, $this->fieldStructure, $params) ?>
		<?php endif; ?>
									<?php endif; ?>
									<br />
								</div>
								...
								<?php if(!empty($this->ratingDataTopbox) && !empty($this->noReviewCheck)):?>
									...
									<?php foreach($this->ratingDataTopbox as $reviewcatTopbox): ?>
										...
										<li class="splb_ror">
											...
											<?php if(!empty($reviewcatTopbox['reviewcat_name'])): ?>
												<div class="splb_rorr">
													<ul class='rating-box-small <?php echo $rating_valueTopbox; ?>' style="background-image: url(<?php echo $this->layout()->staticBaseUrl; ?>application/modules/Sitepagereview/externals/images/rating-img-small.png);">
							...
							<?php if($flag == 1): ?>
								<li>
									<div class="splb_tab_cont_list_img">
										...
										<?php else: ?>
											<?php echo $this->htmlLink($sitepagedocument->getHref(), '<img src="'.$this->layout()->staticBaseUrl.'application/modules/Sitepagedocument/externals/images/sitepagedocument_thumb.png" />', array('target'=>"_blank") , array('title' => $sitepagedocument->sitepagedocument_title)) ?>
										<?php endif;?>
									</div>
									<div class='splb_tab_cont_list_title'>
					 ...
					 <?php if($this->EnabledListingTypes):?>
                                         <?php foreach($this->EnabledListingTypes as $listingTypeId): ?>
                                            ...
                                            <?php if(count($listings)):
                                                $listingtTypeName = str_replace(' ','_',$listingtTypeName); ?>
                            <?php   if($this->value["$listingtTypeName"]=="true"): ?>
                                                <div class='' id = "<?php echo $resultName ?>" <?php if ($this->current_activetab == $resultName ): ?> style="display:block;" <?php else: ?> style="display:none;" <?php endif; ?> >
							<ul class="splb_tab_cont_list">
								<?php foreach ($listings as $listing): ?>
								<?php $flag = 1; ?>
								<?php if($listing->draft == 0 && $listing->approved == 1 ):?>
									<?php $flag = 1; ?>
								<?php elseif($this->level_id == 1 || $this->viewer_id == $this->sitepage->owner_id || $this->viewer_id == $listing->owner_id || $this->can_edit == 1): ?>
									<?php $flag = 1; ?>
								<?php endif; ?>
							<?php if($flag == 1): ?>
								<li>
									<div class="splb_tab_cont_list_img">
										<?php echo $this->htmlLink($listing->getHref(array('profile_link' => 1)), $this->itemPhoto($listing, 'thumb.icon')) ?>
									</div>
									<div class='splb_tab_cont_list_title'>
										<?php echo $this->htmlLink($listing->getHref(), Engine_Api::_()->sitepage()->truncation($listing->title,$this->titleturncation), array('target'=>"_blank"), array('title' => $listing->title)) ?>
									</div>
								</li>
								<?php endif; ?>
								<?php endforeach; ?>
