	...
	?>
	  <?php if( !$this->noList ): ?><li id="activity-item-<?php echo $action->action_id ?>"><?php endif; ?>
	    ...
	    <div class='splb_feed_item_photo'>
            <?php
              $itemPhoto = (isset($action->getTypeInfo()->is_object_thumb) && !empty($action->getTypeInfo()->is_object_thumb)) ? $action->getObject() : $action->getSubject();
              ...
              echo $this->htmlLink($itemPhoto->getHref(), $this->itemPhoto($itemPhoto, 'thumb.icon', $itemPhoto->getTitle()), array('target' => '_blank')); 
            ?>
	    </div>
