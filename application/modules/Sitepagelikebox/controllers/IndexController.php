...
class Sitepagelikebox_IndexController extends Core_Controller_Action_Standard {
  ...
  public function indexAction() {
    ...
    if (isset($_GET['page_id']) && !empty($_GET['page_id'])) {
        ...
        $this->view->sitepage = $sitepage = Engine_Api::_()->getItem('sitepage_page', $_GET['page_id']);
      if (empty($sitepage)) {
        return;
      }
...
//       $this->view->header = $value['header'] = $_GET['header'] ;
    ...
    if ($value['stream'] == "true") {
      ...
      $this->view->discussionresult = array();
      ...
      //CHECK AND GET THE CONTENT OF MUSIC.
       $this->view->playlistsresult = array();
      ...
      //CHECK AND GET THE CONTENT OF DOCUMENTS.
       $this->view->ratingDataTopbox = array();
      ...
      //CHECK AND GET THE CONTENT OF DOCUMENTS.
       $this->view->documentsresult = array();
      ...
      //Check and get content of Listings 
      if (Engine_Api::_()->hasModuleBootstrap('sitereview')) {
                $this->view->EnabledListingTypes = $EnabledListingTypes = Engine_Api::_()->getApi('settings', 'core')->getSetting('sitepagelikebox.listing.types');
  ...
  public function likeBoxAction() {
    ...
    if (Engine_Api::_()->getDbtable('modules', 'core')->isModuleEnabled('sitereview')) {
            ...
            $values = Engine_Api::_()->getApi('settings', 'core')->getSetting('sitepagelikebox.listing.types');
            foreach ($listingTypes as $listingType) {
                if (Engine_Api::_()->getApi('settings', 'core')->getSetting("sitereview.listing." . $listingType->listingtype_id, 0))
                    $multiOptions["$listingType->listingtype_id"] = str_replace(" ", "_", $listingType->title_plural);;
            }
        }
  ...
  public function getLikeCodeAction() {
    ...
    if(Engine_Api::_()->getDbtable('modules', 'core')->isModuleEnabled('sitereview')){
        $listingTypes = Engine_Api::_()->getDbTable('listingtypes', 'sitereview')->getListingTypes();
            ...
            foreach ($listingTypes as $listingType) {
                if (Engine_Api::_()->getApi('settings', 'core')->getSetting("sitereview.listing." . $listingType->listingtype_id, 0)) {
                    if (isset($_GET[$listingType->title_plural]))
                        $value[$listingType->title_plural] = $_GET[$listingType->title_plural];
                }
            }
        }
    ...
    $this->view->code = $code = '&lt;iframe src="' . "http://" . $_SERVER['HTTP_HOST'] . $this->view->url(array('action' => 'index'), 'sitepagelikebox_general', true) . '?page_id=' . urlencode($value['page_id']) . '&amp;amp;width=' . urlencode($value['width']) . '&amp;amp;height=' . urlencode($value['height']);
    ...
    if (Engine_Api::_()->getDbtable('modules', 'core')->isModuleEnabled('sitereview')) {
              foreach ($listingTypes as $listingType) {
                  if (Engine_Api::_()->getApi('settings', 'core')->getSetting("sitereview.listing." . $listingType->listingtype_id, 0)) {
                      if (isset($_GET[$listingType->title_plural]))
                          $code.='&amp;amp;' . $listingType->title_plural . '=' . urlencode($value[$listingType->title_plural]);
                  }
              }
          }
