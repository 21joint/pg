...
//        'value' => 1 ,
    ...
    $modules_sitepagelikebox = Engine_Api::_()->getApi( 'settings' , 'core' )->getSetting( 'modules.sitepagelikebox' ) ;
    $values['modules_sitepagelikebox'] = unserialize( $modules_sitepagelikebox ) ;
    ...
    if ( !empty( $enableSubModules ) ) {
      ...
      $this->addElement( 'MultiCheckbox' , 'modules_sitepagelikebox' , array (
        'description' => $desc_modules ,
        'label' => 'Tabs from Apps' ,
        'multiOptions' => $enableSubModules ,
        'value' => $values['modules_sitepagelikebox'] ,
          ) ) ;
      $this->modules_sitepagelikebox->addDecorator( 'Description' , array ( 'placement' => Zend_Form_Decorator_Abstract::PREPEND , 'escape' => false ) ) ;
    }
    ...
    if(Engine_Api::_()->hasModuleBootstrap('sitereview')){
        ...
        foreach ($listingTypes as $listingType) {
            ...
            $this->addElement( 'MultiCheckbox' , 'sitepagelikebox_listing_types' , array (
        'description' => "Select the Listing types from which tabs should be available for display in embeddable Business Badges / Like Boxes :" ,
        'label' => 'Listing types Tab' ,
