    ...
    function viewMoreTabMutualFriend()
    {
		...
		function userWidgetRequestSend (member_id, group_id) 
		{
			...
			en4.core.request.send(new Request.HTML({
				...
				'onSuccess' : function(responseTree, responseElements, responseHTML, responseJavaScript)
				{
					document.getElementById('more_results_shows_'+member_id).innerHTML = '<?php echo $this->string()->escapeJavascript($this->translate("You have successfully approve this member.")); ?>';
		...
		function rejectmemberRequestSend (member_id, group_id, user_id) {
			...
			en4.core.request.send(new Request.HTML({
				...
				'onSuccess' : function(responseTree, responseElements, responseHTML, responseJavaScript) { 
					document.getElementById('more_results_shows_'+member_id).innerHTML = '<?php echo $this->string()->escapeJavascript($this->translate("You have ignored the invite to the group.")); ?>';
					setTimeout("hideField(" + member_id + ")", 1000);
...
<?php foreach( $this->paginator as $value ): ?>
  ...
  <div class="item_member_list" id="more_results_shows_<?php echo $value->member_id; ?>">
    <div class="item_member_thumb">
      <?php echo $this->htmlLink($value->getHref(), $this->itemPhoto($value->getOwner(), 'thumb.icon'), array('target' => '_parent', 'rel'=>'user'.' '.$value->user_id)); ?>
    </div>
    <div class="item_member_option">
			<?php if($value->active == 0 &&  $value->user_approved == 0 ) : ?>
			
				<a href="javascript:void(0);" onclick="userWidgetRequestSend('<?php echo $value->member_id ?>', '<?php echo $value->group_id ?>');" class="icon_sitegroup_accept buttonlink"><?php echo $this->translate('Approve Member')?></a>
				
				<a href="javascript:void(0);" onclick="rejectmemberRequestSend('<?php echo $value->member_id ?>', '<?php echo $value->group_id ?>', '<?php echo $value->user_id ?>');" class="icon_sitegroup_cancel buttonlink"><?php echo $this->translate('Reject Request')?></a>
			<?php endif;?>
		</div>
    <div class="item_member_details">
      <div class="item_member_name">
        <?php echo $this->htmlLink($value->getHref(), $value->getTitle(), array('title' => $value->getTitle(), 'target' => '_parent', 'rel'=>'user'.' '.$value->user_id)); ?>
      </div>
    </div>
  </div>
<?php endforeach;?>
...
<script type="text/javascript">
 function smoothboxclose () {
  parent.window.location.href = "<?php echo $this->url(array('group_url' => Engine_Api::_()->sitegroup()->getGroupUrl($value->group_id), 'tab' => $this->tab_selected_id), 'sitegroup_entry_view', true) ?>";
