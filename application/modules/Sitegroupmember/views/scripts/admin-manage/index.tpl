...
<p>
  <?php echo $this->translate('This group lists all those members of your site who have joined the Groups created on your site. If you need to search for a specific member, then enter your search criteria in the fields below. Here, you can also make members featured / un-featured.');?>
</p>
...
<script type="text/javascript">
  ...
  var changeOrder = function(order, default_direction){
	...
	{
		return confirm('<?php echo $this->string()->escapeJavascript($this->translate("Are you sure you want to delete selected group offers ?")) ?>');
	}
  ...
  <div class="search">
    <form method="post" class="global_form_box" action="">
      ...
      <?php if(count($categories) > 0) :?>
        <div>
          <label>
            <?php echo  $this->translate("Group Category") ?>
          </label>
           <select id="" name="category_id" onchange="subcategory(this.value, '', '', '');">
			...
			<div id="subcategory_backgroundimage" class="cat_loader"><img src="<?php echo $this->layout()->staticBaseUrl ?>application/modules/Sitegroup/externals/images/loading.gif" /></div>
		 	<div id="subcategory_id-label">
				<label>
						<?php echo  $this->translate("Group Subcategory") ?>	
				</label>
      ...
      <div id="subsubcategory_backgroundimage" class="cat_loader"><img src="<?php echo $this->layout()->staticBaseUrl ?>application/modules/Sitegroup/externals/images/loading.gif" /></div>
		 	<div id="subsubcategory_id-label">
				<label>
						<?php echo  $this->translate('Group 3%s Level Category', "<sup>rd</sup>") ?>
				</label>
...
<div class='admin_search'>
  ...
  ?><br />
	...
	<br />
	  <form id='multidelete_form' method="post" action="<?php echo $this->url(array('module' => 'sitegroupmember', 'controller' => 'manage', 'action' => 'multi-delete-member'), 'admin_default'); ?>" onSubmit="return multiDelete()">
		<table class='admin_table' border="0" width="50%">
			<thead>
		      ...
		      <th style='width: 4%;' align="left"><a href="javascript:void(0);" onclick="javascript:changeOrder('title', 'ASC');"><?php echo $this->translate('Joined Groups'); ?></a></th>
				</tr>
			</thead>
			<tbody>
					...
					<?php foreach( $this->paginator as $item ):  ?>
						<tr>
							...
							<?php if($item->featured_member == 1):   ?>
								<td align="center" class="admin_table_centered"> <?php echo $this->htmlLink(array('route' => 'default', 'module' => 'sitegroupmember', 'controller' => 'admin-manage', 'action' => 'featured', 'user_id' => $item->user_id), $this->htmlImage($this->layout()->staticBaseUrl . 'application/modules/Sitegroup/externals/images/sitegroup_goldmedal1.gif', '', array('title'=> $this->translate('Make Un-featured')))); ?>
							<?php else: ?>
								<td align="center" class="admin_table_centered"> <?php echo $this->htmlLink(array('route' => 'default', 'module' => 'sitegroupmember', 'controller' => 'admin-manage', 'action' => 'featured', 'user_id' => $item->user_id), $this->htmlImage($this->layout()->staticBaseUrl . 'application/modules/Sitegroup/externals/images/sitegroup_goldmedal0.gif', '', array('title'=> $this->translate('Make Featured')))) ?>
								</td>
							<?php endif; ?>
							<td class='admin_table_bold admin_table_centered'>
							<?php echo $this->htmlLink(array('route' => 'default', 'module' => 'sitegroupmember', 'controller' => 'admin-manage', 'action' => 'group-join', 'user_id' => $item->user_id), $this->translate($this->locale()->toNumber($item->JOINP_COUNT)), array('onclick' => 'owner(this);return false')); ?>
						</tr>
					<?php endforeach; ?>
				<?php endif; ?>
			</tbody>
