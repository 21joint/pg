...
class Sitegroupmember_Form_Searchwidget extends Engine_Form {
  ...
  public function init() {
    ...
    $this->setAction($view->url(array(), 'sitegroupmember_browse', true))->getDecorator('HtmlTag')->setOption('class', '');
  }
  ...
  public function getAdditionalOptionsElement() {
    ...
    $select_content = $coreContent_table->select()->where('name = ?', 'sitegroupmember.search-sitegroupmember');
      ...
      $this->addElement('Text', 'title', array(
		...
		if (in_array("2", $enabledColumns)) {
			$this->addElement('Select', 'orderby', array(
				...
				'order' => $row->order - 1,
				'onchange' => $this->gethasMobileMode() ? '' : 'searchSitegroupvideos();',
			));
    }
