...
$allowedAdminTitle = array('core.html-block', 'core.rich-text-block'); ?>
...
<script type="text/javascript">
  ...
  var setWidgetParams = function(params) {
    ...
    var contentParams = currentEditingElement.retrieve('contentParams') || {};
    var oldParams = contentParams.params || {};
    var element = currentEditingElement;
    contentParams.params = params;
    currentEditingElement.store('contentParams', contentParams);
    currentEditingElement = false;

    // Signal modification
    pushModification('main');
    var allowAdminTitle = <?php echo $this->jsonInline($allowedAdminTitle) ?>;
    if( allowAdminTitle.indexOf(params.name) === -1 ) {
        return;
    }
    var oldAdminTitle = oldParams.adminTitle || contentByName[params.name]['title'];
    var updatedAdminTitle = params.adminTitle || contentByName[params.name]['title'];
    var updateHtml = element.get('html').replace(oldAdminTitle, updatedAdminTitle);
    element.set('html', updateHtml);
  }
                        ...
                        ?>
                          ...
                          <?php elseif( empty($structThreeInfo['canHaveChildren']) ): ?>
                            <li id="admin_content_<?php echo $structThree['identity'] ?>" class="admin_content_cell admin_content_buildable admin_content_draggable admin_content_<?php echo $structThree['type'] . '_' . $structThree['name'] ?><?php if( !empty($structThreeInfo['special']) ) echo ' htmlblock' ?>">
                              <?php
                                // showing admin title
                                if( in_array($structThree['name'], $allowedAdminTitle) && (!empty($structThreeNE['params']['adminTitle'])) ):
                                  echo $this->translate($structThreeNE['params']['adminTitle']);
                                else:
                                  echo $this->translate($this->contentByName[$structThree['name']]['title']) ;
                                endif;
                              ?>
                              <span class="open">
                                |
                                  ...
                                  ?>
                                    ...
                                    <?php else: ?>
                                      <li id="admin_content_<?php echo $structFour['identity'] ?>" class="admin_content_cell admin_content_buildable admin_content_draggable admin_content_<?php echo $structFour['type'] . '_' . $structFour['name'] ?>">
                                        <?php
                                          // showing admin title
                                          if( in_array($structThree['name'], $allowedAdminTitle) && (!empty($structFourNE['params']['adminTitle'])) ):
                                            echo $this->translate($structFourNE['params']['adminTitle']);
                                          else:
                                            echo $this->translate($this->contentByName[$structFour['name']]['title']) ;
                                          endif;
                                        ?>
