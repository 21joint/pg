...
<head>
  <base
    href="<?php echo rtrim($this->serverUrl($this->baseUrl()), '/') . '/' ?>"/>
  ...
  if ($this->subject() && $this->subject()->getPhotoUrl()) {
    $this->headMeta()->setProperty(
      'og:image', $this->absoluteUrl($this->subject()->getPhotoUrl())
    );
  }
  ...
  $this->headLink(
    array(
      'rel' => 'shortcut icon',
      'href' => $staticBaseUrl . (isset($this->layout()->favicon)
  ...
  $currentTheme = APPLICATION_PATH . '/application/themes/' . $themes[0]
    . '/default.tpl';
  $currentThemeHeader = APPLICATION_PATH . '/application/themes/' . $themes[0]
    . '/head.tpl';
  ?>
  ...
  <script type="text/javascript">
    ...
    <?php endif; ?>
    if ( <?php echo(Engine_Api::_()->getDbtable(
      'settings', 'core'
    )->core_dloader_enabled ? 'true' : 'false') ?> ) {
