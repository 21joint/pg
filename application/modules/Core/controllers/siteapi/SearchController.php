            ...
            if (isset($forumSearch) && !empty($forumSearch)) {
                ...
                if (is_array($availableTypes) && count($availableTypes) > 0) {
                    ...
                    if ((_ANDROID_VERSION && _ANDROID_VERSION >= '1.7.7.1') || (_IOS_VERSION && _IOS_VERSION >= '1.6.5')) {
                        ...
                        $notIncludeArray = array('activity', 'messages', 'core', 'cometchat');
                        ...
                        foreach ($menuObj as $menu) {
                            ...
                            if ($menu->module == 'sitevideo') {
                                if ($menu->dashboard_label == 'Advanced Videos') {

                                    $options[$menu->module . '_video'] = $this->translate($menu->dashboard_label);
                                } elseif ($menu->dashboard_label == 'Videos Channels') {

                                    $options[$menu->module . '_channel'] = $this->translate($menu->dashboard_label);
                                } else {
                                    $options[$menu->module . '_playlist'] = $this->translate($menu->dashboard_label);
                                }
                            } else
            ...
            foreach ($paginator as $item) {
               ...
               if(strtolower($module_name) == 'video'){
                ...
                if (isset($itemArray['content_url']) && !isset($itemArray['uri']))
                 ...
                 if (strstr($itemArray['item_type'], 'video')&& !strstr($itemArray['item_type'], 'channel') && !strstr($itemArray['item_type'], 'palylist')){
