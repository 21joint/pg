  ...
  public function sendSystem($recipient, $type, array $params = array())
  {
    ...
    // Queue
      ...
      foreach( $recipients as $oneRecipient ) {
        ...
        } else if( is_numeric($oneRecipient) ) {
          $mailRecipientsTable->insert(array(
            'mail_id' => $mail_id,
            'user_id' => $oneRecipient,
          ));
        }
      }
    }
  ...
  public function sendSystemRaw($recipient, $type, array $params = array())
  {
    ...
    foreach( $recipients as $recipient ) {
      ...
      if( $recipient instanceof Core_Model_Item_Abstract ) {
        ...
        $photoUrl = $recipient->getPhotoUrl('thumb.normal');
        $view = Zend_Registry::get('Zend_View');
        $photoUrl = $view->absoluteUrl($photoUrl);
        $recipientImg = $view->htmlImage($photoUrl, '', array(
          'max-width' => '100px',
          ));

        $rParams['recipient_photo'] = $recipientImg;
      ...
      if( !$bodyHtmlHeader ) {
        $bodyHtmlHeader = nl2br($bodyTextHeader);
      }

      if( !$bodyHtmlFooter ) {
        $bodyHtmlFooter = nl2br($bodyTextFooter);
      }
