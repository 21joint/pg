 ...
 * @version    $Id: windowlivelogin.php 2011-05-05 9:40:21Z SocialEngineAddOns $
...
class WindowsLiveLogin {
   ...
   */
  public function __construct($appid=null, $secret=null, $securityalgorithm=null, $force_delauth_nonprovisioned=null, $policyurl=null, $returnurl=null) {
  ...
  public static function initFromXml($settingsFile) {
    ...
    $settings['appid'] = Engine_Api::_()->getApi('settings', 'core')->getSetting('windowlive.apikey');
    $settings['secret'] = Engine_Api::_()->getApi('settings', 'core')->getSetting('windowlive.secretkey');
    $settings['securityalgorithm'] = 'wsignin1.0';
    $settings['returnurl'] = ( _ENGINE_SSL ? 'https://' : 'http://' ) . $_SERVER['HTTP_HOST'] . Zend_Controller_Front::getInstance()->getBaseUrl() . '/sitepageinvite/usercontacts/getwindowlivecontacts';
    $settings['policyurl'] = Engine_Api::_()->getApi('settings', 'core')->getSetting('windowlive.policyurl');
    if (empty($settings['policyurl'])) {
      $settings['policyurl'] = 'http://' . $_SERVER['HTTP_HOST'] . Zend_Controller_Front::getInstance()->getBaseUrl() . '/help/privacy';
    }
    //$settings['policyurl']	= 'http://devaddons.socialengineaddons.com/LiveContactsPHP/policy.html';
  ...
  public function setPolicyUrl($policyurl) {
    ...
    if (!$policyurl) {
      if ($_force_delauth_nonprovisioned) {
        echo Zend_Registry::get('Zend_Translate')->_('Please make sure that you have entered the policy url for Window Live settings.');
        die;
  ...
  public function decodeAndValidateToken($token, $cryptkey=null, $signkey=null, $internal_allow_recursion=true) {
    if (!$cryptkey) {
