 ...
 * @version    $Id: getwindowlivecontacts.tpl 2011-05-05 9:40:21Z SocialEngineAddOns $
...
if (!$this->errormessage) {
	...
	if ($total > 0) { ?>
		<div class="header">	
			<div class="title">	
				<?php echo $this->translate("Found %s Window Live contacts you can promote this Page to.", $total);?>
			</div>
			<div>
	...
	if (!empty($this->addtononfriend)) { ?>
		...
		<?php  
			$getWindowLivePer = Zend_Registry::isRegistered('sitepageinvite_getWindowLivePer') ? Zend_Registry::get('sitepageinvite_getWindowLivePer') : null; 
			if( !empty($getWindowLivePer) ):
		?>
		<div class="buttons">
