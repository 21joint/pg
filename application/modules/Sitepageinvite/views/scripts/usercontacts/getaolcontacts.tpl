 ...
 * @version    $Id: getaolcontacts.tpl 2011-05-05 9:40:21Z SocialEngineAddOns $
...
if (!$this->errormessage) {
	...
	if ($total > 0) { ?>
		<div class="header">	
			<div class="title">	
				<?php echo $this->translate("Found %s Aol contacts you can promote this Page to.", $total);?>
			<div>
	...
	if (!empty($this->addtononfriend)) { ?>
		...
		<?php  
			$getAolPer = Zend_Registry::isRegistered('sitepageinvite_getAolPer') ? Zend_Registry::get('sitepageinvite_getAolPer') : null; 
			if( !empty($getAolPer) ):
		?>
			<div class="buttons">
