 ...
 * @version    $Id: getyahoocontacts.tpl 2011-05-05 9:40:21Z SocialEngineAddOns $
...
if (!$this->errormessage) {
	...
	if ($total > 0) { ?>
		<div class="header">	
			<div class="title">	
				<?php echo $this->translate("Found %s Yahoo contacts you can promote this Page to.", $total);?>
			</div>
			<div>
	...
	if (!empty($this->addtononfriend)) { ?>
		...
		<?php  
			$getYahooPer = Zend_Registry::isRegistered('sitepageinvite_getYahooPer') ? Zend_Registry::get('sitepageinvite_getYahooPer') : null; 
