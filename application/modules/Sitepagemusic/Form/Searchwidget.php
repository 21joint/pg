 ...
 */
class Sitepagemusic_Form_Searchwidget extends Engine_Form {
  ...
  public function init() {
    ...
    $this->setAction($view->url(array(), 'sitepagemusic_browse', true))->getDecorator('HtmlTag')->setOption('class', '');
  }
  ...
  public function getAdditionalOptionsElement() {
    ...
    $select_content = $coreContent_table->select()->where('name = ?', 'sitepagemusic.search-sitepagemusic');
    ...
    if (in_array("4", $enabledColumns)) {
      $this->addElement('Text', 'search_music', array(
          'label' => 'Playlist Name',
          'order' => $row->order - 4,
      ));
    }
    ...
    if (in_array("1", $enabledColumns)) {
      ...
      $show_multiOptions["creation_date"] = "Everyone's Music";

      if (!empty($viewer_id)) {
        $show_multiOptions["my_music"] = 'My Music';
        $show_multiOptions["my_like"] = 'Music of Pages I Like';
      }

      $show_multiOptions["featured"] = "Featured Music";
      $sitepagePackageEnable = Engine_Api::_()->getApi('settings', 'core')->getSetting('sitepage.package.enable', 1);
      if ($sitepagePackageEnable) {
        $show_multiOptions["sponsored music"] = 'Sponsored Music';
      }
      ...
      $this->addElement('Select', 'show', array(
          ...
          'onchange' => $this->gethasMobileMode() ? '' : 'searchSitepagemusics();',
          'order' => $row->order - 2,
      ));
    }
    ...
    if (in_array("5", $enabledColumns)) {
      ...
      if (count($categories) != 0) {
        ...
        //For Mobile Mode Subcategory Work
