...
Composer.Plugin.SitepageMusic = new Class({
  ...
  activate : function() {
    ...
    this.elements.formInput = new Element('input', {
      'id' : 'compose-sitepagemusic-form-input',
    ...
    if( this.options.fancyUploadEnabled && this.options.fancyUploadOptions ) {
      ...
      this.elements.formFancyFile = new Element('a', {
        ...
        'id' : 'compose-sitepagemusic-form-fancy-file',
        'class' : 'buttonlink',
...
'<div style="display:none;">\n\
  ...
  <div class="current-text"></div>\n\
      ...
      try {
        this.elements.formFancyUpload = new FancyUpload2(
          this.elements.formFancyStatus,
          this.elements.formFancyList,
          opts
        );
      } catch( e ) {
    ...
    this.elements.formSubmit = new Element('button', {
      'id' : 'compose-sitepagemusic-form-submit',
  ...
  deactivate : function() {
    if (this.params.song_id)
      new Request.JSON({
        url: en4.core.basePath + '/sitepagemusic/remove-song',
        data: {
          format: 'json',
  ...
  doProcessResponse : function(responseJSON) {
    ...
    */
    ...
    this.elements.preview = new Element('a', {
      ...
      'class': 'compose-sitepagemusic-link',
      'events' : {
        'click' : function(event) {
          event.stop();
          $(this).toggleClass('compose-sitepagemusic-link-playing');
          $(this).toggleClass('compose-sitepagemusic-link');
