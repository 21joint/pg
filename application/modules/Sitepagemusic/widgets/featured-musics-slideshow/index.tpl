...
foreach ($this->show_slideshow_object as $type => $music) {
  ...
  $musicPhoto = $this->htmlLink(
	$music->getHref(),
	$this->itemPhoto($music, null, $music->getTitle())
  );
  $content_info = null;
  if(Engine_Api::_()->getApi('settings', 'core')->getSetting('sitepage.postedby', 1)):
  $content_info .= $this->timestamp(strtotime($music->creation_date)) . $this->translate(' - created by ') . $this->htmlLink($music->getOwner()->getHref(), $music->getOwner()->getTitle(),array('title' => $music->getOwner()->getTitle()));
  endif;
  //$content_info = '<div class="sitepage_sidebar_list_details"></div>';
  $content_info.='<p>';
  $content_info.=$this->translate(array('%s comment', '%s comments', $music->comment_count), $this->locale()->toNumber($music->comment_count)) . ', ';
  $content_info.=$this->translate(array('%s view', '%s views', $music->view_count), $this->locale()->toNumber($music->view_count)) . ', ';
  $content_info.=$this->translate(array('%s like', '%s likes', $music->like_count), $this->locale()->toNumber($music->like_count));
  $content_info.='</p>';

  $description = $music->description;

  $content_link = $this->htmlLink($music->getHref(array('tab' => $tab_id)), $this->translate('View Music &raquo;'), array('class' => 'featured_slideshow_view_link'));

  $image_text_var .= "<div class='featured_slidebox'>";
  $image_text_var .= "<div class='featured_slidshow_img'>" . $musicPhoto . "</div>";
  ...
  if (!empty($music->title)) {
    ...
    $title = $this->htmlLink($music->getHref(array('tab' => $tab_id)), $this->string()->chunk($this->string()->truncate($music->getTitle(), 45), 10),array('title' => $music->getTitle()));

    $image_text_var .='<h5>' . $this->htmlLink($music->getHref(array('tab' => $tab_id)), $music->title) . '</h5>';
    $sitepage_object = Engine_Api::_()->getItem('sitepage_page', $music->page_id);
    ...
    $image_text_var .= $this->translate("in ") . $this->htmlLink(Engine_Api::_()->sitepage()->getHref($music->page_id, $music->owner_id, $music->getSlug()),  $page_title,array('title' => $sitepage_object->title)); 
    $image_text_var .= "</div>";
  ...
  if (!empty($description)) {
    ...
    $image_text_var .= "<p>" . $truncate_description . " " . $this->htmlLink($music->getHref(array('tab' => $tab_id)), $this->translate('More &raquo;')) . "</p>";
  }
    ...
    window.addEvent('domready',function(){
     ...
     for(var i=0;i < divElements.length;i++)
      ...
      var nS8 = new noobSlide({
        box: $('sitepagemusic_featured_music_im_te_advanced_box'),
        items: $$('#sitepagemusic_featured_music_im_te_advanced_box h3'),
        size: (width-10),
        handles: $$('#handles8 span'),
        addButtons: {previous: $('sitepagemusic_featured_music_prev8'), stop: $('sitepagemusic_featured_music_stop8'), play: $('sitepagemusic_featured_music_play8'), next: $('sitepagemusic_featured_music_next8') },
        interval: 5000,
        fxOptions: {
          duration: 500,
          transition: '',
        ...
        onWalk: function(currentItem,currentHandle){
          ...
          {
            $('sitepagemusic_featured_music_caption').innerHTML =   current_caption_link;
          }
          else {
            $('sitepagemusic_featured_music_caption').innerHTML =  '';
          }


          $('sitepagemusic_featured_music_current_numbering').innerHTML =  current_index + '/' + num_of_slidehsow ;
        }
      });
...
<div class="featured_slideshow_wrapper">
  <div class="featured_slideshow_mask">
    <div id="sitepagemusic_featured_music_im_te_advanced_box" class="featured_slideshow_advanced_box">
  ...
  <div class="featured_slideshow_option_bar">
    <div>
      <p class="buttons">
        <span id="sitepagemusic_featured_music_prev8" class="featured_slideshow_controllers-prev featured_slideshow_controllers prev" title=<?php echo $this->translate("Previous") ?> ></span>
        <span id="sitepagemusic_featured_music_stop8" class="featured_slideshow_controllers-stop featured_slideshow_controllers" title=<?php echo $this->translate("Stop") ?> ></span>
        <span id="sitepagemusic_featured_music_play8" class="featured_slideshow_controllers-play featured_slideshow_controllers" title=<?php echo $this->translate("Play") ?> ></span>
        <span id="sitepagemusic_featured_music_next8" class="featured_slideshow_controllers-next featured_slideshow_controllers" title=<?php echo $this->translate("Next") ?> ></span>
      </p>
    </div>
    <span id="sitepagemusic_featured_music_caption"></span>
    <span id="sitepagemusic_featured_music_current_numbering" class="featured_slideshow_pagination"></span>
  </div>
</div>  
