 ...
 */
class Sitepagemusic_Widget_SearchSitepagemusicController extends Engine_Content_Widget_Abstract {
  ...
  public function indexAction() {
    ...
    //FORM CREATION
    $this->view->form = $form = new Sitepagemusic_Form_Searchwidget(array('type' => 'sitepage_page'));
    $populateValue = 0;
    if( isset($form->show) ) {
			$sponsoredmusic = Zend_Controller_Front::getInstance()->getRequest()->getParam('sponsoredmusic', null);
			if(!empty($sponsoredmusic)) {
				$populateValue = 1;
				$form->show->setValue("sponsored music");
			}
		
			$featuredmusic = Zend_Controller_Front::getInstance()->getRequest()->getParam('featuredmusic', null);
			if(!empty($featuredmusic)) {
				$populateValue = 1;
				$form->show->setValue("featured");
			}
    }
    $commentedmusic = Zend_Controller_Front::getInstance()->getRequest()->getParam('commentedmusic', null);
    if(!empty($commentedmusic)) {
      $populateValue = 1;
			$form->orderby_browse->setValue("comment_count");
    }

    $likedmusic = Zend_Controller_Front::getInstance()->getRequest()->getParam('likedmusic', null);
    if(!empty($likedmusic)) {
      $populateValue = 1;
			$form->orderby_browse->setValue("like_count");
    } 

    $popularmusic = Zend_Controller_Front::getInstance()->getRequest()->getParam('popularmusic', null);
    if(!empty($popularmusic)) {
      $populateValue = 1;
			$form->orderby_browse->setValue("play_count");
    } 
