  ...
  <script type="text/javascript">
    ...
    sm4.core.runonce.add(function (){
      $.mobile.activePage.find('#short_player_<?php echo $random ?>').smMusicShortPlayer({
        mp3: "<?php echo $songs[0]->getFilePath() ?>",
        oga: "<?php echo $songs[0]->getFilePath() ?>"
      },{
        ...
        onPlayCallback:function(){
          ...
          if(!tallied[<?php echo $songs[0]->song_id ?>]){
            ...
            $.ajax({
              type: "POST", 
              dataType: "json",
              'url' : sm4.core.baseUrl + 'pagemusictally/song/<?php echo $songs[0]->song_id ?>/tally',
              'data' : {
                format : 'json',
...
<!--  FULL PLAYER FOR PLAYLIST PLAY ON MUSIC PROFILE PAGE-->
  <div class="music_player_wrapper playlist_<?php echo $playlist->getIdentity() ?>" id="music_player_<?php echo $random ?>">
    <div class="music-player "<?php if (isset($this->id))
    echo "id='{$this->id}'" ?> <?php if ($this->short_player): ?>style="display:none;"<?php endif; ?>>
      ...
      <div class="tracklist">
        <ul class="tracks music-player-tracks">
          ...
          <?php foreach ($songs as $song): if (!empty($song)): ?>
              <?php
              $songsList[] = array(
                  'mp3' => $song->getFilePath(),
                  'oga' => $song->getFilePath(),
                  'title' => $song->getTitle(),
        ...
        onPlayCallback:function(current , song){
          ...
          if(!tallied[song.song_id]){
            ...
            $.ajax({
              type: "POST", 
              dataType: "json",
              'url' : sm4.core.baseUrl + 'pagemusictally/song/' + song.song_id + '/tally',
              'data' : {
                format : 'json',
                song_id: song.song_id,
