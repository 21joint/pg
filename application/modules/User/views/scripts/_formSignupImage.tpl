...
<br/>
  ...
  onResize : lassoSetCoords
    ...
    var uploadSignupPhoto =function(){
      ...
      $('thumbnail-controller').innerHTML = "<div><img class='loading_icon' src='application/modules/Core/externals/images/loading.gif'/><span class='loading-text'><?php echo $this->translate('Loading...');?></span></div>";
      $('SignupForm').submit();
      $('Filedata-wrapper').innerHTML = "";
    }
    var lassoEnd = function(){
      $('lassoImg').setStyle('display', 'block');
      $('thumbnail-controller').innerHTML = '<a href="javascript:void(0);" onclick="lassoStart();"><?php echo $this->string()->escapeJavascript($this->translate('Edit Thumbnail'));?></a>';
      $('lassoMask').destroy();
    }
