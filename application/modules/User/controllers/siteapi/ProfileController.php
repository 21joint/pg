...
class User_ProfileController extends Siteapi_Controller_Action_Standard {
    ...
    public function init() {
        if ($this->getRequestParam('user_id') && (0 !== ($user_id = (int) $this->getRequestParam('user_id')) &&
...
// Set item count per page and current page number
    ...
    private function _gutterMenus() {
        ...
        if ($viewer_id == $subject_id) {
            ...
            $menus[] = Engine_Api::_()->getApi('Siteapi_Core', 'user')->userFriendship($subject);
        }
...
//one way
        ...
        if (_IOS_VERSION >= '2.0.5' && $viewer_id == $subject_id) {
        ...
        if ($viewer_id != $subject_id && _IOS_VERSION>='2.1.5' && !empty($seocoreFollowTable)) {
...
//CHECK USER VALIDATION
    ...
    private function _getAlbumCount() {
        try {
            ...
            $isAdvModEnabled = Engine_Api::_()->getDbtable('modules', 'core')->isModuleEnabled("sitealbum");
...
// Get forums allowed to be viewed by current user
    ...
    private function _profileTAbsContainer($subject) {
        ...
        } elseif (_IOS_VERSION < '2.0.5' || $viewer_id != $subject_id) {
            $select = $subject->membership()->getMembersOfSelect();
            $response[] = array(
                'label' => $this->translate('Friends'),
                'name' => 'friends',
...
// sitegroup module code
        if ((_ANDROID_VERSION && (_ANDROID_VERSION >= '1.7.1')) || (_IOS_VERSION && (_IOS_VERSION >= '1.7.3'))) {
            $isModEnabled = Engine_Api::_()->getDbtable('modules', 'core')->isModuleEnabled("sitegroup");
            if (!empty($isModEnabled)) {
