...
class User_Api_Siteapi_FormValidators extends Siteapi_Api_Validators {
    ...
    /**
     * Validation: user login form
     * 
     * @return array
     */
    public function getLoginFormValidators() {
        $validators = array();
        $options = array('Db_NoRecordExists' => false);
        $validators['email'] = $this->getEmailValidator($options);
        $validators['password'] = $this->getPasswordValidator();
        return $validators;
    }
    ...
    public function getChangePasswordValidators() {
        ...
        $formValidators['passwordConfirm'] = array(
            'required' => true,
            'allowEmpty' => false
        );
    ...
    private function _getSignupAccountFormValidations($formValidators = array()) {
        ...
        if (empty($_REQUEST['facebook_uid']) && empty($_REQUEST['twitter_uid']) && empty($random)) {
