...
class Sitemenu_Installer extends Engine_Package_Installer_Module {
    ...
    function onPreinstall() {
        ...
        $SocialEngineAddOns_version = '4.9.4p13';
    ...
    public function onInstall() {
        ...
        //IF MULTICURRENCY IS INSTALLED THAN DO ENTERY IN MENUITEMS
        $select = new Zend_Db_Select($db);
        $select->from('engine4_core_modules')
                ->where('name = ?', "sitemulticurrency")
                ->where('enabled = ?', 1);
        $isSitemulticurrencyEnabled = $select->query()->fetchObject();

        if (!empty($isSitemulticurrencyEnabled)) {
            $db->query("INSERT IGNORE INTO `engine4_core_menuitems` (`name`, `module`, `label`, `plugin`, `params`, `menu`, `submenu`, `order`) VALUES('sitemenu_mini_currency', 'sitemulticurrency', 'Change Currency', 'Sitemenu_Plugin_Menus', '', 'core_mini', '', 0);");
        }

        //IF MAGENTO INTEGRATION PLUGIN IS INSTALLED THEN DO ENTERY IN MENUITEMS
        $select = new Zend_Db_Select($db);
        $select->from('engine4_core_modules')
                ->where('name = ?', "sitemagento")
                ->where('enabled = ?', 1);
        $isSitemagentoEnabled = $select->query()->fetchObject();

        if (!empty($isSitemagentoEnabled)) {
            $db->query("INSERT IGNORE INTO `engine4_core_menuitems` (`name`, `module`, `label`, `plugin`, `params`, `menu`, `submenu`, `order`) VALUES('sitemenu_mini_magentocart', 'sitemagento', 'Cart', 'Sitemenu_Plugin_Menus', '', 'core_mini', '', 2);");
        }

        //WORK FOR COMPATIBILITY WITH SITETHEME
        $this->sitethemeCompatibility();
        $db->query("UPDATE `engine4_core_menuitems` SET enabled =0 WHERE name='core_mini_update' and module='activity';");
    ...
    public function onDisable() {
        ...
        $db->query("UPDATE `engine4_core_menuitems` SET enabled =1 WHERE name='core_mini_update' and module='activity';");
            ...
            if (!empty($parentContentId)) {
                ...
                if (!empty($isMiniMenuWidgetChange)) {
                    ...
                    foreach ($miniMenusLink as $linkName) {
                        ...
                        } else if ($linkName == 'sitemenu_mini_magentocart') {
                            $db->query("UPDATE `engine4_core_menuitems` SET `order` = '2' WHERE `name` = 'sitemenu_mini_magentocart' LIMIT 1 ;");
                        } else if ($linkName == 'sitemenu_mini_friend_request') {
                            $db->query("UPDATE `engine4_core_menuitems` SET `order` = '3' WHERE `name` = 'sitemenu_mini_friend_request' LIMIT 1 ;");
                        } else if ($linkName == 'core_mini_messages') {
                            $db->query("UPDATE `engine4_core_menuitems` SET `order` = '4' WHERE `name` = 'core_mini_messages' LIMIT 1 ;");
                        } else if ($linkName == 'sitemenu_mini_notification') {
                            $db->query("UPDATE `engine4_core_menuitems` SET `order` = '5' WHERE `name` = 'sitemenu_mini_notification' LIMIT 1 ;");
                        } else if ($linkName == 'core_mini_settings') {
                            $db->query("UPDATE `engine4_core_menuitems` SET `order` = '6' WHERE `name` = 'core_mini_settings' LIMIT 1 ;");
                        } else if ($linkName == 'core_mini_profile') {
                            $db->query("UPDATE `engine4_core_menuitems` SET `order` = '7' WHERE `name` = 'core_mini_profile' LIMIT 1 ;");
                        } else if ($linkName == 'core_mini_admin') {
                            $db->query("UPDATE `engine4_core_menuitems` SET `order` = '8' WHERE `name` = 'core_mini_admin' LIMIT 1 ;");
                        ...
                        } else if ($linkName == 'sitemenu_mini_currency') {
                            $db->query("UPDATE `engine4_core_menuitems` SET `order` = '100' WHERE `name` = 'sitemenu_mini_currency' LIMIT 1 ;");
                        }
                    }
                }
