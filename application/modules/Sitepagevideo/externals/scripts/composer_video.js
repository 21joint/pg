...
/* $Id: composer_sitepagevideo.js 7244 2010-09-01 01:49:53Z john $ */


Composer.Plugin.SitepageVideo = new Class({
  ...
  activate : function() {
    ...
    this.elements.formInput = new Element('select', {
      'id' : 'compose-sitepagevideo-form-type',
      'class' : 'compose-form-input',
      'option' : 'test',
      'events' : {
        'change' : this.updateSitepagevideoFields.bind(this)
      }
    }).inject(this.elements.body);    
    var options = 0;
    $('compose-sitepagevideo-form-type').options[options++] = new Option(this._lang('Choose Source'), '0');
    if (this.options.youtubeEnabled == 1) {
      $('compose-sitepagevideo-form-type').options[options++] = new Option(this._lang('YouTube'), '1');
    }
    $('compose-sitepagevideo-form-type').options[options++] = new Option(this._lang('Vimeo'), '2');
    
    this.elements.formInput = new Element('input', {
      'id' : 'compose-sitepagevideo-form-input',
      'class' : 'compose-form-input',
      'type' : 'text',
      'style': 'display:none;'
    }).inject(this.elements.body);
    ...
    if (this.options.allowed == 1 && this.options.type != 'message'){
      $('compose-sitepagevideo-form-type').options[options++] = new Option(this._lang('My Device'), '3');
    }
      this.elements.previewDescription = new Element('div', {
        'id' : 'compose-sitepagevideo-upload',
        'class' : 'compose-sitepagevideo-upload',
        'html' : this._lang('To upload a video from your device, please use our <a href="'+en4.core.baseUrl+ videoRouteURL + '/create/'+en4.core.subject.id+'/type/3">full uploader</a>.'),
        'style': 'display:none;'
      }).inject(this.elements.body);
    }
    else{
    if (this.options.allowed == 1 && this.options.type != 'message'){
      $('compose-sitepagevideo-form-type').options[options++] = new Option(this._lang('My Computer'), '3');
    }
      this.elements.previewDescription = new Element('div', {
        'id' : 'compose-sitepagevideo-upload',
        'class' : 'compose-sitepagevideo-upload',
    ...
    this.elements.formSubmit = new Element('button', {
      'id' : 'compose-sitepagevideo-form-submit',
  ...
  deactivate : function() {
    ...
    if (this.params.video_id)
      new Request.JSON({
        url: en4.core.basePath + 'sitepagevideo/index/delete',
        data: {
          format: 'json',
  ...
  doAttach : function(e) {
    ...
    var sitepagevideo_element = document.getElementById("compose-sitepagevideo-form-type");
    var type = sitepagevideo_element.value;
  ...
  doProcessResponse : function(responseJSON, responseText) {
    ...
    this.elements.preview = Asset.image(responseJSON.src, {
      'id' : 'compose-sitepagevideo-preview-image',
  ...
  doImageLoaded : function() {
    ...
    this.elements.previewInfo = new Element('div', {
      'id' : 'compose-sitepagevideo-preview-info',
      'class' : 'compose-preview-info'
    }).inject(this.elements.body);

    this.elements.previewTitle = new Element('div', {
      'id' : 'compose-sitepagevideo-preview-title',
    ...
    this.elements.previewDescription = new Element('div', {
      'id' : 'compose-sitepagevideo-preview-description',
  ...
  updateSitepagevideoFields : function(element) {
    var sitepagevideo_element = document.getElementById("compose-sitepagevideo-form-type");
    var url_element = document.getElementById("compose-sitepagevideo-form-input");
    var post_element = document.getElementById("compose-sitepagevideo-form-submit");
    var upload_element = document.getElementById("compose-sitepagevideo-upload");
    // clear url if input field on change
    $('compose-sitepagevideo-form-input').value = "";

    // If sitepagevideo source is empty
    if (sitepagevideo_element.value == 0)
    {
    ...
    // If sitepagevideo source is youtube or vimeo
    if (sitepagevideo_element.value == 1 || sitepagevideo_element.value == 2)
    {
      upload_element.style.display = "none";
    ...
    // if sitepagevideo source is upload
    if (sitepagevideo_element.value == 3)
    {
