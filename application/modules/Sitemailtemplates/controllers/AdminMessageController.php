...
class Sitemailtemplates_AdminMessageController extends Core_Controller_Action_Admin
{
  public function mailAction()
  {
    ...
    if(isset($values['sitemailtemplates_send_mail']) && empty($values['sitemailtemplates_send_mail'])) {
      ...
      $user_ids = preg_split('/[,]+/', $values['toValues']);
    }
    else {
			//GET LEVELS
    ...
    $mail
      ...
      ->setBodyHtml("Hello ".$user->displayname.",<br>".$values['body']);

    if( !empty($values['body_text']) ) {
      $mail->setBodyText("Hello ".$user->displayname.",<br>".$values['body_text']);
    } else {
      $mail->setBodyText("Hello ".$user->displayname.",<br>".$values['body']);
    }
