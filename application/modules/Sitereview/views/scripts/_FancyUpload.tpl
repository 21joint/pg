 ...
 * @version    $Id: _FancyUpload.tpl 6590 2013-04-01 00:00:00Z SocialEngineAddOns $
...
// url is read from the form, so you just have to change one place
  ...
  fileListMax: <?php echo $photoCount;?>,
		 ...
		 *
		 * This example creates a list of error elements directly in the file sitereview, which
		 * hide on click.
		 */
		...
		onFileStart: function() {
		  uploadCount += 1;
              ...
              onFileRemove: function(file) {
                  ...
                  request = new Request.JSON({
                   'format' : 'json',
                   'url' : '<?php echo $this->url(Array('module' => 'sitereview', 'controller' => 'photo', 'action' => 'remove'), 'default') ?>',
                   'data': {
                     'photo_id' : file_id,
                     'is_ajax' : 1,
		  ...
		  if (uploadCount == 0)
		  {
    		    ...
    		    var demositereview = document.getElementById("demo-list");
		    var demosubmit = document.getElementById("submit-wrapper");
		    democlear.style.display = "none";
		    demositereview.style.display = "none";
...
<fieldset id="demo-fallback">
  ...
  <label for="demo-photoupload">
    ...
    <input type="file" id="input-img" name="Filedata"  onchange="showSaveButton();" />
  </label>
</fieldset>

<div id="demo-status" class="hide">
  <div>
    <?php echo $this->translate("Click 'Add Photos' to select one or more photos from your computer. After you have selected the photos, they will begin to upload right away. When your upload is finished, click the button below your photo to save them to your $listing_singular_lc.");?>
  </div>
  <?php if(Engine_Api::_()->sitereview()->hasPackageEnable() && !empty($photoCount)):?>
    <div class="tip">
      <span>
        <?php echo $this->translate("You are allowed to upload maximum $photoCount photos.");?>
      </span>
    </div>
  <?php endif;?>
