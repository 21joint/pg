...
PinBoardSR = new Class({
      ...
      onSuccess : function(responseTree, responseElements, responseHTML, responseJavaScript) {
        ...
        if(params.requestParams.contentpage==1){
          var elem=new Element('div',{
            id:'sr_board_wrapper_'+params.widgetId,
            'class':'sr_board_wrapper'
          }).inject(params.responseContainer);
          params.responseContainer.getElements('.sr_list_wrapper').inject(elem);
  ...
  setMasonry:function(params){
    ...
    params.responseContainer.pinBoardSRMasonry({
...
//**********
var PinBoardSRObject=new Array();
var PinBoardSRViewMoreObjects=new Array();
var PinBoardSRColumn = new Array('middle','left','right');
for(var i=0;i<PinBoardSRColumn.length;i++)
  PinBoardSRObject[PinBoardSRColumn[i]]= new PinBoardSR(PinBoardSRColumn[i]);
en4.core.runonce.add(function(){
  for(var i=0;i<PinBoardSRColumn.length;i++){
    PinBoardSRObject[PinBoardSRColumn[i]].start();  
  }
  window.addEvent('scroll', function(){
    for(var i=0;i<PinBoardSRColumn.length;i++){
      PinBoardSRObject[PinBoardSRColumn[i]].viewMore();
    }
    en4.srpinboard.comments.setLayout(true);
  });
});

//********
PinBoardSRViewMore = new Class({
  Implements : [Events, Options],
  options : {
  },
  params:{},
  ...
  pinBoardLayout:function(){ 
    ...
    (function(){
      self.params.responseContainer.pinBoardSRMasonry({
...
en4.srpinboard={
  ...
  setMasonryLayout:function(){
    for(var i=0;i< en4.srpinboard.masonryArray.length;i++){
      if(en4.srpinboard.masonryArray[i].allowId && en4.srpinboard.masonryWidgetAllow[en4.srpinboard.masonryArray[i].allowId]){
        en4.srpinboard.setMasonry(en4.srpinboard.masonryArray[i]);
      }
    }
  },
  setMasonry:function(params){
    if(!params.responseContainer)
      return;
    params.responseContainer.pinBoardSRMasonry(params); 
  }
}

en4.srpinboard.comments = {
  ...
  setLayout:function(force){
    ...
    (function(){
      for(var i=0;i<PinBoardSRColumn.length;i++){
        PinBoardSRObject[PinBoardSRColumn[i]]. setAllPinBoardLayout();
      }
      for(var i=0;i<PinBoardSRViewMoreObjects.length;i++){
        PinBoardSRViewMoreObjects[i].pinBoardLayout();
      }
      en4.srpinboard.setMasonryLayout();
      en4.srpinboard.comments.setLayoutActive=false;
    }).delay(delay);
  },
  ...
  loadComments : function(type, id, page,widget_id){
    en4.core.request.send(new Request.HTML({
      url : en4.core.baseUrl + 'sitereview/pin-board-comment/list',
      data : {
        format : 'html',
        type : type,
        id : id,
      ...
      onSuccess : function(responseJSON) {
        en4.srpinboard.comments.setLayout(); 
      }
    }), {
  ...
  submit:function(formElement,type,id,widget_id){
    ...
    en4.core.request.send(new Request.JSON({
      url : en4.core.baseUrl + 'sitereview/pin-board-comment/create',
      data : form_values,
      onSuccess : function(responseJSON) {
        en4.srpinboard.comments.setLayout();
  ...
  like : function(type, id, widget_id, comment_id) {
    en4.core.request.send(new Request.JSON({
      url : en4.core.baseUrl + 'sitereview/pin-board-comment/like',
      data : {
        format : 'json',
        type : type,
        id : id,
  ...
  unlike : function(type, id, widget_id, comment_id) {
    en4.core.request.send(new Request.JSON({
      url : en4.core.baseUrl + 'sitereview/pin-board-comment/unlike',
      data : {
        format : 'json',
        type : type,
        id : id,
