...
class Sitepagenote_Installer extends Engine_Package_Installer_Module {
  ...
  function onInstall() {
    ...
    $db->query('UPDATE  `engine4_activity_notificationtypes` SET  `body` =  \'{item:$subject} has created a page note {item:$object}.\' WHERE  `engine4_activity_notificationtypes`.`type` =  "sitepagenote_create";');
...
//    //END CODE FOR LIGHTBOX    
    ...
    $noteTable = $db->query('SHOW TABLES LIKE \'engine4_sitepagenote_notes\'')->fetch();
    if(!empty($noteTable)) {     
        
        $featuredColumn = $db->query("SHOW COLUMNS FROM engine4_sitepagenote_notes LIKE 'featured'")->fetch();        
        if(!empty($featuredColumn)) {
            $featuredIndex = $db->query("SHOW INDEX FROM `engine4_sitepagenote_notes` WHERE Key_name = 'featured'")->fetch();   
            if(empty($featuredIndex)) {
              $db->query("ALTER TABLE `engine4_sitepagenote_notes` ADD INDEX ( `featured` )");
            }         
        }

        $categoryIdColumn = $db->query("SHOW COLUMNS FROM engine4_sitepagenote_notes LIKE 'category_id'")->fetch();        
            if(!empty($categoryIdColumn)) {
            $categoryIdIndex = $db->query("SHOW INDEX FROM `engine4_sitepagenote_notes` WHERE Key_name = 'category_id'")->fetch();     
            if(empty($categoryIdIndex)) {
              $db->query("ALTER TABLE `engine4_sitepagenote_notes` ADD INDEX ( `category_id` )");
            }     
        }
    }
...
('sitepagenote','1')");
  ...
  public function setActivityFeeds() {
    ...
    if ($is_nestedcomment_object) {
        $db->query('INSERT IGNORE INTO `engine4_activity_actiontypes` (`type`, `module`, `body`, `enabled`, `displayable`, `attachable`, `commentable`, `shareable`, `is_generated`) VALUES ("nestedcomment_sitepagenote_note", "sitepagenote", \'{item:$subject} replied to a comment on {item:$owner}\'\'s page note {item:$object:$title}: {body:$body}\', 1, 1, 1, 1, 1, 1)');
        $db->query('INSERT IGNORE INTO `engine4_activity_actiontypes` (`type`, `module`, `body`, `enabled`, `displayable`, `attachable`, `commentable`, `shareable`, `is_generated`) VALUES ("nestedcomment_sitepagenote_photo", "sitepagenote", \'{item:$subject} replied to a comment on {item:$owner}\'\'s page note photo {item:$object:$title}: {body:$body}\', 1, 1, 1, 1, 1, 1)');
    }
  }
}
