 ...
 */
class Sitepagenote_Form_Searchwidget extends Engine_Form {
  ...
  public function init() {
    ...
    $this->setAction($view->url(array(), 'sitepagenote_browse', true))->getDecorator('HtmlTag')->setOption('class', '');
  }
  ...
  public function getAdditionalOptionsElement() {
    ...
    $select_content = $coreContent_table->select()->where('name = ?', 'sitepagenote.search-sitepagenote');
    ...
    //$row = $this->_searchForm->getFieldsOptions('sitepage', 'search');
     ...
     if (in_array("4", $enabledColumns)) {
      $this->addElement('Text', 'search_note', array(
              'label' => 'Note Keywords',
              'order' => $row->order - 4,
              'filters' => array(
     ...
     if (in_array("3", $enabledColumns)) {
      ...
      $show_multiOptions["creation_date"] = "Everyone’s Notes";

			if(!empty($viewer_id)) {
				$show_multiOptions["my_note"] = 'My Notes';
				$show_multiOptions["my_like"] = 'Notes of Pages I Like';
			}

      $show_multiOptions["featured"] = "Featured Notes";
      $sitepagePackageEnable = Engine_Api::_()->getApi('settings', 'core')->getSetting('sitepage.package.enable', 1);
      if($sitepagePackageEnable) {
				$show_multiOptions["sponsored note"] = 'Sponsored Notes';
      }
      ...
      $this->addElement('Select', 'show', array(
              ...
              'onchange' => $this->gethasMobileMode() ? '' : 'searchSitepagenotes();',
              'order' => $row->order - 2,
      ));
   ...
   if (in_array("2", $enabledColumns)) {
    ...
    if (in_array("6", $enabledColumns)) {
      $multiOptions = array();
      $multiOptions[] = '';
      foreach( Engine_Api::_()->getDbtable('categories', 'sitepagenote')->select()->order('title ASC')->query()->fetchAll() as $row ) {    
      $multiOptions[$row['category_id']] = $row['title'];
    }
      
      $this->addElement('Select', 'note_category_id', array(
        'label' => 'Note Category',
        'order' => $i--,
