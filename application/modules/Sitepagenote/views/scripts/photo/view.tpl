...
$this->headScript()
        ...
        ->appendFile($this->layout()->staticBaseUrl. 'application/modules/Sitepagenote/externals/scripts/core.js');
...
<?php if (empty($this->isajax)): ?>
  <div class="sitepage_viewpages_head">
    ...
    <h2>	
      ...
      <?php $note_title = $this->htmlLink(array('route' => 'sitepagenote_detail_view', 'user_id' => $this->sitepagenote->owner_id, 'note_id' => $this->sitepagenote->note_id, 'slug' => $this->sitepagenote->getSlug(), 'tab' => $this->tab_selected_id), $this->translate($this->sitepagenote->getTitle())) ?>
      <?php $notes = $this->htmlLink(array('route' => 'sitepage_entry_view', 'page_url' => Engine_Api::_()->sitepage()->getPageUrl($this->sitepage->page_id), 'tab' => $this->tab_selected_id), $this->translate('Notes')) ?>
      <?php echo $this->translate('%1$s  &raquo; %2$s  &raquo; %3$s', $title, $notes, $this->sitepagenote->__toString()); ?>
    </h2>
  </div>	
  <div class='sitepagenote_viewmedia'>
    <div class="sitepagenote_viewmedia_nav">
      <div id= 'photo_navigation1'>
        <?php
        echo $this->translate('Photo %1$s of %2$s in %3$s', $this->locale()->toNumber($this->photo->getCollectionIndex() + 1), $this->locale()->toNumber($this->sitepagenotePhoto->count()), $this->sitepagenote->getTitle())
        ?>
      </div>
      <?php if ($this->sitepagenotePhoto->count() > 1): ?>
    ...
    <div id='image_div'>      
    <?php endif; ?>
    <div class='sitepagenote_viewmedia_info'> 
      ...
      <?php endif; ?>
      <div class='sitepagenote_viewmedia_container' id='media_image_div'>
        <a id='media_image_next'   <?php if ($this->sitepagenotePhoto->count() > 1): ?> onclick="photopagination('<?php echo $this->escape($this->photo->getNextCollectible()->getHref() . '/tab/' . $this->tab_selected_id) ?>')" <?php endif; ?> title="<?php echo $this->photo->getTitle(); ?>">
          <?php
      ...
      <?php if ($this->can_edit): ?>
        <div class="sitepagenote_photo_right_options">
          <a class=" icon_sitepagenote_photos_rotate_ccw" href="javascript:void(0)" onclick="$(this).set('class', 'page_ccw icon_loading');en4.sitepagenote.rotate(<?php echo $this->photo->getIdentity() ?>, 90).addEvent('complete', function(){ this.set('class', 'icon_sitepagenote_photos_rotate_ccw') }.bind(this)); loadingImage();" title="<?php echo $this->translate("Rotate Left"); ?>" >&nbsp;</a>
          <a class="icon_sitepagenote_photos_rotate_cw" href="javascript:void(0)" onclick="$(this).set('class', 'page_cw icon_loading');en4.sitepagenote.rotate(<?php echo $this->photo->getIdentity() ?>, 270).addEvent('complete', function(){ this.set('class', 'icon_sitepagenote_photos_rotate_cw') }.bind(this)); loadingImage();" title="<?php echo $this->translate("Rotate Right"); ?>" >&nbsp;</a>
          <a class="icon_sitepagenote_photos_flip_horizontal" href="javascript:void(0)" onclick="$(this).set('class', 'page_horizontal icon_loading');en4.sitepagenote.flip(<?php echo $this->photo->getIdentity() ?>, 'horizontal').addEvent('complete', function(){ this.set('class', 'icon_sitepagenote_photos_flip_horizontal') }.bind(this));loadingImage();" title="<?php echo $this->translate("Flip Vertical"); ?>" >&nbsp;</a>
          <a class="icon_sitepagenote_photos_flip_vertical" href="javascript:void(0)" onclick="$(this).set('class', 'page_vertical icon_loading');en4.sitepagenote.flip(<?php echo $this->photo->getIdentity() ?>, 'vertical').addEvent('complete', function(){ this.set('class', 'icon_sitepagenote_photos_flip_vertical') }.bind(this));loadingImage();" title="<?php echo $this->translate("Flip Horizontal"); ?>" >&nbsp;</a>
          <input type="hidden" id='canReload' />
        </div>
      <?php endif ?>

      <?php if ($this->photo->getTitle()): ?>
        <div class="sitepagenote_photo_title">
          <?php echo $this->photo->getTitle(); ?>
        </div>
      <?php endif; ?>
      <?php if ($this->photo->getDescription()): ?>
        <div class="sitepagenote_photo_description">
      ...
      <?php $editurl = $this->url(array('action' => 'photoedit', 'photo_id' => $this->photo->photo_id, 'note_id' => $this->sitepagenote->note_id, 'page_id' => $this->sitepage->page_id, 'tab' => $this->tab_selected_id), 'sitepagenote_photoedit', true); ?>
      <?php $deleteurl = $this->url(array('action' => 'remove', 'photo_id' => $this->photo->photo_id, 'note_id' => $this->sitepagenote->note_id, 'page_id' => $this->sitepage->page_id, 'owner_id' => $this->photo->user_id, 'tab' => $this->tab_selected_id), 'sitepagenote_removeimage', true); ?>
      <div class="sitepagenote_photo_date">
        ...
        <?php if ($this->viewer_id == $this->sitepagenote->owner_id || $this->can_edit == 1): ?>			
          ...
          - <a href="javascript:void(0);" onclick="showSmoothbox('<?php echo $this->photo->photo_id; ?>', 'edit', 'null', 'null', '<?php echo $this->sitepage->page_id ?>',  '<?php echo $this->sitepagenote->note_id; ?>', '<?php echo $this->photo->user_id ?>', '<?php echo $editurl; ?>');">
            <?php echo $this->translate('Edit'); ?>
          </a>
          - <a href="javascript:void(0);" onclick="showSmoothbox('<?php echo $this->photo->photo_id; ?>', 'delete', 'null', 'null','<?php echo $this->sitepage->page_id ?>', '<?php echo $this->sitepagenote->note_id; ?>', '<?php echo $this->photo->user_id ?>', '<?php echo $deleteurl; ?>');">
            <?php echo $this->translate('Delete'); ?>
          </a>
        <?php endif; ?>
				<?php if ($this->viewer_id):?>
					<?php if (SEA_PHOTOLIGHTBOX_SHARE): ?>
						- <a href="javascript:void(0);" onclick="showSmoothbox('<?php echo $this->photo->photo_id; ?>', 'share', '<?php echo $this->photo->getType() ?>', '<?php echo $this->photo->getIdentity() ?>','<?php echo $this->sitepage->page_id ?>', '<?php echo $this->sitepagenote->note_id; ?>', '<?php echo $this->photo->user_id ?>');">
							<?php echo $this->translate('Share'); ?>
						</a>
					<?php endif; ?>
					<?php if (SEA_PHOTOLIGHTBOX_REPORT): ?>		
						-
						<a href="javascript:void(0);" onclick="showSmoothbox('<?php echo $this->photo->getGuid(); ?>', 'report','<?php echo $this->photo->getType() ?>', '<?php echo $this->photo->getIdentity() ?>','<?php echo $this->sitepage->page_id ?>', '<?php echo $this->sitepagenote->note_id; ?>', '<?php echo $this->photo->user_id ?>');">
...
function showSmoothbox(photo_id, action, type, id, page_id, note_id, owner_id, url) 
{
