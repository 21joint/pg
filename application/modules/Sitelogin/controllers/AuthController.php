...
class Sitelogin_AuthController extends Core_Controller_Action_Standard {
    ...
    public function googleAction() {
        ...
        $sitelogingooglesignup = Zend_Registry::isRegistered('sitelogingooglesignup') ? Zend_Registry::get('sitelogingooglesignup') : null;
        if (empty($sitelogingooglesignup))
            return $this->_helper->redirector->gotoRoute(array('action' => 'home'), 'user_general', true);
            ...
            if (!isset($_SESSION['access_token'])) {
                ...
                if (!$loggedin) {
                    if (!empty($quickSignup)) {
                        ...
                        } elseif ($redirectlink == 2) {
                            $url = $_SESSION['redirectURL'] . 'profile/' . $viewer->getIdentity();
                        } elseif ($redirectlink == 3) {
                            $url = $_SESSION['redirectURL'] . 'members/edit/profile';
                        } else {
                            $customeUrl = $redirectlink = Engine_Api::_()->getApi('settings', 'core')->getSetting('sitelogin.customurl', '');
                            $url = $_SESSION['redirectURL'] . $customeUrl;
                        }
    ...
    public function linkedinAction() {
        ...
        $siteloginlinkedinsignup = Zend_Registry::isRegistered('siteloginlinkedinsignup') ? Zend_Registry::get('siteloginlinkedinsignup') : null;
        if (empty($siteloginlinkedinsignup))
            return $this->_helper->redirector->gotoRoute(array('action' => 'home'), 'user_general', true);
    ...
    public function vkAction() {
        ...
        try {
            $vk_id = $userDetails->response[0]->uid;
            //Login user
            if (!empty($vk_id)) {
    ...
    public function saveUser($userLoginDetails, $loginType = NULL) {
        ...
        $siteloginquicksignup = Zend_Registry::isRegistered('siteloginquicksignup') ? Zend_Registry::get('siteloginquicksignup') : null;
        if (empty($siteloginquicksignup))
            return $this->_helper->redirector->gotoRoute(array('action' => 'home'), 'user_general', true);
