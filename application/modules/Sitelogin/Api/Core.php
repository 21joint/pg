...
class Sitelogin_Api_Core extends Core_Api_Abstract
{
  ...
  public function getProfileTypes()
  {
    ...
    $profileFields = array();
    if (count($topStructure) == 1
      && $topStructure[0]->getChild()->type == 'profile_type'
    ) {
      $profileTypeField = $topStructure[0]->getChild();
      $options = $profileTypeField->getElementParams('user');
      if (isset($options['options']['multiOptions'])
        && !empty($options['options']['multiOptions'])
        && is_array($options['options']['multiOptions'])
      ) {
  ...
  {
    $levelMultiOptions = Engine_Api::_()->getDbtable('levels', 'authorization')
      ->getLevelsAssoc();
    $publicLevel = Engine_Api::_()->getDbtable('levels', 'authorization')
      ->getPublicLevel();
  ...
  public function getDefaultSubscriptionPlan()
  {
    ...
    $package = $packagesTable->fetchRow(
      array(
  ...
  public function outlookfetchImage($subject)
  {
    ...
    if ($code == 200) {
      $random = rand(0, 10000);
      $tmpfile = APPLICATION_PATH_TMP.DS.md5($service_url.$random)
        .'_outlook.jpg';
  ...
  public function isSubsciptionStepEnabled()
  {
    ...
    if (Engine_Api::_()->hasModuleBootstrap("sitesubscription")) {
      $stepSelect = $stepTable->select()->where(
        'class = ?', 'Sitesubscription_Plugin_Signup_Subscription'
      );
    } else {
      $stepSelect = $stepTable->select()->where(
        'class = ?', 'Payment_Plugin_Signup_Subscription'
      );
    }
  ...
  public function getGoogleRedirectUrl()
  {
    ...
    return $domainUrl.'/sitelogin/auth/google?google_connected=1';
  }

  public function getLinkedinRedirectUrl()
  {
    $baseParentUrl = Zend_Controller_Front::getInstance()->getBaseUrl();
    $domainUrl = (_ENGINE_SSL ? 'https://' : 'http://')
      .$_SERVER['HTTP_HOST'];
    ...
    return $domainUrl.'/sitelogin/auth/linkedin';
  }
  ...
  public function getInstagramRedirectUrl()
  {
    ...
    return $domainUrl.'/sitelogin/auth/instagram';
  }

  public function facebookIntegrationEnabled()
  {

    $facebookSettings = (array)Engine_Api::_()->getApi(
      'settings', 'core'
    )->sitelogin_facebook;
    $coreSettings = (array)Engine_Api::_()->getApi(
      'settings', 'core'
    )->core_facebook;

    $client_id = isset($coreSettings['appid']) ? $coreSettings['appid'] : 0;
    $client_secret = isset($coreSettings['secret']) ? $coreSettings['secret']
      : 0;
    $loginEnable = isset($facebookSettings['facebookOptions'])
      ? $facebookSettings['facebookOptions'] : 0;
  ...
  {
    $twitterSettings = (array)Engine_Api::_()->getApi(
      'settings', 'core'
    )->sitelogin_twitter;
    $coreSettings = (array)Engine_Api::_()->getApi(
      'settings', 'core'
    ...
    $client_secret = isset($coreSettings['secret']) ? $coreSettings['secret']
      : 0;
    $loginEnable = isset($twitterSettings['twitterOptions'])
      ? $twitterSettings['twitterOptions'] : 0;
  ...
  public function getStats()
  {
    ...
    $flickrresult = Engine_Api::_()->getDbTable('flickr', 'sitelogin')
      ->getCount();
    $googleresult = Engine_Api::_()->getDbTable('google', 'sitelogin')
      ->getCount();
    $instagramresult = Engine_Api::_()->getDbTable('instagram', 'sitelogin')
      ->getCount();
    $linkedinresult = Engine_Api::_()->getDbTable('linkedin', 'sitelogin')
      ->getCount();
    $outlookresult = Engine_Api::_()->getDbTable('outlook', 'sitelogin')
      ->getCount();
    $pinterestresult = Engine_Api::_()->getDbTable('pinterest', 'sitelogin')
      ->getCount();
  ...
  function getfacebookcount()
  {
    ...
    $select->from(
      $facebookTableName,
      array('COUNT('.$facebookTableName.'.user_id) as count')
    )
      ->join(
        $userTableName,
        $userTableName.'.user_id = '.$facebookTableName.'.user_id AND '
        .$userTableName.'.password ="" ', array()
      );
  ...
  function gettwittercount()
  {
    ...
    $select->from(
      $twitterTableName, array('COUNT('.$twitterTableName.'.user_id) as count')
    )
      ->join(
        $userTableName,
        $userTableName.'.user_id = '.$twitterTableName.'.user_id AND '
        .$userTableName.'.password ="" ', array()
      );
