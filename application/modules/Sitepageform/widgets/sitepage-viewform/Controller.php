 ...
 */
class Sitepageform_Widget_SitepageViewformController extends Seaocore_Content_Widget_Abstract {

    //protected $_childCount;
    public function indexAction() {
        global $sitepageform_mail;
        ...
        $getPackageFormMail = Engine_Api::_()->sitepage()->getPackageAuthInfo('sitepageform');
        $sitepageform_tabInfo = Zend_Registry::isRegistered('sitepageform_tabInfo') ? Zend_Registry::get('sitepageform_tabInfo') : null;
        $Showtab_Fornonloggedin = Engine_Api::_()->getApi('settings', 'core')->getSetting('sitepageform_formtabseeting', 1);
        if (empty($viewr_id) && $Showtab_Fornonloggedin == 0) {
            return $this->setNoRender();
        }
        if (empty($sitepageform_tabInfo)) {
            return $this->setNoRender();
        }

        $ITEM = Engine_Api::_()->core()->getSubject();
        $layout_type = Engine_Api::_()->getApi('settings', 'core')->getSetting('sitepage.layoutcreate', 0);
        $canEditTabName = Engine_Api::_()->getApi('settings', 'core')->getSetting('sitepageform.edit.name', 1);
        if (empty($layout_type) && !empty($canEditTabName)) {
        ...
        //GET THE SUBJECT OF SITEPAGE
        $this->view->page_id = $page_id = $ITEM->page_id;
        $page_name = $ITEM->title;

        $this->view->sitepage_object = $sitepage_object = Engine_Api::_()->getItem('sitepage_page', $page_id);

        // PACKAGE BASE PRIYACY START
        if (Engine_Api::_()->sitepage()->hasPackageEnable()) {
            if (!Engine_Api::_()->sitepage()->allowPackageContent($sitepage_object->package_id, "modules", "sitepageform")) {
                return $this->setNoRender();
            }
        } else {
            $isPageOwnerAllow = Engine_Api::_()->sitepage()->isPageOwnerAllow($sitepage_object, 'form');
...
//		$isManageAdmin = Engine_Api::_()->sitepage()->isManageAdmin($sitepage_object, 'view');
...
//		}
        ...
        //GET THE OBJECT OF PAGEQUESTION TABLE$this->view->page_url
        ...
        $select_sitepageform = $itepageforms_table->select()->where('page_id = ?', $page_id);
        $select_sitepageform_result = $itepageforms_table->fetchRow($select_sitepageform);
        $sitepageform_id = $select_sitepageform_result->sitepageform_id;
        ...
        if (!Engine_Api::_()->core()->hasSubject()) {
            if (null !== $page_id) {
                $subject = Engine_Api::_()->getItem('sitepageform', $sitepageform_id);
        ...
        $form = $this->view->form = new Sitepageform_Form_Standard(array(
            'item' => $subject,
            'topLevelId' => 1,
            'topLevelValue' => $option_id,
        ));
        $page_owner = Engine_Api::_()->getItem('user', $owner_id);
        $this->view->show_msg = $form->show_error_msg;
        //GET THE OBJECT OF MANAGEADMIN TABLE
        $table_manageadmin = Engine_Api::_()->getDbTable('manageadmins', 'sitepage');
        //$table_manageadmin_name = $table_manageadmin->info('name'); 
        $select_manageadmin = $table_manageadmin->select()->where('page_id =?', $page_id);
        ...
        $sender_email = $page_owner->email;
        if (!empty($viewr_id)) {
            $sendme = $viewer->email;
            $viewer_name = $viewer->displayname;
            if ($select_sitepageform_result->activeemail == 1) {
                $form->sender_email->setValue("$sendme");
            }
            if ($select_sitepageform_result->activeyourname == 1) {
        ...
        $this->view->content_id = Engine_Api::_()->sitepage()->GetTabIdinfo('sitepageform.sitepage-viewform', $page_id, $layout);
        ...
        if (!empty($isajax) || ($currenttabid == $this->view->identity) || ($front->getRequest()->isPost() && empty($isajax)) || ($widgets == 0)) {
            ...
            if ($front->getRequest()->isPost() && !$form->isValid($front->getRequest()->getPost()) && empty($isajax)) {
                ...
                //CLEAR THE SUBJECT OF SITEPAGEFORM
                Engine_Api::_()->core()->clearSubject();
                $subject = null;
                if (!Engine_Api::_()->core()->hasSubject()) {
            ...
            * Please complete this field - it is required.');
                ...
                if ($select_sitepageform_result->activemessage == 1) {
            ...
            * Please complete this field - it is required.');
                ...
                //GET THE OBJECT OF SITEPAGE TABLE
                Engine_Api::_()->sitepageform()->setFormPackages();
                ...
                foreach ($values as $key => $value) {
                    ...
                    if ($parts[0] == 1 && $parts[1] == $option_id) {
                        //GET THE OBJECT OF SITEPAGEFORM META TABLE
                        $table_option = Engine_Api::_()->fields()->getTable('sitepageform', 'meta');
                        ...
                        if ($result->canHaveDependents()) {
                            if (is_array($value)) {
                                ...
                                //GET THE OBJECT OF SITEPAGEFORM OPTION TABLE
                                $table_option = Engine_Api::_()->fields()->getTable('sitepageform', 'options');
                                $table_option_name = $table_option->info('name');
                                $select_options = $table_option->select();
                                $select_options
                ...
                if ($select_sitepageform_result->activemessage == 1 || (count($Question_ans > 0))) {
                    $message .= '<br /><br />';
                }
                if ($select_sitepageform_result->activemessage == 1) {
                ...
                if (!empty($sitepageform_mail) && !empty($getPackageFormMail)) {
                    Engine_Api::_()->getApi('mail', 'core')->sendSystem($email_user, 'SITEPAGEFORM_QUESTION_EMAIL', array(
                        ...
                        'heading' => '',
                        'message' => $message,
                        'object_link' => Engine_Api::_()->sitepage()->getHref($sitepage->page_id, $viewr_id, $sitepage->getSlug()),
                        'email' => $sender_email,
                        'queue' => false
                    ));
                }
                ...
                if ((!empty($values_form['send_me'])) && !empty($sitepageform_mail) && !empty($getPackageFormMail)) {
                    Engine_Api::_()->getApi('mail', 'core')->sendSystem($sendme_viewer, 'SITEPAGEFORM_QUESTION_EMAIL', array(
                        'host' => $_SERVER['HTTP_HOST'],
                        'sender_name' => $sender,
                        'sender_email' => $sendme,
                        'page_name' => $page_name,
                        'heading' => '',
                        'message' => $message,
                        'object_link' => Engine_Api::_()->sitepage()->getHref($sitepage->page_id, $sitepage->owner_id, $sitepage->getSlug()),
                        'email' => $sendme,
                        'queue' => false
                    ));
                }
                ...
                if (!Engine_API::_()->seaocore()->checkSitemobileMode('fullsite-mode')) {
                    $tab_id = Engine_Api::_()->sitepage()->GetTabIdinfo('sitepageform.sitepage-viewform', $page_id, 1);
                    $request->setParams(array(
                        'parentRedirect' => $sitepage_object->getHref(array('tab' => $tab_id)),
