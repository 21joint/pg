 ...
 * @version    $Id: Standard.php 2011-05-05 9:40:21Z SocialEngineAddOns $
 * @author     SocialEngineAddOns
 */
class Sitepageform_Form_Standard extends Engine_Form {
    ...
    public function getFieldStructure() {
        ...
        $a = $this->getFieldsStructureFull('sitepageform', $this->_topLevelId, $this->_topLevelValue);

        return $this->getFieldsStructureFull('sitepageform', $this->_topLevelId, $this->_topLevelValue);
    }

    // Main
    public function generate() {
        $page_url = Zend_Controller_Front::getInstance()->getRequest()->getParam('page_url', null);
        if (Engine_Api::_()->core()->hasSubject() && isset(Engine_Api::_()->core()->getSubject()->page_id)) {
            $page_id = Engine_Api::_()->core()->getSubject()->page_id;
        } else {
        ...
        $form_title = $select_sitepageform_result->title;
        $form_description = $select_sitepageform_result->description;

        if ($select_sitepageform_result->activeyourname == 1 || $select_sitepageform_result->activeemail == 1 || ($select_sitepageform_result->activeemail == 1) || $select_sitepageform_result->activemessage == 1) {
            $var = 0;
        } else {
            $var = 1;
        }
        ...
        $action = Zend_Controller_Front::getInstance()->getBaseUrl() . '/widget/index/mod/sitepageform/name/sitepage-viewform';
        ...
        // SEND COPY TO ME
        if (($select_sitepageform_result->activeemail == 1) && ($select_sitepageform_result->activeemailself == 1)) {
        ...
        if (Engine_Api::_()->getApi('settings', 'core')->getSetting('sitepageform.captcha', 1) && empty($viewer_id)) {
    ...
    public function getFieldsStructureFull($spec, $parent_field_id = null, $parent_option_id = null) {
        ...
        $structure = array();
        foreach (Engine_Api::_()->fields()->getFieldsMaps($type)->getRowsMatching('field_id', (int) $parent_field_id) as $map) {
            // Skip maps that don't match parent_option_id (if provided)
            if (null !== $parent_option_id && $map->option_id != $parent_option_id && !empty($map->option_id)) {
                continue;
            }
            // Get child field
            ...
            // Get children
            if ($field->canHaveDependents()) {
                $structure += Engine_Api::_()->fields()->getFieldsStructureFull($spec, $map->child_id);
            }
        }
