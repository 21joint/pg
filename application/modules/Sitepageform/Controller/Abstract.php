 ...
 * @version    $Id: Abstract.php 2011-05-05 9:40:21Z SocialEngineAddOns $
 * @author     SocialEngineAddOns
 */
class Sitepageform_Controller_Abstract extends Seaocore_Controller_Action_Standard {
  ...
  public function indexAction() {
    ...
    $this->view->pagename = $page_name = $this->_getParam('pagename');

    //FORM GENERATION
    $this->view->createform = $createform = new Sitepageform_Form_Create();

    //GET FORM DATA
    $formSelectData = Engine_Api::_()->getDbtable('sitepageforms', 'sitepageform')->getFormData($page_id);
    ...
    //GET FORM DATA
    $values = $_POST;
    $sitepageform = Engine_Api::_()->getItem('sitepageform', $formSelectData->sitepageform_id);
    $sitepageform->setFromArray($values);
    $createform->populate($values);
    $sitepageform->save();

    //TO GET THE OBJECT OF MAP TABLE
    $formMapTable = Engine_Api::_()->fields()->getTable('sitepageform', 'maps');
    ...
    if (!$this->_requireProfileType) {
      ...
      if (!empty($option_id)) {
        ...
        $canAddquestions = Engine_Api::_()->getApi('settings', 'core')->getSetting('sitepageform.add.question', 1);
  ...
  public function fieldCreateAction() {
    ...
    global $sitepageform_fieldCreate;
    $getPackageFormQuestion = Engine_Api::_()->sitepage()->getPackageAuthInfo('sitepageform');
    if ((!$this->_helper->requireUser()->isValid()) || empty($getPackageFormQuestion))
      return;

    if (empty($sitepageform_fieldCreate)) {
      return;
    }
    ...
    $FormClass = 'Sitepageform_Form_Field';
    ...
    $isModType = Engine_Api::_()->getApi('settings', 'core')->getSetting('sitepageform.set.type', 0);
    if (empty($isModType)) {
      Engine_Api::_()->getApi('settings', 'core')->setSetting('sitepageform.form.page', convert_uuencode($sitepageModHostName));
    }
