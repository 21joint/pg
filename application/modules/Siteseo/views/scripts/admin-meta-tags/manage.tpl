...
<?php endif; ?>
<p>All the widgetized pages of your website are listed here. If you need to search for a specific page, enter your search criteria in the fields below. Here, you can set titles, meta keywords and meta descriptions for your pages.</p>
<div>
	<b><?php echo "Note: "; ?></b>
...
<?php if(empty($settings->getSetting("siteseo.opengraph.enable", 1))): ?>
	<div class="tip">
		<span>You have disabled open graph from Global settings.</span>
	</div>
<?php endif; ?>
<?php if(empty($settings->getSetting("siteseo.twittercards.enable", 1))): ?>
	<div class="tip">
...
<?php $keywordTemplate = '[subject_keywords], [subject_category],[subject_subcategory],[subject_subsubcategory],[subject_type],[subject_location]'; ?>
<?php if (count($this->paginator) > 0): ?>
	<div class="admin_table_form">
		...
		<table class='admin_table siteseo_admin_table' id="manage-meta-tags">
			<thead>
				<tr>
					<th style='width: 5%;'><a href="javascript:void(0);" onclick="javascript:changeOrder('page_id', 'DESC');">Page ID</a></th>
					...
					<th style='width: 35%;'><a href="javascript:void(0);" onclick="javascript:changeOrder('keywords', 'ASC');">Meta Keywords</a></th>
					<th style='width: 5%;' class='admin_table_image'>Custom Image</th>
					<!-- DISPLAY OPEN GRAPH AND TWITTER CARD ONLY IF SITEMETATAG IS INSTALLED -->
					<?php if(Engine_Api::_()->hasModuleBootstrap('sitemetatag')): ?>
						<th style='width: 2%;' class=''>OG</th>
						<th style='width: 2%;' class=''>TC</th>
					<?php endif; ?>
				...
				<?php if (count($this->paginator)): ?>
					<?php foreach ($this->paginator as $pageObject): ?>
						<tr data-id="<?php echo $pageObject->getIdentity(); ?>">
							...
							<td class="inline_edit_column">
							<?php if(strpos($pageObject->name, '_view') !== false) : ?>
								<?php echo $pageObject->keywords ? $pageObject->keywords : $keywordTemplate; ?>
							<?php else: ?>
								<?php echo $pageObject->keywords; ?>
							<?php endif; ?>
							</td>
							...
							<td class='admin_table_centered'>
								<a class='smoothbox' href='<?php echo $this->url(array('action' => 'edit', 'page_id' => $pageObject->getIdentity())); ?>'>
									Edit
								</a> 
							</td>
						</tr>
