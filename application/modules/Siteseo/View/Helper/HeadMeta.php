 ...
 * @version    $Id: HeadMeta.php 2017-03-27 9:40:21Z SocialEngineAddOns $
 * @author     SocialEngineAddOns
 */
class Siteseo_View_Helper_HeadMeta extends Zend_View_Helper_Placeholder_Container_Standalone {
    /**
     * Types of attributes
     * @var array
     */
    ...
    public function itemToString(stdClass $item)
    {
        ...
        $type = $item->type;
        //FORMAT DESCRIPTION AND KEYWORDS
        if (isset($item->name) && $item->name == 'description')
            $item->content = $this->siteseoDescription($item);
        if (isset($item->name) && $item->name == 'keywords')
            $item->content = $this->siteseoKeywords($item);
        //FORMAT DESCRIPTION AND KEYWORDS
        $modifiersString = '';
    ...
    public function siteseoDescription($item) {
        $page = Engine_Api::_()->siteseo()->getCurrentPageinfo();
        if (empty($page))
            return $item->content;
        return Engine_Api::_()->siteseo()->formatMetaDescription($item->content);
    }

    // RETURNS FORMATTED META KEYWORDS
    public function siteseoKeywords($item) {
        $page = Engine_Api::_()->siteseo()->getCurrentPageinfo();
        $content = $item->content;
        if ($page) {
            $content = ($page->keywords && strpos($content, $page->keywords) !== false) ? $content : $content . ', ' . $page->keywords;
            $content = Engine_Api::_()->siteseo()->formatMetaKeywords($content);
        }
        return $content;
    }
}