...
class Video_IndexController extends Siteapi_Controller_Action_Standard {
    ...
    public function browseAction() {
        ...
        $items_count = $this->getRequestParam("limit", 20); //( int ) Engine_Api::_()->getApi('settings', 'core')->getSetting('video.page', 12);
...
//        $values['search'] = 1;
    ...
    public function createAction() {
        ...
        if (($current_count >= $quota) && !empty($quota))
            $this->respondWithError('exceed_allow_uplod_limit');
        ...
        if ($this->getRequest()->isGet()) {
            ...
            try {
                ...
                if (Engine_Api::_()->getDbtable('modules', 'core')->isModuleEnabled('sitevideo') && isset($values['type']) && $values['type'] != 3 && $values['type'] != 'iframely') {
                    try {
