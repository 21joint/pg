 ...
 * @version    $Id: create.tpl 2014-02-16 5:40:21Z SocialEngineAddOns $
...
<div>
  <?php echo $this->htmlLink(array('action' => 'index', 'reset' => false), 'Back to Manage Static Pages', array('class' => 'seaocore_icon_back buttonlink')) ?>
</div>
...
<script type="text/javascript">
  ...
  window.addEvent('domready', function() {
    ...
    if (multiLanguage == 1 && languageCount > 1) {
      ...
      $('multiLanguageLinkHide').inject(bodyParent, 'after');
			<?php foreach ($this->languageData as $value):
				if ($value == 'en')
				$value = '';
				else 
				$value = "_$value";
  ...
  function PageUrlBlur() {
    ...
    if ($('page_url_alert') == null) {
      ...
      newdiv.innerHTML = '<img src="<?php echo $this->layout()->staticBaseUrl ?>application/modules/Sitestaticpage/externals/images/loading.gif" />';
      pageurlcontainer.insertBefore(newdiv, pageurlcontainer.childNodes[3]);
    }
    else {
      $('page_url_alert').innerHTML = '<img src="<?php echo $this->layout()->staticBaseUrl ?>application/modules/Sitestaticpage/externals/images/loading.gif" />';
    }
  ...
  $(window).on('load', function()
  {
    ...
    if ($('page_url_address')) {
      $('page_url_address').innerHTML = $('page_url_address').innerHTML.replace(url, '<span id="page_url_address_text"><?php echo 'URL-COMPONENT'; ?></span>');
    }
   ...
   $('title').addEvent('keyup', function() {
	...
	$(window).on('load', function() {
			...
			$('menu-0').addEvent('click', function(){
					$('link_title-wrapper').setStyle('display', ($(this).get('value') == '0'?'block':'none'));
			});
