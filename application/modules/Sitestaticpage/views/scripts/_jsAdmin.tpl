 ...
 * @version    $Id: _jsAdmin.tpl 2014-02-16 5:40:21Z SocialEngineAddOns $
...
<script type="text/javascript">
  ...
  var readIdentifiers = function(string, throwException) {
    ...
    if( !$type(throwException) || throwException ) {
      throw '<?php echo $this->string()->escapeJavascript("Unable to find identifiers in text:") ?> ' + string;
    } else {
      return false;
    }
  }
    ...
    $H(htmlArr).each(function(html, key) {
      ...
      if( oldEl && !newEl ) { // Remove
        ...
        if( ids.option_id == topLevelOptionId ) {
          ...
          if( !targetEl ) {
            //throw '<?php echo $this->string()->escapeJavascript("could not find target element") ?>';
          } else {
  ...
  var uiToggleOptions = function(spec, forceClose) {
    if( $type(spec) == 'event' || $type(spec) == 'domevent' ) {
      ...
      throw '<?php echo $this->string()->escapeJavascript("cannot toggle, no event or element") ?>';
    }
  ...
  var uiConfirmOptionDelete = function(spec) {
    if( $type(spec) == 'event' || $type(spec) == 'domevent' ) {
      ...
      throw '<?php echo $this->string()->escapeJavascript("unable to find option_id") ?>';
    }
    ...
    if( confirm('<?php echo $this->string()->escapeJavascript("Are you sure you want to delete this option?") ?>') ) {
      doOptionDelete(ids.suboption_id);
    }
  }

  var uiSmoothTopOptionDelete = function(spec) {
    if( confirm('<?php echo $this->string()->escapeJavascript("Are you sure you want to delete the current form?") ?>') ) {
  ...
  var uiSmoothCreateField = function(spec) {
    if( $type(spec) == 'event' || $type(spec) == 'domevent' ) {
      ...
      throw '<?php echo $this->string()->escapeJavascript("unable to find option_id for field") ?>';
    }
  ...
  var uiConfirmDeleteField = function(spec) {
    ...
    var ids = readIdentifiers(parentEl.get('id'));
    var url = urls.field.edit;
    if( confirm('<?php echo $this->string()->escapeJavascript("Are you sure you want to delete this field?") ?>') ) {
