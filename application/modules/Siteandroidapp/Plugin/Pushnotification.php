...
//            }
            ...
            //push notification work for MLT plugin add listingtype_id in object params & send listing_id in case of sitereview_listing instead of review_id
            if (isset($tempMessage['object_params']['type']) && !empty($tempMessage['object_params']['type']) && isset($tempMessage['object_params']['id']) && !empty($tempMessage['object_params']['id']) && strstr($tempMessage['object_params']['type'], 'sitereview')) {

                if ($tempMessage['object_params']['type'] == 'sitereview_listing') {
                    $sitereviewObj = Engine_Api::_()->getItem('sitereview_listing', $tempMessage['object_params']['id']);
                    if (isset($sitereviewObj) && !empty($sitereviewObj)) {
                        $tempMessage['object_params']['listingtype_id'] = $sitereviewObj->listingtype_id;
                    }
                } else if ($tempMessage['object_params']['type'] == 'sitereview_review') {
                    $reviewObj = Engine_Api::_()->getItem('sitereview_review', $tempMessage['object_params']['id']);
                    if (isset($reviewObj) && !empty($reviewObj) && !empty($reviewObj->resource_id)) {
                        $sitereviewObj = Engine_Api::_()->getItem('sitereview_listing', $reviewObj->resource_id);
                        if (isset($sitereviewObj) && !empty($sitereviewObj)) {
                            $tempMessage['object_params']['listingtype_id'] = $sitereviewObj->listingtype_id;
                            $tempMessage['object_params']['id'] = $sitereviewObj->listing_id;
                        }
                    }
                }
            }
            ...
            if ($notificationSubject->getIdentity()) {
                $messageGCMData['title'] = $notificationSubject->getTitle();
                $getContentImages = Engine_Api::_()->getApi('Core', 'siteapi')->getContentImage($notificationSubject);
                $messageGCMData['imgUrl'] = $getContentImages['image_icon'];
            } else {
