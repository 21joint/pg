...
class Siteandroidapp_AdminAppBuilderController extends Core_Controller_Action_Admin {
    ...
    protected $_enabledTabsArray = array(
        ...
        7 => 'siteandroidapp_admin_people',
    );
    protected $_advertisingEnabledTabsArray = array(
        1 => 'siteandroidapp_admin_advertising_type',
        2 => 'siteandroidapp_admin_advertising_modules',
    );
    ...
    protected $_getAPPImageDetailsForUpload = array(
        ...
        'notification_icon' => array(
            'title' => 'Notification APP Icon (48 x 48)',
            'width' => 48,
            'height' => 48,
            'directoryName' => 'App_Icons',
            'fileName' => 'Notification icon 48 x 48'
        ),
        'comet_chat_icon' => array(
            'title' => 'Comet Chat APP Icon (48 x 48)',
            'width' => 48,
            'height' => 48,
            'directoryName' => 'App_Icons',
            'fileName' => 'Comet Chat App icon 48 x 48'
        ),
        'app_icon_3' => array(
    ...
    public function init() {
        ...
        unset($tempImageArray['comet_chat_icon']);
    ...
    public function editGoogleAdModulesAction() {
        ...
        if ($this->getRequest()->isPost()) {
            ...
            if (!empty($values)) {
                ...
                $values['module_ad_type'] = $values['module_ad_type'];
...
//            }
            ...
            $explodeXMLToArray = !empty($getDefaultTranslatedLanguageArray) ? $getDefaultTranslatedLanguageArray : $this->_ExplodeXMLToArray($path, 1);
            $valuesArray = $explodeXMLToArray['values'] ? $explodeXMLToArray['values'] : $explodeXMLToArray;
...
//            return $this->_redirect($_SERVER['REQUEST_URI'], array('prependBase' => false));
    ...
    public function languageIndexAction() {
        ...
        // remove the language which was earlier a part of the website but now is not.
        $fileList = glob(APPLICATION_PATH . DIRECTORY_SEPARATOR . $this->_getAPPBuilderBaseURL . DIRECTORY_SEPARATOR . "Languages_App/*");

        $langaugeKeys = array_keys($this->_getAPPLanguageDetailsForUpload());

        foreach($fileList as $row => $value){
            $pathArray = explode("/", $value);
            $fileending = $pathArray[count($pathArray)-1];
            $languageArray = explode("_language", $fileending);
            if(!in_array($languageArray[0] , $langaugeKeys)){
                unlink($value);
            }
        }
    ...
    public function createAction() {
        ...
        if ($enabledTab == 5) {
            ...
            $this->view->subsubnavigation = $navigation = Engine_Api::_()->getApi('menus', 'core')->getNavigation('siteandroidapp_admin_advertising', array(), $this->_advertisingEnabledTabsArray[$subTab]);
            $multiOptions = array();
        ...
        if (@file_exists(APPLICATION_PATH . DIRECTORY_SEPARATOR . $this->_getAPPBuilderBaseURL . DIRECTORY_SEPARATOR . $values['directoryName'] . '/' . 'google-services.json')) {
            ...
            if (isset($form->firebase_google_services)) {
                ...
                $form->firebase_google_services->getDecorator('Description')->setOptions(array('placement' => 'PREPEND', 'escape' => false));
            }
        }
...
//                    if(($key != 'feature_graphics') && ($key != 'hi_res_icon'))
                    ...
                    if ($key == 'notification_icon') {
                        $form->$key->setDescription('Icon should be of white color with transparent background.' . $description);
                    }
...
//            }
                if (in_array($element, array('siteapi_validate_ssl'))) {
                    if (!isset($values[$element]))
                        $formError[] = $form->$element->getLabel() . " - Field  is required";
                }else if (in_array($element, array('app_webview'))) {
                    if (!isset($values[$element]))
                        $formError[] = $form->$element->getLabel() . " - Field  is required";
                }
...
//                $formError[] = 'You have chosed `I am running my website on https` but there are no SSL available, please change your option OR enable the SSL.';
            ...
            //Calculate no of Slideshow Images Uploaded
            $slideshowCount = 0;
            if (@file_exists(APPLICATION_PATH . DIRECTORY_SEPARATOR . $this->_getAPPBuilderBaseURL . DIRECTORY_SEPARATOR . 'Slideshow_Images/Big_Images_Tablet 720 x 1280/first.png') && @file_exists(APPLICATION_PATH . DIRECTORY_SEPARATOR . $this->_getAPPBuilderBaseURL . DIRECTORY_SEPARATOR . 'Slideshow_Images/Small_Images_Mobile 480 x 800/first.png')) {
                $slideshowCount++;
                $values['slideshow_1'] = 1;
            }
            if (@file_exists(APPLICATION_PATH . DIRECTORY_SEPARATOR . $this->_getAPPBuilderBaseURL . DIRECTORY_SEPARATOR . 'Slideshow_Images/Big_Images_Tablet 720 x 1280/second.png') && @file_exists(APPLICATION_PATH . DIRECTORY_SEPARATOR . $this->_getAPPBuilderBaseURL . DIRECTORY_SEPARATOR . 'Slideshow_Images/Small_Images_Mobile 480 x 800/second.png')) {
                $slideshowCount++;
                $values['slideshow_2'] = 1;
            }
            if (@file_exists(APPLICATION_PATH . DIRECTORY_SEPARATOR . $this->_getAPPBuilderBaseURL . DIRECTORY_SEPARATOR . 'Slideshow_Images/Big_Images_Tablet 720 x 1280/third.png') && @file_exists(APPLICATION_PATH . DIRECTORY_SEPARATOR . $this->_getAPPBuilderBaseURL . DIRECTORY_SEPARATOR . 'Slideshow_Images/Small_Images_Mobile 480 x 800/third.png')) {
                $slideshowCount++;
                $values['slideshow_3'] = 1;
            }
            $values['slideshowCount'] = $slideshowCount;
            //Calculate no of Slideshow Images Uploaded work ends
            ...
            try {
                ...
                foreach ($this->_getAPPImageDetailsForUpload as $key => $values) {
                    ...
                    if (empty($_FILES[$key]["tmp_name"])) {
                        ...
                        if (in_array($key, array('slideshow_slide_image_2_1', 'slideshow_slide_image_2_2', 'slideshow_slide_image_3_1', 'slideshow_slide_image_3_2', 'comet_chat_icon')))
                            continue;
...
//                        }
            ...
            if (!empty($errors)) {
                ...
                return;
            } else if ($enabledTab == 5 && isset($subTab) && !empty($subTab) && $subTab == 1) {
                $this->_helper->redirector->gotoRoute(array('tab' => $enabledTab, 'subTab' => ++$subTab));
                return;
            } else if ($enabledTab < 6) {
                $this->_helper->redirector->gotoRoute(array('tab' => ++$enabledTab));
                return;
            } else if ($enabledTab == 7) {
                $this->_helper->redirector->gotoRoute(array('tab' => 6));
                return;
            }
        }
    ...
    public function savePrimemessengerFiles() {
        ...
        if (isset($subdomain) && !empty($subdomain)) {
            ...
            //Save Setting File
            if (isset($getCurlArray['firebaseSettings']['android']) && !empty($getCurlArray['firebaseSettings']['android'])) {
                $temp_doc_file = APPLICATION_PATH . DIRECTORY_SEPARATOR . $this->_getAPPBuilderBaseURL . DIRECTORY_SEPARATOR . 'Primemessenger';
                $content = file_get_contents($getCurlArray['firebaseSettings']['android']);
            ...
            if (isset($getCurlArray['languageSettings']) && !empty($getCurlArray['languageSettings'])) {
                foreach ($getCurlArray['languageSettings'] as $key => $files) {
                    if (isset($files['android']) && !empty($files['android'])) {
                        ...
                        $content = file_get_contents($files['android']);
                        if (isset($content) && !empty($content)) {
                            $fileName = $temp_doc_file . '/' . $key . '_language_android_mobileapp.xml';
                            file_put_contents($fileName, $content);
                        }
                    }
                }
