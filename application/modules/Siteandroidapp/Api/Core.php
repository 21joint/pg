    ...
    private function _addListingTypeInMenu($listingTypes, $availableListingTypesArray) {
        if (!empty($listingTypes)) {
            ...
            foreach ($listingTypes as $listingType) {
                if(isset($listingType->listingtype_id))
                    $tempArray['listingtype_id'] = $listingType->listingtype_id;
                
                if(isset($listingType->title_singular))
                    $tempArray['header_label_singular'] = $listingType->title_singular;
                ...
                if (!in_array($listingType->listingtype_id, $availableListingTypesArray)) {
                    ...
                    $row->default = 1;
                    $row->default = 1;
                    $row->params = @serialize($tempArray);
                    $row->save();
                }
            }
        }
