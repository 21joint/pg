 ...
 */
class Event_TopicController extends Siteapi_Controller_Action_Standard {
    ...
    /**
     * Show discussion on event profile page.
     * 
     * @return array
     */
    public function indexAction() {
        ...
        $select = $table->select()
                ->where('event_id = ?', Engine_Api::_()->core()->getSubject()->getIdentity())
                ->order('sticky DESC')
    ...
    public function createAction() {
        ...
        if ($this->getRequest()->isGet()) {
            ...
            try {
                ...
                $action = $activityApi->addActivity($viewer, $event, 'event_topic_create', null, array('child_id' => $topic->getIdentity()));
                if ($action) {
    ...
    /**
     * Profile page of event discussion.
     * 
     * @return array
     */
...
//    $response = array();
        ...
        $canEdit = false;
        $canDelete = false;
        if (Engine_Api::_()->authorization()->isAllowed($event, null, 'topic.edit')) {
            $canEdit = true;
        }
        if (Engine_Api::_()->authorization()->isAllowed($event, null, 'topic.delete')) {
            $canDelete = true;
        }

//    $response['canEdit'] = $topic->canEdit(Engine_Api::_()->user()->getViewer());
//    $response['officerList'] = $event->getOfficerList();
//
//    $canPost = $event->authorization()->isAllowed($viewer, 'comment');
            ...
            if ($viewer->getIdentity()) {
                if (!$isWatching) {
                    $bodyParams['gutterMenu'][] = array(
                        ...
                        'name' => 'watch_topic',
                        'url' => 'events/topic/watch/topic_id/' . $topic->getIdentity() . '/watch/1',
                        'urlParams' => array(
                        )
                    );
                } else {
                    $bodyParams['gutterMenu'][] = array(
                        ...
                        'url' => 'events/topic/watch/topic_id/' . $topic->getIdentity() . '/watch/0',
                        'urlParams' => array(
                        )
                    );
                }
                ...
                if (!empty($canEdit)) {
                    if (!$topic->sticky) {
                        $bodyParams['gutterMenu'][] = array(
                            ...
                            'name' => 'make_sticky',
                            'url' => 'events/topic/sticky/topic_id/' . $topic->getIdentity() . '/sticky/1',
                            'urlParams' => array(
                            )
                        );
                    } else {
                        $bodyParams['gutterMenu'][] = array(
                            ...
                            'name' => 'remove_sticky',
                            'url' => 'events/topic/sticky/topic_id/' . $topic->getIdentity() . '/sticky/0',
                            'urlParams' => array(
                            )
                        );
                    }
                    ...
                    if (!$topic->closed) {
                        $bodyParams['gutterMenu'][] = array(
                            ...
                            'name' => 'close',
                            'url' => 'events/topic/close/topic_id/' . $topic->getIdentity() . '/close/1',
                            'urlParams' => array(
                            )
                        );
                    } else {
                        $bodyParams['gutterMenu'][] = array(
                            ...
                            'name' => 'open',
                            'url' => 'events/topic/close/topic_id/' . $topic->getIdentity() . '/close/0',
                            'urlParams' => array(
                            )
                        );
                    }
                    ...
                    $bodyParams['gutterMenu'][] = array(
                        ...
                        'name' => 'rename',
                        'url' => 'events/topic/rename/topic_id/' . $topic->getIdentity(),
                        'urlParams' => array(
                        )
                    );
                }
                ...
                if (!empty($canDelete)) {
                    $bodyParams['gutterMenu'][] = array(
                        ...
                        'name' => 'delete',
                        'url' => 'events/topic/delete/topic_id/' . $topic->getIdentity(),
                        'urlParams' => array(
                        )
                    );
                }
    ...
    public function postAction() {
        ...
        if (Engine_Api::_()->core()->hasSubject())
            $topic = Engine_Api::_()->core()->getSubject('event_topic');
            ...
            try {
                ...
                foreach ($userTable->find($notifyUserIds) as $notifyUser) {
                    ...
                    if ($notifyUser->isSelf($topicOwner)) {
                        $type = 'event_topic_response';
                    } else {
                        $type = 'event_topic_reply';
                    }
