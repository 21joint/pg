    ...
    <table class='admin_table'>
        <thead>
            <tr>
                ...
                <?php if(!empty($_GET['seaddons'])) : ?>
                <th align="left">
                    <input type="checkbox" class="seaocore_multi-upgrade_checkall" name="multi-upgrade" onclick="checkUncheckAll(this)">
                </th>
                <?php endif; ?>
            </tr>
        </thead>
...
//				$versionInfo = 0;
                ...
                if (!empty($running_version) && !empty($product_version)) {
                    ...
                    <tr>
                        ...
                        <td>
                    <?php if(!empty($shouldUpgrade) && !empty($_GET['seaddons'])) : ?>
    ...
    <?php if(!empty($_GET['seaddons'])) : ?>
     <button title="Upgrade Selected" style="font-size:11px;padding:5px; float: right; margin: 10px" onclick="upgradeSelectedPlugins()">Upgrade Selected</button>
     <?php endif; ?>
    <br />
<div id="seaocore_light" class="seaocore_downloading_white_content">
    <?php echo $this->translate('Downloading packages ! please wait '); ?>
    <img src="<?php echo $this->layout()->staticBaseUrl; ?>application/modules/Document/externals/images/document-uploading.gif" alt="" />
</div>
...
<script type="text/javascript">
    ...
    function upgradeSelectedPlugins() {
        ...
        if(ready == true) {
            ...
            sendRequest(elements[selectedItemLength]);
    ...
    //$$('.file-package-remove').each(function(item){ item.getFirst('a').click(); })
</script>
