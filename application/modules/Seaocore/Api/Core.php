...
class Seaocore_Api_Core extends Core_Api_Abstract {
    ...
    public function setDefaultConstant() {
        ...
        if (!empty($showLightboxOptionDisplay)) {
            ...
            define('SEA_SITEVIDEO_LIGHTBOX', in_array('sitevideo', $showLightboxOptionDisplay) || $seaocore_display_lightbox);
...
//      define('SEA_SITEREVIEW_LIGHTBOX', in_array('sitereview', $showLightboxOptionDisplay) || $seaocore_display_lightbox);
            ...
            define('SEA_SITEVIDEO_LIGHTBOX', $seaocore_display_lightbox);
...
Stores - Notes Extension ", "sitestoreevent" => "Directory / Stores - Events Extension ", "siteevent" => 'SEAO - Advanced Events ', "advancedactivity" => "SEAO - Advanced Activity Feeds ");
        ...
        if (!empty($enableModules)) {
            ...
            define('SEA_VIDEOLIGHTBOX_DOWNLOAD', 1);
            define('SEA_VIDEOLIGHTBOX_REPORT', 1);
            define('SEA_VIDEOLIGHTBOX_MAKEPROFILEPHOTO', 1);
            define('SEA_VIDEOLIGHTBOX_SHARE', 1);
            define('SEA_VIDEOLIGHTBOX_EDITLOCATION', 1);
            define('SEA_VIDEOLIGHTBOX_GETLINK', 1);
            define('SEA_VIDEOLIGHTBOX_SENDMAIL', 1);
            define('SEA_VIDEOLIGHTBOX_MAKECHANNELCOVER', 1);
            define('SEA_VIDEOLIGHTBOX_MOVETOOTHERCHANNEL', 1);
        } else {
            ...
            define('SEA_VIDEOLIGHTBOX_DOWNLOAD', 0);
            define('SEA_VIDEOLIGHTBOX_REPORT', 0);
            define('SEA_VIDEOLIGHTBOX_MAKEPROFILEPHOTO', 0);
            define('SEA_VIDEOLIGHTBOX_SHARE', 0);
            define('SEA_VIDEOLIGHTBOX_GETLINK', 0);
            define('SEA_VIDEOLIGHTBOX_SENDMAIL', 0);
            define('SEA_VIDEOLIGHTBOX_MAKECHANNELCOVER', 0);
            define('SEA_VIDEOLIGHTBOX_MOVETOOTHERCHANNEL', 0);
            define('SEA_VIDEOLIGHTBOX_EDITLOCATION', 0);
        }
    }
...
//     }
        ...
        switch ($resource_type) {
            ...
            case 'video':
                if (Engine_Api::_()->getDbtable('modules', 'core')->isModuleEnabled('sitevideo')) {
                    $Table = Engine_Api::_()->getItemTable('sitevideo_video_category');
                } elseif (Engine_Api::_()->getDbtable('modules', 'core')->isModuleEnabled('sesvideo')) {
                    $Table = Engine_Api::_()->getItemTable('sesvideo_category');
                } else if (!Engine_Api::_()->getDbtable('modules', 'core')->isModuleEnabled('video')) {
    ...
    public function getContentPhotoHref($content) {
        $href = '';
        if (isset($content->photo_id)) {
            $photo_id = $content->photo_id;
        } else if (isset($content->file_id)) {
            $photo_id = $content->file_id;
        }
        $tab = Engine_Api::_()->getDbtable('photos', strtolower($content->getModuleName()));
        $photo_id = $tab->select()->from($tab->info('name'), 'photo_id')->where('file_id =?', $photo_id)->query()->fetchColumn();
        if ($photo_id) {
    ...
    public function tinymceEditorOptions($upload_url = false) {
        ...
        if ($coreversion < '4.7.0') {
            ...
            $plugins = "directionality,advlist,autolink,lists,link,image,charmap,print,preview,hr,anchor,"
                    ...
                    . "media,nonbreaking,save,table,contextmenu,directionality,emoticons,paste,textcolor";
            $uploadPlugin = '';
            if (!empty($upload_url)) {
                $plugins .= ',jbimages';
            ...
            if ($coreversion >= '4.9.0') {
                $plugins .= ",imagetools,colorpicker,autosave";
            }
            return array(
                'upload_url' => $upload_url,
                'menubar' => true,
    ...
    public function getMyLocationDetailsCookie($fromCookiesOnly = 0) {
        ...
        if ($fromCookiesOnly) {
            if (!is_array($seaocore_myLocationDetails)) {
                return !empty($seaocore_myLocationDetails) ? Zend_Json::decode($seaocore_myLocationDetails) : array();
            }
            return array();
        }

        if (!empty($seaocore_myLocationDetails) && !is_array($seaocore_myLocationDetails)) {
            $seaocore_myLocationDetails = !empty($seaocore_myLocationDetails) ? Zend_Json::decode($seaocore_myLocationDetails) : array();
        }
    ...
    // to retrieve super admin of site.
    public function getSuperAdmin() {

        $userTable = Engine_Api::_()->getDbtable('users', 'user');
        $userTableName = $userTable->info('name');
        $userId = $userTable->select()->from($userTableName, 'user_id')->where('level_id =?', 1)->order('user_id ASC')->query()->fetchColumn();
        $superAdmin = Engine_Api::_()->getItem('user', $userId);
        return $superAdmin;
    }
    ...
    public function getModuleContent($contentDetails = array()) {
        ...
        switch ($contentDetails['module_name']) {
            ...
            case 'album':
                ...
                break;
            case 'event':
                $params = array('search' => 1, 'order' => $orderby);
            ...
            default:
                $select = $table_name->select();
                if (!empty($orderby)) {
                    $select = $select->order($orderby . ' DESC');
                }
                break;
        }
    ...
    public function checkModuleNameAndNavigation() {
        if (Engine_Api::_()->hasModuleBootstrap('spectacular') || Engine_Api::_()->hasModuleBootstrap('captivate')) {
    ...
    public function verifyYotubeApiKey($key) {
        ...
        curl_close($curl);
        $responseObj = !empty($json_response) ? Zend_Json::decode($json_response) : array();
    ...
    public function getEnabledModuleNames() {
        ...
        return $enabledModuleNames = $coreTable->select()
                ->from($coreTableName, '*')
                ->where('enabled = ?', true)
                ->where('name in (?)', array('sitevideo'))
                ->query()
                ->fetchAll();
    }
    ...
    public function getValueOfCssVariables($file, $search) {
        $explodeVariableValue = 0;
        if (!empty($file) && !empty($search)) {
            // Make an array of the each row and search the respective string row and change it to new. 
            if (@file_exists($file)) {
                @chmod($file, 0777);
            ...
            if (!empty($tempValue))
                $explodeVariableValue = (int) str_replace("px;", "", end(explode(":", $tempValue)));
        }

        return $explodeVariableValue;
    }

    public function getWidgetContentInfo($content_id, $widgetInfo) {
        ...
        if( !empty($contentModule) ) {
          ...
          foreach( $contentModule as $modules ) {
            if( $modules['name'] == 'sitereview' ) {
               ...
               $contentModuleArray = array_merge($contentModuleArray,$contentItem);
          }else {
               $contentModuleArray = array_merge($contentModuleArray,$this->getContentItem($modules['name']));
          }
          }
        }
