...
var SEAOAutocompleter = new Class({
		...
		if( !this.options.alwaysOpen ) {
                ...
                if (this.options.autocompleteType=='message') {
                  ...
                  if (token.friends){
                    ...
                    this.doAddValueToHidden(token.label, friend_ids, this.options.elementValues, true, ' tag_friend');
                    // code to expend out the suggestion list
                    /*
                    for (var id in token.friends) {
	this.doAddValueToHidden(token.friends[id], id, this.options.elementValues, true);
                    }*/
                  }
                 
                 else this.doAddValueToHidden(choice.id, token.id, this.options.elementValues, true);
                }
        ...
        doPushSpan: function(name, toID, newItem, hideLoc, list){
          ...
          if (newItem){
            myElement.id = "tospan_"+name+"_"+toID;
						myElement.innerHTML = name+" <a href='javascript:void(0);' onclick='this.parentNode.destroy();removeFromToValue(\""+toID+"\", \""+this.options.elementValues+"\" , \""+this.element.id+"\", \""+hideLoc+"\");'>x</a>";
					} 
          else{
            myElement.id = "tospan_"+name+"_"+toID;
						myElement.innerHTML = name+" <a href='javascript:void(0);' onclick='this.parentNode.destroy();removeFromToValue(\""+toID+"\", \""+this.options.elementValues+"\" , \""+this.element.id+"\", \""+hideLoc+"\");'>x</a>";
          }
          $(this.options.elementValues+'-wrapper').setStyle('height', 'auto');


          if (list == null) list = "";
          myElement.addClass("tag"+list);

					document.getElementById(this.options.elementValues+'-element').appendChild(myElement);
          this.fireEvent('push');
        },
...
/* compatibility */
 ...
 SEAOAutocompleter.Base = SEAOAutocompleter;
