        ...
        <ul class="edit-button">
          <li> 
            ...
            <ul class="seaocore_profile_options_pulldown">
              ...
              <li>
                <?php echo $this->htmlLink($this->url(array('action' => 'get-albums-photos', 'user_id' => $this->user->user_id, 'recent' => 1, 'special' => 'profile'), 'siteusercoverphoto_profilepage', true), $this->translate('Choose from Albums'), array(' class' => 'seaocore_profile_cover_icon_photo_view smoothbox')); ?>
              </li> 
...
<div class="seaocore_profile_cover_head_section_inner" id="seaocore_profile_cover_head_section_inner">
  ...
  <?php if (is_array($this->showContent) && (($this->profile_like_button == 1) || in_array('friendShipButton', $this->showContent) || in_array('composeMessageButton', $this->showContent) || in_array('updateInfoButton', $this->showContent) || in_array('settingsButton', $this->showContent) || in_array('optionsButton', $this->showContent))): ?>
    <div class="seaocore_profile_coverinfo_buttons">
      ...
      <div>
        <?php $followEnabled = Engine_Api::_()->getApi("settings", "core")->getSetting('user.friends.direction', 1) && Engine_Api::_()->getApi("settings", "core")->getSetting('sitemember.user.follow.enable', 1); ?>
        <?php if ($followEnabled && in_array('followButton', $this->showContent)): ?>
          <?php echo $this->content()->renderWidget("seaocore.seaocore-follow"); ?>
        <?php endif; ?>
      </div>
      <?php if (in_array('reviewbutton', $this->showContent)): ?>
            <?php if (Engine_Api::_()->hasModuleBootstrap('sitemember')): ?>
                <?php echo $this->content()->renderWidget("sitemember.review-button", array('cover_photo' => 1)); ?>
            <?php endif; ?>
        <?php endif; ?>
