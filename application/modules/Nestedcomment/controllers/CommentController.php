...
class Nestedcomment_CommentController extends Core_Controller_Action_Standard {
    ...
    public function listAction() {
        ...
        $this->view->postComment = $postComment= Engine_Api::_()->getApi('settings', 'core')->getSetting('nestedcomment.comment.public', 1);
        $subjectParent = $subject;
        if($postComment==1){ 
         
            $modules = Engine_Api::_()->getItemTable('nestedcomment_modules');
            $publicComment  = $modules->select()->from($modules->info('name'), array('publicComment'))
            ...
            if($publicComment==0){
                $publicComment = $this->_getParam('nestedcomment_comment_public',0);
            }
            
          
            $this->view->postComment = $publicComment;
             if($subject->getType()=='advancedactivity') {
                  $this->view->postComment=0;
             }
        }
       ...
       if (($viewer->getIdentity() && $canComment) || $this->view->postComment==1) {
    ...
    public function createAction() {
        ...
        if (!$this->_helper->requireUser()->isValid()) {
            $commentData = $this->_getAllParams();
            $commentNamespace = new Zend_Session_Namespace('Comment');
            $commentNamespace->comment = serialize($commentData);
            return;
        }
        ...
        try {
            ...
            if (!empty($attachmentData) && !empty($attachmentData['type'])) {
                ...
                if ($attachmentData['type'] == 'sticker' && isset($attachmentData['stikcer_guid']))
                    $attachment = Engine_Api::_()->getItemByGuid($attachmentData['stikcer_guid']);
            ...
            $userIds = array();
            $allowedNotification = Engine_Api::_()->getApi('settings', 'core')->getSetting('aaf.notification.onoff.enable', 1);
            if($allowedNotification) {
                $offNotification = Engine_Api::_()->getDbtable('notificationsettings', 'advancedactivity');
                $userIds = $offNotification->getUserIdsByActionId($action->getIdentity());
            }
            $tagsArray = array();
            ...
            if (!empty($tagsArray)) {
                ...
                foreach ($tagsArray as $key => $tagStrValue) {
                    ...
                    if ($action && $tag && ($tag instanceof User_Model_User) && !$tag->isSelf($viewer) &&  !in_array($tag->getIdentity(), $userIds)) {
                        ...
                        foreach ($tag->getPageAdmins() as $owner) {
                            if ($action && $owner && ($owner instanceof User_Model_User) && !$owner->isSelf($viewer) && !in_array($owner->getIdentity(), $userIds)) {
            ...
            if ((strpos($subject->getType(), "sitepage") === false) || (strpos($subject->getType(), "sitegroup") === false) || (strpos($subject->getType(), "sitestore") === false ) || (strpos($subject->getType(), "sitebusiness") === false) || (strpos($subject->getType(), "sitestoreproduct") !== false)) {
                if ($subjectOwner->getType() == 'user' && $subjectOwner->getIdentity() != $viewer->getIdentity() && !in_array($subjectOwner->getIdentity(), $userIds)) {
            ...
            foreach (Engine_Api::_()->getDbtable('comments', 'nestedcomment')->comments($subject)->getAllCommentsUsers() as $notifyUser) {
                if ($notifyUser->getIdentity() == $viewer->getIdentity() || $notifyUser->getIdentity() == $subjectOwner->getIdentity() || !in_array($notifyUser->getIdentity(), $userIds))
                    continue;
    ...
    public function likeAction() {
        ...
        try {
            ...
            if (!Engine_Api::_()->getDbtable('likes', 'core')->isLike($commentedItem, $viewer))
                $like = Engine_Api::_()->getDbtable('likes', 'nestedcomment')->likes($commentedItem)->addLike($viewer);
            else
                $like = Engine_Api::_()->getDbtable('likes', 'core')->getLike($commentedItem,$viewer);

            $reaction = $this->getParam('reaction');
            if(!empty($reaction))
            {
                $like->reaction = $this->getParam('reaction');
                $like->save();
                $attachment = $subject;
            ...
            if (strpos($subject->getType(), "sitepage_page") != 'sitepage_page' || strpos($subject->getType(), "sitebusiness_business") != 'sitebusiness_business' || strpos($subject->getType(), "sitegroup_group") != 'sitegroup_group' || strpos($subject->getType(), "sitestore_store") != 'sitestore_store') {
                if ($owner->getType() == 'user' && $owner->getIdentity() != $viewer->getIdentity()  && !in_array($owner->getIdentity(), $userIds)) {
                    $notifyApi = Engine_Api::_()->getDbtable('notifications', 'activity');
    ...
    public function updateAction() {
        ...
        try {
            ...
            if (!$attachmentData && ($comment->attachment_type)) {
                if (!in_array($comment->attachment_type, array('sitereaction_sticker'))) {
            ...
            if (!empty($tagsArray)) {
                ...
                foreach ($tagsArray as $key => $tagStrValue) {
                    $tag = Engine_Api::_()->getItemByGuid($key);
                    if ($tag && ($tag instanceof User_Model_User) && !$tag->isSelf($viewer) && !in_array($tag->getIdentity(), $userIds)) {
                        $notificationAPi->addNotification($tag, $viewer, $action, 'tagged', array(
                            'object_type_name' => $type_name,
                            'label' => $type_name,
                        ));
