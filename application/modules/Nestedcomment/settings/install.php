...
class Nestedcomment_Installer extends Engine_Package_Installer_Module {
    ...
    function onInstall() {
        ...
        if (!empty($tableNewObj)) {
            $publicComment = $db->query("SHOW COLUMNS FROM `engine4_nestedcomment_modules` LIKE 'publicComment' ")->fetch();
            if (empty($publicComment)) {
                $db->query("ALTER TABLE  `engine4_nestedcomment_modules` ADD  `publicComment` tinyint(1) DEFAULT  '0';");
            }
        }
...
VALUES ('sitecrowdfunding', 'sitecrowdfunding_project', 0)");
        ...
        // sitenews integration
        $select = new Zend_Db_Select($db);
        $select
                ->from('engine4_core_modules')
                ->where('name = ?', 'sitenews');
        $is_sitenews_object = $select->query()->fetchObject();
        if ($is_sitenews_object) {

            $db->query("INSERT IGNORE INTO `engine4_nestedcomment_modules` (`module`, `resource_type`, `enabled`) 
VALUES ('sitenews', 'sitenews_news', 0)");
        }
    }
