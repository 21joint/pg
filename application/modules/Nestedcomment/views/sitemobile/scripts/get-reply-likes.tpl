 ...
 * @version    $Id: get-reply-likes.tpl 6590 2013-06-03 00:00:00Z SocialEngineAddOns $
  ...
  <?php if ($this->likes->getTotalItemCount() > 0): // COMMENTS -------  ?>
    ...
    <?php if ($this->page == 1): ?>
      <div class="sm-ui-popup-top ui-header ui-bar-a">
        <?php if (!empty($action)): ?> 
          <a data-iconpos="notext" data-role="button" data-icon="chevron-left" data-corners="true" data-shadow="true" class="ui-btn-left " onclick="$('#reply-activity-item-' + <?php echo $action->action_id ?>).css('display', 'block');$('#like-reply-item-' + <?php echo $action->action_id ?>).css('display', 'none');"><?php //echo $this->translate('back');?></a>
        <?php else : ?>
      ...
      <div class="sm-ui-popup-likes sm-content-list">
        <ul id="likereplymembers_ul" class="ui-member-list" data-role="listview" data-icon="none">
        <?php endif; ?>
        ...
        <div class="like_viewmore" id="like_replyviewmore" style="display: none;">
          <?php
          echo $this->htmlLink('javascript:void(0);', $this->translate('View More'), array(
              'id' => 'like_reply_viewmore_link',
              'class' => 'buttonlink icon_viewmore',
              'onclick' => 'sm4.activity.reply_likes(' . $this->action_id . ',' . $this->comment_id . ',' . ($this->page + 1) . ');'
          ))
          ?>
        </div>
      </div>
...
<?php if ($this->page && $this->likes->getCurrentPageNumber() >= $this->likes->count()): ?>
         var nextlikereplypage = 0;
<?php else: ?>
         var nextlikereplypage = 1;
<?php endif; ?>
<?php if (!empty($action)): ?>
            window.onscroll = sm4.activity.doOnScrollLoadReplyLikes('<?php echo $action->action_id; ?>', '<?php echo $this->comment_id; ?>', '<?php echo ($this->page + 1); ?>');    
