    ...
    <?php endif; ?>
            ...
            <div class="aaf_pulldown_contents_wrapper">
                <div class="aaf_pulldown_contents">
                  ...
                  <?php if (!$this->subject()): ?>
                    ...
                    <?php endif; ?>
                        <li>
                             <a href="javascript:void(0);" onclick='showLinkPost("<?php echo $action->getHref(array('show_comments' => true)) ?>")'><?php echo $this->translate('Feed Link'); ?></a>
                        </li>
                        <li class="sep"></li>
                        <li>
        ...
        <div class='feed_item_body'>
          <?php // Main Content  ?>
            ...
            ?>
            <?php echo $this->getContent($action, false, $groupedFeeds, array('similarActivities'=> $this->similarActivities)); ?>
            </span>
            <div><?php echo $allowEdit ? $this->editPost($action): '';?></div>
          <?php // Attachments ?>
                ...
                <span class='feed_attachment_<?php echo $attachment->meta->type ?>'>
                    ...
                    <div class="feed_attachment_aaf">
                          <?php if ($attachment->item->getPhotoUrl()): ?>
                            ...
                            <?php if (strpos($attachment->meta->type, '_photo')): ?>
                              ...
                              <?php else: ?>
                                ...
                                <?php if ($this->showLargePhoto || $attachment->item->getType() == 'album_photo'): ?>
                                  ...
                                  switch( $count ):
                                    case 1:
                                      $photoContent = $this->itemPhoto($attachment->item, 'thumb.main', $attachment->item->getTitle(), array('class' => "aaf-feed-photo-1", 'data-gif-scroll-play' => 1));
                                      break;
                                    case 2:
                                      $photoContent = $this->itemBackgroundPhoto($attachment->item, 'thumb.profile', $attachment->item->getTitle(), array('class' => "aaf-feed-photo-2"));
                                      break;
                                    case 3:
                                      if( !$isIncludeFirstAttachment ):
                                        $photoContent = $this->itemBackgroundPhoto($attachment->item, 'thumb.medium', $attachment->item->getTitle(), array('class' => "aaf-feed-photo-3-big"));
                                      else:
                                        $photoContent = $this->itemBackgroundPhoto($attachment->item, ($attachment->item->getType() == 'album_photo' ? 'thumb.normal' : 'thumb.medium'), $attachment->item->getTitle(), array('class' => "aaf-feed-photo-3-small"));
                                      endif;
                                      break;
                                    case 4:
                                      if( !$isIncludeFirstAttachment ):
                                        $photoContent = $this->itemBackgroundPhoto($attachment->item, 'thumb.medium', $attachment->item->getTitle(), array('class' => "aaf-feed-photo-4-big"));
                                      else:
                                        $photoContent = $this->itemBackgroundPhoto($attachment->item, ($attachment->item->getType() == 'album_photo' ? 'thumb.normal' : 'thumb.medium'), $attachment->item->getTitle(), array('class' => "aaf-feed-photo-4-small"));
                                      endif;
                                      break;
                                    case 5:
                                      if( !$isIncludeFirstAttachment || $isIncludeSecondAttachment == 2 ):
                                        $photoContent = $this->itemBackgroundPhoto($attachment->item, 'thumb.medium', $attachment->item->getTitle(), array('class' => "aaf-feed-photo-5-big"));
                                      elseif( $isIncludeFirstAttachment ):
                                        $photoContent = $this->itemBackgroundPhoto($attachment->item, ($attachment->item->getType() == 'album_photo' ? 'thumb.normal' : 'thumb.medium'), $attachment->item->getTitle(), array('class' => "aaf-feed-photo-5-small"));
                                      endif;
                                      break;
                                    case $count > 5:
                                      if( !$isIncludeFirstAttachment || $isIncludeSecondAttachment == 2 ):
                                      ...
                                      break;
                                    default :
                                      $photoContent = $this->itemBackgroundPhoto($attachment->item, 'thumb.normal', $attachment->item->getTitle(), array('class' => "aaf-feed-photo-9"));
                                  endswitch;
                           ...
                           // echo $this->htmlLink($attachment->item->getHref(), $photoContent, $attribs); ?>
                              <?php $thumbType = in_array($attachment->item->getType(), array('core_link')) ? 'thumb.main': 'thumb.profile'; ?>
                              <?php echo $this->htmlLink($attachment->item->getHref(), $this->itemPhoto($attachment->item, $thumbType, $attachment->item->getTitle()), $attribs) ?>
                            <?php endif; ?>
                          <?php endif; ?>
...
document.getElementById("' . $this->commentForm->body->getAttrib('id') . '").focus();document.getElementById("' . "comment-likes-activityboox-item-$actionBaseId" . '").style.display = "block";', 'title' => $this->translate('Leave a comment'))) ?>
                ...
                <?php if ($action->getTypeInfo()->shareable && $action->shareable && $this->viewer()->getIdentity()): ?>
                  <?php if (Engine_Api::_()->getApi('settings', 'core')->getSetting('aaf.social.share.enable', 1)) : ?>
                    <?php $shareIcons = $this->shareIcons($action) ?>
                    <?php if ($shareIcons) : ?>
                        <li class="feed_item_option_share seao_icons_toolbar_attach"> 
                        <?php echo $shareIcons; ?> 
                            <a href="javascript:void(0);" class='share_icons_link'><?php echo $this->translate('ADVACADV_SHARE')?></a>
                            <span>&#183;</span>
                        </li>
                    <?php endif; ?>
                  <?php else: ?>
...
<?php endif;?>
<?php if ($this->settings('sitereaction.collection.active', 1) && Engine_Api::_()->getDbtable('modules', 'core')->isModuleEnabled('sitereaction')): ?>
