...
?>
<?php $this->addHelperPath(
  APPLICATION_PATH . '/application/modules/Nestedcomment/View/Helper',
  'Nestedcomment_View_Helper'
); ?>
<?php $photoEnabled = false; ?>
<?php $smiliesEnabled = false; ?>
<?php $activityTaggingContent = false; ?>
<?php $row = Engine_Api::_()->seaocore()->checkEnabledNestedComment(
  'advancedactivity', array('checkModuleExist' => true)
); ?>
<?php if ($row): ?>
  ...
  <?php if (!empty($decodedParams['showComposerOptions'])): ?>
    <?php if (Engine_Api::_()->getDbtable('modules', 'core')->isModuleEnabled(
      'album'
    )
    ) : ?>
      <?php $photoEnabled = in_array(
        "addPhoto", $decodedParams['showComposerOptions']
      ); ?>
    <?php endif; ?>
    <?php $smiliesEnabled = in_array(
      "addSmilies", $decodedParams['showComposerOptions']
    ); ?>
  <?php endif; ?>
  <?php $showAsLike = $decodedParams['showAsLike']; ?>
  <?php $showAsNested = isset($decodedParams['showAsNested'])
    ? $decodedParams['showAsNested'] : 1; ?>
  <?php $defaultViewReplyLink = isset($decodedParams['defaultViewReplyLink'])
    ? $decodedParams['defaultViewReplyLink'] : 0; ?>
  <?php $showDislikeUsers = $decodedParams['showDislikeUsers']; ?>
  <?php $showLikeWithoutIcon = $decodedParams['showLikeWithoutIcon']; ?>
  <?php $showLikeWithoutIconInReplies
    = $decodedParams['showLikeWithoutIconInReplies']; ?>
  <?php $taggingEnabled
    =
  $activityTaggingContent = implode($decodedParams['taggingContent'], ","); ?>
  <?php $this->commentShowBottomPost
    = $decodedParams['advancedactivity_comment_show_bottom_post']; ?>
  ...
  <?php endif; ?>
  <?php if (!$showAsLike): ?>
    <?php $this->allowReaction = $this->allowReaction
      && $this->settings(
...
$this->headLink()->appendStylesheet(
  ...
  ->appendStylesheet(
    $this->layout()->staticBaseUrl
    . 'application/modules/Seaocore/externals/styles/style_icon_toolbar.css'
  )
  ->appendStylesheet(
    $this->layout()->staticBaseUrl
