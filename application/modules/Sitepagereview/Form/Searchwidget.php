 ...
 */
class Sitepagereview_Form_Searchwidget extends Engine_Form {
  ...
  public function init() {
    ...
    $this->setAction($view->url(array(), 'sitepagereview_browse', true))->getDecorator('HtmlTag')->setOption('class', '');
  }
  ...
  public function getAdditionalOptionsElement() {
    ...
    $select_content = $coreContent_table->select()->where('name = ?', 'sitepagereview.search-sitepagereview');
    ...
    //$row = $this->_searchForm->getFieldsOptions('sitepage', 'search');
     ...
     if (in_array("4", $enabledColumns)) {
      $this->addElement('Text', 'search_review', array(
              'label' => 'Review Title',
              'order' => $row->order - 4,
              'filters' => array(
     ...
     if (in_array("3", $enabledColumns)) {
      ...
      $show_multiOptions["creation_date"] = "Everyone’s Reviews";

			if(!empty($viewer_id)) {
				$show_multiOptions["my_friend_review"] = 'My Friends’ Reviews';
				$show_multiOptions["my_like"] = 'Reviews of Pages I Like';
			}

      $show_multiOptions["featured"] = "Featured Reviews";
      ...
      $this->addElement('Select', 'show', array(
              ...
              'onchange' => $this->gethasMobileMode() ? '' : 'searchSitepagereviews();',
              'order' => $row->order - 2,
      ));
