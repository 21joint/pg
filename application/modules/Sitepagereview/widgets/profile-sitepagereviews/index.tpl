...
<?php if (!empty($this->show_content)) : ?>
	<script type="text/javascript">
		...
		var paginateSitepageReview = function(page) {
			var url = en4.core.baseUrl + 'widget/index/mod/sitepagereview/name/profile-sitepagereviews';
...
<?php if (!empty($this->show_content)) : ?>
  <?php if($this->showtoptitle == 1):?>
      ...
      <?php echo $this->translate($this->sitepage->getTitle());?><?php echo $this->translate("'s Reviews");?>
		</div>
	<?php endif;?>	
	<?php if(Engine_Api::_()->getDbtable('modules', 'core')->isModuleEnabled('communityad') && Engine_Api::_()->getApi('settings', 'core')->getSetting('sitepage.communityads', 1) && Engine_Api::_()->getApi('settings', 'core')->getSetting('sitepage.adreviewwidget', 3) && $page_communityad_integration && Engine_Api::_()->sitepage()->showAdWithPackage($this->sitepage)) :?>
			<div class="layout_right" id="communityad_review">

<?php
			echo $this->content()->renderWidget("communityad.ads", array( "itemCount"=>Engine_Api::_()->getApi('settings', 'core')->getSetting('sitepage.adreviewwidget', 3),"loaded_by_ajax"=>1,'widgetId'=>"page_reviews")); 			 
		?>
			</div>
		...
		<div class="seaocore_add">
			<?php
				echo $this->htmlLink(array(
							'route' => 'sitepagereview_create',
							'page_id' => $this->sitepage->page_id,
							'tab' => $this->identity_temp,
					), $this->translate('Write a Review'), array('class' => 'icon_sitepages_review buttonlink'));
			?>
		</div>
	<?php endif; ?>
	
<!--start top box code-->
<?php if(!empty($this->ratingDataTopbox) && !empty($this->noReviewCheck) && $this->current_page <= 1):?>
<ul class="sitepagereview_rating">
	<?php $iteration = 1;?>
	<?php foreach($this->ratingDataTopbox as $reviewcatTopbox): ?>
		...
		<li class="sitepagereview_overall_rating">
			...
			<?php else:?>
				<div class="review_cat_rating">
					<ul title="<?php echo $rating_valueTitle.$this->translate(" rating"); ?>" class='rating <?php echo $rating_valueTopbox; ?>' style="background-image: url(<?php echo $this->layout()->staticBaseUrl; ?>application/modules/Sitepagereview/externals/images/show-star-matrix.png);">
						<li id="1" class="rate one">1</li>
						<li id="2" class="rate two">2</li>
						<li id="3" class="rate three">3</li>
						<li id="4" class="rate four">4</li>
...
<!--end top box code-->
	...
	<?php if (($this->paginator->getTotalItemCount() > 0) && !empty($this->getPackageReviewView)): ?>
		<ul class="sitepagereview_profile">
			<?php foreach ($this->paginator as $review): ?>
				<li>
					...
					<div class="sitepagereview_overallrating">
					...
					<div class="sitepagereview_profile_info">
						<div class="sitepagereview_profile_title">
							...
							<?php endif;?>
							<?php echo $this->htmlLink($review->getHref(), Engine_Api::_()->sitepagereview()->truncateText($review->title, 60), array('title' => $review->title)) ?>
						</div>
							
						<div class="sitepagereview_profile_info_date">
						...
						<div class='sitepagereview_profile_info_des'>
							<?php 
						...
						<div class="sitepagereview_profile_options">
							...
							<?php
								...
								echo $this->htmlLink(array(
										...
										'slug' => $slug,
										'tab' => $this->identity_temp,
								), $this->translate('View Review'), array('class' => 'buttonlink icon_sitepages_review'));
							?>
							...
							<?php if ($this->viewer_id == $review->owner_id): ?>
								<?php
									echo $this->htmlLink(array(
											...
											'page_id' => $this->sitepage->page_id,
											'tab' => $this->identity_temp,
									), $this->translate('Edit Review'), array('class' => 'buttonlink icon_sitepages_edit'));
								?>
							<?php endif; ?>
								...
								<?php
									echo $this->htmlLink(
										array(
														...
														'page_id' => $this->sitepage->page_id,
														'tab' => $this->identity_temp,
										), $this->translate('Delete Review'), array('class' => 'buttonlink icon_sitepages_delete')); 
								?>
							<?php endif; ?>
						</div>
	...
	<?php else: ?>
		<div class="tip">
			<span>
				<?php echo $this->translate('No reviews have been posted for this Page yet.'); ?>
				<?php if ($this->viewer_id)?>
					<?php if($this->can_create == 1 && empty($this->is_manageadmin)): ?>
						<?php	$show_link = $this->htmlLink(
															array('route' => 'sitepagereview_create', 'page_id' => $this->sitepage->page_id, 'tab' => $this->identity_temp,),
															$this->translate('Click here to write a review'));
							//$show_label = Zend_Registry::get('Zend_Translate')->_('Click %s to write a review.');
							//$show_label = sprintf($show_link);
							echo $show_link;
						?>
				<?php endif; ?>
			</span>
...
<script type="text/javascript">
  var review_ads_display = '<?php echo Engine_Api::_()->getApi('settings', 'core')->getSetting('sitepage.adreviewwidget', 3);?>';
  ...
  var page_url = '<?php echo Engine_Api::_()->sitepage()->getPageUrl($this->sitepage->page_id)?>';
	...
	if (ReviewTabIdCurrent == ReviewtabId) {
		if(page_showtitle != 0) {
			if($('profile_status')  && show_widgets == 1) {
			  $('profile_status').innerHTML = "<h2><?php echo $this->string()->escapeJavascript($this->sitepage->getTitle())?><?php echo $this->translate(' &raquo; ');?><?php echo $this->translate('Reviews');?></h2>";	
			}		
			if($('layout_review')) {
		   $('layout_review').style.display = 'block';
		  }
		}
    hideWidgetsForModule('sitepagereview');
    ...
    if ($('id_' + prev_tab_id) != null && prev_tab_id != 0 && prev_tab_id != '<?php echo $this->content_id; ?>') {
      $$('.'+ prev_tab_class).setStyle('display', 'none');
		...
		if(execute_Request_Review == false) {
			ShowContent('<?php echo $this->content_id; ?>', execute_Request_Review, '<?php echo $this->identity_temp?>', 'review', 'sitepagereview', 'profile-sitepagereviews', page_showtitle,'<?php echo Engine_Api::_()->sitepage()->getPageUrl($this->sitepage->page_id)?>', review_ads_display, page_communityad_integration, adwithoutpackage);
			execute_Request_Review = true;    		
		} 
		if('<?php echo Engine_Api::_()->getApi('settings', 'core')->getSetting('sitepage.communityads', 1);?>' && review_ads_display == 0)
