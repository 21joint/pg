...
$image_count = 1;
$photo_review = Engine_Api::_()->getApi('settings', 'core')->getSetting('sitepagereview.photo', 1);
foreach ($this->show_slideshow_object as $type => $review) {
  $sitepage_object = Engine_Api::_()->getItem('sitepage_page', $review->page_id);
  ...
  if(!empty($photo_review)) {
    $user = Engine_Api::_()->getItem('user', $review->owner_id);
		...
		$reviewPhoto = $this->htmlLink(Engine_Api::_()->sitepage()->getHref($sitepage_object->page_id, $sitepage_object->owner_id, $sitepage_object->getSlug()), $this->itemPhoto($sitepage_object, 'thumb.normal'), array('title' => $sitepage_object->getTitle()));
  }
  $content_info = null;
  ...
  if (!empty($review->title)) {
    ...
    $image_text_var .= $this->translate("on ") . $this->htmlLink(Engine_Api::_()->sitepage()->getHref($review->page_id, $review->owner_id, $review->getSlug()),  $page_title,array('title' => $sitepage_object->title,'class' => 'bold')); 
    $image_text_var .= "</div>";
    ...
    window.addEvent('domready',function(){
     ...
     for(var i=0;i < divElements.length;i++)
      ...
      var nS8 = new noobSlide({
        box: $('sitepagereview_featured_review_im_te_advanced_box'),
        items: $$('#sitepagereview_featured_review_im_te_advanced_box h3'),
        size: (width-10),
        handles: $$('#handles8 span'),
        addButtons: {previous: $('sitepagereview_featured_review_prev8'), stop: $('sitepagereview_featured_review_stop8'), play: $('sitepagereview_featured_review_play8'), next: $('sitepagereview_featured_review_next8') },
        interval: 5000,
        fxOptions: {
          duration: 500,
          transition: '',
        ...
        onWalk: function(currentItem,currentHandle){
          ...
          {
            $('sitepagereview_featured_review_caption').innerHTML =   current_caption_link;
          }
          else {
            $('sitepagereview_featured_review_caption').innerHTML =  '';
          }


          $('sitepagereview_featured_review_current_numbering').innerHTML =  current_index + '/' + num_of_slidehsow ;
        }
      });
...
<div class="featured_slideshow_wrapper">
  <div class="featured_slideshow_mask">
    <div id="sitepagereview_featured_review_im_te_advanced_box" class="featured_slideshow_advanced_box">
  ...
  <div class="featured_slideshow_option_bar">
    <div>
      <p class="buttons">
        <span id="sitepagereview_featured_review_prev8" class="featured_slideshow_controllers-prev featured_slideshow_controllers prev" title=<?php echo $this->translate("Previous") ?> ></span>
        <span id="sitepagereview_featured_review_stop8" class="featured_slideshow_controllers-stop featured_slideshow_controllers" title=<?php echo $this->translate("Stop") ?> ></span>
        <span id="sitepagereview_featured_review_play8" class="featured_slideshow_controllers-play featured_slideshow_controllers" title=<?php echo $this->translate("Play") ?> ></span>
        <span id="sitepagereview_featured_review_next8" class="featured_slideshow_controllers-next featured_slideshow_controllers" title=<?php echo $this->translate("Next") ?> ></span>
      </p>
    </div>
    <span id="sitepagereview_featured_review_caption"></span>
    <span id="sitepagereview_featured_review_current_numbering" class="featured_slideshow_pagination"></span>
  </div>
</div>  
