    ...
    <table class="sm-rating-table">
      <?php foreach($this->ratingData as $rating):?>
        <?php if (!empty($rating['reviewcat_name'])): ?>
          <?php 
            $showRatingImage = Engine_Api::_()->sitepagereview()->showRatingImage($rating['avg_rating'], 'box');
        ...
        <?php else: ?>
				<?php
					$showRatingImage = Engine_Api::_()->sitepagereview()->showRatingImage($rating['avg_rating'], 'star');
					$rating_value = $showRatingImage['rating_value'];
...
<?php if ($this->paginator->getTotalItemCount() > 0): ?>
  <div class="sm-content-list" id="profile_sitepagereviews">
     <ul data-role="listview" data-inset="false">
		...
		<?php if ($this->paginator->count() > 1): ?>
			...
			echo $this->paginationAjaxControl(
							$this->paginator, $this->identity, "profile_sitepagereviews");
			?>
		<?php endif; ?>
	</div>
<?php else: ?>
    ...
    <span>
      ...
      <?php if ($this->can_create == 1 && empty($this->is_manageadmin)): ?>
        ...
        $show_link = $this->htmlLink(
                array('route' => 'sitepagereview_create', 'page_id' => $this->page_id, 'tab' => $this->identity), $this->translate('here'));
