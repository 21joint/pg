...
<script type="text/javascript">
	function doRating(element_id, reviewcat_id, classstar) {
		$(element_id + '_' + reviewcat_id).getParent().getParent().className= 'rating-box ' + classstar;
		$('review_rate_' + reviewcat_id).value = $(element_id + '_' + reviewcat_id).getParent().id;
	}

	function doDefaultRating(element_id, reviewcat_id, classstar) {
   
		$(element_id + '_' + reviewcat_id).getParent().getParent().className= 'rating ' + classstar;
		$('review_rate_' + reviewcat_id).value = $(element_id + '_' + reviewcat_id).getParent().id;
	}
</script>
<?php 
...
<?php if(Engine_Api::_()->getDbtable('modules', 'core')->isModuleEnabled('communityad') && Engine_Api::_()->getApi('settings', 'core')->getSetting('sitepage.communityads', 1) && Engine_Api::_()->getApi('settings', 'core')->getSetting('sitepage.adreviewedit', 3) && $page_communityad_integration && Engine_Api::_()->sitepage()->showAdWithPackage($this->sitepage)):?>
	<div class="layout_right" id="communityad_reviewedit">
		<?php
			echo $this->content()->renderWidget("communityad.ads", array( "itemCount"=>Engine_Api::_()->getApi('settings', 'core')->getSetting('sitepage.adreviewedit', 3),"loaded_by_ajax"=>0,'widgetId'=>"page_reviewedit")); 			 
		?>
	</div>
<?php endif;?>
<div class="layout_middle">
	<form id="sitepage_review_create" enctype="application/x-www-form-urlencoded" class="global_form" action='' method="post">
		<div>
			<div>
				<h3><?php echo $this->translate("Edit Your Review");?></h3>
				<p><?php echo $this->translate('Edit your review below and click "Save Changes" to save it.') ?></p>
				...
				<div class="form-elements">
					...
					<?php if(!empty($this->total_reviewcats)): ?>
						<?php foreach($this->reviewCategory as $reviewcat): ?>
							...
							<div class="form-wrapper">
								...
								<div class="form-element">
									<ul id= 'rate_<?php echo $reviewcat->reviewcat_id; ?>' class='rating-box <?php echo $rating_value; ?>'  style="position:relative;">
					...
					<div id="body-wrapper" class="form-wrapper">
						...
						<div id="body-element" class="form-element">
							<textarea name="body" id="body" cols="180" rows="24" style="width:300px; max-width:300px;height:120px;"><?php echo $this->escape($this->prefield_body) ?></textarea>
						</div>
					</div>
					...
					<?php if($this->showRecommendField): ?>
						<div id="end_settings-wrapper">
							...
							<div class="form-element" id="end_settings-element">
								<p class="description"><?php echo $this->translate("Would you recommend ").$this->page_title_with_link ?> <?php echo $this->translate(" to a friend?");?></p>
...
.rating{background-image: url(<?php echo $this->layout()->staticBaseUrl; ?>application/modules/Sitepagereview/externals/images/star-matrix.png);}		
