...
class Music_PlaylistController extends Siteapi_Controller_Action_Standard {
    ...
    public function init() {
        ...
        // Get subject
        if (null !== ($playlist_id = $this->getRequestParam('playlist_id')) &&
    ...
    public function viewAction() {
        ...
        foreach ($getPlaylistSongs as $songObj) {
            ...
            $songArray['filePath'] = (empty($staticBaseUrl)) ? $songHost . $songObj->getFilePath() : $songObj->getFilePath();
...
//    }
        ...
        else if (0 >= ($playlist_id = $this->getRequestParam('playlist_id')) &&
                false != ($type = $this->getRequestParam('type'))) {
            $playlist = $playlistTable->getSpecialPlaylist($viewer, $type);
            Engine_Api::_()->core()->setSubject($playlist);
        }
