...
// // preload chat sound
soundManager.setup({url: en4.core.baseUrl + 'externals/soundmanager/swf/'});
soundManager.onready(function() {
  if( soundManager.supported() ) {
    soundManager.createSound({
      id: '_chat_ding',
      url: en4.core.staticBaseUrl + 'application/modules/Chat/externals/ding.mp3',
      autoLoad: true,
      volume: 50
    });
  }
...
ChatHandler = new Class({
  ...
  loop : function() {
    ...
    try {
      ...
      var completed = false;
      var cancelled = false;
      var timeoutFunctionDelay = false;
      request.addEvents({
        'complete' : function() {
          completed = true;
          if( timeoutFunctionDelay ) {
            $clear(timeoutFunctionDelay);
          }
          if( !cancelled ) {
            this.loop.delay(this.options.delay, this);
          }
      ...
      timeoutFunctionDelay = timeoutFunc.delay(this.options.requestTimeout, this);
      ...
      if( $type(responseJSON) == 'object' ) {
        ...
        if( $type(responseJSON.events) == 'object' ) {
          for( var x in responseJSON.events ) {
            ...
            if (type == 'chat' && responseJSON.events[x].sender_id != en4.user.viewer.id && 'object' == $type(soundManager)) {
            //if ( (type == 'chat' || type == 'groupchat') && responseJSON.events[x].sender_id != en4.user.viewer.id && 'object' == $type(soundManager)) {
              // ding!
              try {
                soundManager.getSoundById('_chat_ding').play();
              } catch (e) {}
...
ChatHandler_Room = new Class({
  ...
  render : function() {
    ...
    }).inject(this.container || $(this.options.container) || $('global_content') || document.body);
    ...
    */
    ...
    if( this.handler._supportsContentEditable() ) {
      ...
      // Input
      this.elements.input = new Element('textarea', {
    ...
    if( parseInt(data.state) >= 1 ) {
      if( !userEl ) {
        userEl = new Element('li', {
          'id' : userElId,
          'html' : '<span class="chat_user_photo"><img src="' + (data.photo || en4.core.staticBaseUrl + 'application/modules/User/externals/images/nophoto_user_thumb_icon.png') + '" /></span>' + '<span class="chat_user_name"><a href="' + data.href + '" target="_blank">' + data.title + '</a></span>'
  ...
  onGroupChat : function(data, ref) {
    ...
    if( $type(data.system) && parseInt(data.system)==1 ) {
      ...
      var tmpMsgEl = (new Element('span', {'html' : body, 'class' : 'chat_message_info_body chat_message_info_body_system'}));
      tmpMsgEl.inject(tmpDivEl);
    ...
    else
    {
      ...
      */
      ...
      (new Element('div', {
        ...
        'html' : '<a href="' + user.href + '" target="_blank"><img src="' + (user.photo || en4.core.staticBaseUrl + 'application/modules/User/externals/images/nophoto_user_thumb_icon.png') + '" alt="" /></a>'
      })).inject(msgWrpr);

      var tmpMsgEl = (new Element('div', {
        'class' : 'chat_message_info',
        'html' : '\n\
...
ChatHandler_Whispers = new Class({
...
ChatHandler_Whispers_UI_Abstract = new Class({
...
ChatHandler_Whispers_UI_Settings = new Class({
  ...
  render : function() {
    ...
    if (notify.isSupported) {
      ...
      this.elements.main.set('id', 'im_main_notification');

      // Tooltip
      this.elements.tooltip = new Element('span', {
        'class' : 'im_item_tooltip_settings',
        'html' : en4.core.language.translate('Toggle Notifications')
      }).inject(this.elements.main);
      ...
      this.elements.item = new Element('span', {
        'class' : 'im_item im_item_'  + name,
        'events' : {
          'click' : this.toggleNotification.bind(this)
        }
      }).inject(this.elements.main);
...
ChatHandler_Whispers_UI_Friends = new Class({
...
ChatHandler_Whispers_UI_Conversation = new Class({
  ...
  render : function() {
    ...
    if( this.handler.handler._supportsContentEditable() ) {
      ...
      this.elements.menuInput = new Element('textarea', {
        'class' : 'im_menu_input im_menu_'  + this.options.name + '_input'
      }).inject(this.elements.menuFooter);
  ...
  onChat : function(data, user, ref) {
    ...
    if( $type(data.system) && data.system ) {
      ...
      var tmpMsgEl = (new Element('span', {'class' : 'im_convo_messages_body', 'html' : body}).inject(messageEl));
      tmpMsgEl.enableLinks();
    }
    ...
    else
    {
      ...
      if (notify.isSupported ) {
        var notificationsEnable = document.id('im_main_notification').hasClass('im_main_settings_notifications_on');
...
ChatHandler_Utility = {
  ...
  imageSpec : en4.core.staticBaseUrl + 'application/modules/Chat/externals/images/smilies/:name:.png',
