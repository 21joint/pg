 ...
 * @version    $Id: _video_view.tpl 6590 2016-3-3 00:00:00Z SocialEngineAddOns $
...
if ($this->paginatorVideoView) {
    $this->paginator = $this->paginatorVideoView;
}
?>
...
<?php if ($this->paginator->getTotalItemCount() > 0): ?>
    ...
    <ul class='videos_manage siteevideo_videos_view' id='videos_manage<?php echo "_" . $identity; ?>'>
            ...
            <li>
                <div class="site_video_calendar">
                    <div class="cal_date">
                        ...
                        <span class="mon"><?php echo $this->locale()->toDate($item->creation_date, array('format' => 'MMM')); ?></span>
                        <span class="day"><?php echo $this->locale()->toDate($item->creation_date, array('format' => 'dd')); ?></span>
                    </div>
                </div>
                <div class="sitevideo_thumb_wrapper sitevideo_thumb_viewer"  style=" width:<?php echo $this->videoViewWidth; ?>px; height:<?php echo $this->videoViewHeight; ?>px;">
                    <?php $fsDuration = ''; ?>
                    ...
                    <?php
                    if ($item->photo_id) {
                        echo $this->htmlLink($item->getHref(), "<span class='video_overlay'></span> <span class='play_icon'></span>" . $fsContent . $fsDuration . "<i style='background-image:url(" . $item->getPhotoUrl($this->videoViewThumbnailType) . ")'></i>", array('class' => 'sitevideo_thumb'));
                    } else {
                        ...
                        <div class="sitevideo_bottom_info">
                            ...
                            <div class="clr">
                                ...
                                <?php if (in_array('owner', $this->videoOption)) : ?>
                                    ...
                                    ?>
                                    <?php $ownerNchannelTitle = $this->translate('by %s', $this->htmlLink($item->getOwner()->getHref(), $this->string()->truncate($this->string()->stripTags($item->getOwner()->getTitle()), 17))); ?>
                                ...
                                <?php else : ?>
                                    <?php if ($itemTypeValue == 'user') : ?>
                                        ...
                                        if ($channel) :
                                            ?> 
                                            <?php $ownerNchannelTitle = $this->htmlLink($channel->getHref(), $this->string()->truncate($this->string()->stripTags($channel->getTitle()), 17)); ?>
                                            <?php
                                            if ($channel->file_id) {
                                ...
                                <?php if($contentitem):?>
                                        <?php $ownerNchannelTitle = $this->htmlLink($contentitem->getHref(), $this->string()->truncate($this->string()->stripTags($contentitem->getTitle()), 17)); ?>
                                        <?php
                                        echo $this->htmlLink($contentitem->getHref(), $this->itemPhoto($contentitem, 'thumb.icon'), array('class' => 'sitevideo_author_logo'));
                                        ?>
                                <?php endif; ?>
                                    <?php endif; ?>
                                <?php endif; ?>
                                ...
                                <div class="sitevideo_stats fleft">
                                    <span class="video_views dblock">
                                        <span class="site_video_author_name">
                                            <?php if ($channel) : ?>
                                                <?php echo $ownerNchannelTitle; ?>
                                            <?php else: ?>
                                                <?php if ($itemTypeValue == 'user') : ?>
                                                    <?php echo $this->htmlLink($item->getOwner()->getHref(), $this->string()->truncate($this->string()->stripTags($item->getOwner()->getTitle()), 17)); ?><?php else: ?> <?php if($contentitem):?><?php echo $this->htmlLink($contentitem->getHref(), $this->string()->truncate($this->string()->stripTags($contentitem->getTitle()), 17)); ?> <?php endif; ?> <?php endif; ?>
                                    ...
                                    <div class="fleft">
                                        <?php if (in_array('view', $this->videoOption)) : ?>
                                            ...
                                            <span class="sitevideo_bottom_info_views" title ="<?php echo $countText; ?>">
                                                <?php echo $count ?></span>
                                            <?php endif; ?>
                                    ...
                                    <?php if (in_array('rating', $this->videoOption)) : ?>
                                        <div class="sitevideo_ratings fright">
                                            <?php echo $this->ratingInfo($item, array('widget_id' => $identity . '_a')); ?>
                                        </div>
                                    <?php endif; ?>
...
<?php if (empty($this->is_ajax) && $this->isViewMoreButton) : ?>
    <script type="text/javascript">
        function viewMoreVideos(viewFormat)
        {
