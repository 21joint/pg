...
class Sitevideo_IndexController extends Siteapi_Controller_Action_Standard {
    ...
    public function indexAction() {
        ...
        if ($moduleName == 'sitepage' || $moduleName == 'sitebusiness' || $moduleName == 'sitegroup' || $moduleName == 'sitestore') {
            ...
            //END PACKAGE WORK
            $canCreate = Engine_Api::_()->$moduleName()->isManageAdmin($subject, 'svcreate');
            ...
            $videoCount = Engine_Api::_()->$moduleName()->getTotalCount($subject->getIdentity(), 'sitevideo', 'videos');
            //AUTHORIZATION CHECK
            $canCreate = Engine_Api::_()->siteevent()->allowVideo($subject, $viewer, $videoCount);
            if (empty($canCreate) && empty($videoCount) && empty($canEdit)) {
                $this->respondWithError('no_record');
            }
            ...
            $canCreate = Engine_Api::_()->sitereview()->allowVideo($subject, $viewer, $this->view->videoCount);
        ...
        try {
            foreach ($paginator as $video) {
                ...
                $browseVideo["rating_count"] = Engine_Api::_()->getDbTable('ratings', 'sitevideo')->ratingCount($video->getIdentity());
    ...
    private function _videoPaginator($params = array(), $isManage = false, $tempParam = array()) {
        ...
        foreach ($paginator as $video) {
            ...
            if (isset($params['showVideo']) && !empty($params['showVideo'])) {
                ...
                $video = Engine_Api::_()->getItem('video', $video->getIdentity());
                $browseVideo = $video->toArray();
            } else
                $browseVideo = $video->toArray();
...
//GET API
    ...
    public function createAction() {
        ...
        if ($this->getRequest()->isGet()) {
            ...
            if (!empty($_GET['post_attach']) || !empty($_GET['message'])) {
                $this->respondWithSuccess(Engine_Api::_()->getApi('Siteapi_Core', 'sitevideo')->advancedActivityVideoForm());
            } else {
                $this->respondWithSuccess(Engine_Api::_()->getApi('Siteapi_Core', 'sitevideo')->getVideoFrom());
            }
        } else if ($this->getRequest()->isPost()) {
    ...
    private function _uploadVideoAction($values) {
        ...
        try {
            ...
            $video->title = (!empty($values['title'])) ? $values['title'] : $_FILES['filedata']['name'];
