...
// only show videos if authorized
        ...
        $id = $this->getRequestParam('channel_id', $this->getRequestParam('id', null));
...
// Check if edit/delete is allowed
    ...
    private function _profileTAbsContainer($subject) {
        ...
        if (!empty($subject_id)) {
            $subsTotal = Engine_Api::_()->getApi('Siteapi_Core', 'sitevideo')->countSubscriber($subject->getIdentity());
...
//GET VIEWER
        ...
        try {
            ...
            foreach ($subscribedUsersId as $value) {
                ...
                if (!empty($user_subject) && empty($user_subject->toArray()))
                    continue;
            ...
            $response['totalItemCount'] = count($response['members']);
            $this->respondWithSuccess($response);
        } catch (Exception $ex) {
            $this->respondWithValidationError('internal_server_error', $ex->getMessage());
        }
