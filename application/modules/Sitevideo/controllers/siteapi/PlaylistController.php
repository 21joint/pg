...
// only show playlist if authorized
        ...
        $id = $this->getRequestParam('playlist_id', $this->getRequestParam('id', null));

        if ($id) {
            $palaylist = Engine_Api::_()->getItem('sitevideo_playlist', $id);

            if ($palaylist)
                Engine_Api::_()->core()->setSubject($palaylist);
        }
    }
