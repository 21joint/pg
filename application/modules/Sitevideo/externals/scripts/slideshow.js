...
(function () {
    ...
    Slideshow = new Class({
        ...
        options: {/*
         ...
         onEnd: $empty,
         onStart: $empty,*/
            accesskeys: {'first': {'key': 'shift left', 'label': 'First [Shift + Leftwards Arrow]'}, 'prev': {'key': 'left', 'label': 'Prev [Leftwards Arrow]'}, 'pause': {'key': 'p', 'label': 'Play / Pause [P]'}, 'next': {'key': 'right', 'label': 'Next [Rightwards Arrow]'}, 'last': {'key': 'shift right', 'label': 'Last [Shift + Rightwards Arrow]'}},
            captions: true,
            center: true,
            ...
            hu: '',
            linked: false,
            loader: false, //{'animate': [url_base+'application/modules/Siteslideshow/externals/images/spinner_temp.gif']},
            loop: true,
            match: /\?slide=(\d+)$/,
            overlap: true,
            paused: false,
        ...
        /**
         ...
         Syntax:
         var myShow = new Siteslideshow(element, data, options);
         */
...
//					title = caption.replace(/<.+?>/gm, '').replace(/</g, '&lt;').replace(/>/g, '&gt;').replace(/"/g, "'");
                        thumbnail = obj.thumb ? obj.thumb : (obj.thumbnail ? this.options.hu + obj.thumbnail.trim()
                                : image.replace(this.options.replace[0], this.options.replace[1])),
                        title = caption.replace(/<.+?>/gm, '').replace(/</g, '&lt;').replace(/>/g, '&gt;').replace(/"/g, "'");
    ...
    var Caption = new Class({
        ...
        update: function (fast) {
            ...
            if (fast) {
                ...
                : function (caption) {
                    if (caption == '' || caption == '<span></span>') {
                        var hide_caption_strip = 'hidden';
                    }
                    else {
                        var hide_caption_strip = 'visible';
                    }
                    this.caption.store('timer', setTimeout(function (caption) {
                        this.caption.set('html', caption).morph(this.classes.get('captions', hide_caption_strip));
