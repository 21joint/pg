  ...
  public function init()
  {
    ...
    if( isset($allowedSources[4]) && !empty($ffmpeg_path) && $allowed_upload && isset($allowedSources_level[4]) ) {
      if( Engine_Api::_()->hasModuleBootstrap('mobi') && Engine_Api::_()->mobi()->isMobile() ) {
        ...
        $video_options['upload'] = "<span class='sitevideo_icon_mysystem' title='My Computer'>My Computer</span>";
      }
    }
    if( isset($allowedSources[5]) && isset($allowedSources_level[5]) ) {
      $video_options['embedcode'] = "<i class='sitevideo_icon_embed' title='Embed Code'></i><br /><p class='description'>You need to upload a video thumbnail image for the videos which have different video sources other than Vimeo, Dailymotion and Youtube.</p>";
    }
