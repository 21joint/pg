 ...
 * @version    $Id: send.tpl 2017-03-08 00:00:00Z SocialEngineAddOns $
...
<div>
  <p> Here, you can view the information of users to whom you have send the credits. You can also send credits to all users, to users as per different member level or to a specific user via ‘Send Credits’ pop-up. </p>
</div>
<br/>
<div class="mbot10">
  <?php 
  echo $this->htmlLink(array('route' => 'admin_default', 'module' => 'sitecredit', 'controller' => 'user', 'action' => 'credit-send'), $this->translate('Send Credits'), array('class' => 'smoothbox link_button'));
  ?>
</div>
<div class="search transaction_search">
  ...
  <div>
    <label><?php echo  $this->translate("Credit Values") ?></label>
    <div class="form-element">
        <input type="text" name="order_min_amount" onkeypress="return isNumberKey(event)" placeholder="Min" value="<?php echo empty($this->order_min_amount)?'':$this->order_min_amount; ?>" style="width: 100px;" />
        <input type="text" name="order_max_amount" onkeypress="return isNumberKey(event)" placeholder="Max" value="<?php echo empty($this->order_max_amount)?'':$this->order_max_amount; ?>" style="width: 100px;" />
    </div>
  </div>
...
<?php if( count($this->paginator) ): ?>
  <form id='multidelete_form' method="post" action="<?php echo $this->url();?>" onSubmit="return multiDelete()">
    <table class='admin_table'>
      <thead>
        <tr>
          <?php $class = ( $this->order == 'bonus_id' ? 'admin_table_ordering admin_table_direction_' . strtolower($this->order_direction) : '' ) ?>
          <th class="<?php echo $class ?>" align="left"><a href="javascript:void(0);" onclick="javascript:changeOrder('bonus_id', 'ASC');"><?php echo $this->translate("Id") ?></a></th>
          <th>Send By</th>
          <th>Send to</th>
          <?php $class = ( $this->order == 'credit_point' ? 'admin_table_ordering admin_table_direction_' . strtolower($this->order_direction) : '' ) ?>
          <th class="<?php echo $class ?>" align="left"><a href="javascript:void(0);" onclick="javascript:changeOrder('credit_point', 'ASC');"><?php echo $this->translate("Credit Values") ?></a></th>
      ...
      <tbody>
       ...
       if($level->getTitle()): ?>
       <td><?php echo $level->getTitle() ?></td>
     <?php endif; ?>
   <?php else : ?>
    <td> All member levels </td>
  <?php endif; ?>
  <td><?php echo date('dS F Y ', strtotime($item->creation_date)) ?></td>
  <td><?php echo $this->htmlLink(
    array('route' => 'admin_default', 'module' => 'sitecredit', 'controller' => 'user', 'action' => 'reason-view', 'id' => $item->bonus_id),
    $this->translate("View"),
    array('class' => 'smoothbox')) ?></td>
  </tr>
<?php endforeach; ?>
</tbody>
</table>
