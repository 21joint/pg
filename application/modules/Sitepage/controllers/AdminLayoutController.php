 ...
 */
class Sitepage_AdminLayoutController extends Core_Controller_Action_Admin {
  ...
  public function layoutAction() {
    ...
    $userlayoutcontentpagesTable = Engine_Api::_()->getDbtable('contentpages', 'sitepage');
  ...
  public function updateAction() {
    ...
    $pageTable = Engine_Api::_()->getDbtable('pages', 'core');
    $contentTable = Engine_Api::_()->getDbtable('admincontent', 'sitepage');
    $userlayoutcontentTable = Engine_Api::_()->getDbtable('content', 'sitepage');
    ...
    $select = $userlayoutcontentTable->select()->from($userlayoutcontentTablename, array('contentpage_id'))->limit($limit, $offset)->group('contentpage_id');
    ...
    try {
      ...
      foreach ($structure as $element) {
        ...
        if (!empty($contentRow) && is_object($contentRow)) {
          ...
          foreach ($content_pages_id as $value) {
            ...
            $content_page_id = $value->contentpage_id;

            if ($contentRow->type == 'container' && $contentRow->name == 'middle') {
              $select_parent = $userlayoutcontentTable->select()->where('type =?', 'container')->where('name =?', $parentContentRow->name)->where('contentpage_id =?', $content_page_id);
              $content_parent = $userlayoutcontentTable->fetchRow($select_parent);
              $select = $userlayoutcontentTable->select()->where('type =?', $contentRow->type)->where('name =?', $contentRow->name)->where('contentpage_id =?', $content_page_id)->where('parent_content_id = ?', $content_parent->content_id);
            } else {
              $select = $userlayoutcontentTable->select()->where('type =?', $contentRow->type)->where('name =?', $contentRow->name)->where('contentpage_id =?', $content_page_id);
            }
            ...
            if ($usercontentRow->type == 'container') {
              ...
              $userlayoutcontentTable->update(array('order' => $newOrder), array('type = ?' => $contentRow->type, 'contentpage_id = ?' => $content_page_id, 'name = ?' => $contentRow->name, 'widget_admin = ?' => 1));
            }

            if (isset($session->edittitles) && in_array($element['name'], $session->edittitles)) {
							$userlayoutcontentTable->update(array('params' => $element['params']), array('type = ?' => $contentRow->type, 'contentpage_id = ?' => $content_page_id, 'name = ?' => $contentRow->name, 'widget_admin = ?' => 1, 'parent_content_id = ?' => $usercontentRow->parent_content_id, 'params =?' => $oldContentRowParams));
            }

            $parent_id = 0;
            if (!empty($parentContentRow)) {
              $select = $userlayoutcontentTable->select()->where('type =?', $parentContentRow->type)->where('name =?', $parentContentRow->name)->where('contentpage_id =?', $content_page_id);
              ...
              if (!empty($parentContentRow->parent_content_id)) {
                ...
                if (!empty($parentparentContentRow)) {
                  $select = $userlayoutcontentTable->select()->where('type =?', $parentparentContentRow->type)->where('name =?', $parentparentContentRow->name)->where('contentpage_id =?', $content_page_id);
                  $contentParent_pages = $userlayoutcontentTable->fetchRow($select);
                  if ($contentParent_pages->name != $parentparentContentRow->name) {
                    $select = $userlayoutcontentTable->select()->where('type =?', $parentContentRow->type)->where('name =?', $parentContentRow->name)->where('contentpage_id =?', $content_page_id)->where('parent_content_id <> ?', $content_pages->parent_content_id);
              ...
              $parent_id = $content_pages->content_id;
            } else {
              $userlayoutcontentTable->update(array('parent_content_id' => new Zend_Db_Expr('NULL'), 'order' => $newOrder), array('type = ?' => $contentRow->type, 'contentpage_id = ?' => $content_page_id, 'name = ?' => $contentRow->name, 'widget_admin = ?' => 1));
            }
          ...
          foreach ($content_pages_id as $value) {
            ...
            if ($element['type'] == 'container') {
              ...
              if ($element['name'] != 'core.html-block' && $element['name'] != 'core.ad-campaign' && $element['name'] != 'sitepageintegration.profile-items' && $element['name'] != 'sitepagemember.profile-sitepagemembers') {
                $select = $userlayoutcontentTable->select()->where('name =?', $element['name'])->where('contentpage_id =?', $content_page_id);
                $userlayoutpages = $userlayoutcontentTable->fetchRow($select);
              } else {
                $select = $userlayoutcontentTable->select()->where('name =?', $element['name'])->where('contentpage_id =?', $content_page_id)->where('params =?', json_encode($element['params']));
                $userlayoutpages = $userlayoutcontentTable->fetchRow($select);
              }
            }
            ...
            if (empty($userlayoutpages) && isset($element['type']) && isset($element['name'])) {
              ...
              $usercontentRow->contentpage_id = $content_page_id;
              $usercontentRow->order = $newOrder;
              $usercontentRow->type = $element['type'];
              $usercontentRow->name = $element['name'];
              if (!empty($parentContentRow)) {
                $select = $userlayoutcontentTable->select()->where('type =?', $parentContentRow->type)->where('name =?', $parentContentRow->name)->where('contentpage_id =?', $content_page_id)->order('content_id DESC')->order('content_id DESC');
                $content_pages = $userlayoutcontentTable->fetchRow($select);
                if (!empty($content_pages)) {
                  $usercontentRow->parent_content_id = $content_pages->content_id;
                }
                if (empty($usercontentRow->parent_content_id)) {
          ...
          foreach ($content_pages_id as $value) {
            ...
            if (!empty($parentContentRow)) {
              ...
              if (!empty($parentparentContentRow)) {
                $select = $userlayoutcontentTable->select()->where('type =?', $parentparentContentRow->type)->where('name =?', $parentparentContentRow->name)->where('contentpage_id =?', $content_page_id);
                $contentParent_pages = $userlayoutcontentTable->fetchRow($select);
                if ($contentParent_pages && $contentParent_pages->name != $parentparentContentRow->name) {
                  $select = $userlayoutcontentTable->select()->where('type =?', $parentContentRow->type)->where('name =?', $parentContentRow->name)->where('contentpage_id =?', $content_page_id)->where('parent_content_id <> ?', $contentParent_pages->content_id);
                  $content_pages = $userlayoutcontentTable->fetchRow($select);
                }
              }
              $parent_id = $content_pages->content_id;
            ...
            if (!empty($parent_id)) {
              if ( $contentRow->name != 'core.ad-campaign' && $contentRow->name != 'sitepageintegration.profile-items') {
                $select = $userlayoutcontentTable->select()->where('type =?', $contentRow->type)->where('name =?', $contentRow->name)->where('contentpage_id =?', $content_page_id)->where('parent_content_id =?', $parent_id);
              } else {
                $select = $userlayoutcontentTable->select()->where('type =?', $contentRow->type)->where('name =?', $contentRow->name)->where('contentpage_id =?', $content_page_id)->where('parent_content_id =?', $parent_id)->where('params =?', $oldContentRow);
              }
              $content_pages = $userlayoutcontentTable->fetchRow($select);
              if (!empty($content_pages)) {
                $userlayoutcontentTable->delete(array(
                    'name =?' => $contentRow->name, 'contentpage_id =?' => $content_page_id, 'parent_content_id =?' => $parent_id, 'content_id =?' => $content_pages->content_id, 'widget_admin = ?' => 1
                ));
              }
            } else {
              if (($contentRow->name == 'top' || $contentRow->name == 'bottom') && $contentRow->type == 'container') {
                ...
                $userlayoutcontentTable->delete(array('type =?' => 'container',
                    'name =?' => 'middle', 'contentpage_id =?' => $content_page_id, 'parent_content_id = ?' => $content_pages->content_id, 'widget_admin = ?' => 1
                ));
                $userlayoutcontentTable->delete(array(
                    'name =?' => $contentRow->name, 'contentpage_id =?' => $content_page_id, 'content_id =?' => $content_pages->content_id, 'widget_admin = ?' => 1
                ));
              }
            }
          }
      ...
      if (!empty($top)) {
        $select = $contentTable->select()->where('page_id =?', $page)->where('name =?', 'middle')->where('parent_content_id =?', $top->admincontent_id);


        $middle = $contentTable->fetchRow($select);

				if (!empty($middle)) {
					$select = $contentTable->select()->where('page_id =?', $page)->where('parent_content_id =?', $middle->admincontent_id);
					...
					foreach ($content_pages_id as $value) {
						$content_page_id = $value->contentpage_id;
						$select = $userlayoutcontentTable->select()->where('name =?', 'top')->where('contentpage_id =?', $content_page_id);
						$user_top = $userlayoutcontentTable->fetchRow($select);
						if (!empty($user_top)) {
							$select = $userlayoutcontentTable->select()->where('name =?', 'middle')->where('contentpage_id =?', $content_page_id)->where('parent_content_id =?', $user_top->content_id);
							$user_middile = $userlayoutcontentTable->fetchRow($select);
							if ($user_middile) {
								foreach ($name as $name_value) {
									$userlayoutcontentTable->update(array('parent_content_id' => $user_middile->content_id), array('contentpage_id = ?' => $content_page_id, 'name = ?' => $name_value, 'widget_admin = ?' => 1));
								}
							}
						}
					}
        ...
        if (!empty($bottom)) {
          $select = $contentTable->select()->where('page_id =?', $page)->where('name =?', 'middle')->where('parent_content_id =?', $bottom->admincontent_id);
          $middle = $contentTable->fetchRow($select);
          $select = $contentTable->select()->where('page_id =?', $page)->where('parent_content_id =?', $middle->admincontent_id);
          $middle_block = $contentTable->fetchAll($select);
          $name = array();
          ...
          foreach ($content_pages_id as $value) {
            $content_page_id = $value->contentpage_id;
            $select = $userlayoutcontentTable->select()->where('name =?', 'bottom')->where('contentpage_id =?', $content_page_id);
            $user_bottom = $userlayoutcontentTable->fetchRow($select);
            if (!empty($user_bottom)) {
              $select = $userlayoutcontentTable->select()->where('name =?', 'middle')->where('contentpage_id =?', $content_page_id)->where('parent_content_id =?', $user_bottom->content_id);
              $user_middile = $userlayoutcontentTable->fetchRow($select);
              if ($user_middile) {
                foreach ($name as $name_value) {
                  $userlayoutcontentTable->update(array('parent_content_id' => $user_middile->content_id), array('contentpage_id = ?' => $content_page_id, 'name = ?' => $name_value, 'widget_admin = ?' => 1));
      ...
      if (empty($edit_layout_setting)) {
        ...
        if (empty($user_widgetlink)) {
          foreach ($content_pages_id as $value) {
            $content_page_id = $value->contentpage_id;
            $userlayoutcontentTable->delete(array(
                'name =?' => 'sitepage.widgetlinks-sitepage', 'contentpage_id =?' => $content_page_id));
          }
        }
      }
    ...
    if ($page_reload <= $reload_count) {
      ...
      $this->_forward('success', 'utility', 'core', array(
          ...
          'redirect' => $this->view->url(array('module' => 'sitepage', 'controller' => 'layout', 'action' => 'update', 'page' => $page, 'page_reload' => $page_reload, 'admin_sitepage_layout' => $newLayout), 'admin_default', true),
      ...
      $this->_forward('success', 'utility', 'core', array(
          'smoothboxClose' => false,
          'redirect' => $this->view->url(array('module' => 'sitepage', 'controller' => 'layout', 'action' => 'layout', 'page' => $page), 'admin_default', true),
          'parentRedirectTime' => '10',
          //'format' => 'smoothbox',
  ...
  public function saveAction() {
    ...
    $pagecoreTable = Engine_Api::_()->getDbtable('pages', 'core');
    ...
    if ($option == 1) {
      ...
      if (empty($admincontentrow)) {
        foreach ($contentpagesrow as $value) {
          Engine_Api::_()->getDbTable('content', 'sitepage')->delete(array('name =?' => $widgetname));
        }
      }
      if (empty($row)) {
