...
class Sitepage_Model_DbTable_Admincontent extends Engine_Db_Table {
    ...
    public function getContentId($contentpage_id, $sitepage) {
        ...
        if (!empty($content_id)) {
            ...
            $select_photo = $select
                    ...
                    ->where('parent_content_id = ?', $content_id)
                    ->where('type = ?', 'widget')
                    ->where('name = ?', 'sitepage.photos-sitepage')->where('page_id = ?', $contentpage_id)
                ...
                switch ($content_name) {
                    ...
                    case 'sitevideo.contenttype-videos':
                        if (Engine_Api::_()->getDbtable('modules', 'core')->isModuleEnabled('sitevideo')) {
                            ...
                            //TOTAL VIDEOS
                            $videoCount = Engine_Api::_()->sitepage()->getTotalCount($sitepage->page_id, 'sitevideo', 'videos');
                            $isManageAdmin = Engine_Api::_()->sitepage()->isManageAdmin($sitepage, 'svcreate');
                            if (!empty($isManageAdmin) || !empty($videoCount)) {
                                $flag = true;
                            } else {
                        ...
                        if (Engine_Api::_()->getDbtable('modules', 'core')->isModuleEnabled('document')) {
                            ...
                            //TOTAL DOCUMENTS
                            $documentCount = Engine_Api::_()->sitepage()->getTotalCount($sitepage->page_id, 'document', 'documents');
                            $isManageAdmin = Engine_Api::_()->sitepage()->isManageAdmin($sitepage, 'sdcreate');
                            if (!empty($isManageAdmin) || !empty($documentCount)) {
                                $flag = true;
                            } else {
                    ...
                    case 'sitepagetwitter.feeds-sitepagetwitter':
                        ...
                        break;
                    case 'sitecrowdfunding.contenttype-projects':
                        if (Engine_Api::_()->getDbtable('modules', 'core')->isModuleEnabled('sitecrowdfunding')) {
                            if (Engine_Api::_()->sitepage()->hasPackageEnable()) {
                                if (Engine_Api::_()->sitepage()->allowPackageContent($sitepage->package_id, "modules", "sitecrowdfunding") == 1) {
                                    $flag = true;
                                }
                            } else {
                                $isPageOwnerAllow = Engine_Api::_()->sitepage()->isPageOwnerAllow($sitepage, 'sprcreate');
                                if (!empty($isPageOwnerAllow)) {
                                    $flag = true;
                                }
                            }
                            //TOTAL PROJECTS
                            $projectCount = Engine_Api::_()->sitepage()->getTotalCount($sitepage->page_id, 'sitecrowdfunding', 'projects');
                            $isManageAdmin = Engine_Api::_()->sitepage()->isManageAdmin($sitepage, 'sprcreate');
                            if (!empty($isManageAdmin) || !empty($projectCount)) {
                                $flag = true;
                            } else {
                                $flag = false;
                            }
    ...
    public function getContentInformation($page_id) {
        $select = $this->select()->from($this->info('name'), array('admincontent_id', 'name'))
                        ...
                        ->where('page_id = ?', $page_id)->order('admincontent_id ASC');
                ...
                if (!empty($container)) {
                    ...
                    if (!empty($middle)) {
                        ...
                        if ($name != 'sitepageintegration.profile-items') {
                            ...
                            $this->settabpageintwidget('document', '{"title":"Documents","resource_type":"document_0","nomobile":"0","name":"sitepageintegration.profile-items"}', $middle_id, $page_id);

                            $this->settabpageintwidget('sitefaq', '{"title":"FAQs","resource_type":"sitefaq_faq_0","nomobile":"0","name":"sitepageintegration.profile-items"}', $middle_id, $page_id);

                            $this->settabpageintwidget('sitetutorial', '{"title":"Tutorials","resource_type":"sitetutorial_tutorial_0","nomobile":"0","name":"sitepageintegration.profile-items"}', $middle_id, $page_id);

                            $this->settabpageintwidget('sitegroup', '{"title":"Groups","resource_type":"sitegroup_group_0","nomobile":"0","name":"sitepageintegration.profile-items"}', $middle_id, $page_id);

                            $this->settabpageintwidget('sitebusiness', '{"title":"Businesses","resource_type":"sitebusiness_business_0","nomobile":"0","name":"sitepageintegration.profile-items"}', $middle_id, $page_id);

                            $this->settabpageintwidget('sitestoreproduct', '{"title":"Products","resource_type":"sitestoreproduct_product_0","nomobile":"0","name":"sitepageintegration.profile-items"}', $middle_id, $page_id);

                            $this->settabpageintwidget('list', '{"title":"Listings","resource_type":"list_listing_0","nomobile":"0","name":"sitepageintegration.profile-items"}', $middle_id, $page_id);

                            $this->settabpageintwidget('folder', '{"title":"Folders","resource_type":"folder_0","nomobile":"0","name":"sitepageintegration.profile-items"}', $middle_id, $page_id);

                            $this->settabpageintwidget('quiz', '{"title":"Quiz","resource_type":"quiz_0","nomobile":"0","name":"sitepageintegration.profile-items"}', $middle_id, $page_id);
                        }
                    }
                }
            }
    ...
    public function getContents($contentpage_id) {
        ...
        $selectPageAdmin = $this->select()
                ...
                ->where('page_id =?', $contentpage_id)
                ->where("name IN ('sitepage.overview-sitepage', 'sitepage.photos-sitepage', 'sitepage.discussion-sitepage', 'sitepagenote.profile-sitepagenotes', 'sitepagepoll.profile-sitepagepolls', 'sitepageevent.profile-sitepageevents', 'sitepagevideo.profile-sitepagevideos', 'sitepageoffer.profile-sitepageoffers', 'sitepagereview.profile-sitepagereviews', 'sitepagedocument.profile-sitepagedocuments', 'sitepageform.sitepage-viewform','sitepage.info-sitepage', 'seaocore.feed','advancedactivity.home-feeds', 'activity.feed', 'sitepage.location-sitepage', 'core.profile-links', 'sitepagemusic.profile-sitepagemusic', 'sitepagemember.profile-sitepagemembers', 'sitepageintegration.profile-items','sitepagetwitter.feeds-sitepagetwitter', 'siteevent.contenttype-events', 'sitevideo.contenttype-videos', 'document.contenttype-documents','sitecrowdfunding.contenttype-projects')");
