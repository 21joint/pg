...
//     );
        ...
        $this->addElement('Hidden', 'fancyuploadfileids',array( 'order' => 336,));
    ...
    public function saveValues() {
        ...
        if (count($values['file'] > 0)) {
            ...
            if ($activityFeedType) {
                $action = $api->addActivity(Engine_Api::_()->user()->getViewer(), $sitepage, $activityFeedType, null, array('child_id' => $album->getIdentity(), 'count' => count($values['file'])));
            ...
            if (!empty($action)) {
                if ($sitepageVersion >= '4.2.9p3') {
                    Engine_Api::_()->sitepage()->sendNotificationEmail($album, $action, 'sitepagealbum_create', 'SITEPAGEALBUM_CREATENOTIFICATION_EMAIL', 'Pageevent Invite', count($values['file']));
                    ...
                    if (!empty($isPageAdmins)) {
                        ...
                        Engine_Api::_()->sitepage()->sendNotificationToFollowers($album, $action, 'sitepagealbum_create', count($values['file']));
                    }
                }
            }
