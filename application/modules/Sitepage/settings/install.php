...
class Sitepage_Installer extends Engine_Package_Installer_Module {
    ...
    function onPreInstall() {
        ...
        $SocialEngineAddOns_version = '4.8.13';
...
//     if (!empty($is_enabled)) {
  ...
  KEY `name` (`name`)
        ...
        $categoriesTable = $db->query('SHOW TABLES LIKE \'engine4_sitepage_categories\'')->fetch();
        if (!empty($categoriesTable)) {
            $file_id = $db->query("SHOW COLUMNS FROM engine4_sitepage_categories LIKE 'file_id'")->fetch();
            if (empty($file_id)) {
                $db->query("ALTER TABLE `engine4_sitepage_categories` ADD `file_id` int(11) NOT NULL DEFAULT '0';");
            }
        }
