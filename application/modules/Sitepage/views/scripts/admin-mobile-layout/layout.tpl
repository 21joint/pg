...
<?php $showHideHeaderFooter = Engine_Api::_()->getApi('settings', 'core')->getSetting('sitepage.show.hide.header.footer', 'default');?>
...
<script type="text/javascript">
  ...
  var saveChanges = function()
  {
    ...
    var url = '<?php echo $this->url(array('action' => 'update', 'controller' => 'mobile-layout', 'module' => 'sitepage'), 'admin_default', true)?>';
    var page_reload = '<?php echo Zend_Controller_Front::getInstance()->getRequest()->getParam('page_reload', 1);?>';
    var reload_count = '<?php echo round($this->totalPages / $this->limit);?>';
    Smoothbox.open('<center><img src="<?php echo $this->layout()->staticBaseUrl ?>application/modules/Sitepage/externals/images/loading.gif" /></center>');
    var request = new Request.HTML({
      ...
      'data' : {
        ...
        'admin_sitepage_layout' : currentLayout
      },
  ...
  var openWidgetParamEdit = function(name, element) {
    ...
    var url = '<?php echo $this->url(array('action' => 'widget', 'controller' => 'mobile-layout', 'module' => 'sitepage'), 'admin_default', true)?>';
  ...
  var saveCurrentPageInfo = function(formElement) {
    var url = '<?php echo $this->url(array('action' => 'save', 'controller' => 'mobile-layout', 'module' => 'sitepage'), 'admin_default', true)?>';
...
<h2><?php echo $this->translate('Page Profile Layout Editor for Mobile / Tablet'); ?></h2>
<p>
  <?php echo $this->translate('Page Profile Layout DESCRIPTION'); ?>
</p>
...
<div id='admin_cms_wrapper'>
    ...
    <ul>
      <li class="admin_layoutbox_menu_generic" id="admin_layoutbox_menu_openpage">
        ...
        <a href="javascript:void(0);">
           <?php echo $this->pageObject->displayname ?>
        </a>
      </li>
  ...
  <div class="admin_layoutbox_wrapper">
    ...
    <?php // Normal editing ?>
    <?php if( substr($this->pageObject->name, 0, 6) !== 'header' && substr($this->pageObject->name, 0, 6) !== 'footer' && ($showHideHeaderFooter == 'default' || $showHideHeaderFooter == 'default-simple' || $showHideHeaderFooter == '')): ?>
          ...
          <span>
            ...
            <span>
              <a href="javascript:void(0);" onclick="toggleGlobalLayout($(this).getParent('div.admin_layoutbox_header'));">
                ...
                <?php echo ( empty($showHideHeaderFooter) || $showHideHeaderFooter == 'default' ? "({$this->translate('hide on this page')})" : "({$this->translate('show on this page')})" ) ?>
              </a>
            </span>
          </span>
        </div>
    ...
    <?php elseif( (substr($this->pageObject->name, 0, 6) == 'header' || substr($this->pageObject->name, 0, 6) == 'footer') && ($showHideHeaderFooter == 'default' || $showHideHeaderFooter == 'default-simple' || $showHideHeaderFooter == '')): ?>
            ...
            <ul class='admin_content_sortable admin_content_stock_sortable'>
              ...
              <?php foreach( $categoryAreas as $info ):
                ...
                <?php //if( empty($info['canHaveChildren']) ): ?>
                  <li class="<?php echo $class ?> admin_sitepage_content_draggable" title="<?php echo $this->escape($info['description']) ?>"<?php if( $onmousedown ): ?> onmousedown="<?php echo $onmousedown ?>"<?php endif; ?>>
                      <?php if($category == 'Page Profile') :?>
                        <div>
                        ...
                        <div id="hide_<?php echo $this->translate($info['name'])?>" <?php if(!in_array($info['name'], $this->hideWidgets)) :?>style="display:block;"<?php else:?> style="display:none;"<?php endif;?>  title='<?php echo $this->translate('If you have NOT placed this widget in the Page Profile Placement area and you "lock" it, then this widget will NOT be available to Page Owners for placing on the Page. If you have placed this widget in the Page Profile Placement area and you "lock" it, then Page Owners will NOT be able to remove it or drag-n-drop it.')?>'>
	                        <a href="javascript:void(0);" onclick="widgetshowhide('<?php echo $this->translate($info['name'])?>', 1);"><?php echo $this->translate('lock')?></a>
                       	</div> 
                       
                       	<div id="show_<?php echo $this->translate($info['name'])?>" <?php if(in_array($info['name'], $this->hideWidgets)) :?>style="display:block;"<?php else:?> style="display:none;"<?php endif;?> title="<?php echo $this->translate('Unlock this widget and make it available to Page Owners for arranging, adding on, or removing from their Page Profile.')?>">
...
<script type="text/javascript">
	...
	var widgetshowhide = function(widgetname, option) {
		  ...
		  if(option == 1) {
		  	var confirmation = confirm("<?php echo $this->string()->escapeJavascript($this->translate("You are about to Lock this widget. If you have placed this widget in the main Page area, then the Page Admins of the Pages where this widget is placed, will NOT be able to remove it or drag-and-drop to arrange it on their Pages. If you have not placed this widget in the main Page area, then this will be removed from all the Pages where it has been placed, and will not be available to Page Admins in the layout managing area.")); ?>");
		  	if(!confirmation) {
		  		return;
		  	}
		  ...
		  $('backgroundimage_' + widgetname).innerHTML = '<div class="form-label"></div><div  class="form-element"><img src="<?php echo $this->layout()->staticBaseUrl ?>application/modules/Sitepage/externals/images/loading.gif" /></center></div>';
			var url = '<?php echo $this->url(array('action' => 'show-hide-widget', 'controller' => 'mobile-layout', 'module' => 'sitepage'), 'admin_default', true)?>';
    var request = new Request.HTML({
      'url' : url,
      'data' : {
        'format' : 'html',
...
<?php else :?>
		...
		<div class="tip">
	  	<span><?php echo $this->translate('You have disabled Page Profile Layout editing by their owners from the "Edit Page Layout" field in Global Settings. If you enable it, then from here you will be able to choose which widgets should be available to users on their Page Profile, and which ones will they be able to arrange, add or remove.Currently, you can configure Page Profile Layout from the "Layout" > "Layout Editor" section by selecting "Page Profile" from the "Editing" dropdown.'); ?></span> 
	  </div>
