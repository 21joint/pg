...
foreach ($this->show_slideshow_object as $type => $item) {
  ...
  if ($priceEnable && $item->price) {
    $content_price = $this->translate("Price: ") . " " . Engine_Api::_()->sitepage()->getPriceWithCurrency($item->price);
  }

  $description = $item->body;

  $content_link = $this->htmlLink(Engine_Api::_()->sitepage()->getHref($item->page_id, $item->owner_id, $item->getSlug()), $this->translate('View Page &raquo;'), array('class' => 'seaocore_slideshow_view_link'));
...
if (!empty($this->sitepage_sponsored)) {
  ?>
    ...
    window.addEvent('domready',function(){
     ...
     for(var i=0;i < divElements.length;i++)
      ...
      var nS8 = new noobSlide({
        box: $('sitepage_seaocore_te_advanced_box'),
        items: $$('#sitepage_seaocore_te_advanced_box h3'),
        size: (width-10),
        handles: $$('#handles8 span'),
        addButtons: {previous: $('sitepage_seaocore_prev8'), stop: $('sitepage_seaocore_stop8'), play: $('sitepage_seaocore_play8'), next: $('sitepage_seaocore_next8') },
        interval: 5000,
        fxOptions: {
          duration: 500,
          transition: '',
        ...
        onWalk: function(currentItem,currentHandle){
          ...
          {
            $('sitepage_seaocore_caption').innerHTML =   current_caption_link;
          }
          else {
            $('sitepage_seaocore_caption').innerHTML =  '';
          }


          $('sitepage_seaocore_current_numbering').innerHTML =  current_index + '/' + num_of_slidehsow ;
        }
      });
...
<div class="seaocore_slideshow_wrapper">
  <div class="seaocore_slideshow_mask">
    <div id="sitepage_seaocore_te_advanced_box" class="seaocore_slideshow_advanced_box">
<?php echo $image_text_var ?>
  ...
  <div class="seaocore_slideshow_option_bar">
    <div>
      <p class="buttons">
        <span id="sitepage_seaocore_prev8" class="seaocore_slideshow_controllers-prev seaocore_slideshow_controllers prev" title="<?php echo $this->translate('Previous'); ?> "></span>
        <span id="sitepage_seaocore_stop8" class="seaocore_slideshow_controllers-stop seaocore_slideshow_controllers" title="<?php echo $this->translate('Stop'); ?> "></span>
        <span id="sitepage_seaocore_play8" class="seaocore_slideshow_controllers-play seaocore_slideshow_controllers" title="<?php echo $this->translate('Play'); ?>"></span>
        <span id="sitepage_seaocore_next8" class="seaocore_slideshow_controllers-next seaocore_slideshow_controllers" title="<?php echo $this->translate('Next'); ?> "></span>
      </p>
    </div>
    <span id="sitepage_seaocore_caption"></span>
    <span id="sitepage_seaocore_current_numbering" class="seaocore_slideshow_pagination"></span>
  </div>
</div>