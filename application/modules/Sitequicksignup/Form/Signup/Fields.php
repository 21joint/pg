 ...
 */
class Sitequicksignup_Form_Signup_Fields extends Sitequicksignup_Form_Signup_Standard {

    protected $_fieldType = 'user';
    protected $_signupForm;
    ...
    public function init() {
        ...
        $this
			    ...
			    ->setAttrib('class','sitequicksignup_form global_form');
        parent::init();
    ...
    public function accountForm() {
        ...
        $tabIndex = 100;
        $this->_signupForm = Engine_Api::_()->getDbTable('fieldorder', 'sitequicksignup');
        // Init form
        $view = Zend_Registry::get('Zend_View');
        if (Engine_Api::_()->getApi('settings', 'core')->getSetting('sitequicksignup.allow.title', 1)) {
            $title = Engine_Api::_()->getApi('settings', 'core')->getSetting('sitequicksignup.title', 'Create Account');
            $translatedText = $view->translate($title);
            $this->setTitle($translatedText);
        }

        if (Engine_Api::_()->getApi('settings', 'core')->getSetting('sitequicksignup.allow.description', 1)) {
            $description = Engine_Api::_()->getApi('settings', 'core')->getSetting('sitequicksignup.description', 'You can signup!!');
            $translatedText = $view->translate($description);
            $this->setDescription($translatedText);
        }
...
//        $this->name->getValidator('StringLength')->setMessage('An error has occured, please try again later.');
        $rowEmail = $this->_signupForm->getFieldsOptions('email');
        // Element: email
        $emailElement = $this->addEmailElement(array(
            'label' => 'Email Address',
            'description' => Engine_Api::_()->getApi('settings', 'core')->getSetting('sitequicksignup.field.description', 1) ? 'You will use your email address to login.' : '',
            'required' => true,
            'allowEmpty' => false,
            'validators' => array(
                array('NotEmpty', true),
        ...
        if (Engine_Api::_()->getApi('settings', 'core')->getSetting('sitequicksignup.confirm.email', 0)) {

            // Element: passconf
            $this->addElement('Text', 'emailconf', array(
                'label' => 'Email Again',
                'description' => Engine_Api::_()->getApi('settings', 'core')->getSetting('sitequicksignup.field.description', 1) ? 'Enter your email again for confirmation.' : '',
                'required' => true,
                'validators' => array(
                    array('NotEmpty', true),
                ),
                'tabindex' => $tabIndex++,
                'order' => $rowEmail->order + 1
            ));
        ...
        if ($settings->getSetting('user.signup.inviteonly') > 0) {
            $rowInvite = $this->_signupForm->getFieldsOptions('user.signup.inviteonly');
        ...
        if ($settings->getSetting('user.signup.random', 0) == 0 &&
            ...
            $rowPassword = $this->_signupForm->getFieldsOptions('password');
            // Element: password
            $this->addElement('Password', 'password', array(
                'label' => 'Password',
                'description' => Engine_Api::_()->getApi('settings', 'core')->getSetting('sitequicksignup.field.description', 1) ? 'Passwords must be at least 6 characters in length.' : '',
                'required' => true,
                'allowEmpty' => false, 
                'validators' => array(
                    array('NotEmpty', true),
            ...
            if (Engine_Api::_()->getApi('settings', 'core')->getSetting('sitequicksignup.confirm.password', 1)) {
                // Element: passconf
                $this->addElement('Password', 'passconf', array(
                    'label' => 'Password Again',
                    'description' => Engine_Api::_()->getApi('settings', 'core')->getSetting('sitequicksignup.field.description', 1) ? 'Enter your password again for confirmation.' : '',
                    'required' => true,
                    'validators' => array(
                        array('NotEmpty', true),
                    ),
                    'tabindex' => $tabIndex++,
                    'order' => $rowPassword->order + 1
                ));
        ...
        if ($settings->getSetting('user.signup.username', 1) > 0) {
            $rowUsername = $this->_signupForm->getFieldsOptions('user.signup.username');
            ...
            $this->addElement('Text', 'username', array(
                ...
                'description' => Engine_Api::_()->getApi('settings', 'core')->getSetting('sitequicksignup.field.description', 1) ? $description : '',
                'required' => true,
                'allowEmpty' => false,
                'validators' => array(
                    array('NotEmpty', true),
        ...
        if (Engine_Api::_()->getApi('settings', 'core')->getSetting('sitequicksignup.timezone', 0)) {
            $rowTimezone = $this->_signupForm->getFieldsOptions('sitequicksignup.timezone');
            // Element: timezone
        ...
        if (count($languageNameList) > 1 && Engine_Api::_()->getApi('settings', 'core')->getSetting('sitequicksignup.language', 0)) {
            $rowLanguage = $this->_signupForm->getFieldsOptions('sitequicksignup.language');
            $this->addElement('Select', 'language', array(
                'label' => 'Language',
                'multiOptions' => $languageNameList,
                'tabindex' => $tabIndex++,
                'order' => $rowLanguage->order
            ));
        ...
        if (Engine_Api::_()->getApi('settings', 'core')->core_spam_signup) {
            $rowCaptcha = $this->_signupForm->getFieldsOptions('core.spam.signup');
            $this->addElement('captcha', 'captcha', Engine_Api::_()->core()->getCaptchaOptions(array(
                        'tabindex' => $tabIndex++,
                        'order' => $rowCaptcha->order
            )));
        }
        ...
        if ($settings->getSetting('user.signup.terms', 1) == 1) {
            ...
            $rowTerms = $this->_signupForm->getFieldsOptions('user.signup.terms');
            ...
            $this->terms->clearDecorators()
                    ...
                    ->addDecorator('Description', array('placement' => Zend_Form_Decorator_Abstract::PREPEND, 'tag' => 'label', 'class' => 'null', 'escape' => false, 'for' => 'terms'))
        ...
        $this->addElement('Button', 'submit', array(
            'label' => $view->translate('Create an Account'),
            'type' => 'submit',
            'order' => 999999999,
            'ignore' => true,
            'tabindex' => $tabIndex++,
