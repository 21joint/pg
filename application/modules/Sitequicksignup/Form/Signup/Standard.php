 ...
 * @version    $Id: Standard.php 2017-03-16 9:40:21Z SocialEngineAddOns $
 * @author     SocialEngineAddOns
 */
class Sitequicksignup_Form_Signup_Standard extends Engine_Form {
    /* Custom */
    ...
    public function generate() {
        $isSubscription = Engine_Api::_()->hasModuleBootstrap('sitesubscription');
        $subscriptionAllowed = (Engine_Api::_()->getApi('settings', 'core')->getSetting('sitequicksignup.subscription.enabled', 0));
        $fieldMappingEnabled = $subscriptionAllowed && $isSubscription && (Engine_Api::_()->getApi('settings', 'core')->getSetting('sitesubscription.profile.mapping', '2') == 1);
        if($fieldMappingEnabled){
            return;
        }
        $struct = $this->getFieldStructure();
        $rowProfileTypes = Engine_Api::_()->getDbTable('fieldorder', 'sitequicksignup')->getFieldsOptions('profiletypes');
        $orderIndex = $rowProfileTypes->order;
        ...
        foreach ($struct as $fskey => $map) {
            ...
            if ($params['type'] == 'ProfileType') {
                $params['options']['multiOptions']['']=$view->translate('Profile Type');
            }
            // Heading
...
//    }
    ...
    public function isValid($data) {
        ...
        foreach ($otherElements as $key => $element) {
            if (isset($data[$key])) {
                $valid = $element->isValid($data[$key], $data) && $valid;
            } else {
                $valid = $element->isValid($data) && $valid;
            }

            if ($valid == true && $this->isEmailAntispamEnabled()) {
                if (!empty($data[$this->_orgEmailFieldName])) {
                    $valid = false;
                } else {
                    $emailElementName = $this->getEmailElementFieldName();
                    $this->{$this->_orgEmailFieldName}->setValue($data[$emailElementName]);
                }
            }
        }
