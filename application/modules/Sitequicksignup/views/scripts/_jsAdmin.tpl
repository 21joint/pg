 ...
 * @version    $Id: _jsAdmin.tpl 2017-03-16 9:40:21Z SocialEngineAddOns $
...
<script type="text/javascript">
    ...
    var urls = {
        ...
        heading: {
            ...
            enable: '<?php echo $this->url(array('action' => 'heading-enable')) ?>',
            disable: '<?php echo $this->url(array('action' => 'heading-disable')) ?>'
        },
    ...
    var registerEvents = function () {
        ...
        // Attach edit field
        $$('.field > .item_options > a:first-child').removeEvents().addEvent('click', uiSmoothEnableHeading);

        // Attach delete field
        $$('.field > .item_options > a + a').removeEvents().addEvent('click', uiSmoothDisableHeading);

        // Attach heading edit
        $$('.heading > .item_options > a:first-child').removeEvents().addEvent('click', uiSmoothEnableHeading);

        // Attach heading edit
        $$('.heading > .item_options > a:last-child').removeEvents().addEvent('click', uiSmoothDisableHeading);
    ...
    var uiSmoothEnableHeading = function (spec) {
        if ($type(spec) == 'event' || $type(spec) == 'domevent') {
            element = $(spec.target);
        } else if ($type(spec) == 'element' || $type(spec) == 'domelement') {
            element = $(element);
        ...
        var parentEl = element.getParent('.admin_field');
        var ids = readIdentifiers(parentEl.get('id'));
        var url = urls.heading.enable;
        url += '/field_id/' + ids.child_id;
        window.location.href = url;
    }

    var uiSmoothDisableHeading = function (spec) {
        if ($type(spec) == 'event' || $type(spec) == 'domevent') {
            element = $(spec.target);
        } else if ($type(spec) == 'element' || $type(spec) == 'domelement') {
            element = $(element);
