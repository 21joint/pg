...
class Siteiosapp_Api_Core extends Core_Api_Abstract {
    ...
    public function validatePreviousMobileAPP() {
     ...
     /*
     * Synchronised review listing types to iOS menus.
     * 
     * @return empty
     */

    public function synchroniseDashboardMenus() {
        if(!_IOS_VERSION || (_IOS_VERSION < '1.5.2'))
            return;
    ...
    private function _addListingTypeInMenu($listingTypes, $availableListingTypesArray) {
        if (!empty($listingTypes)) {
            ...
            foreach ($listingTypes as $listingType) {
                ...
                $listingTypeInfo = Engine_Api::_()->getItem('sitereview_listingtype', $listingType->listingtype_id);
                if (!in_array($listingType->listingtype_id, $availableListingTypesArray)) {             
                    ...
                    $row->status = 1;
                    $row->default = 1;
                    $row->params = @serialize(array("listingtype_id" => $listingType->listingtype_id, "header_label_singular" => $listingTypeInfo->title_singular ));
                    $row->save();
                }
            }
        }
        ...
        if (!empty($availableListingTypesArray)) {
            foreach($availableListingTypesArray as $menu_id => $listingtype_id) {
                if(!in_array($listingtype_id, $this->_listingTypeArray))
                    Engine_Api::_()->getItem('siteiosapp_menus', $menu_id)->delete();
            }
        }
