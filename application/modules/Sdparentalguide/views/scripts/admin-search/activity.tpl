...
function changeOrder(elm, column_name, direction){
    ...
    var url = '<?= $this->url(array('module' => 'sdparentalguide', 'controller' => 'search', 'action' => 'activity'), '', true) ?>';
...
<div class="admin_table_form sd_layout_middle">
    ...
    <table class='admin_table'>
        <thead>
            <tr>
                <th><a href="javascript:void(0);" class="<?= $this->order_column == 'search_text'?'sort_active '.strtolower($this->order):''; ?>" onclick="changeOrder(this, 'search_text','<?= $this->order ;?>')"><?= $this->translate("Search Activity") ?></a></th>
                <th><a href="javascript:void(0);" class="<?= $this->order_column == 'gg_user_created'?'sort_active '.strtolower($this->order):''; ?>" onclick="changeOrder(this, 'gg_user_created', '<?= $this->order ;?>')"><?= $this->translate("User") ?></a></th>
        ...
        <tbody>
            <?php if( count($this->paginator) ): ?>
                ...
                <?php endforeach; ?>
            <?php else: ?>
            <tr><td colspan="3"><div class="tip"><span><?= $this->translate("No search activities found."); ?></span></td></tr>
            <?php endif; ?>
        </tbody>
    </table>
    <div>
