<?php if($this->paginator->getTotalItemCount() > 0): ?>
    <script type="text/javascript">
        ...
        en4.core.runonce.add(function(){
            $('sdparentalguide_pagination_struggles_previous').style.display = '<?php echo ($this->paginator->getCurrentPageNumber() == 1 ? 'none' : '' ) ?>';
            $('sdparentalguide_pagination_struggles_next').style.display = '<?php echo ( $this->paginator->count() == $this->paginator->getCurrentPageNumber() ? 'none' : '' ) ?>';
        });
    </script>

    <div class="px-3 pagination-holder-narrow">
            <div id="sdparentalguide_pagination_struggles_previous" class="paginator_previous">
                <a  class="d-flex  align-items-center fs-18" href="javascript:void(0);" onclick="filterAjaxStruggleContent( {'page': <?php echo $this->paginator->getCurrentPageNumber() - 1 ?>} )">
