...
<ul class="topic_holder d-flex">
    ...
    <li class="struggle_holder left-side d-block d-sm-flex px-xl-4 py-xl-3 px-lg-4 py-lg-3 px-1 py-3 mr-4 mb-0">
        ...
        <div class="struggle_box_left">
            <div class="struggle_left_side d-none d-sm-block">
                <a href="<?php $question->getOwner()->getHref();?>" class="struggle_owner_image">
                    <?= $this->itemPhoto($question->getOwner(), 'thumb.icon', array('class'=> 'owner_thumb')) ?>
                </a>
            </div>

            <div class="struggle_right-side w-sm-100 m-0">
                <a href="<?= $question->getHref();?>" class="struggle_title">
                <?= $question->getTitle();?></a>
                <ul class="struggle_info d-flex">
                    <li class="struggle_time_created pr-2">
                        <?= 'asked '. Engine_Api::_()->ggcommunity()->time_elapsed_string($question->creation_date);?>
                    </li>
                    <li class="pr-2">᛫</li>
        ...
        <div class="struggle_box_right col-xl-3 col-lg-3 col-5 d-none d-sm-block">
            <ul class="struggle_count_info <?= ($question->open==1 ? 'primary' : 'count_close_closed');?>">
                <li class="count_info" id="vote_count">
                    <span class="count votes counter <?= ($question->open==1 ? 'primary' : 'count_close');?>"><?= $question->up_vote_count;?></span>
                    <p class="count_title <?= ($question->open==1 ? 'title_active' : 'title_close');?>"><?= $this->translate('votes');?></p>
                </li>
                <li class="count_info" id="answer_count">
                    <span class="count answers counter <?= ($question->open==1 ? 'primary' : 'count_close');?> <?= ( (($question->open == 1) && ($question->accepted_answer==1)) ? 'open_accepted_answer' : '');?> <?= ( (($question->open == 0) && ($question->accepted_answer==1)) ? 'closed_accepted_answer' : '');?>"><?= $question->answer_count;?></span>
                    <p class="count_title title_active"><?= $this->translate('theories');?></p>
                </li>
                <li class="count_info" id="comment_count">
                    <span class="count comments counter <?= ($question->open==1 ? 'primary' : 'count_close');?>"><?= $question->comment_count;?></span>
                    <p class="count_title <?= ($question->open==1 ? 'title_active' : 'title_close');?>"><?= $this->translate('comments');?></p>
                </li>
            </ul>
        ...
        <div class="struggle-box_right-mobile d-block d-sm-none pt-1">
            <div class="holder-bottom-mobile d-flex justify-content-between">
                <ul class="d-flex  align-items-center col-3 justify-content-between left-side mx-0 px-0 ">
                    ...
                    <!-- answer count -->
                    <li class="count_info d-flex  align-items-center mr-3 <?= ($question->open==1 ? 'primary' : 'count_close');?> <?= ( (($question->open == 1) && ($question->accepted_answer == 1)) ? 'svg-icons-one' : '');?> <?= ( (($question->open == 0) && ($question->accepted_answer==1)) ? 'svg-icons-two' : '');?> " id="answer_count">

                        <div class="first-icon">
                            <?= ($question->open == 1 ? '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 13.98 13.34"  width="13"><title>star_unanswered_open</title><path d="M6.67,2.15,8.09,3.94l.4.5.6-.15,2.37-.57L10.22,5.38,9.75,6l.49.61,1.47,1.83-2.62-1-.79-.3-.43.72L6.19,10.72l.18-3.16V6.77l-.76-.22L3,5.77l2.82-.68.72-.17V4.18l.11-2M6.09,0a.31.31,0,0,0-.31.3L5.56,4.12.23,5.4a.31.31,0,0,0,0,.6L5.39,7.5l-.3,5.34a.46.46,0,0,0,.43.49h0A.45.45,0,0,0,6,13.1L8.76,8.37l4.7,1.82h.14a.39.39,0,0,0,.3-.63L11,6l2.44-3.2a.31.31,0,0,0-.25-.5h-.12l-4.25,1L6.33.12A.31.31,0,0,0,6.09,0Z" transform="translate(0 0)" fill="#5cc7cd"/></svg>'  : '<svg  width="13" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 13.98 13.33"><title>star_unanswered_closed</title><path d="M6.67,2.15,8.09,3.94l.4.5.6-.15,2.37-.57L10.22,5.38,9.75,6l.49.61,1.47,1.83-2.62-1-.79-.3-.43.72L6.19,10.72l.18-3.16V6.77l-.76-.22L3,5.77l2.82-.68.72-.17V4.18l.11-2M6.09,0a.31.31,0,0,0-.31.3L5.56,4.12.23,5.4a.31.31,0,0,0,0,.6L5.39,7.5l-.3,5.34a.46.46,0,0,0,.43.49h0A.45.45,0,0,0,6,13.1L8.76,8.37l4.7,1.82h.14a.39.39,0,0,0,.3-.63L11,6l2.44-3.2a.31.31,0,0,0-.25-.5h-.12l-4.25,1L6.33.12A.31.31,0,0,0,6.09,0Z" transform="translate(0 0)" fill="#b9cfd1"/></svg>');?>
                        </div>

                        <div class="second-icon">
                            <?= ( (($question->open == 1) && ($question->accepted_answer == 1)) ? '<svg xmlns="http://www.w3.org/2000/svg" width="13" viewBox="0 0 13.97 13.33"><title>star_answered_open</title><path d="M13.61,10.22h-.15L8.75,8.37,5.93,13.1a.46.46,0,0,1-.4.23.45.45,0,0,1-.44-.46v0l.29-5.33L.22,6a.32.32,0,0,1,0-.61L5.56,4.12,5.76.3A.31.31,0,0,1,6.08,0h0a.3.3,0,0,1,.24.12l2.56,3.2,4.25-1h.07a.31.31,0,0,1,.25.5L11,6l2.88,3.61a.38.38,0,0,1-.28.61Z" transform="translate(0 0)" fill="#5cc7cd"/></svg>' : '');?>
                        </div>
                        
                        <div class="third-icon">
                            <?= ( (($question->open == 0) && ($question->accepted_answer==1)) ? '<svg xmlns="http://www.w3.org/2000/svg"  width="13" viewBox="0 0 13.97 13.33"><title>star_answered_closed</title><path d="M13.61,10.22h-.15L8.75,8.37,5.93,13.1a.46.46,0,0,1-.4.23.45.45,0,0,1-.44-.46v0l.29-5.33L.22,6a.32.32,0,0,1,0-.61L5.56,4.12,5.76.3A.31.31,0,0,1,6.08,0h0a.3.3,0,0,1,.24.12l2.56,3.2,4.25-1h.07a.31.31,0,0,1,.25.5L11,6l2.88,3.61a.38.38,0,0,1-.28.61Z" transform="translate(0 0)" fill="#b9cfd1"/></svg>' : '');?>
                        </div>

                        <div class="holder-counter-number pl-1 <?= ($question->open==1 ? 'primary' : 'count_close');?>">
                            <?= $question->answer_count;?>
                        </div>

                    </li>
                    <!-- comment -->
                    <li class="count_info d-flex  align-items-center  <?= ($question->open==1 ? 'primary' : 'count_close');?>" id="comment_count">
    ...
    <!-- Answer side -->
    <!-- left side -->
    <li class="struggle_holder right-side d-block d-sm-flex px-xl-4 py-xl-3 px-lg-4 py-lg-3 px-1 py-3 mr-4 mb-0 <?= (($item->accepted ==1) ? 'chosen_theory' : '');?>">
        <div class="struggle_box_left w-100">
            <div class="struggle_left_side">
                <a href="<?= $item->getOwner()->getHref();?>" class="struggle_owner_image">
                    <?= $this->itemPhoto($item->getOwner(), 'thumb.icon', array('class'=> 'owner_thumb')) ?>
                </a>
            </div>
                ...
                <ul class="struggle_info d-flex">
                    <li class="struggle_time_created  ">
                        ...
                        <div class="top-holder">
                            <a class="struggle_title" href="<?= $question->getHref();?>">
                                <?=  mb_strimwidth($item->body,0,30,'...'); ?>
                            </a>
