<?php if($this->paginator->getTotalItemCount() > 0): ?>
    <script type="text/javascript">
        ...
        en4.core.runonce.add(function(){
            $('sdparentalguide_pagination_theories_previous').style.display = '<?= ($this->paginator->getCurrentPageNumber() == 1 ? 'none' : '' ) ?>';
            $('sdparentalguide_pagination_theories_next').style.display = '<?= ( $this->paginator->count() == $this->paginator->getCurrentPageNumber() ? 'none' : '' ) ?>';
        });
    </script>

    <div class="px-3 pagination-holder-narrow py-3">
            <div id="sdparentalguide_pagination_theories_previous" class="paginator_previous">
                <a  class="d-flex  align-items-center fs-18" href="javascript:void(0);" onclick="filterAjaxTheoriesContent( {'page': <?= $this->paginator->getCurrentPageNumber() - 1 ?>} )">
