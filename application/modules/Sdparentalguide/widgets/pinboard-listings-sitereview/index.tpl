...
<script type="text/javascript">
  window.totalListingCount = <?php echo sprintf('%d', $this->totalCount) ?>;
  ...
  function loadAjaxListings() {
    ...
    var pinboard = $("seaocore_board_wrapper_<?php echo $this->identity ?>");
    if (pinboard) {
      pinboard.set("id", "pinboard_<?php echo $this->identity ?>");
    ...
    window.loadingAjaxListings = new Request.HTML({
      ...
      onSuccess: function (responseTree, responseElements, responseHTML, responseJavaScript) {
        ...
        if ($$(".seaocore_board_wrapper").length > 0) {
          $$(".seaocore_board_wrapper").set("id", "seaocore_board_wrapper_<?php echo $this->identity ?>");
        }
        ...
        try {
          ...
          PinBoardSeaoObject["middle"].widgets.each(function (widget, index) {
            if (widget.widgetId == "<?php echo $this->identity ?>") {
...
<?php else: ?>
    ...
    <div class="seaocore_list_wrapper" style="width:<?php echo $this->params['itemWidth'] ?>px;">
      <div class="seaocore_board_list b_medium <?php echo $sitereview->featured; ?>_featured"
           style="width:<?php echo $this->params['itemWidth'] - 18 ?>px;">
        <div>
            <?php if ($sitereview->featured): ?>
              <span class="seaocore_list_featured_label"
                    title="<?php echo $this->translate('Featured'); ?>"><?php echo $this->translate('FEATURED'); ?></span>
          ...
          <div class="seaocore_board_list_btm">
            ...
            <div class="o_hidden seaocore_stats seaocore_txt_light">
                <?php if ($this->postedby): ?>
                  <b><?php echo $this->htmlLink($sitereview->getOwner()->getHref(), $sitereview->getOwner()->getTitle()) ?></b>
                  <br/>
                <?php endif; ?>
            ...
            <?php if (!empty($sitereview->sponsored)): ?>
              <div class="seaocore_list_sponsored_label"
                   style="background: <?php echo $listingType->sponsored_color; ?>">
              ...
              <div class="seaocore_board_list_action_links">
                  ...
                  <?php if ((in_array('comment', $this->show_buttons) || in_array('like', $this->show_buttons)) && $alllowComment): ?>
                      ...
                      <a href='javascript:void(0);'
                         onclick="en4.seaocorepinboard.comments.addComment('<?php echo $sitereview->getGuid() . "_" . $this->identity ?>')"
                         class="seaocore_board_icon comment_icon" title="Comment">
                        <!--<?php echo $this->translate('Comment'); ?>--></a>
                      <?php endif; ?>
                      <?php if (in_array('like', $this->show_buttons)): ?>
                      <a href="javascript:void(0)" title="Like"
                         class="seaocore_board_icon like_icon <?php echo $sitereview->getGuid() ?>like_link"
                         id="<?php echo $sitereview->getType() ?>_<?php echo $sitereview->getIdentity() ?>like_link"
                         <?php if ($sitereview->likes()->isLike($this->viewer())): ?>style="display: none;"
                         <?php endif; ?>onclick="en4.seaocorepinboard.likes.like('<?php echo $sitereview->getType() ?>', '<?php echo $sitereview->getIdentity() ?>');">
                        <!--<?php echo $this->translate('Like'); ?>--></a>

                      <a href="javascript:void(0)" title="Unlike"
                         class="seaocore_board_icon unlike_icon <?php echo $sitereview->getGuid() ?>unlike_link"
                         id="<?php echo $sitereview->getType() ?>_<?php echo $sitereview->getIdentity() ?>unlike_link"
                         <?php if (!$sitereview->likes()->isLike($this->viewer())): ?>style="display:none;" <?php endif; ?>
                         onclick="en4.seaocorepinboard.likes.unlike('<?php echo $sitereview->getType() ?>', '<?php echo $sitereview->getIdentity() ?>');">
                  ...
                  <?php if (in_array('pinit', $this->show_buttons)): ?>
                    <a href="http://pinterest.com/pin/create/button/?url=<?php echo $urlencode; ?>&media=<?php echo urlencode((!preg_match("~^(?:f|ht)tps?://~i", $sitereview->getPhotoUrl('thumb.profile')) ? (((!empty($_ENV["HTTPS"]) && 'on' == strtolower($_ENV["HTTPS"])) ? "https://" : "http://") . $_SERVER['HTTP_HOST']) : '') . $sitereview->getPhotoUrl('thumb.profile')); ?>&description=<?php echo $sitereview->getTitle(); ?>"
                       class="pb_ch_wd seaocore_board_icon pin_icon" title="Pin It">
                      <!--<?php echo $this->translate('Pin It') ?>--></a>
                  <?php endif; ?>
    ...
    <h3><?php echo $this->response; ?></h3>
    <?php endif; ?>
    <?php if (!$this->autoload && !$this->is_ajax_load): ?>
    <div class="seaocore_view_more mtop10 dnone" id="seaocore_view_more_<?php echo $this->identity ?>">
      <a href="javascript:void(0);" id=""
         class="buttonlink icon_viewmore"><?php echo $this->translate('View More') ?></a>
    </div>
