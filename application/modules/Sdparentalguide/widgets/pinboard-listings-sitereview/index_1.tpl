...
<?php
$this->headScript()
  ->appendFile($this->layout()->staticBaseUrl . 'application/modules/Sdparentalguide/externals/scripts/core.js')
  ->appendFile($this->layout()->staticBaseUrl . 'application/modules/Sdparentalguide/externals/scripts/pinboard/pinboard.js')
  ->appendFile($this->layout()->staticBaseUrl . 'application/modules/Sdparentalguide/externals/scripts/pinboard/mooMasonry.js');
?>

<?php $this->headLink()->prependStylesheet($this->layout()->staticBaseUrl . 'application/modules/Sdparentalguide/externals/styles/style_board.css'); ?>
<?php $this->headLink()->prependStylesheet($this->layout()->staticBaseUrl . 'application/modules/Sdparentalguide/externals/styles/style_sdparentalguide.css'); ?>
<?php
$this->headLink()
...
<?php else: ?>
  ...
  <?php foreach ($this->listings as $sitereview): ?>
    ...
    ?>
    <div class="seaocore_list_wrapper" style="width:<?= $this->params['itemWidth'] ?>px;">
      <div class="seaocore_board_list b_medium <?= $sitereview->featured; ?>_featured"
           style="width:<?= $this->params['itemWidth'] - 18 ?>px;">
        <div>
          <?php if ($sitereview->featured): ?>
            <span class="seaocore_list_featured_label sd_featured_img" title="<?= $this->translate('Featured'); ?>"><img
                src='<?= $this->layout()->staticBaseUrl; ?>application/modules/Sdparentalguide/externals/images/featured_small2.png'/></span>
          <?php elseif ($sitereview->newlabel): ?>
            <i class="seaocore_list_new_label" title="<?= $this->translate('New'); ?>"></i>
          <?php endif; ?>
          <div class="seaocore_board_list_thumb">
            <a href="<?= $sitereview->getHref(array('profile_link' => 1)) ?>" class="seaocore_thumb">
              <table>
                <tr valign="middle">
                  <td>
                    ...
                    <?= $this->itemPhoto($sitereview, ($this->params['itemWidth'] > 300) ? 'thumb.main' : 'thumb.profile', '', $options); ?>
          ...
          <div class="seaocore_board_list_btm">
            <?php if ($this->postedby): ?>
              <?= $this->htmlLink($sitereview->getOwner()->getHref(), $this->itemPhoto($sitereview->getOwner(), 'thumb.icon', '', array())) ?>
            <?php endif; ?>
            <div class="o_hidden seaocore_stats seaocore_txt_light">
              <?php if ($this->postedby): ?>
                <b><?= $this->htmlLink($sitereview->getOwner()->getHref(), $sitereview->getOwner()->getTitle()) ?></b>
                <br/>
              <?php endif; ?>
              <?= $this->translate("in %s", $this->htmlLink($sitereview->getCategory()->getHref(), $this->translate($sitereview->getCategory()->getTitle(true)))) ?>
              -
          ...
          <?php if (!empty($sitereview->sponsored)): ?>
            <div class="seaocore_list_sponsored_label" style="background: <?= $listingType->sponsored_color; ?>">
          ...
          <div class="seaocore_board_list_cont">
            <div class="seaocore_title">
              <?= $this->htmlLink($sitereview->getHref(), $sitereview->getTitle()) ?>
            </div>

            <?php if ($this->truncationDescription): ?>
              <div class="seaocore_description">
                <?= Engine_Api::_()->seaocore()->seaocoreTruncateText($sitereview->getDescription(), $this->truncationDescription) ?>
              </div>
            <?php endif; ?>
            <?php if (!empty($sitereview->price) && $sitereview->price > 0 && Zend_Registry::get('listingtypeArray' . $sitereview->listingtype_id)->price && isset($this->params['price']) && $this->params['price']): ?>
              <div class="seaocore_stats seaocore_txt_light mtop5">
                ...
                <span><?= Engine_Api::_()->sitereview()->getPriceWithCurrency($sitereview->price); ?></span>
              </div>
            <?php endif; ?>
            <?php if (isset($this->params['location']) && $this->params['location'] && Zend_Registry::get('listingtypeArray' . $sitereview->listingtype_id)->location && !empty($sitereview->location)): ?>
              ...
              <b>
                <?= $this->htmlLink(array('route' => 'seaocore_viewmap', "id" => $sitereview->listing_id, 'resouce_type' => 'sitereview_listing'), $this->translate("Get Directions"), array('class' => 'smoothbox')); ?>
              </b>
              </span>
              </div>
            <?php endif; ?>
          ...
          <?php if ($this->userComment): ?>
            <div class="seaocore_board_list_comments o_hidden">
              <?= $this->action("list", "pin-board-comment", "seaocore", array("type" => $sitereview->getType(), "id" => $sitereview->listing_id, 'widget_id' => $this->identity)); ?>
            </div>
          <?php endif; ?>
          <?php if (!empty($this->show_buttons)): ?>
            <div class="seaocore_board_list_action_links sd_listing_action_links">
              ...
              <?php if ((in_array('comment', $this->show_buttons) || in_array('like', $this->show_buttons)) && $alllowComment): ?>
                <?php if (in_array('comment', $this->show_buttons)): ?>
                  <a href='javascript:void(0);'
                     ...
                     class="seaocore_board_icon comment_icon"
                     title="Comment"><!--<?= $this->translate('Comment'); ?>--></a>
                <?php endif; ?>
                <?php if (in_array('like', $this->show_buttons)): ?>
                  <a href="javascript:void(0)"
                     ...
                     <?php if ($sitereview->likes()->isLike($this->viewer())): ?>style="display: none;"
                  ...
                  <a href="javascript:void(0)"
                     ...
                     <?php if (!$sitereview->likes()->isLike($this->viewer())): ?>style="display:none;" <?php endif; ?>
              ...
              <?php if (in_array('pinit', $this->show_buttons)): ?>
                <a href="http://pinterest.com/pin/create/button/?url=<?= $urlencode; ?>&media=<?= urlencode((!preg_match("~^(?:f|ht)tps?://~i", $sitereview->getPhotoUrl('thumb.profile')) ? (((!empty($_ENV["HTTPS"]) && 'on' == strtolower($_ENV["HTTPS"])) ? "https://" : "http://") . $_SERVER['HTTP_HOST']) : '') . $sitereview->getPhotoUrl('thumb.profile')); ?>&description=<?= $sitereview->getTitle(); ?>"
                   class="pb_ch_wd seaocore_board_icon pin_icon"
                   title="Pin It"><!--<?= $this->translate('Pin It') ?>--></a>
              <?php endif; ?>
  ...
  <?php endif; ?>
  <?php if (!$this->autoload && !$this->is_ajax_load): ?>
    <div class="seaocore_view_more mtop10 dnone" id="seaocore_view_more_<?= $this->identity ?>">
      <a href="javascript:void(0);" id="" class="buttonlink icon_viewmore"><?= $this->translate('View More') ?></a>
    </div>
    <div class="seaocore_loading dnone" id="seaocore_loading_<?= $this->identity ?>">
