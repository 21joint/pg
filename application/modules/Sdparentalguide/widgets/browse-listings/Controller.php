 ...
 */
class Sdparentalguide_Widget_BrowseListingsController
...
//    $headLink->appendStylesheet('/styles/reviews_home.bundle.css');
    ...
    $this->view->truncation
      = $params['truncation'] = $this->_getParam('truncation', 32);
    $this->view->truncationDescription
      = $params['truncationDescription'] = $this->_getParam(
      'truncationDescription', 1024
    );
...
//                ->where($listingTableName . '.approved = ?', '1')
    ...
    if (!empty($formValues['listing_type'])) {
      $select->where(
        "$listingTableName.listingtype_id = ?", $formValues['listing_type']
      );
    }
    if (!empty($formValues['category_id'])) {
      $select->where(
        "$listingTableName.category_id = ?", $formValues['category_id']
      );
    }
    if (!empty($formValues['subcategory_id'])) {
      $select->where(
        "$listingTableName.subcategory_id = ?", $formValues['subcategory_id']
      );
    }
    ...
    foreach ($formValues['forms'] as $formData) {
      ...
      $sitereview = Engine_Api::_()->getItem(
        "sitereview_listing", $formData['listing_id']
      );
      if (empty($sitereview)) {
        continue;
      }
