 ...
 *
 */
class Sdparentalguide_Widget_AjaxNotificationsController
  ...
  public function indexAction()
  {
    ...
    if (!$this->view->isajax) {
      ...
      $modules = Engine_Api::_()->getDbtable('modules', 'core')
        ->getModulesAssoc();
      $notificationTypes = Engine_Api::_()->getDbtable(
        'notificationTypes', 'activity'
      )->getNotificationTypes();
      $notificationSettings = Engine_Api::_()->getDbtable(
        'notificationSettings', 'activity'
      )->getEnabledNotifications($subject);
      ...
      foreach ($notificationTypes as $type) {
        if (isset($modules[$type->module])) {
          ...
          if ($translateCategory === $category) {
            ...
            $elementName = preg_replace(
              '/[^a-zA-Z0-9]+/', '_', strtolower($translateCategory)
            );
          }
        } else {
          $elementName = 'misc';
      ...
      $notificationTypesAssoc = array_filter(
        array_merge(
          array(
            'general' => array(),
            'misc'    => array(),
