...
class Sdparentalguide_Widget_AssignedBadgesSliderController
  extends Engine_Content_Widget_Abstract
{
  public function indexAction()
  {
    ...
    $assignedTable = Engine_Api::_()->getDbtable(
      'assignedBadges', 'sdparentalguide'
    );
    $assignedTableName = $assignedTable->info("name");

    $selectAssigned = $table->select()->setIntegrityCheck(false)->from(
      $tableName
    )
      ->joinLeft(
        $assignedTableName, "$assignedTableName.badge_id = $tableName.badge_id",
        array("$assignedTableName.active as assigned_active")
      )
    ...
    $this->view->assignedBadges
      = $assignedBadges = $table->fetchAll($selectAssigned);
