...
class Sdparentalguide_Api_Core extends Core_Api_Abstract
{
  ...
  public function getDateTime($datefromtable)
  {
    ...
    if ($total_months > 12) {
      ...
      $total_months = $view->translate(
        array('%s Year', '%s Years', $total_months), $total_months
      );
    } else {
      $total_months = $view->translate(
        array('%s Month', '%s Months', $months), $months
      );
    }
  ...
  public function getUserCredits($user = null)
  {
    ...
    $param['count'] = 1;
    $credits = Engine_Api::_()->getDbtable('credits', 'sitecredit')->Credits(
      $param
    );
  ...
  public function getUserCredibility($user = null)
  {
    ...
    return $view->translate(
      array('Credibility Score %s', 'Credibility Score %s', $score),
      $this->locale()->toNumber($score)
    );
  }
  ...
  public function getUserBadge($credit)
  {
    ...
    $select = $table->select();
    $select->where("credit_count <= ?", (int)$credit)->order(
      "credit_count DESC"
    )->limit(1);
...
'5' => 'Bronze',
  ...
  public function getListingTypesArray()
  {
    ...
    $select = $listingTypeTable->select()->from(
      $listingTypeTableName, array('title_plural', 'listingtype_id')
   ...
   * @return array
   */
  public function validateUserArray(User_Model_User $user,
    $ignoreParams = array()
  ) {
    try {
      $restrictedFields = array('email', 'password', 'salt', 'creation_ip',
                                'lastlogin_ip');
  ...
  public function getContentImage($subject, $getOwnerImage = false, $key = false
    ...
    $type = (empty($getOwnerImage)) ? $subject->getType()
      : $subject->getOwner()->getType();
    $images = array();
    if (empty($getOwnerImage)) { // Getting content images
      // If image url already contains http://
      if (strstr($subject->getPhotoUrl('thumb.main'), 'http://')
        || strstr(
          $subject->getPhotoUrl('thumb.main'), 'https://'
        )
      ) {
        $host = '';
      }
      ...
      $images[$tempKey] = (($thumbMain = $subject->getPhotoUrl('thumb.main'))
        && !empty($thumbMain)) ? (!strstr($thumbMain, "application/modules"))
        ? $host.$subject->getPhotoUrl('thumb.main')
        : $this->getDefaultImage(
          $type, 'main'
        ) : $this->getDefaultImage($type, 'main');
      if (!strstr($images[$tempKey], 'http')) {
        $images[$tempKey] = $getParentHost.DIRECTORY_SEPARATOR.$baseParentUrl
          .$images[$tempKey];
      }

      $images[$tempKey.'_normal'] = (($thubNormal = $subject->getPhotoUrl(
          'thumb.normal'
        ))
        && !empty($thubNormal)) ? (!strstr($thubNormal, "application/modules"))
        ? $host.$subject->getPhotoUrl('thumb.normal')
        : $this->getDefaultImage(
          $type, 'normal'
        ) : $this->getDefaultImage($type, 'normal');
      if (!strstr($images[$tempKey.'_normal'], 'http')) {
        $images[$tempKey.'_normal'] = $getParentHost.DIRECTORY_SEPARATOR
          .$baseParentUrl.$images[$tempKey.'_normal'];
      }

      $images[$tempKey.'_profile'] = (($thumbProfile = $subject->getPhotoUrl(
          'thumb.profile'
        ))
        && !empty($thumbProfile))
        ? (!strstr(
          $thubNormal, "application/modules"
        )) ? $host.$subject->getPhotoUrl('thumb.profile')
          : $this->getDefaultImage($type, 'profile')
        : $this->getDefaultImage(
          $type, 'profile'
        );
      if (!strstr($images[$tempKey.'_profile'], 'http')) {
        $images[$tempKey.'_profile'] = $getParentHost.DIRECTORY_SEPARATOR
          .$baseParentUrl.$images[$tempKey.'_profile'];
      }

      $images[$tempKey.'_icon'] = (($thumbIcon = $subject->getPhotoUrl(
          'thumb.icon'
        ))
        && !empty($thumbIcon)) ? (!strstr($thubNormal, "application/modules"))
        ? $host.$subject->getPhotoUrl('thumb.icon')
        : $this->getDefaultImage(
          $type, 'icon'
        ) : $this->getDefaultImage($type, 'icon');
      if (!strstr($images[$tempKey.'_icon'], 'http')) {
        $images[$tempKey.'_icon'] = $getParentHost.DIRECTORY_SEPARATOR
          .$baseParentUrl.$images[$tempKey.'_icon'];
      }
      ...
      if (strstr($subject->getOwner()->getPhotoUrl('thumb.main'), 'http://')
        || strstr($subject->getOwner()->getPhotoUrl('thumb.main'), 'https://')
      ) {
        $host = '';
      }

      $tempKey = empty($key) ? 'owner_image' : $key.'_owner_image';
      $images[$tempKey] = ($subject->getOwner()->getPhotoUrl('thumb.main'))
        ? $host.$subject->getOwner()->getPhotoUrl('thumb.main')
        : $this->getDefaultImage($type, 'main');
      if (!strstr($images[$tempKey], 'http')) {
        $images[$tempKey] = $getParentHost.DIRECTORY_SEPARATOR.$baseParentUrl
          .$images[$tempKey];
      ...
      $images[$tempKey.'_normal'] = ($subject->getOwner()->getPhotoUrl(
        'thumb.normal'
      )) ? $host.$subject->getOwner()->getPhotoUrl('thumb.normal')
        : $this->getDefaultImage($type, 'normal');
      if (!strstr($images[$tempKey.'_normal'], 'http')) {
        $images[$tempKey.'_normal'] = $getParentHost.DIRECTORY_SEPARATOR
          .$baseParentUrl.$images[$tempKey.'_normal'];
      ...
      $images[$tempKey.'_profile'] = ($subject->getOwner()->getPhotoUrl(
        'thumb.profile'
      )) ? $host.$subject->getOwner()->getPhotoUrl('thumb.profile')
        : $this->getDefaultImage($type, 'profile');
      if (!strstr($images[$tempKey.'_profile'], 'http')) {
        $images[$tempKey.'_profile'] = $getParentHost.DIRECTORY_SEPARATOR
          .$baseParentUrl.$images[$tempKey.'_profile'];
      ...
      $images[$tempKey.'_icon'] = ($subject->getOwner()->getPhotoUrl(
        'thumb.icon'
      )) ? $host.$subject->getOwner()->getPhotoUrl('thumb.icon')
        : $this->getDefaultImage($type, 'icon');
      if (!strstr($images[$tempKey.'_icon'], 'http')) {
        $images[$tempKey.'_icon'] = $getParentHost.DIRECTORY_SEPARATOR
          .$baseParentUrl.$images[$tempKey.'_icon'];
  ...
  {
    return _ENGINE_SSL ? 'https://'.$_SERVER['HTTP_HOST']
