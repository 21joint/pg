 ...
 */
class Sitegroupalbum_Form_Searchwidget extends Engine_Form {
  ...
  public function init() {
    ...
    $this->setAction($view->url(array(), 'sitegroupalbum_browse', true))->getDecorator('HtmlTag')->setOption('class', '');
  }
  ...
  public function getAdditionalOptionsElement() {
    ...
    $select_content = $coreContent_table->select()->where('name = ?', 'sitegroupalbum.search-sitegroupalbum');
    ...
    //$row = $this->_searchForm->getFieldsOptions('sitegroup', 'search');
     ...
     if (in_array("4", $enabledColumns)) {
      $this->addElement('Text', 'search_album', array(
              'label' => 'Album Title',
              'order' => $row->order - 4,
      ));
     ...
     if (in_array("3", $enabledColumns)) {
      ...
      $show_multiOptions["creation_date"] = "Everyone’s Albums";

			if(!empty($viewer_id)) {
				$show_multiOptions["my_album"] = 'My Albums';
				$show_multiOptions["my_like"] = 'Albums of Groups I Like';
			}

      $show_multiOptions["featured"] = "Featured Albums";
			$sitegroupPackageEnable = Engine_Api::_()->getApi('settings', 'core')->getSetting('sitegroup.package.enable', 1);
      if($sitegroupPackageEnable) {
				$show_multiOptions["sponsored album"] = 'Sponsored Albums';
      }
      ...
      $this->addElement('Select', 'show', array(
              ...
              'onchange' => $this->gethasMobileMode() ? '' : 'searchSitegroupalbums();',
              'order' => $row->order - 2,
      ));
   ...
   if (in_array("2", $enabledColumns)) {
		$this->addElement('Select', 'orderby_browse', array(
					'label' => 'Browse By',
					'multiOptions' => array(
              '' => '',
    ...
    $this->addElement('Hidden', 'itemCount', array(
            'order' => $i++,
    ));
