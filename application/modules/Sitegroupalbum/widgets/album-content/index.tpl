...
<div class="sitegroup_viewgroups_head">
	...
	<?php if (Engine_Api::_()->getDbtable('modules', 'core')->isModuleEnabled('sitetagcheckin')) :?>
		<div class="seaotagsitegroupalbumcheckinshowlocation" style="overflow:hidden;">
			<?php
...
<div class="layout_middle">
  <div class="sitegroup_album_options">
		...
		<?php  if ($enable_facebookse) { ?>
				<div class="mbot15">
					<script type="text/javascript">
						var fblike_moduletype = 'sitegroup_album';
    ...
    <div class="tip">
			<span>
				<?php echo $this->translate('You can not make this album as "Album of the Day" or "Featured Album" because its group privacy is not set to "Everyone" or "All Registered Members" and thus cannot be highlighted to users.');?>
			</span>
    </div>
    <?php endif;?>
    ...
    <?php if ($this->can_edit):?>
			...
			<?php if($this->allowView ): ?>    
				<a href="javascript:void(0);" class="buttonlink seaocore_icon_featured" onclick='featured("<?php echo $this->album->album_id;?>");' ><span id="featured_sitegroupalbum" <?php if($this->album->featured): ?> style="display:none;" <?php endif;?> title="<?php echo $this->translate("Make Featured"); ?>" ><?php echo $this->translate("Make Featured"); ?> </span> <span id="un_featured_sitegroupalbum" <?php if(!$this->album->featured): ?> style="display:none;" <?php endif;?> title="<?php echo $this->translate("Make Un-Featured"); ?>" > <?php echo $this->translate("Make Un-featured"); ?> </span></a>
				<?php echo $this->htmlLink(array('route' => 'default','module'=> 'sitegroup', 'controller'=>'album','action' => 'add-album-of-day', 'album_id' => $this->album->album_id, 'format' => 'smoothbox'), $this->translate('Make Album of the Day'), array(
				'class' => 'buttonlink smoothbox sitegroup_icon_photos_manage'
			)) ?>
			<?php endif;?>
		<?php endif;?>
  </div>
...
<script type="text/javascript">
  ...
  function viewMorePhoto()
  {
    ...
    en4.core.request.send(new Request.HTML({
      ...
      'url' : en4.core.baseUrl + 'widget/index/mod/sitegroupalbum/name/album-content',
      'data' : {
        format : 'html',
        isajax : 1,
        itemCountPerGroup : '20',
      ...
      onSuccess : function(responseTree, responseElements, responseHTML, responseJavaScript) { 
        Elements.from(responseHTML).inject($('sitegroupalbum_content').getElement('.thumbs_nocaptions'));
    ...
    function hideViewMoreLink(){
        $('view_more').style.display = '<?php echo ( $this->maxgroup == $this->currentGroupNumbers || $this->total_images == 0 ? 'none' : '' ) ?>';
    }
</script>
<script type="text/javascript">
  function featured(album_id)
  {
    en4.core.request.send(new Request.HTML({
      method : 'post',
      'url' : en4.core.baseUrl + 'sitegroup/album/featured',
      'data' : {
        format : 'html',
        'album_id' : album_id
      },
      onSuccess : function(responseTree, responseElements, responseHTML, responseJavaScript) {       
       if($('featured_sitegroupalbum').style.display=='none'){
        $('featured_sitegroupalbum').style.display="";
        $('un_featured_sitegroupalbum').style.display="none";
       }else{
          $('un_featured_sitegroupalbum').style.display="";
        $('featured_sitegroupalbum').style.display="none";
       }
      }
    }));
