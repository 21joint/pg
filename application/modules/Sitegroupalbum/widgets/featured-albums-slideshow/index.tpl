...
?>
<?php 
$this->headLink()->appendStylesheet($this->layout()->staticBaseUrl . 'application/modules/Sitegroupalbum/externals/styles/style_sitegroupalbum.css');
...
foreach ($this->show_slideshow_object as $type => $album) {
  ...
  $content_info = null;
  if(Engine_Api::_()->getApi('settings', 'core')->getSetting('sitegroup.postedby', 1)):
  ...
  //$content_info = '<div class="sitegroup_sidebar_list_details"></div>';
  $content_info.='<p>';
  ...
  if (!empty($album->title)) {
    ...
    $sitegroup_object = Engine_Api::_()->getItem('sitegroup_group', $album->group_id);
    ...
    $image_text_var .= $this->translate("in ") . $this->htmlLink(Engine_Api::_()->sitegroup()->getHref($album->group_id, $album->owner_id, $album->getSlug()),  $group_title,array('title' => $sitegroup_object->title)); 
    $image_text_var .= "</div>";
    ...
    window.addEvent('domready',function(){
     ...
     for(var i=0;i < divElements.length;i++)
      ...
      var nS8 = new noobSlide({
        box: $('sitegroupalbum_featured_album_im_te_advanced_box'),
        items: $$('#sitegroupalbum_featured_album_im_te_advanced_box h3'),
        size: (width-10),
        handles: $$('#handles8 span'),
        addButtons: {previous: $('sitegroupalbum_featured_album_prev8'), stop: $('sitegroupalbum_featured_album_stop8'), play: $('sitegroupalbum_featured_album_play8'), next: $('sitegroupalbum_featured_album_next8') },
        interval: 5000,
        fxOptions: {
          duration: 500,
          transition: '',
        ...
        onWalk: function(currentItem,currentHandle){
          ...
          {
            $('sitegroupalbum_featured_album_caption').innerHTML =   current_caption_link;
          }
          else {
            $('sitegroupalbum_featured_album_caption').innerHTML =  '';
          }


          $('sitegroupalbum_featured_album_current_numbering').innerHTML =  current_index + '/' + num_of_slidehsow ;
        }
      });
...
<div class="featured_slideshow_wrapper">
  <div class="featured_slideshow_mask">
    <div id="sitegroupalbum_featured_album_im_te_advanced_box" class="featured_slideshow_advanced_box">
  ...
  <div class="featured_slideshow_option_bar">
    <div>
      <p class="buttons">
        <span id="sitegroupalbum_featured_album_prev8" class="featured_slideshow_controllers-prev featured_slideshow_controllers prev" title=<?php echo $this->translate("Previous") ?> ></span>
        <span id="sitegroupalbum_featured_album_stop8" class="featured_slideshow_controllers-stop featured_slideshow_controllers" title=<?php echo $this->translate("Stop") ?> ></span>
        <span id="sitegroupalbum_featured_album_play8" class="featured_slideshow_controllers-play featured_slideshow_controllers" title=<?php echo $this->translate("Play") ?> ></span>
        <span id="sitegroupalbum_featured_album_next8" class="featured_slideshow_controllers-next featured_slideshow_controllers" title=<?php echo $this->translate("Next") ?> ></span>
      </p>
    </div>
    <span id="sitegroupalbum_featured_album_caption"></span>
    <span id="sitegroupalbum_featured_album_current_numbering" class="featured_slideshow_pagination"></span>
  </div>
</div>  
