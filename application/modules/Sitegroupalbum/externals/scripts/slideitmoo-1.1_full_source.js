...
/**
	...
	- navigators ( forward/back ) no longer added by script. Instead, add them into overallContainer making their display from CSS and after add the CSS selector class to navs parameter
		IE: navs:{ 
				fwd:'.Sitegroupalbum_SlideItMoo_forward',
				bk:'.Sitegroupalbum_SlideItMoo_back'
			}
...
**/
var SitegroupalbumSlideItMoo = new Class({
  ...
  options: {
    ...
    itemsSelector: null, /* css class for inner elements ( ie: .Sitegroupalbum_SlideItMoo_element ) */
    ...
    navs:{ /* starting this version, you'll need to put your back/forward navigators in your HTML */
      fwd:'.Sitegroupcontent_SlideItMoo_forward', /* forward button CSS selector */
      bk:'.Sitegroupcontent_SlideItMoo_back' /* back button CSS selector */
    },
  ...
  setContainersSize: function(){
    ...
    if( this.options.slideVertical ){
      ...
      if( this.options.showControls ){
        var s1 = $('Sitegroupalbum_SlideItMoo_forward').getSize();
        var s2 = $('Sitegroupalbum_SlideItMoo_back').getSize();
    ...
    if (fwdbck_click == 1 ) {
      ...
      if (call_count == 1) {
        ...
        if(direction ==1){
          ...
          document.getElementById('Sitegroupalbum_SlideItMoo_forward_disable').style.display= 'none';
          document.getElementById('Sitegroupalbum_SlideItMoo_forward_loding').style.display= 'block';
        }else{
          document.getElementById('Sitegroupalbum_SlideItMoo_back').style.display= 'none';
          document.getElementById('Sitegroupalbum_SlideItMoo_back_disable').style.display= 'none';
          document.getElementById('Sitegroupalbum_SlideItMoo_back_loding').style.display= 'block';

        } 
        var obj = this;
        var request = new Request.HTML({
          url : urlcarosel_sitegroupalbum,
          method: 'GET',
          data : {
            format : 'html',
            'task': 'ajax',
          ...
          {						
            $('Sitegroupalbum_SlideItMoo_items').innerHTML = responseHTML;												
            custom_sliditmoo (obj, direction, startindex_temp, forward_link);
            document.getElementById('Sitegroupalbum_SlideItMoo_forward_loding').style.display= 'none';
            document.getElementById('Sitegroupalbum_SlideItMoo_back_loding').style.display= 'none';
