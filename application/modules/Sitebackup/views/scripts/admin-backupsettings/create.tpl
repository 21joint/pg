...
<style type="text/css">
  ...
  .sitebackup_success	{
    ...
    border-radius:  5px;
    background-image: url(application/modules/Sitebackup/externals/images/success.png);
    background-repeat:no-repeat;
    background-position:6px 6px;
    margin-top:15px;
    margin-bottom:15px;
  }
  table.sitebackup_table{
    margin-top:5px;
    border:1px solid #EEEEEE;
    border-bottom:none;
    min-width:450px;
  }
  table.sitebackup_table tr:nth-child(2n) {
    background-color:#F8F8F8;
  }

  table.sitebackup_table tr td,
  table.sitebackup_progressbar_table tr td {
  ...
  table.sitebackup_progressbar_table{
    margin-top:15px;
    border:1px solid #EEEEEE;
    border-bottom:none;
    width:100%;
  ...
  table.sitebackup_progressbar_table tr{
    background-color:rgba(63, 200, 244, 0.5);
  }
  table.sitebackup_table tr td{
    font-size:11px;
  }
  ...
  a.button
  {
    ...
    color: #fff;
    background-image: url(application/modules/Seaocore/externals/images/buttonbg.png);
...
<?php if( !$this->flage && $this->destination_mode1 != 3 && $this->backup_completecode != 1 && !empty($imploded_string) ): ?>
  <div class="tip">
    <span>
      <?php
      echo $this->translate("Note: If you are getting any 'Error' during the Backup Process then you should exclude some directories or files before clicking on 'Backup Now !' button by using 'File Directories to Backup in $base_url/' option. For example, you may exclude these directories/files: $imploded_string.");
      ?>
    </span>
  </div>
<?php endif; ?>
...
<div class="settings">
  <div class="sitebackup_form">
    <div class="sitebackup_form_inner">
      <?php if( !$this->flage ): ?>
        ...
        <table class="sitebackup_progressbar_table">
          <tr>
            <td>
              <?php echo $this->htmlImage('application/modules/Sitebackup/externals/images/sign.png', '', array('class' => 'icon', 'style' => 'vertical-align:middle;')) ?>
          ...
          <?php if( $this->initial_code == 0 ): ?>
            <tr style="font-weight:bold;">
              <td>
                <?php echo $this->htmlImage('application/modules/Sitebackup/externals/images/arrow-right.png', '', array('class' => 'icon', 'style' => 'vertical-align:middle;')) ?>&nbsp;
            ...
            <?php if( $this->initial_code > 0 ): ?>
              ...
              <tr>
                <td>
                  <?php echo $this->htmlImage('application/modules/Sitebackup/externals/images/sign.png', '', array('class' => 'icon', 'style' => 'vertical-align:middle;')) ?>
                <?php endif; ?>
                <?php echo (empty($this->destination_mode)) ? $this->translate('Backing up database tables.') : $this->translate('Backing up database tables and uploading to ' . $this->destinationname . ' destination.') ?>
              </td>
            </tr>
          <?php endif; ?>
            ...
            <?php if( $this->initial_code > 0 ): ?>
              ...
              <tr>
                <td>
                  <?php echo $this->htmlImage('application/modules/Sitebackup/externals/images/sign.png', '', array('class' => 'icon', 'style' => 'vertical-align:middle;')) ?>
                <?php endif; ?>
                <?php echo (!empty($this->destination_mode) && !empty($this->backup_options) && $this->backup_options != 1) ? $this->translate('Backing up database and files and uploading to ' . $this->destinationname . ' & ' . $this->code_destinationname . ' destinations.') : $this->translate('Backing up database and files.') ?></td>
            </tr>
          <?php endif; ?>
            ...
            <?php if( $this->initial_code > 0 && $this->start_code ): ?>
              ...
              <tr>
                <td>
                  <?php echo $this->htmlImage('application/modules/Sitebackup/externals/images/sign.png', '', array('class' => 'icon', 'style' => 'vertical-align:middle;')) ?>
                <?php endif; ?>
                <?php echo (!empty($this->backup_options) && $this->backup_options != 1) ? $this->translate('Backing up files and uploading to ' . $this->code_destinationname . ' destination.') : $this->translate('Backing up files.') ?>
              </td>
            </tr>
          <?php endif; ?>
          <tr>
        ...
        <?php if( ($this->backup_completecode == 0) && $this->initial_code > 0 && $this->start_code ): ?>
          <div style="display:block;height:15px;width:128px;background-image:url(application/modules/Sitebackup/externals/images/backup-uploading.gif);background-position:0 0;float:left;font-size:0px;">
            <img src="application/modules/Sitebackup/externals/images/backup-uploading.gif" alt="" style="height:15px;width:128px;" />
          </div>
        <?php endif; ?>
        <?php if( ($this->backup_completecode == 2) && $this->initial_code > 0 ): ?>
          <div style="display:block;height:15px;width:128px;background-image:url(application/modules/Sitebackup/externals/images/backup-uploading.gif);background-position:0 0;float:left;font-size:0px;">
            <img src="application/modules/Sitebackup/externals/images/backup-uploading.gif" alt="" style="height:15px;width:128px;" />
          </div>
        <?php endif; ?>
        <?php if( ($this->backup_completecode == 1) && $this->initial_code > 0 ): ?>
          <div style="display:block;height:15px;width:128px;background-image:url(application/modules/Sitebackup/externals/images/backup-uploading.gif);background-position:0 0;float:left;font-size:0px;">
            <img src="application/modules/Sitebackup/externals/images/backup-uploading.gif" alt="" style="height:15px;width:128px;" />
          </div>
        <?php endif; ?>
      ...
      if( $this->initial ) {
        $base_url = Zend_Controller_Front::getInstance()->getBaseUrl();
        $parameter = '<form action="' . $base_url . '/admin/sitebackup/backupsettings/create?backup_options=' . $this->backup_options . '&backup_completecode=' . $this->backup_completecode . '&destination_id=' . $this->destination_id . '&lockoption=' . $this->lockoption . '&code_destination_id=' . $this->code_destination_id . '" method="POST" name="admin_sitebackup1" id="admin_sitebackup1">';
        $parameter .= '<input type="hidden" name="backup_options" value=' . $this->backup_options . '>';
        $parameter .= '</form>';
        $page_submit = $parameter . '<script language="javascript" type="text/javascript">setTimeout("document.admin_sitebackup1.submit()", 1);</script>';
      ...
      <?php if( $this->flage ): ?>
        ...
        <div style="background:#E9F4FA;padding:5px;">
          <b><?php echo $this->translate('100% backup complete!') ?></b>
        </div>
        ...
        <?php if( $this->backup_completecode == 0 ): ?>
          ...
          <table class="sitebackup_table">
            ...
            <tr>
              <td title="<?php if( file_exists(APPLICATION_PATH . '/public/' . $file_name . '/' . $this->code_filename) ): echo $this->translate('Download backup file');
      endif;
          ?>">
                ...
                <?php if( file_exists(APPLICATION_PATH . '/public/' . $file_name . '/' . $this->code_filename) ): ?>
                  ...
                  <?php echo $this->htmlImage('application/modules/Sitebackup/externals/images/download.png', '', array('style' => 'vertical-align:middle')) ?>
                <?php else: ?>
  ...
  <?php if( $this->backup_completecode == 1 && $this->backup_code != 4 ): ?>
          ...
          <table class="sitebackup_table">  
            ...
            <tr>
              <td title="<?php if( file_exists(APPLICATION_PATH . '/public/' . $file_name . '/' . $this->database_filename) ): echo $this->translate('Download backup file');
    endif;
    ?>">
                <u><?php echo $this->translate('Backup file generated') ?></u>:
    ...
    <?php if( $this->backup_completecode == 2 ): ?>
      ...
      <?php else: ?>
        ...
        <div style="margin-top:15px;clear:both;">  
          ...
          <?php if( $this->backup_completecode == 0 ): ?>
            <?php echo $this->htmlLink(array('route' => 'admin_default', 'module' => 'sitebackup', 'controller' => 'codebackup', 'action' => 'index'), $this->translate('View Files Backup History'), array('target' => '_parent', 'class' => 'button')) ?>
          <?php endif; ?>
          <?php if( $this->backup_completecode == 1 ): ?>
            <?php echo $this->htmlLink(array('route' => 'admin_default', 'module' => 'sitebackup', 'controller' => 'manage', 'action' => 'index'), $this->translate('View Database Backup History'), array('target' => '_parent', 'class' => 'button')) ?>&nbsp;
  <?php endif; ?>
        <?php if( $this->backup_completecode == 2 ): ?>
          <?php echo $this->htmlLink(array('route' => 'admin_default', 'module' => 'sitebackup', 'controller' => 'manage', 'action' => 'index'), $this->translate('View Database Backup History'), array('target' => '_parent', 'class' => 'button')) ?>&nbsp;
    <?php echo $this->htmlLink(array('route' => 'admin_default', 'module' => 'sitebackup', 'controller' => 'codebackup', 'action' => 'index'), $this->translate('View Files Backup History'), array('target' => '_parent', 'class' => 'button')) ?>
