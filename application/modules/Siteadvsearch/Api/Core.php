...
class Siteadvsearch_Api_Core extends Core_Api_Abstract {
    ...
    public function makeWidgetizePage($params = array(), $content_id = 0) {
        ...
        switch ($resourceType) {
            ...
            case "sitenews_news":
                $name = "siteadvsearch_index_browse-page_sitenews_news";
                $displayName = "Advanced Search - SEAO - News";
                $searchWidgetName = 'sitenews.search-news-sitenews';
                $browseWidgetName = 'sitenews.browse-news-sitenews';
                $browseWidgetParams = '{"title":"","titleCount":true,"viewType":["gridView","listView"],"defaultViewType":"gridView","newsOption":["title","provider"],"categoryids":null,"subcategoryids":null,"hidden_category_id":"","hidden_subcategory_id":"","hidden_subsubcategory_id":"","rss":"0","gridViewHeight":"200","gridViewWidth":"250","titleInsideGrid":"1","showPhotosInJustifiedView":"0","rowHeight":"205","maxRowHeight":"300","margin":"30","lastRow":"justify","descriptionInsideGrid":"0","orderby":"creationDate","show_content":"2","titleTruncation":"22","descriptionTruncation":"100","detactLocation":"0","defaultLocationDistance":"1000","listDescription":"1","gridDescription":"0","itemCountPerPage":"6","nomobile":"0","name":"sitenews.browse-news-sitenews"}';
                $searchWidgetParams = '{"title":"SEARCH","titleCount":true,"viewType":"vertical","showAllCategories":"1","locationDetection":"1","nomobile":"0","name":"sitenews.search-news-sitenews"}';
                $newLinkWidgetExist = 1;
                $db->query("INSERT IGNORE INTO `engine4_siteadvsearch_contents` (`module_name`, `resource_type`, `resource_title`, `listingtype_id`, `widgetize`, `content_tab`, `main_search`, `order`, `file_id`, `default`, `enabled`) VALUES ('sitenews', 'sitenews_news', 'News', '0', '0', '0', '1', '999', '0', '1', '1');");
                break;
    ...
    public function setContentItems($moduleName = null, $resourceType = null, $resourceId = null) {
        ...
        if (!empty($resourceType) && !empty($resourceId)) {
            if ($coreApi->getSetting($moduleName . '.fs.markers', 1) && $contentItem->featured) {
                echo '<i title="' . $view->translate('Featured') . '" class="seaocore_list_featured_label">' .$view->translate('Featured'). '</i>';
            }
    ...
    public function getCommonContentItem($contentItem, $params = array()) {
        ...
        if (in_array('category', $showOptions) && $contentItem->getCategory()) {
            $url = $contentItem->getCategory()->getHref();
            echo '<div class="seaocore_browse_list_info_date seaocore_txt_light">' . '<a href="' . $url . '">' . $view->translate($contentItem->getCategory()->getTitle(true)) . '</a>' . '</div>';
        }
        $user = Engine_Api::_()->user()->getUser($contentItem->owner_id);
        ...
        if (!empty($contentItem->price) && $coreApi->getSetting($moduleName . '.price.field', 1)) {
            ...
            echo $this->getPriceWithCurrency($contentItem->price) . ' ';
        }
        ...
        if (empty($resourceTypekey)) {
            if (!empty($slug_plural)) {
                ...
                if (!empty($isModuleEnabled)) {
                    ...
                    elseif ($params['module_name'] == 'sitecrowdfunding')
    ...
    public function getPriceWithCurrency($price) {
        ...
        if (Engine_Api::_()->hasModuleBootstrap('sitemulticurrency') && Engine_Api::_()->getDbtable('modules', 'core')->isModuleEnabled('sitemulticurrency') && Engine_Api::_()->getDbtable('modules', 'sitemulticurrency')->isModuleEnable('siteadvsearch')) {
    ...
    public function getEnabled($listingTypeId, $enabled) {
        ...
        $tableName = $contentTable->info('name');
        $select = $contentTable->select()
                ->setIntegrityCheck(false)
                ->from($tableName)
                ->where($tableName . '.resource_type = ?', "sitereview_listingtype_$listingTypeId");
        $row = $select->query()->fetchColumn();
        if (!empty($row)) {
            $db->query("UPDATE `engine4_siteadvsearch_contents` SET `content_tab` = '" . $enabled . "', `main_search` = '" . $enabled . "', `enabled` = '" . $enabled . "' WHERE `engine4_siteadvsearch_contents`.`resource_type` = 'sitereview_listingtype_" . $listingTypeId . "' LIMIT 1 ;");
        }
    }
