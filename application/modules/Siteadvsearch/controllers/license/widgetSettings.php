...
if (!empty($is_enabled)) {
    ...
    //Make the layout work for Sitenews
    
    $containerCount = 0;
    $widgetCount = 0;
    $page_id = $db->select()
            ->from('engine4_core_pages', 'page_id')
            ->where('name = ?', 'siteadvsearch_index_browse-page_sitenews_news')
            ->limit(1)
            ->query()
            ->fetchColumn();
    if (!$page_id) {
    ...
    $select
            ->from('engine4_siteadvsearch_contents')
            ->where('resource_type = ?', 'sitenews_news');
    $sitenews_news = $select->query()->fetchObject();
    if (!$sitenews_news) {
