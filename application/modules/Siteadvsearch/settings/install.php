...
class Siteadvsearch_Installer extends Engine_Package_Installer_Module {
    ...
    function onInstall() {
        ...
        $select
                ...
                ->where('enabled = ?', 1);
        $is_sitecrowdfunding_object = $select->query()->fetchObject();
        if ($is_sitecrowdfunding_object) {
            $engine4siteadvsearchTable = $db->query("SHOW TABLES LIKE 'engine4_siteadvsearch_contents'")->fetch();
            if ($engine4siteadvsearchTable) {
                $select = new Zend_Db_Select($db);
                $select
        ...
        //Add row if Socialengine Magento Integration plugin is installed
        $select = new Zend_Db_Select($db);
        $select
                ->from('engine4_core_modules')
                ->where('name = ?', 'sitemagento')
                ->where('enabled = ?', 1);
        $is_sitemagento_object = $select->query()->fetchObject();
        if ($is_sitemagento_object) {
            $engine4siteadvsearchTable = $db->query("SHOW TABLES LIKE 'engine4_siteadvsearch_contents'")->fetch();
            if ($engine4siteadvsearchTable) {
                $select = new Zend_Db_Select($db);
                $select
                        ->from('engine4_siteadvsearch_contents')
                        ->where('resource_type = ?', 'sitemagento');
                $sitemagento_isActivate_object = $select->query()->fetchObject();
                if (!$sitemagento_isActivate_object) {
                    $db->query("INSERT IGNORE INTO `engine4_siteadvsearch_contents` ( `module_name`, `resource_type`, `resource_title`, `listingtype_id`, `widgetize`, `content_tab`, `main_search`, `order`, `file_id`, `default`, `enabled`) VALUES ( 'sitemagento', 'sitemagento', 'Stores', '0', '1', '1', '1', '999', '', '1', '1');");
                }
            }
        }
        //Add column if news plugin is installed
        ...
        if ($pluginInstalled) {
            $engine4siteadvsearchTable = $db->query("SHOW TABLES LIKE 'engine4_siteadvsearch_contents'")->fetch();
            if ($engine4siteadvsearchTable) {
                $isModExist = $db->query("SELECT * FROM `engine4_siteadvsearch_contents` WHERE `module_name` LIKE 'sitenews' LIMIT 1")->fetch();
                if (empty($isModExist)) {
                    $db->query("INSERT IGNORE INTO `engine4_siteadvsearch_contents` (`module_name`, `resource_type`, `resource_title`, `widgetize`, `content_tab`, `main_search`, `order`, `file_id`, `default`, `enabled`) VALUES ('sitenews', 'sitenews_news', 'News', '0', '0', '1', '999', '0', '1', '1');");
                }
            }
        }
        $page_id = $db->select()
                ...
                ->fetchColumn();
        if (!empty($page_id)) {
            $db->query("DELETE FROM engine4_core_content WHERE `name`='core.search-mini' and type='widget' and page_id=$page_id");
        }
        parent::onInstall();
    }
