...
class Video_IndexController extends Core_Controller_Action_Standard {
    ...
    public function browseAction() {
        ...
        $this->view->form = $form = new Sitemobile_modules_Video_Form_Filter_Search();
        ...
        $items_count = 9; //(int) Engine_Api::_()->getApi('settings', 'core')->getSetting('video.page', 12);
    ...
    public function createAction() {
        ...
        // Upload video
//    if (isset($_GET['ul']) ||
//            (isset($_FILES['Filedata']) && !empty($_FILES['Filedata']['name']))) {
//      return $this->_forward('upload-video', null, null, array('format' => 'json'));
//    }
        // Render only in case of browser
        if (!Engine_Api::_()->sitemobile()->isApp()) {
            $this->_helper->content
                    //->setNoRender()
                    ->setEnabled()
        ...
        if (!Engine_Api::_()->hasModuleBootstrap('sitevideo')) {
            ...
            $this->view->quota = $quota = Engine_Api::_()->authorization()->getPermission($viewer->level_id, 'sitevideo_video', 'sitevideo_max_allowed_video');
        }
        $this->view->current_count = $paginator->getTotalItemCount();


        if (Engine_Api::_()->sitemobile()->isApp() && ($this->view->current_count >= $this->view->quota) && !empty($this->view->quota)) {
            $this->view->notSuccessMessage = true;
            return $this->_forward('success', 'utility', 'core', array(
                        'redirect' => $this->_helper->url->url(array('action' => 'manage'), 'video_general', true),
        ...
        // Create form
        $this->view->form = $form = new Sitemobile_modules_Video_Form_Video();
        if (Engine_Api::_()->sitemobile()->isApp()) {
            ...
            $this->view->form->setAttrib('id', 'form_video_creation');
            Zend_Registry::set('setFixedCreationFormId', '#form_video_creation');
            $this->view->form->removeElement('upload');
            //$this->view->form->removeElement('cancel');
            //$this->view->form->removeDisplayGroup('buttons');
            $form->setTitle('');
        }
        //$form->url->removeAttrib('maxlength');
// 		$form->getElement('url')->setMaxlength('500');
...
//     return $this->_helper->redirector->gotoRoute(array('user_id' => $viewer->getIdentity(), 'video_id' => $video->getIdentity()), 'video_view', true);
        ...
        return $this->_forward('success', 'utility', 'core', array(
                    'redirect' => $this->_helper->url->url(array('user_id' => $viewer->getIdentity(), 'video_id' => $video->getIdentity()), 'video_view', true),
                    'messages' => array(Zend_Registry::get('Zend_Translate')->_('Your video has been created successfully.')),
        ));
    }
    ...
    public function deleteAction() {
        ...
        if (Engine_Api::_()->sitemobile()->isApp()) {
            Zend_Registry::set('setOnlyHeaderTitle', $form->getTitle());
            $form->setTitle('');
...
//     ));
    ...
    public function editAction() {
        ...
        return $this->_forward('success', 'utility', 'core', array(
                    'redirect' => $this->_helper->url->url(array('action' => 'manage'), 'video_general', true),
                    'messages' => array(Zend_Registry::get('Zend_Translate')->_('Your video has been edited successfully.')),
        ));
    }
    ...
    public function viewAction() {
        ...
        if ($video->status == 1) {
            ...
            $embedded = Engine_Api::_()->getApi('video', 'sitemobile')->getRichContent(true, array(), $video);
        }
