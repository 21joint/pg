 ...
 * @license    http://www.socialengine.com/license/
 */
class Sitemobile_modules_User_Form_Signup_Account extends Engine_Form {
    ...
    public function init() {
        ...
        $this->setTitle('Create Account')
                ->setAttrib('id', 'signup_account_form');
        ...
        if (isset($_SESSION['signup_email']) && isset($_POST['email']))
            $antispam = $_SESSION['signup_email'];
        else {
            $_SESSION['signup_email'] = $antispam = $this->generateRandomString(20);
        }
        // Element: email
        ...
        $this->$antispam->getDecorator('Description')->setOptions(array('placement' => 'APPEND'));
        $this->$antispam->getValidator('NotEmpty')->setMessage('Please enter a valid email address.', 'isEmpty');
        $this->$antispam->getValidator('Db_NoRecordExists')->setMessage('Someone has already registered this email address, please use another one.', 'recordFound');
        $this->$antispam->getValidator('EmailAddress')->getHostnameValidator()->setValidateTld(false);
        // Add banned email validator
        $bannedEmailValidator = new Engine_Validate_Callback(array($this, 'checkBannedEmail'), $this->$antispam);
        $bannedEmailValidator->setMessage("This email address is not available, please use another one.");
        $this->$antispam->addValidator($bannedEmailValidator);

        if (!empty($inviteSession->invite_email)) {
            $this->$antispam->setValue($inviteSession->invite_email);
        }
        ...
        // Element: captcha
        if ($settings->sitemobile_spam_signup && !Engine_Api::_()->sitemobile()->isApp()) {
