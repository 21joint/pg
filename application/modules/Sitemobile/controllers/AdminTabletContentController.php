 ...
 * @version    $Id: AdminTabletContentController.php 6590 2013-06-03 00:00:00Z SocialEngineAddOns $
 * @author     SocialEngineAddOns
 */
class Sitemobile_AdminTabletContentController extends Core_Controller_Action_Admin {
  ...
  public function init() {
    $this->_contentTable = Engine_Api::_()->getDbtable('tabletcontent', 'sitemobile');
    $this->_pageTable = Engine_Api::_()->getDbtable('tabletpages', 'sitemobile');
  ...
  public function indexAction() {
    ...
    $this->view->subNavigation = Engine_Api::_()->getApi('menus', 'core')->getNavigation('sitemobile_admin_main_content', array(), 'sitemobile_admin_main_content_tablet');
    // GET PAGE PARAM.
...
//      } else if ($pageRow->fragment) {
 ...
 public function updateAction()
  ...
  public function widgetAction() {
    ...
    if (isset($contentInfo[$name]['adminFormTablet']) && !empty($contentInfo[$name]['adminFormTablet'])) {

      if (is_string($contentInfo[$name]['adminFormTablet'])) { // Core_Form_Admin_Widget_*
        $formClass = $contentInfo[$name]['adminFormTablet'];
        Engine_Loader::loadClass($formClass);
        $this->view->form = $form = new $formClass(array('mode' => 'tablet'));
      } else if (is_array($contentInfo[$name]['adminFormTablet'])) {
        $this->view->form = $form = new Engine_Form($contentInfo[$name]['adminFormTablet']);
      } else {
...
//    }
  ...
  public function manageWidgetAction() {
    ...
    $form->setTitle('Site-wide Tablet Widget Settings');
    $form->setDescription('Here, you can manage the settings which will affect all the widgets of Tablet Layouts.');
    if ($this->getRequest()->isPost() && $form->isValid($this->getRequest()->getPost())) {
      $values = $form->getValues();
...
sitebusinessoffer_index_browse', 'sitebusinesspoll_index_browse', 'sitebusinessreview_index_browse', 'sitebusinessvideo_index_browse', 'sitebusinessevent_index_browse', 'sitebusinessdocument_index_browse', 'sitebusinessmember_index_browse', 'sitebusinessmusic_playlist_browse', 'sitegroup_sitemobile-album_browse', 'sitegroupnote_index_browse', 'sitegroupoffer_index_browse', 'sitegrouppoll_index_browse', 'sitegroupreview_index_browse', 'sitegroupvideo_index_browse', 'sitegroupevent_index_browse', 'sitegroupdocument_index_browse', 'sitegroupmember_index_browse', 'sitegroupmusic_playlist_browse');
  ...
  public function insertWidget($widgetName, $value, $order) {
    ...
    $db = Engine_Db_Table::getDefaultAdapter();
    $db->insert('engine4_sitemobile_tablet_content', array(
