...
<script type="text/javascript">
  ...
  var saveChanges = function () {
    ...
    $('.admin_content_buildable').each(function (__ind, element) {
      ...
      if ($type(contentByName[elData.name]) && $type(contentByName[elData.name].module)) {
        elData.module = contentByName[elData.name].module;
      }

      data.push(elData);
    });

    var url = '<?php echo $this->url(array('action' => 'update', 'controller' => 'tablet-content', 'module' => 'sitemobile'), 'admin_default', true)?>';
  ...
  var openWidgetParamEdit = function (name, element) {
    ...
    var url = '<?php echo $this->url(array('action' => 'widget', 'controller' => 'tablet-content', 'module' => 'sitemobile'), 'admin_default', true)?>';
  ...
  var saveCurrentPageInfo = function (formElement) {
    var url = '<?php echo $this->url(array('action' => 'save', 'controller' => 'tablet-content', 'module' => 'sitemobile'), 'admin_default', true)?>';
  ...
  var changeCurrentLayoutType = function (type) {
    ...
    $H({'top': 'before', 'bottom': 'after'}).each(function (placement, currentType) {
      ...
      var newTbody = new Element('tbody', {}).inject(newTable);

      var newTr = new Element('tr', {}).inject(newTbody);
  ...
  var deleteCurrentPage = function () {
    ...
    var url = '<?php echo $this->url(array('action' => 'delete', 'controller' => 'tablet-content', 'module' => 'sitemobile'), 'admin_default', true)?>';
    var request = new Request.JSON({
      'url': url,
      'data': {
        'format': 'json',
...
<!--<h3><?php echo $this->translate('Tablet Layout Editor'); ?></h3>-->
<p>
  <?php echo $this->translate('Use the layout editor to decide what content appears on specific tablet pages or in your site-wide tablet header and footer. Click and drag the colored "blocks" to arrange the content you want on each page. Drag blocks to and from the "Available Blocks" area to add or remove them from your page. Use "HTML Blocks" if you want to drop in raw HTML or other content.'); ?>
</p>
<br/>
<div class="tip">
  <span>
   <?php echo $this->htmlLink(array('route' => 'admin_default', 'module' => 'sitemobile', 'controller' => 'tablet-content', 'action' => 'manage-widget'),
     $this->translate('Click here'), array()) ?> to manage the settings which will affect all the widgets of Tablet Layouts.
  </span>
</div>
...
<div id='admin_cms_wrapper'>
  ...
  <div class="admin_layoutbox_menu">
    <ul>
      <li class="admin_layoutbox_menu_generic" id="admin_layoutbox_menu_openpage">
        <div class="admin_layoutbox_menu_wrapper_generic admin_layoutbox_menu_pages_wrapper"
             id="admin_layoutbox_menu_pages_wrapper">
      ...
      <li class="admin_layoutbox_menu_generic" id="admin_layoutbox_menu_newpage">
        <div class="admin_layoutbox_menu_wrapper_generic admin_layoutbox_menu_makepage_wrapper"
             id="admin_layoutbox_menu_makepage_wrapper">
          <div class="admin_layoutbox_menu_makepage">
            <form class="admin_layoutbox_menu_makepage_form"
                  action="<?php echo $this->url(array('action' => 'create')) ?>" method="post">
              <div>
              ...
              <div class="admin_layoutbox_menu_makepage_form_name">
                ...
                <span><?php echo $this->translate("Duplicate Existing Page?") ?></span>
                <!-- Add Duplicate Layout Drop Down Box -->
                <select name="duplicate">
                  <option value="null" selected="selected">No, create a blank page</option>
                  <?php foreach ($this->pageListAssoc as $pageType => $pageListSub): ?>
                    <?php foreach ($pageListSub as $page): ?>
                      <?php if ($page->displayname != "Site Header" && $page->displayname != "Site Footer"): ?>
                        <option value="<?php echo $page->page_id ?>"><?php echo $page->displayname ?></option>
                      <?php endif; ?>
                    <?php endforeach; ?>
                  <?php endforeach; ?>
                </select>
                <!-- End of Duplicate Page Drop Down -->
  ...
  <div class="admin_layoutbox_wrapper">
    ...
    <div class="admin_layoutbox_sub_menu">
      <h3>
        <?php echo $this->translate('Tablet Page Block Placement') ?>
      </h3>
      <ul>
        ...
        <?php if (substr($this->pageObject->name, 0, 6) !== 'header' && substr($this->pageObject->name, 0, 6) !== 'footer'): ?>
          <li class="admin_layoutbox_menu_generic" id="admin_layoutbox_menu_pageinfo">
            <div class="admin_layoutbox_menu_wrapper_generic admin_layoutbox_menu_editinfo_wrapper"
                 id="admin_layoutbox_menu_editinfo_wrapper">
              ...
              <span>
                ...
                <div class="admin_layoutbox_menu_editinfo_submit">
                  <button
                    onclick="saveCurrentPageInfo($('admin_content_pageinfo')); return false;"><?php echo $this->translate("Save Changes") ?></button>
                  or <a href="javascript:void(0);"
                        onClick="switchPageMenu(new Event(event), $(this));"><?php echo $this->translate("cancel") ?></a>
                </div>
              </div>
            </div>
            <a href="javascript:void(0);"
               onClick="switchPageMenu(new Event(event), $(this));"><?php echo $this->translate("Edit Page Info") ?></a>
          </li>
          <li class="admin_layoutbox_menu_generic" id="admin_layoutbox_menu_editcolumns">
            <div class="admin_layoutbox_menu_wrapper_generic admin_layoutbox_menu_columnchoices_wrapper"
                 id="admin_layoutbox_menu_columnchoices_wrapper">
              <div class="admin_layoutbox_menu_columnchoices">
                ...
                <div class="admin_layoutbox_menu_columnchoices_cancel">
                  Or, <a href="javascript:void(0);"
                         onClick="switchPageMenu(new Event(event), $(this));"><?php echo $this->translate("cancel") ?></a> <?php echo $this->translate("and keep your current layout.") ?>
                </div>
              </div>
            </div>
            <a href="javascript:void(0);"
               onClick="switchPageMenu(new Event(event), $(this));"><?php echo $this->translate("Edit Columns") ?></a>
          </li>
        <?php endif; ?>
      </ul>
    </div>
    ...
    <?php if (substr($this->pageObject->name, 0, 6) !== 'header' && substr($this->pageObject->name, 0, 6) !== 'footer' && ($this->pageObject->layout == 'default' || $this->pageObject->layout == 'default-simple' || $this->pageObject->layout == '')): ?>
      ...
      <div class='admin_layoutbox'>
        <div
          class='admin_layoutbox_header<?php echo(empty($this->pageObject->layout) || $this->pageObject->layout == 'default' ? '' : ' admin_layoutbox_header_hidden') ?>'>
          <span>
            <?php echo $this->translate('Global Header') ?>
            <span>
              <a href="javascript:void(0);"
                 onclick="toggleGlobalLayout($(this).getParent('div.admin_layoutbox_header'));">
        ...
        try {
          ...
          foreach ((array)@$this->contentStructure as $structOne):
            ...
            ?>
            <table id="admin_content_<?php echo $structOne['identity'] ?>"
                   class="admin_content_block admin_content_buildable admin_content_<?php echo $structOne['type'] . '_' . $structOne['name'] ?>">
              <tbody>
              <tr>
                <script type="text/javascript">
                ...
                foreach ((array)@$structOne['elements'] as $structTwo):
                  ...
                  ?>
                  <td id="admin_content_<?php echo $structTwo['identity'] ?>"
                      class="admin_content_column admin_content_buildable admin_content_<?php echo $structTwo['type'] . '_' . $structTwo['name'] ?>">
                    <script type="text/javascript">
                      $(window).on('domready', function () {
                    ...
                    <ul class="admin_content_sortable">
                      ...
                      <?php if (empty($structThreeInfo)): // Missing widget         ?>
                        <li id="admin_content_<?php echo $structThree['identity'] ?>"
                            class="disabled admin_content_cell admin_content_buildable admin_content_draggable admin_content_<?php echo $structThree['type'] . '_' . $structThree['name'] ?><?php if (!empty($structThreeInfo['special'])) echo ' htmlblock' ?>">
                          <?php echo $this->translate('Missing widget: %s', $structThree['name']) ?>
                          <span class="open"></span>
                          <span class="remove"><a href='javascript:void(0)'
                                                  onclick="removeWidget($(this));">x</a></span>
                        </li>
                      <?php elseif (empty($structThreeInfo['canHaveChildren'])): ?>
                        <li id="admin_content_<?php echo $structThree['identity'] ?>"
                            class="admin_content_cell admin_content_buildable admin_content_draggable admin_content_<?php echo $structThree['type'] . '_' . $structThree['name'] ?><?php if (!empty($structThreeInfo['special'])) echo ' htmlblock' ?>">
                          <?php echo $this->translate($this->contentByName[$structThree['name']]['title']) ?>
                          <span class="open">
                                |
                                <a href='javascript:void(0);'
                                   onclick="openWidgetParamEdit('<?php echo $structThree['name'] ?>', $(this).getParent('li.admin_content_cell')); (new Event(event).stop()); return false;">
                      ...
                      <?php else: ?>
                        <!-- tabbed widgets -->
                        <li id="admin_content_<?php echo $structThree['identity'] ?>"
                            class="admin_content_cell admin_content_buildable admin_content_draggable admin_layoutbox_widget_tabbed_wrapper admin_content_<?php echo $structThree['type'] . '_' . $structThree['name'] ?>">
                              <span class="admin_layoutbox_widget_tabbed_top">
                                <?php echo $this->translate('Tab / Column Container') ?>
                                <span class="open">
                                  <a href='javascript:void(0);'
                                     onclick="openWidgetParamEdit('<?php echo $structThree['name'] ?>', $(this).getParent('li.admin_content_cell')); (new Event(event).stop()); return false;">
                                    <?php echo $this->translate('edit') ?>
                          ...
                          <div class="admin_layoutbox_widget_tabbed">
                            <ul
                              ...
                              <?php if (empty($structFourInfo)): ?>
                                <li id="admin_content_<?php echo $structFour['identity'] ?>"
                                    class="disabled admin_content_cell admin_content_buildable admin_content_draggable admin_content_<?php echo $structFour['type'] . '_' . $structFour['name'] ?>">
                                  <?php echo $this->translate('Missing widget: %s', $structFour['name']) ?>
                                  <span></span>
                                </li>
                              <?php else: ?>
                                <li id="admin_content_<?php echo $structFour['identity'] ?>"
                                    class="admin_content_cell admin_content_buildable admin_content_draggable admin_content_<?php echo $structFour['type'] . '_' . $structFour['name'] ?>">
                                  <?php echo $this->translate($this->contentByName[$structFour['name']]['title']) ?>
                                  <span class="open"> | <a href='javascript:void(0);'
                                                           onclick="openWidgetParamEdit('<?php echo $structFour['name'] ?>', $(this).getParent('li.admin_content_cell')); (new Event(event).stop()); return false;">edit</a></span>
                                  <span class="remove"><a href='javascript:void(0)'
                                                          onclick="removeWidget($(this));">x</a></span>
                                </li>
                              <?php endif; ?>
        ...
        <div
          class='admin_layoutbox_footer<?php echo(empty($this->pageObject->layout) || $this->pageObject->layout == 'default' ? '' : ' admin_layoutbox_footer_hidden') ?>'>
          <span>
            <?php echo $this->translate('Global Footer') ?>
            <span>
              <a href="javascript:void(0);"
                 onclick="toggleGlobalLayout($(this).getParent('div.admin_layoutbox_footer'));">
                <?php echo(empty($this->pageObject->layout) || $this->pageObject->layout == 'default' ? "({$this->translate('hide on this page')})" : "({$this->translate('show on this page')})") ?>
              </a>
            </span>
          </span>
      ...
      <div class='admin_layoutbox'>
        ...
        <?php else: ?>
          ...
          foreach ((array)@$this->contentStructure as $structOne):
            ...
            ?>
            <table id="admin_content_<?php echo $structOne['identity'] ?>"
                   class="admin_content_block admin_content_block_headerfooter admin_content_buildable admin_content_<?php echo $structOne['type'] . '_' . $structOne['name'] ?>">
              <tbody>
              <tr>
                  ...
                  <ul class="admin_content_sortable">
                    ...
                    <?php else: ?>
                      <li id="admin_content_<?php echo $structThree['identity'] ?>"
                        ...
                        <?php echo $this->translate($this->contentByName[$structThree['name']]['title']) ?>
                        <span class="open"> | <a href='javascript:void(0);'
                                                 onclick="openWidgetParamEdit('<?php echo $structThree['name'] ?>', $(this).getParent('li.admin_content_cell')); (new Event(event).stop()); return false;">edit</a></span>
                        <span class="remove"><a href='javascript:void(0)' onclick="removeWidget($(this));">x</a></span>
                      </li>
                    <?php endif;
                      ?>
                  ...
                  <ul class="admin_content_sortable">
                    ...
                    <?php else: ?>
                      <li id="admin_content_<?php echo $structThree['identity'] ?>"
                        ...
                        <span class="open"> | <a href='javascript:void(0);'
                                                 onclick="openWidgetParamEdit('<?php echo $structThree['name'] ?>', $(this).getParent('li.admin_content_cell')); (new Event(event).stop()); return false;"><?php echo $this->translate("edit") ?></a></span>
                        <span class="remove"><a href='javascript:void(0)' onclick="removeWidget($(this));">x</a></span>
                      </li>
                    <?php endif;
                      ?>
  ...
  <div class="admin_layoutbox_pool_wrapper">
    <h3><?php echo $this->translate("Tablet Available Blocks") ?></h3>
    <div class='admin_layoutbox_pool'>
      <div id='stock_div'></div>
      <ul id='column_stock'>
        <?php foreach ($this->contentAreas as $category => $categoryAreas): ?>
          <li>
            <div class="admin_layoutbox_pool_category_wrapper"
                 onclick="$(this); $(this).getElement('.admin_layoutbox_pool_category_show').toggle(); $(this).getElement('.admin_layoutbox_pool_category_hide').toggle(); this.getParent('li').getElement('ul').style.display = ( this.getParent('li').getElement('ul').style.display == 'none' ? '' : 'none' );">
            ...
            <ul class='admin_content_sortable admin_content_stock_sortable'>
              <?php foreach ($categoryAreas as $info):
                ...
                <li class="<?php echo $class ?>"
                    title="<?php echo $this->escape($info['description']) ?>"<?php if ($onmousedown): ?> onmousedown="<?php echo $onmousedown ?>"<?php endif; ?>>
                  <?php echo $this->translate($info['title']) ?>
                  <span class="open"> | <a href='javascript:void(0);'
                                           onclick="openWidgetParamEdit('<?php echo $info['name'] ?>', $(this).getParent('li.admin_content_cell')); (new Event(event).stop()); return false;"><?php echo $this->translate("edit") ?></a></span>
                  <span class="remove"><a href='javascript:void(0);' onclick="removeWidget($(this));">x</a></span>
                </li>
                <?php /* //else: ?>
