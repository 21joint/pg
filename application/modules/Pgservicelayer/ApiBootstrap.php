...
class Pgservicelayer_ApiBootstrap
{
  ...
  protected function _initModules()
  {
    ...
    $default = 'pgservicelayer';

    // Prepare data
    $enabledModuleNames = Engine_Api::_()->getDbtable('modules', 'core')
  ...
  protected function getModuleBootstrap()
  {
    if (self::$_bootstrap === null) {
      ...
      self::$_bootstrap = new Pgservicelayer_ModulesBootstrap(
        Engine_Application::getInstance()
      );
    }
  ...
  protected function _initCache()
  {
    ...
    if (file_exists($file)) {
      ...
      if (is_writable(APPLICATION_PATH.'/temporary/cache')
        || (
          !@is_dir(APPLICATION_PATH.'/temporary/cache')
          && @mkdir(
            APPLICATION_PATH.'/temporary/cache', 0777, true
          )
        )
      ) {
        // Auto default config
    ...
    if (in_array(
      $backend, array('Engine_Cache_Backend_Apc', 'Engine_Cache_Backend_Redis')
    )
    ) {
      $cache = Engine_Cache::factory(
        $frontend, $backend, array(), array(), false, true
      );
    } else {
    ...
    Zend_Controller_Action_HelperBroker::getStack()->offsetSet(
      -1, new Engine_Controller_Action_Helper_Cache()
    );
          ...
          try {
            $log->addWriter(
              new Zend_Log_Writer_Stream(
                APPLICATION_PATH.'/temporary/log/main.log'
              )
            );
          } catch (Exception $e) {
            // Check directory
            if (!@is_dir(APPLICATION_PATH.'/temporary/log')
              && @mkdir(
                APPLICATION_PATH.'/temporary/log', 0777, true
              )
            ) {
              $log->addWriter(
                new Zend_Log_Writer_Stream(
  ...
  protected function _initManifest()
  {
    ...
    $data = array();
    $path = APPLICATION_PATH.DIRECTORY_SEPARATOR."application"
      .DIRECTORY_SEPARATOR."modules";
    $enabledModuleNames = Engine_Api::_()->getDbtable('modules', 'core')
      ->getEnabledModuleNames();
    // Modified existing routes for APIs
    $file = $path.DIRECTORY_SEPARATOR.'Pgservicelayer'
      .'/settings/apiroutes.php';
    ...
    foreach ($enabledModuleNames as $module) {
      ...
      if (file_exists($file)) {
        ...
        if (isset($temp['items'])
          && (!in_array(
            $apiModName, $this->_notIncludeItemType
          ))
        ) {
          $data[$apiModName]['items'] = $temp['items'];
        }

        if (isset($temp['routes'])
          && (is_array($this->_notIncludeRoute)
            && !in_array($apiModName, $this->_notIncludeRoute))
        ) {
          // Edit the SocialEngine default routes.
          if (($apiModName == 'sitereview') && isset($temp['routes'])
            && (is_array($this->_notIncludeRoute)
              && !in_array(
                $apiModName, $this->_notIncludeRoute
              ))
          ) {
            foreach ($temp['routes'] as $key => $value) {
              if (isset($value['route']) && !empty($value['route'])
                && in_array(
                  $value['route'], $this->_notIncludeListingRoute
                )
              ) {
                $temp['routes'][$key]['route'] = 'se'.@ltrim(
                    $value['route'], "/"
                  );
              }
            }
          ...
          // Edit the SocialEngine default routes.
          if (($apiModName == 'siteevent') && isset($temp['routes'])
            && (is_array($this->_notIncludeRoute)
              && !in_array(
                $apiModName, $this->_notIncludeRoute
              ))
          ...
          if (($apiModName == 'sitegroup') && isset($temp['routes'])
            && (is_array($this->_notIncludeRoute)
              && !in_array(
                $apiModName, $this->_notIncludeRoute
              ))
      ...
      if (!empty($apiRoutes[$apiModName])) {
        ...
        $data[$apiModName]['routes'] = array_merge(
          $data[$apiModName]['routes'], $routes
        );
      }
    }
  ...
  protected function _initFrontController()
  {
    ...
    Zend_Controller_Action_HelperBroker::addPath(
      "Engine/Controller/Action/Helper/", 'Engine_Controller_Action_Helper'
    );

    Zend_Controller_Action_HelperBroker::addPath(
      $modulePath."Authorization/Controller/Action/Helper/",
      'Authorization_Controller_Action_Helper'
    );

    Zend_Controller_Action_HelperBroker::addPath(
      $modulePath."Core/Controller/Action/Helper/",
    ...
    if (Engine_Api::_()->getDbtable('modules', 'core')->isModuleEnabled(
      ...
      Zend_Controller_Action_HelperBroker::addHelper(
        new Primemessenger_Controller_Action_Helper_PrimeMessengers()
      );
    }

    Zend_Controller_Action_HelperBroker::addPath(
      $modulePath."Pgservicelayer/Controller/Action/Helper/",
      'Pgservicelayer_Controller_Action_Helper'
    );
  ...
  protected function _loadLicense()
  {
    ...
    if (is_file(
      APPLICATION_PATH
      .'/application/modules/Sitereview/controllers/license/license.php'
    )
    ) {
...
//            include APPLICATION_PATH . '/application/modules/Siteeventticket/controllers/license/license.php';
  ...
  protected function _initRouter()
  {
    ...
    $defaultAdminRoute
      = Engine_Controller_Router_Route_ControllerPrefix::getInstance(
    ...
    foreach ($manifest as $module => $config) {
      ...
      $getEnabledModulesArray = (strstr(
          $_SERVER['REQUEST_URI'], '/api/rest/advancedactivity/feeds?'
        )
        || strstr($_SERVER['REQUEST_URI'], 'api/rest/notifications?')
        || strstr($_SERVER['REQUEST_URI'], '/api/rest/search?')
        || strstr($_SERVER['REQUEST_URI'], '/api/rest/activity/'))
        ? Engine_Api::_()->getDbtable('modules', 'core')->getEnabledModuleNames(
        )
        : array("activity", "advancedactivity", "album", "blog", "classified",
                "core", "event", "forum", "group", "message", "music", "poll",
                "user", "video", "sitetagcheckin", "messages", "siteevent",
                "siteeventrepeat", "sitereview", "sitepage", "sitepagevideo",
                "sitepagereview", "sitepagealbum", "sitehashtag", "sitegroup",
                "sitegroupalbum", "sitegroupvideo", "sitegroupreview",
                "sitegroupmember", "sitegroupoffer", "sitegroupintegration",
                "siteusercoverphoto", "sitecontentcoverphoto", "sitereaction",
                "suggestion", "nestedcomment", "siteeventticket", "sitevideo",
                "sitestore", "sitestoreproduct", "sitestoreoffer",
                "sitestorereview", "communityad", "sitemember",
                "pgservicelayer");
  ...
  protected function _initTranslate()
  {
    ...
    if (empty($_SERVER['REQUEST_METHOD'])
      || !in_array(
        $_SERVER['REQUEST_METHOD'], array('POST', 'PUT')
      )
    ) {
      return;
    }
    ...
    if (APPLICATION_ENV == 'development') {
      ...
      $log->addWriter(
        new Zend_Log_Writer_Stream(
          APPLICATION_PATH.'/temporary/log/translate.log'
        )
      );
    } else {
      $log->addWriter(new Zend_Log_Writer_Null());
    ...
    if ($translationAdapter == 'array') {
      ...
      // Get List of Folders
      $languageFolders = array_filter(
        glob($languagePath.DIRECTORY_SEPARATOR.'*'), 'is_dir'
      );
      // Look inside Folders for PHP array
      $locale_array = array();
      foreach ($languageFolders as $folder) {
        // Get Locale code
        $locale_code = str_replace(
          $languagePath.DIRECTORY_SEPARATOR, "", $folder
        );
      ...
      $translate = new Zend_Translate(
        array(
          'adapter' => 'array',
          'content' => $languagePath.DIRECTORY_SEPARATOR.$locale_array[0]
            .DIRECTORY_SEPARATOR.$locale_array[0].'.php',
      ...
      if ($language_count > 1) {
        for ($i = 1; $i < $language_count; $i++) {
          $translate->addTranslation(
            array(
              'content' => $languagePath.DIRECTORY_SEPARATOR.$locale_array[$i]
                .DIRECTORY_SEPARATOR.$locale_array[$i].'.php',
  ...
  protected function _initSession()
  {
    ...
    Zend_Session::setSaveHandler(
      new $saveHandler($config['saveHandler']['params'])
    );
