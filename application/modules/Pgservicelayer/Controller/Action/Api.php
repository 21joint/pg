...
abstract class Pgservicelayer_Controller_Action_Api
  extends Siteapi_Controller_Action_Standard
{
  ...
  public function init()
  {
    if ($this->isApiRequest()) {
      ...
      $user_id = Engine_Api::_()->getApi('oauth', 'pgservicelayer')
        ->validateOauthToken();
    ...
    $timezone = Engine_Api::_()->getApi(
      'settings', 'core'
    )->core_locale_timezone;
    $viewer = Engine_Api::_()->user()->getViewer();
    $defaultLocale
      =
    $defaultLanguage = Engine_Api::_()->getApi('settings', 'core')->getSetting(
      'core.locale.locale', 'en_US'
    );
    ...
    $view->addHelperPath(
      APPLICATION_PATH.'/application/modules/Sitemailtemplates/View/Helper',
      'Sitemailtemplates_View_Helper'
    );
  ...
  public function isApiRequest()
  {
    ...
    if (empty($consumerKey)) {
      $consumerKey = $request->getParam(
        "oauth_consumer_key", $request->getParam("oauth-consumer-key")
      );
    }
    if (empty($consumerKey)) {
      $consumerKey = $request->getServer(
        "HTTP_POSTMAN_TOKEN", $request->getHeader("HTTP_POSTMAN_TOKEN")
      );
    }
    if (!empty($consumerKey)) {
      return true;
...
//        }
  ...
  {
    return Engine_Api::_()->getApi('oauth', 'pgservicelayer')
      ->isConsumerValidOnly();
  }
  ...
  public function sendResponse()
  {
    ...
    if (empty($accept) || strstr(strtolower($accept), "json")
      || $this->isApiRequest()
    ) {
      header("Content-Type: application/json");
    }
    ...
    if ((($moduleName == 'siteevent' || $moduleName == 'sitereview')
      ...
      || ($moduleName == 'sitegroupmember' && $controllerName == 'member'
        && $actionName == 'join')
    ) {
  ...
  public function dispatch($action)
  {
    ...
    if (empty($this->_noDispatched) && !empty($action)
...
//        $this->_helper->notifyPostDispatch();
    ...
    if (isset($message) && !empty($message)) {
      if (is_array($message)) {
        foreach ($message as $key => $value) {
          $messages[$key] = Engine_Api::_()->getApi('Core', 'siteapi')
            ->translate($value);
        }
        $message = $messages;
      } else {
  ...
  public function requireSubject()
  {
    ...
    $resourceType = Engine_Api::_()->sdparentalguide()->mapPGGResourceTypes(
      $resourceType
    );
  ...
  public function validateParams($params = array())
  {
    ...
    if (!empty($invalidParams)) {
      $this->respondWithError(
        "invalid_parameters", sprintf(
          $this->translate("Extra parameters detected. %s"),
  ...
  public function validatePermission($permission, $viewer = null,
    $subject = null
  ) {
    if (null === $viewer) {
      $viewer = Engine_Api::_()->user()->getViewer();
