...
class Pgservicelayer_Model_DbTable_Files extends Storage_Model_DbTable_Files
{
  ...
  public function setPhoto($photo)
  {
        ...
        if ($photo instanceof Core_Model_Item_Abstract
          ...
          if (is_array($photo) && !empty($photo['tmp_name'])) {
            ...
            if (is_string($photo) && file_exists($photo)) {
              ...
              throw new User_Model_Exception(
                'invalid argument passed to setPhoto'
              );
            }
          }
        }
    ...
    $base = rtrim(
      substr(basename($fileName), 0, strrpos(basename($fileName), '.')), '.'
    );
    ...
    $usingLessVersion = Engine_Api::_()->seaocore()->usingLessVersion(
      'core', '4.8.9'
    );
    ...
    try {
      ...
      @unlink($iSquarePath);
      if ($e->getCode()
        == Storage_Model_DbTable_Files::SPACE_LIMIT_REACHED_CODE
      ) {
  ...
  public function createFile($file, $params)
  {
    ...
    if (!empty($params['user_id'])
      && null != ($user = Engine_Api::_()->getItem(
        'user', $params['user_id']
      ))
    ) {
    ...
    if (null !== $user_id && null !== $level_id) {
      $space_limit = (int)Engine_Api::_()->authorization()->getPermission(
        $level_id, 'user', 'quota'
      );
      ...
      if ($space_limit > 0 && $space_limit < ($space_used + $space_required)) {
        throw new Engine_Exception(
          "File creation failed. You may be over your ".
  ...
  public function uploadImage($inputStream)
  {
    ...
    $file = APPLICATION_PATH.DIRECTORY_SEPARATOR.'temporary'.DIRECTORY_SEPARATOR
    ...
    if ($function == 'imagejpeg' && null !== $quality) {
      ...
      $result = $function(
        $image, $file, round(abs(($quality - 100) / 11.111111))
      );
    } else {
  ...
  public function updatePhotoParent($file_id, Core_Model_Item_Abstract $parent)
  {
    ...
    $this->update(
      array('parent_type' => $parent->getType(),
            'parent_id'   => $parent->getIdentity(), 'user_id' => $user_id),
      array(
