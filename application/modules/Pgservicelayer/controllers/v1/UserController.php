...
class Pgservicelayer_UserController extends Pgservicelayer_Controller_Action_Api
{
  ...
  public function rankingAction()
  {
    ...
    $contributionRangeType = $this->getParam(
      "contributionRangeType", "Overall"
    );
...
//            ->where("enabled = ?", 1)
    ...
    if (!empty($expert)) {
      $select->where(
        "gg_expert_bronze_count > ? OR gg_expert_silver_count > ? OR gg_expert_gold_count > ? OR gg_expert_platinum_count > ?",
        0
      );
    }
    ...
    if (strtolower($contributionRangeType) == "week"
      || strtolower(
        $contributionRangeType
      ) == "month"
      || strtolower($contributionRangeType) == "today"
    ) {
      ...
      if ($orderBy == "contributionPoints") {
        ...
        $select->joinLeft(
          $creditsTableName,
          "$creditsTableName.user_id = $usersTableName.user_id OR $creditsTableName.user_id IS NULL",
          array(new Zend_Db_Expr(
                  "SUM($creditsTableName.credit_point) as gg_contribution",
                  new Zend_Db_Expr(
                    "COUNT($creditsTableName.credit_id) as gg_activities"
                  )
                ))
        )
          ->group("$creditsTableName.user_id");
        $select->where(
          "$creditsTableName.creation_date BETWEEN '$maxDate' AND '$currentDate'"
        );
      } elseif ($orderBy == 'reviewCount'
        || strtolower($orderBy) == 'reviews'
      ) {
        $listingTable = Engine_Api::_()->getDbtable('listings', 'sitereview');
        $listingTableName = $listingTable->info("name");
        $select->joinLeft(
          $listingTableName,
          "$listingTableName.owner_id = $usersTableName.user_id",
          array(new Zend_Db_Expr(
                  "COUNT($listingTableName.listing_id) as gg_review_count"
                ))
        );
        $select->where(
          "($listingTableName.creation_date BETWEEN '$maxDate' AND '$currentDate') OR $listingTableName.listing_id IS NULL"
        )
        ...
        $select->joinLeft(
          $membershipTableName,
          "$membershipTableName.resource_id = $usersTableName.user_id",
          array(new Zend_Db_Expr(
                  "COUNT($membershipTableName.user_id) as gg_followers_count"
                ))
        );
        $select->where(
          "$membershipTableName.creation_date BETWEEN '$maxDate' AND '$currentDate'"
        )
          ->where("$membershipTableName.active = ?", 1)
          ->group("$membershipTableName.resource_id");
      } elseif ($orderBy == 'questionCount'
        || strtolower($orderBy) == 'questions'
      ) {
        $questionsTable = Engine_Api::_()->getDbtable(
          'questions', 'ggcommunity'
        );
        $questionsTableName = $questionsTable->info("name");
        $select->joinLeft(
          $questionsTableName,
          "$questionsTableName.user_id = $usersTableName.user_id",
          array(new Zend_Db_Expr(
                  "COUNT($questionsTableName.question_id) as gg_question_count"
                ))
        );
        $select->where(
          "$questionsTableName.creation_date BETWEEN '$maxDate' AND '$currentDate'"
        )
    ...
    foreach ($paginator as $user) {
      $response['contentType'] = Engine_Api::_()->sdparentalguide()
        ->mapSEResourceTypes($user->getType());
