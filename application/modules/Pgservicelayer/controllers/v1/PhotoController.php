...
class Pgservicelayer_PhotoController
  ...
  public function postAction()
  {
    ...
    if (empty($_FILES['Filedata']['tmp_name']) && $image === false) {
      $this->respondWithValidationError(
        'parameter_missing', $this->translate("Photo missing in Filedata.")
      );
    }
    $avatarPhoto = ucfirst($this->getParam("photoType", ""));
    $table = Engine_Api::_()->getDbTable('files', 'pgservicelayer');
    ...
    try {
      ...
      if (is_resource($image)) {
        ...
        if (empty($image)) {
          $this->respondWithValidationError(
            'parameter_missing', $this->translate("Photo missing in stream.")
          );
        }
      }
