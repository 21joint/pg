...
class Pgservicelayer_ReactionController extends Pgservicelayer_Controller_Action_Api
{
    ...
    public function postAction(){
        ...
        try {
            $voteType = $this->getParam("voteType",1);
            Engine_Api::_()->getApi("V1_Reaction","pgservicelayer")->toggleReaction($subject,$voteType);
            $db->commit();
    ...
    public function deleteAction(){
        ...
        try {
            $voteType = $this->getParam("voteType",1);
            Engine_Api::_()->getApi("V1_Reaction","pgservicelayer")->removeReaction($subject,$voteType);
            $db->commit();
        } catch (Exception $e) {
            $db->rollBack();
            $this->respondWithValidationError('internal_server_error', $e->getMessage());
