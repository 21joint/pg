...
class Pgservicelayer_ReviewsController
  extends Pgservicelayer_Controller_Action_Api
{
  ...
  public function getAction()
  {
    ...
    if ($topicID != -1) {
      $select->joinLeft(
        $listingTypeTableName,
        "$listingTypeTableName.listingtype_id = $listingTableName.listingtype_id",
        array()
      );
    ...
    if ($authorRating != -1) {
      if (strstr($authorRating, ">")) {
        ...
        $select->where(
          "$listingTableName.gg_author_product_rating > ?", (int)$authorRating
        );
      } else {
        $select->where(
          "$listingTableName.gg_author_product_rating = ?", (int)$authorRating
    ...
    foreach ($paginator as $sitereview) {
      $response['contentType'] = Engine_Api::_()->sdparentalguide()
        ->mapSEResourceTypes($sitereview->getType());
  ...
  public function postAction()
  {
    ...
    $level_id = !empty($viewer_id)
      ? $viewer->level_id
      : Engine_Api::_()->getDbtable('levels', 'authorization')->fetchRow(
        array('type = ?' => "public")
      )->level_id;
    $listingtype_id = $this->getParam("typeID", 0);
    $listingType = Engine_Api::_()->getItem(
      "sitereview_listingtype", $listingtype_id
    );
    if (empty($listingType)) {
      $this->respondWithError(
        'no_record', $this->translate("Review category not specified")
      );
    }
    $canCreate = Engine_Api::_()->authorization()->getPermission(
      $level_id, 'sitereview_listing', "create_listtype_$listingtype_id"
    );
    if (!$canCreate) {
      $this->respondWithError('unauthorized');
    }

    $form = Engine_Api::_()->getApi("V1_Forms", "pgservicelayer")
      ->getReviewForm();
    $validators = Engine_Api::_()->getApi("V1_Validators", "pgservicelayer")
      ->getReviewValidators();
    ...
    try {
      ...
      $expiry_setting = Engine_Api::_()->sitereview()->expirySettings(
        $listingtype_id
      );
      ...
      if (Engine_Api::_()->sitereview()->listBaseNetworkEnable()) {
        if (isset($values['networks_privacy'])
          && !empty($values['networks_privacy'])
        ) {
      ...
      if (!empty($sitereview->photo_id)) {
        Engine_Api::_()->getDbTable('files', 'pgservicelayer')
          ->updatePhotoParent($sitereview->photo_id, $sitereview);
      }
      ...
      $sitereview->profile_type = Engine_Api::_()->getDbTable(
        'categories', 'sitereview'
      )->getProfileType($categoryIds, 0, 'profile_type');
      ...
      foreach ($roles as $i => $role) {
        $auth->setAllowed(
          $sitereview, $role, "view_listtype_$listingtype_id", ($i <= $viewMax)
        );
      ...
      if (isset($values['auth_event']) && !empty($values['auth_event'])) {
        ...
        foreach ($roles as $i => $roles) {
          $auth->setAllowed(
            $sitereview, $roles, "event_listtype_$listingtype_id",
            ($i <= $eventMax)
          );
        }
      }
      ...
      if (isset($values['auth_sprcreate'])
        ...
        foreach ($roles as $i => $roles) {
          $auth->setAllowed(
            $sitereview, $roles, "sprcreate_listtype_$listingtype_id",
            ($i <= $projectMax)
          );
        }
      }
      ...
      foreach ($roles as $i => $roles) {
        $auth->setAllowed(
          $sitereview, $roles, "topic_listtype_$listingtype_id",
          ($i <= $topicMax)
        );
        $auth->setAllowed(
          $sitereview, $roles, "photo_listtype_$listingtype_id",
          ($i <= $photoMax)
        );
        $auth->setAllowed(
          $sitereview, $roles, "video_listtype_$listingtype_id",
          ($i <= $videoMax)
        );
      }
    ...
    try {
      ...
      if (empty($row)) {
        Engine_Api::_()->getDbTable('otherinfo', 'sitereview')->insert(
          array(
    ...
    try {
      ...
      if (Engine_Api::_()->getDbtable('modules', 'core')->isModuleEnabled(
        'sitereviewpaidlisting'
      )
      ) {
      ...
      if ($sitereview->draft == 0 && $sitereview->search
        && time() >= strtotime(
          $sitereview->creation_date
        )
        && empty($sitereview_pending)
        && $sitereview->approved
      ) {
        $action = Engine_Api::_()->getDbtable('actions', 'activity')
          ->addActivity(
            $viewer, $sitereview, 'sitereview_new_listtype_'.$listingtype_id
          );
      ...
      $users = Engine_Api::_()->getDbtable('editors', 'sitereview')
        ->getAllEditors($listingtype_id, 0, 1);
      ...
      if ($listingType->subscription) {
        Engine_Api::_()->getDbtable('subscriptions', 'sitereview')
          ->sendNotifications($sitereview);
      }
  ...
  public function putAction()
  {
    ...
    $validators = Engine_Api::_()->getApi("V1_Validators", "pgservicelayer")
      ->getReviewValidators();
    $values = $data = array(
      'typeID'          => $this->getParam(
        "typeID", $sitereview->listingtype_id
      ),
      'title'           => $this->getParam("title", $sitereview->getTitle()),
      'categoryID'      => $this->getParam(
        "categoryID", $sitereview->category_id
      ),
      'subCategoryID'   => $this->getParam(
        "subCategoryID", $sitereview->subcategory_id
      ),
      'longDescription' => $this->getParam(
        "longDescription", $sitereview->body
      ),
      'authorRating'    => (int)$this->getParam(
        "authorRating", $sitereview->gg_author_product_rating
      ),
      'photoID'         => (int)$this->getParam(
        "photoID", $sitereview->photo_id
      ),
    ...
    //Values for database
    $values = array(
      'listingtype_id'           => $this->getParam(
        "typeID", $sitereview->listingtype_id
      ),
      'title'                    => $this->getParam(
        "title", $sitereview->getTitle()
      ),
      'category_id'              => $this->getParam(
        "categoryID", $sitereview->category_id
      ),
      'subcategory_id'           => $this->getParam(
        "subCategoryID", $sitereview->subcategory_id
      ),
      'body'                     => $this->getParam(
        "longDescription", $sitereview->body
      ),
      'gg_author_product_rating' => (int)$this->getParam(
        "authorRating", $sitereview->gg_author_product_rating
      ),
      'photo_id'                 => (int)$this->getParam(
        "photoID", $sitereview->photo_id
      ),
      'search'                   => (int)$this->getParam(
        "search", $sitereview->search
      ),
      'auth_view'                => $this->getParam("authView", "everyone"),
      'auth_comment'             => $this->getParam("authComment", "everyone"),
      'auth_topic'               => $this->getParam("authTopic", "everyone"),
  ...
  public function deleteAction()
  {
    ...
    $sitereviews = Engine_Api::_()->getItemMulti(
      "sitereview_listing", $idsArray
    );
    ...
    try {
      foreach ($sitereviews as $sitereview) {
        $canDelete = Engine_Api::_()->authorization()->getPermission(
          $level_id, 'sitereview_listing',
          "delete_listtype_".$sitereview->listingtype_id
        );
        if (!$canDelete) {
          $this->respondWithError('unauthorized');
        }
  ...
  {
    $listingTypeTable = Engine_Api::_()->getDbTable(
      'listingtypes', 'sitereview'
    );
    $listingTypeTableName = $listingTypeTable->info('name');
    $select = $listingTypeTable->select()->from(
      $listingTypeTableName,
      array('title_plural', 'title_singular', 'listingtype_id')
    )
      ->where('visible = ?', 1)->order("order ASC")->order(
        "listingtype_id ASC"
      );
    ...
    foreach ($listingTypes as $listingType) {
      ...
      foreach ($categories as $category) {
        ...
        $subcategories = $table->getCategoriesList(
          $listingType->getIdentity(), $category->getIdentity()
        );
