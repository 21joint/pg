...
class Pgservicelayer_CommentsController
  extends Pgservicelayer_Controller_Action_Api
{
  ...
  public function getAction()
  {
    ...
    if ($subject->getType() == "core_comment") {
      $subject = Engine_Api::_()->getItem(
        $subject->resource_type, $subject->resource_id
      );
      $parentCommentId = $this->getParam("contentID");
    }
    if (!($subject instanceof Core_Model_Item_Abstract)
      || !$subject->getIdentity()
      || (!method_exists($subject, 'comments')
        && !method_exists(
          $subject, 'likes'
        ))
    ) {
    ...
    $orderByDirection = (strtolower($orderByDirection) == "descending") ? "DESC"
      : "ASC";
    $isLike = Engine_Api::_()->getDbTable("likes", "core")->isLike(
      $subject, $viewer
    );
    ...
    foreach ($comments as $comment) {
      $response['contentType'] = Engine_Api::_()->sdparentalguide()
        ->mapSEResourceTypes($comment->getType());
  ...
  public function postAction()
  {
    ...
    try {
      ...
      $comment->getTable()->update(
        $commentData, array('comment_id = ?' => $comment->getIdentity())
      );

      if ($subject->getType() == "ggcommunity_answer" && $subject->getOwner()
        && !$viewer->isSelf($subject->getOwner())
      ) {
        $actionOwner = Engine_Api::_()->getDbtable('actions', 'activity')
          ->addActivity(
            $subject->getOwner(), $subject, "question_answer_comment", null,
            array(
      ...
      Engine_Api::_()->getDbtable('statistics', 'core')->increment(
        'core.comments'
      );
      if (!empty($comment)) {
        $db->commit();
        ...
        $this->respondWithValidationError(
          'internal_server_error', 'Problem in comment'
        );
      }
    } catch (Exception $e) {
      $db->rollBack();
    ...
    try {
      ...
      foreach ($comments as $comment) {
        ...
        if ($subject->getType() == "ggcommunity_answer") {
          if (($action = $api->hasActivity(
            $subject, 'question_answer_comment', $subject->getOwner()
          ))
          ) {
            $action->delete();
          }
          ...
          if (($action = $api->hasActivity(
            $subject, 'question_answer_author_comment', $viewer
          ))
          ) {
            $action->delete();
          }
        ...
        if ($subject->getType() == "ggcommunity_question") {
          if (($action = $api->hasActivity(
            $subject, 'question_comment', $subject->getOwner()
          ))
          ) {
            $action->delete();
          }
          ...
          if (($action = $api->hasActivity(
            $subject, 'question_author_comment', $viewer
          ))
          ) {
            $action->delete();
          }
