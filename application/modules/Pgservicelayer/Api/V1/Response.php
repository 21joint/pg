...
class Pgservicelayer_Api_V1_Response extends Sdparentalguide_Api_Core
{
  public function getListingOverview(Sitereview_Model_Listing $sitereview)
  {
    ...
    $overview = $tableOtherinfo->getColumnValue(
      $sitereview->getIdentity(), 'overview'
    );
    $staticBaseUrl = Engine_Api::_()->getApi('settings', 'core')->getSetting(
      'core.static.baseurl', null
    );
  ...
  public function getCommentData(Core_Model_Item_Abstract $comment)
  {
    ...
    $childCommentsSelect->where(
      'parent_comment_id =?', $comment->getIdentity()
    );
    $commentInfo = array();
    $poster = Engine_Api::_()->getItem(
      $comment->poster_type, $comment->poster_id
    );
    $commentInfo["commentID"] = (string)$comment->comment_id;
    $commentInfo['contentType'] = Engine_Api::_()->sdparentalguide()
      ->mapSEResourceTypes($comment->resource_type);
    $commentInfo['contentID'] = (string)$comment->resource_id;
    $commentInfo['commentsCount'] = Zend_Paginator::factory(
      $childCommentsSelect
    )->getTotalItemCount();
    $commentInfo["body"] = $comment->body;
    $commentInfo["createdDateTime"] = $this->getFormatedDateTime(
      $comment->creation_date
    );
    $commentInfo["likesCount"] = $comment->likes()->getLikeCount();
    $commentInfo['dislikesCount'] = $dislikesTable->dislikes($comment)
      ->getDislikeCount();
    $commentInfo['totalLikesCount'] = $commentInfo["likesCount"]
      - $commentInfo['dislikesCount'];
    $commentInfo["lastModifiedDateTime"] = $this->getFormatedDateTime(
      $comment->creation_date
    );
  ...
  public function getFormatedDateTime($datetime)
  {
    ...
    return $view->locale()->toDateTime(
      $datetime, array('format' => 'YYYY-MM-d HH:MM:ss')
    );
  }
...
//        $contentImages['photoURL'] = isset($userPhotos['photoURL'.$avatarPhoto])?$userPhotos['photoURL'.$avatarPhoto]:$userPhotos['photoURLIcon'];
    ...
    $expert = ($user->gg_expert_bronze_count || $user->gg_expert_silver_count
  ...
  public function getSearchItemData($item)
  {
    ...
    $searchArray = array(
      'searchRank'    => "1",
      'contentType'   => Engine_Api::_()->sdparentalguide()->mapSEResourceTypes(
        $item->type
      ),
  ...
  public function getReviewData(Sitereview_Model_Listing $sitereview)
  {
    ...
    $listingRating = Engine_Api::_()->getDbTable(
      "listingRatings", "sdparentalguide"
    )->getAvgListingRating($sitereview);
...
//        $contentImages['photoURL'] = isset($sitereviewPhtos['photoURL'.$avatarPhoto])?$sitereviewPhtos['photoURL'.$avatarPhoto]:$sitereviewPhtos['photoURLIcon'];
    ...
    $shortDesc = (Engine_String::strlen($tmpBody) > 100 ? Engine_String::substr(
        $tmpBody, 0, 100
      ).'...' : $tmpBody);
    $sitereviewArray = array(
      ...
      'publishedDateTime'    => $this->getFormatedDateTime(
        $sitereview->approved_date
      ),
      'featured'             => $sitereview->featured,
      'approved'             => $sitereview->approved,
      'createdDateTime'      => $this->getFormatedDateTime(
        $sitereview->creation_date
      ),
      'lastModifiedDateTime' => $this->getFormatedDateTime(
        $sitereview->modified_date
      ),
      ...
      'averageReviewRating'  => sprintf(
        "%.1f", $listingRating['review_rating']
      ),
      'averageProductRating' => sprintf(
        "%.1f", $listingRating['product_rating']
      ),
    );

    $auth = Engine_Api::_()->authorization()->context;
    $roles = array('owner', 'owner_member', 'owner_member_member',
                   'owner_network', 'registered', 'everyone');
    foreach ($roles as $role) {
      if (1 == $auth->isAllowed(
          $sitereview, $role, "view_listtype_$listingtype_id"
        )
      ) {
        $sitereviewArray['privacySettings']['authView'] = $role;
      }
      if (1 == $auth->isAllowed(
          $sitereview, $role, "comment_listtype_$listingtype_id"
        )
      ) {
    ...
    $roles_photo = array('owner', 'owner_member', 'owner_member_member',
                         'owner_network', 'registered');
    foreach ($roles_photo as $role) {
      if (1 == $auth->isAllowed(
          $sitereview, $role, "topic_listtype_$listingtype_id"
        )
      ) {
        $sitereviewArray['privacySettings']['authTopic'] = $role;
      }
      if (1 == $auth->isAllowed(
          $sitereview, $role, "photo_listtype_$listingtype_id"
        )
      ) {
        $sitereviewArray['privacySettings']['authPhoto'] = $role;
      }
      if (1 == $auth->isAllowed(
          $sitereview, $role, "video_listtype_$listingtype_id"
        )
      ) {
  ...
  public function getListingCategorization(Sitereview_Model_Listing $sitereview)
  {
    ...
    $subcategory = Engine_Api::_()->getItem(
      'sitereview_category', $sitereview->subcategory_id
    );
  ...
  {
    $listingType = $sitereview->getListingType();
    $topic = Engine_Api::_()->getItem(
      "sdparentalguide_topic", $listingType->gg_topic_id
    );
...
//        $contentImages['photoURL'] = isset($topicPhotos['photoURL'.$avatarPhoto])?$topicPhotos['photoURL'.$avatarPhoto]:$topicPhotos['photoURLIcon'];
  ...
  public function getQuestionData(Ggcommunity_Model_Question $question)
  {
    $view = Zend_Registry::get("Zend_View");
    $topic = Engine_Api::_()->getItem(
      "sdparentalguide_topic", $question->topic_id
    );
    $questionArray = array();
...
//        $contentImages['photoURL'] = isset($questionPhotos['photoURL'.$avatarPhoto])?$questionPhotos['photoURL'.$avatarPhoto]:$questionPhotos['photoURLIcon'];        
    ...
    $questionArray["closedDateTime"] = $this->getFormatedDateTime(
      $question->date_closed
    );
    ...
    $questionArray['totalVoteCount'] = $question->up_vote_count
      - $question->down_vote_count;
    $questionArray['approvedDateTime'] = $this->getFormatedDateTime(
      $question->approved_date
    );
    $questionArray['status'] = ($question->draft == 0) ? $view->translate(
      "Published"
    ) : $view->translate("Draft");
    $questionArray['questionTopic'] = $this->getTopicData($topic);
    $questionArray['createdDateTime'] = $this->getFormatedDateTime(
      $question->creation_date
    );
    $questionArray['publishedDateTime'] = $this->getFormatedDateTime(
      $question->approved_date
    );
    $questionArray['lastModifiedDateTime'] = $this->getFormatedDateTime(
  ...
  public function getAnswerData(Ggcommunity_Model_Answer $answer,
    $question = null
  ) {
    if (empty($question)) {
      $question = Engine_Api::_()->getItem(
        "ggcommunity_question", $answer->parent_id
      );
    }
    $answerArray = array();
    ...
    $answerArray['totalVoteCount'] = $answer->up_vote_count
      - $answer->down_vote_count;
    $answerArray['createdDateTime'] = $this->getFormatedDateTime(
      $answer->creation_date
    );
    $answerArray['lastModifiedDateTime'] = $this->getFormatedDateTime(
  ...
  public function getFollowData($subject, $row)
  {
    ...
    $rowData = array(
      ...
      'contentType'     => Engine_Api::_()->sdparentalguide()
        ->mapSEResourceTypes(
          $subject->getType()
        ),
  ...
  public function getViewData($view)
  {
    ...
    return array(
      ...
      'author'          => $owner->getIdentity() ? $this->getUserData($owner)
        : array(),
    );
  }

  public function getMemberBadgeData(Sdparentalguide_Model_Badge $badge,
    $member = null
  ) {
  ...
  {
    $topic = Engine_Api::_()->getItem(
      "sdparentalguide_topic", $credit->gg_topic_id
    );
    $activityCredit = Engine_Api::_()->getItem(
      "activitycredit", $credit->type_id
    );
    $contributionData = array(
      'contributionID'       => (string)$credit->getIdentity(),
      'memberID'             => '',
      'contributionPoints'   => (int)$credit->credit_point,
      'contributionDateTime' => $this->getFormatedDateTime(
        $credit->creation_date
      ),
      'contributionDetail'   => '',
      'topic'                => $this->getTopicData($topic),
    );
    ...
    if (!empty($activityCredit) && empty($activityCredit->language_en)) {
      $activity_type = $this->translate(
        'ADMIN_ACTIVITY_TYPE_'.strtoupper($activityCredit->activity_type)
      );
  ...
  public function getPermissionData(User_Model_User $user)
  {
    ...
    $level_id = !empty($user_id)
      ? $user->level_id
      : Engine_Api::_()->getDbtable('levels', 'authorization')->fetchRow(
        array('type = ?' => "public")
      )->level_id;
    $listingtype_id = '0';
    $level = Engine_Api::_()->getItem("authorization_level", $level_id);
    $permissionsTable = Engine_Api::_()->getDbtable(
      'permissions', 'authorization'
    );
    $editListings = $permissionsTable->getAllowed(
      'sitereview_listing', $level_id, "edit_listtype_$listingtype_id"
    );
    $deleteListings = $permissionsTable->getAllowed(
      'sitereview_listing', $level_id, "delete_listtype_$listingtype_id"
    );
    $editQuestions = $permissionsTable->getAllowed(
      'ggcommunity', $level_id, 'edit_question'
    );
    $deleteQuestions = $permissionsTable->getAllowed(
      'ggcommunity', $level_id, 'delete_question'
    );
    $canSelectAnswer = $permissionsTable->getAllowed(
      'ggcommunity', $level_id, 'best_answer'
    );
    $permissionData = array(
      'memberID'                      => (string)$user->getIdentity(),
      'isAdmin'                       => (bool)$user->isAdminOnly(),
      'isModerator'                   => (bool)($level->type == 'moderator'),
      'canCommentOnComments'          => (bool)$permissionsTable->getAllowed(
        'sdparentalguide_custom', $level_id, "comment_comment"
      ),
      'canLikeComments'               => (bool)$permissionsTable->getAllowed(
        'sdparentalguide_custom', $level_id, "like_comment"
      ),

      //Reviews
      'canViewReview'                 => (bool)$permissionsTable->getAllowed(
        'sitereview_listing', $level_id, "view_listtype_$listingtype_id"
      ),
      'canCreateReview'               => (bool)$permissionsTable->getAllowed(
        'sitereview_listing', $level_id, "create_listtype_$listingtype_id"
      ),
      ...
      'canLikeReview'                 => (bool)$permissionsTable->getAllowed(
        'sitereview_listing', $level_id, "comment_listtype_$listingtype_id"
      ),
      'canRateProductReview'          => (bool)$permissionsTable->getAllowed(
        'sitereview_listing', $level_id, "rate_product_listtype_$listingtype_id"
      ),
      'canRateAuthorReview'           => (bool)$permissionsTable->getAllowed(
        'sitereview_listing', $level_id, "rate_author_listtype_$listingtype_id"
      ),
      'canCommentReview'              => (bool)$permissionsTable->getAllowed(
        'sitereview_listing', $level_id, "comment_listtype_$listingtype_id"
      ),
      'canApproveReview'              => (bool)$permissionsTable->getAllowed(
        'sitereview_listing', $level_id, "approved_listtype_$listingtype_id"
      ),
      'canFlagReview'                 => (bool)$permissionsTable->getAllowed(
        'sitereview_listing', $level_id, "flag_listtype_$listingtype_id"
      ),

      //Questions
      'canViewQuestion'               => (bool)$permissionsTable->getAllowed(
        'ggcommunity', $level_id, 'view_question'
      ),
      'canCreateQuestion'             => (bool)$permissionsTable->getAllowed(
        'ggcommunity', $level_id, 'create_question'
      ),
      ...
      'canVoteQuestion'               => (bool)$permissionsTable->getAllowed(
        'ggcommunity', $level_id, 'vote_question'
      ),
      'canCommentQuestion'            => (bool)$permissionsTable->getAllowed(
        'ggcommunity', $level_id, 'comment_question'
      ),
      'canAnswerQuestion'             => (bool)$permissionsTable->getAllowed(
        'ggcommunity', $level_id, 'answer_question'
      ),
      'canVoteAnswer'                 => (bool)$permissionsTable->getAllowed(
        'ggcommunity', $level_id, 'vote_answer'
      ),
      'canCommentAnswer'              => (bool)$permissionsTable->getAllowed(
        'ggcommunity', $level_id, 'comment_answer'
      ),
      ...
      'canApproveQuestion'            => (bool)$permissionsTable->getAllowed(
        'ggcommunity', $level_id, 'approve_question'
      ),
      'canChangeCloseDate'            => (bool)$permissionsTable->getAllowed(
        'ggcommunity', $level_id, 'edit_close_date'
      ),
      'canFlagQuestion'               => (bool)$permissionsTable->getAllowed(
        'ggcommunity', $level_id, 'flag_question'
      ),
      'canFlagAnswer'                 => (bool)$permissionsTable->getAllowed(
        'ggcommunity', $level_id, 'flag_answer'
      ),
    );
    if (APPLICATION_ENV === 'production') {
