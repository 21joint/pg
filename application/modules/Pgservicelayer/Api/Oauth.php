...
class Pgservicelayer_Api_Oauth extends Core_Api_Abstract
{
  ...
  protected $_getOauthConsumer = 'default';
    ...
    if (function_exists("apache_request_headers")) {
      ...
      foreach ($authParams as $param) {
        if (!empty($headers[$param])) {
          $header = $headers[$param];
          $param = strtolower($param);
          $param = str_replace("-", "_", $param);
          $this->_params[$param] = $header;
        }
      }
      $this->_params['oauth_signature_method'] = 'PLAINTEXT';
      $this->_generateRandomStringFormate = 'string';
...
//                echo json_encode($_REQUEST);exit;
  ...
  public function getAccessOauthToken($user)
  {
    ...
    $select = $tokenTable->getSelect(
      array('consumer_id' => $this->_consumer->consumer_id,
            'user_id'     => $user->getIdentity())
    );
    $token = $tokenTable->fetchRow($select);

    if (empty($token)) {
      $token = Engine_Api::_()->getDbTable('tokens', 'siteapi')->createToken(
        $this->_consumer->consumer_id
      );
  ...
  protected function setConsumer()
  {
    ...
    $siteapiOauthConsumerType = Zend_Registry::isRegistered(
      'siteapiOauthConsumerType'
    ) ? Zend_Registry::get('siteapiOauthConsumerType') : null;
    ...
    if (isset($this->_params['oauth_version'])
      && '1.0' != $this->_params['oauth_version']
    ) {
      $this->_throughError(
        array(
          'status_code' => 400,
    ...
    if (!isset($this->_params['oauth_consumer_key'])
      || !isset($this->_params['oauth_signature_method'])
    ) {
      $this->_throughError(
        array(
          'status_code' => 400,
    ...
    if (strlen($this->_params['oauth_consumer_key'])
      != $this->_OauthKeyLenght
    ) {
      $this->_throughError(
        array(
          'status_code' => 400,
    ...
    if (!in_array(
      $this->_params['oauth_signature_method'], array('HMAC-SHA1', 'PLAINTEXT')
    )
    ) {
      $this->_throughError(
        array(
...
//        }
    ...
    $select = $consumersTable->getSelect(
      array('key' => $this->_params['oauth_consumer_key'])
    );
   ...
   */
  public function generateRandomString($lenght = 32,
    $dummyString = '0123456789abcdefghijklmnopqrstuvwxyz'
  ) {
    $tempStr = '';
  ...
  public function validateOauthToken()
  {
    ...
    $tokenTable = Engine_Api::_()->getDbTable('tokens', 'siteapi');
    $select = $tokenTable->getSelect(
      array('token' => $this->_params['oauth_token'])
    );
    $token = $tokenTable->fetchRow($select);
