...
<div class="eventlocation_browse_location" id="eventlocation_browse_location" >
	...
	<div class="eventlocation_map_container_list" id="eventlocation_content_content">
<?php endif; ?>
  ...
  <a id="eventlocation_location_map_anchor" class="pabsolute"></a>
					...
					<li>
						...
						<div class='seaocore_browse_list_info'>
							<div class="seaocore_browse_list_info_title">
								...
								//echo $this->htmlLink($item->getHref(), $item->title) ?>
								<div class="fright">
									<?php if (($item->price>0)): ?>
										<span>
							...
							<div class="seaocore_browse_list_info_date">
									<?php echo $this->translate("in ") . $this->htmlLink(Engine_Api::_()->sitepage()->getHref($item->page_id, $item->owner_id, $item->getSlug()),  $item->page_title) ?>
							</div>
							<div class="seaocore_browse_list_info_date">
							<?php echo $this->translate('Led by %s', $this->htmlLink($item->getOwner(), $item->getOwner()->getTitle())) ?>
							...
							<?php if(!empty($item->location)): ?>
								...
								- <b>
										<?php if (!empty($this->mobile)) : ?>
											<?php echo  $this->htmlLink(array('route' => 'seaocore_viewmap', "id" => $item->seao_locationid, 'resouce_type' => 'page_event', 'is_mobile' => $this->mobile), $this->translate("Get Directions"), array('target' => '_blank')) ; ?>
										<?php else: ?>
											<?php if (!empty($this->is_ajax)) : ?>
												<?php echo  $this->htmlLink(array('route' => 'seaocore_viewmap', "id" => $item->seao_locationid, 'resouce_type' => 'page_event'), $this->translate("Get Directions"), array('onclick' => 'owner(this);return false')) ; ?>
												<?php else : ?>
													<?php echo  $this->htmlLink(array('route' => 'seaocore_viewmap', "id" => $item->seao_locationid, 'resouce_type' => 'page_event'), $this->translate("Get Directions"), array('class' => 'smoothbox')) ; ?>
												<?php endif; ?>
										<?php endif; ?>
									</b>
								</div>	
							...
							<?php if (!empty($item->description)): ?>
								<div class="seaocore_browse_list_info_blurb">
									<?php echo $this->viewMore($item->description); ?><br />
								</div>
							<?php endif; ?>
						</div>
					</li>
		...
		<?php else: ?>
			<div class="tip">
				<span><?php echo $this->translate('Nobody has created a page event with that criteria.'); ?></span>
			</div>
		<?php endif; ?>
		<?php if (empty($this->is_ajax)) : ?>
	</div>
...
<script type="text/javascript" >
  ...
  window.addEvent('domready',function() {
    //smallLargeMap(1);
    var Clientwidth = $('global_content').getElement(".layout_sitepageevent_location_search").clientWidth;
...
<?php if (!empty($location->location)) : ?>
	...
	var contentString = '<div id="content">'+
		...
		'<?php echo $this->htmlLink($location->getHref(), $this->string()->escapeJavascript($location->getTitle()), array('title' => $this->string()->escapeJavascript($location->getTitle()), 'target' => '_parent')); ?>'+ '<div class="clr"></div>'+ '</div>'+
