...
foreach ($this->show_slideshow_object as $type => $event) {
  ...
  $eventPhoto = $this->htmlLink(
						$event->getHref(),
						$this->itemPhoto($event, 'thumb.normal', $event->getTitle())
					);
  $content_info = null;
  if(Engine_Api::_()->getApi('settings', 'core')->getSetting('sitepage.postedby', 1)):
  $content_info .= $this->timestamp(strtotime($event->creation_date)) . $this->translate(' - created by ') . $this->htmlLink($event->getOwner()->getHref(), $event->getOwner()->getTitle(),array('title' => $event->getOwner()->getTitle()));
  endif;
  //$content_info = '<div class="sitepage_sidebar_list_details"></div>';
  $content_info.='<p>';
  $content_info.=$this->translate(array('%s view', '%s views', $event->view_count), $this->locale()->toNumber($event->view_count)) . ', ';
  $content_info.=$this->translate(array('%s guest', '%s guests', $event->member_count), $this->locale()->toNumber($event->member_count));
  $content_info.='</p>';

  $description = $event->description;

  $content_link = $this->htmlLink($event->getHref(), $this->translate('View Event &raquo;'), array('class' => 'featured_slideshow_view_link'));

  $image_text_var .= "<div class='featured_slidebox'>";
  $image_text_var .= "<div class='featured_slidshow_img'>" . $eventPhoto . "</div>";
  ...
  if (!empty($event->title)) {
    ...
    $title = $this->htmlLink($event->getHref(), $this->string()->chunk($this->string()->truncate($event->getTitle(), 45), 10),array('title' => $event->getTitle()));

    $image_text_var .='<h5>' . $this->htmlLink($event->getHref(), $event->title) . '</h5>';
    $sitepage_object = Engine_Api::_()->getItem('sitepage_page', $event->page_id);
    ...
    $image_text_var .= $this->translate("in ") . $this->htmlLink(Engine_Api::_()->sitepage()->getHref($event->page_id, $event->owner_id, $event->getSlug()),  $page_title,array('title' => $sitepage_object->title)); 
    $image_text_var .= "</div>";
  ...
  if (!empty($description)) {
    ...
    $image_text_var .= "<p>" . $truncate_description . " " . $this->htmlLink($event->getHref(), $this->translate('More &raquo;')) . "</p>";
  }
    ...
    window.addEvent('domready',function(){
     ...
     for(var i=0;i < divElements.length;i++)
      ...
      var nS8 = new noobSlide({
        box: $('sitepageevent_featured_event_im_te_advanced_box'),
        items: $$('#sitepageevent_featured_event_im_te_advanced_box h3'),
        size: (width-10),
        handles: $$('#handles8 span'),
        addButtons: {previous: $('sitepageevent_featured_event_prev8'), stop: $('sitepageevent_featured_event_stop8'), play: $('sitepageevent_featured_event_play8'), next: $('sitepageevent_featured_event_next8') },
        interval: 5000,
        fxOptions: {
          duration: 500,
          transition: '',
        ...
        onWalk: function(currentItem,currentHandle){
          ...
          {
            $('sitepageevent_featured_event_caption').innerHTML =   current_caption_link;
          }
          else {
            $('sitepageevent_featured_event_caption').innerHTML =  '';
          }


          $('sitepageevent_featured_event_current_numbering').innerHTML =  current_index + '/' + num_of_slidehsow ;
        }
      });
...
<div class="featured_slideshow_wrapper">
  <div class="featured_slideshow_mask">
    <div id="sitepageevent_featured_event_im_te_advanced_box" class="featured_slideshow_advanced_box">
  ...
  <div class="featured_slideshow_option_bar">
    <div>
      <p class="buttons">
        <span id="sitepageevent_featured_event_prev8" class="featured_slideshow_controllers-prev featured_slideshow_controllers prev" title=<?php echo $this->translate("Previous") ?> ></span>
        <span id="sitepageevent_featured_event_stop8" class="featured_slideshow_controllers-stop featured_slideshow_controllers" title=<?php echo $this->translate("Stop") ?> ></span>
        <span id="sitepageevent_featured_event_play8" class="featured_slideshow_controllers-play featured_slideshow_controllers" title=<?php echo $this->translate("Play") ?> ></span>
        <span id="sitepageevent_featured_event_next8" class="featured_slideshow_controllers-next featured_slideshow_controllers" title=<?php echo $this->translate("Next") ?> ></span>
      </p>
    </div>
    <span id="sitepageevent_featured_event_caption"></span>
    <span id="sitepageevent_featured_event_current_numbering" class="featured_slideshow_pagination"></span>
  </div>
</div>  
