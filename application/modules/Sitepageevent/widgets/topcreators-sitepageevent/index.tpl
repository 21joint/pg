  ...
  <?php foreach ($this->paginator as $event):?>
        <li>
            ...
            <?php echo $this->htmlLink($sitepage_object->getHref(), Engine_Api::_()->sitepage()->truncation($sitepage_object->getTitle()), array('title' => $sitepage_object->getTitle())); ?> 
						...
						<?php endif; ?>
						<div class="sitepage_sidebar_list_details">
						 <?php echo $this->htmlLink($sitepage_object->getHref(array('tab'=> $tab_id)),$this->translate(array('%s event', '%s events', $event['item_count']), $this->locale()->toNumber($event['item_count']))); ?>
				    </div>	
        </div>
      </li>
	<?php endforeach; ?>
