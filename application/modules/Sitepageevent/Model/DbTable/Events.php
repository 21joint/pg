...
class Sitepageevent_Model_DbTable_Events extends Engine_Db_Table {
  ...
  public function getSitepageeventsSelect($params = array()) {
    ...
    if (isset($params['show_count']) && $params['show_count'] == 1) {
      ...
      $select = $select
              ->setIntegrityCheck(false)
              ->from($tableEventName, array('event_id', 'page_id', 'user_id', 'title', 'creation_date', 'view_count', 'description', 'search', 'starttime', 'endtime', 'location', 'host', 'member_count', 'photo_id', 'approval', 'invite', 'featured', 'seao_locationid'))
    ...
    if (isset($params['searchable'])) {
      $select->where($tableEventName . '.search = ?', $params['searchable']);
    }    
